...
TL;DR: 研究发现，针对头部姿态校正和图像恢复的AI技术，在选择性结合使用CFR-GAN和CodeFormer时，能够有效提升生物特征人脸识别在真实场景中的准确性。


<details>
  <summary>Details</summary>
Motivation: 生物特征人脸识别模型在处理真实世界图像（通常质量差、姿态非正面、有遮挡）时准确性显著下降，需要研究AI驱动的头部姿态校正和图像恢复技术是否能改善识别准确率。

Method: 使用模型无关的大规模法医评估流程，评估三种恢复方法：3D重建（NextFace）、2D正面化（CFR-GAN）和特征增强（CodeFormer），并研究它们的组合应用效果。

Result: 单纯应用这些技术会显著降低人脸识别准确性，但选择性结合使用CFR-GAN和CodeFormer能够带来有意义的改进。

Conclusion: AI驱动的头部姿态校正和图像恢复技术需要谨慎选择和组合应用，CFR-GAN与CodeFormer的选择性结合是提升真实场景人脸识别准确性的有效策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有可微分渲染方法（如NeRF和3DGS）在大规模动态场景重建中存在可扩展性限制，需要标注来解耦角色运动；现有自监督方法虽然尝试消除显式标注，但仍受限于逐场景优化和对超参数调整的敏感性。

Method: Flux4D采用完全无监督方式，仅使用光度损失并强制执行"尽可能静态"的正则化，直接预测3D高斯及其运动动态来重建传感器观测，通过跨多个场景训练学习从原始数据中分解动态元素。

Result: 在室外驾驶数据集上的实验表明，Flux4D在可扩展性、泛化能力和重建质量方面显著优于现有方法，能够在几秒内高效重建动态场景，并能有效扩展到大型数据集，泛化到未见过的环境包括罕见和未知物体。

Conclusion: Flux4D提供了一个简单、可扩展且完全无监督的4D动态场景重建框架，解决了现有方法在可扩展性、标注依赖和泛化能力方面的限制，为大规模动态场景重建提供了有效解决方案。
...
Result: 模型在FSC-147数据集上达到了与最先进零样本方法相当的性能，在某些情况下甚至超越了现有方法
...
TL;DR: 提出了一种基于大语言模型的两阶段方法，直接从带有粗略分割的3D点云推断材料组成，将物体识别与材料分配解耦，无需特定任务训练。


<details>
  <summary>Details</summary>
Motivation: 现有3D形状数据集和模型大多只关注几何特征，忽略了决定物体外观的材料属性。缺乏可靠的材料标注数据集限制了材料理解的研究。

Method: 采用两阶段大语言模型方法：第一阶段预测物体的语义类别，第二阶段根据推断的语义为每个几何片段分配合理的材料。两个阶段都以零样本方式运行，无需特定任务训练。
...
Result: 该方法仅需每个ISO设置下的单张噪声图像和单张暗帧，无需简化参数模型或大量干净-噪声图像对，在多个低光去噪基准测试中达到最先进性能。

Conclusion: 提出的噪声合成方法既准确又实用，能够生成真实的噪声数据用于训练去噪器，解决了低光图像去噪训练数据难以获取的问题。
...
TL;DR: PixPerfect是一个像素级精炼框架，通过可微分的判别像素空间、全面的伪影模拟管道和直接像素空间精炼方案，解决了潜在扩散模型在图像修复和局部编辑中产生的像素级不一致问题。


<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在图像修复和局部编辑方面取得了显著进展，但其固有的潜在压缩经常引入像素级不一致问题，如色移、纹理不匹配和编辑边界处的可见接缝。现有解决方案在实践中通常无法完全消除这些伪影，并且无法很好地泛化到不同的潜在表示或任务中。

Method: PixPerfect采用三个关键技术：(1)可微分的判别像素空间，放大和抑制细微的颜色和纹理差异；(2)全面的伪影模拟管道，在训练期间将精炼器暴露于真实的局部编辑伪影；(3)直接像素空间精炼方案，确保在不同潜在表示和任务中的广泛适用性。

Result: 在修复、对象移除和插入基准测试上的广泛实验表明，PixPerfect显著提高了感知保真度和下游编辑性能，为鲁棒和高保真的局部图像编辑建立了新标准。

Conclusion: PixPerfect提供了一个像素级精炼框架，能够为不同的LDM架构和任务提供无缝、高保真的局部编辑，解决了现有方法在消除像素级不一致方面的局限性。
...
TL;DR: 提出PyroFocus两阶段深度学习管道，用于实时机载/星载野火检测，包括火灾分类和火辐射功率估计，在NASA MASTER数据上验证了速度与精度的良好平衡。
...
Method: 提出PyroFocus两阶段管道：第一阶段进行火灾分类（无火、活跃火、后火条件），第二阶段进行火辐射功率回归或分割。系统评估了多种深度学习架构，包括自定义CNN和Transformer模型。

Result: 实验结果表明，提出的两阶段管道在速度和精度之间实现了良好的权衡，展示了在未来野火监测任务中实时边缘部署的显著潜力。

Conclusion: PyroFocus两阶段管道为实时机载野火检测提供了有效的解决方案，在NASA MASTER数据上验证了其性能，适合未来野火监测任务的边缘部署。
...
TL;DR: H²U3D是一个用于房屋尺度3D场景理解的数据集，包含多楼层环境，通过自动化标注流程生成层次化视觉表示和多样化问答对。SpatialReasoner是一个主动感知框架，通过两阶段训练策略（监督冷启动+强化学习）在3D场景中自主调用空间工具进行探索，在H²U3D上取得SOTA性能，且平均仅需3-4张图像。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在大型3D环境中的空间推理能力有限，通常局限于房间尺度场景。需要开发能够处理房屋尺度多楼层复杂环境的3D视觉问答数据集和相应的方法。

Method: 1. 提出H²U3D数据集：包含多楼层环境（最多三层，10-20个房间，超过300平方米），通过自动化标注流程构建从粗到细的层次化视觉表示，生成多样化问答对并带有思维链标注。2. 提出SpatialReasoner框架：主动感知框架，基于文本查询自主调用空间工具探索3D场景。采用两阶段训练策略：监督冷启动后接强化学习，使用自适应探索奖励促进高效探索同时避免冗余操作。

Result: SpatialReasoner在H²U3D数据集上取得了最先进的性能，超越了包括GPT-4o和Gemini-2.5-Pro在内的强基线模型。特别值得注意的是，该方法平均仅需3-4张图像就能达到优异结果，而基线方法需要16+张图像，证明了从粗到细的主动探索范式的有效性。

Conclusion: H²U3D数据集为房屋尺度的3D场景理解提供了有价值的基准，而SpatialReasoner框架通过主动感知和高效探索策略，在大型3D环境的空间推理任务上展现了优越性能，为未来3D视觉语言模型的发展提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确的环境表示，传统高清地图无法实时更新，而标准导航地图分辨率不足。需要一种方法能够在线融合低精度导航地图先验和高精度传感器数据，构建实时更新的环境表示

Method: 提出NavMapFusion扩散模型框架，通过迭代去噪过程，以高保真传感器数据和低保真导航地图为条件。将先验地图与在线感知之间的差异视为扩散过程中的噪声，一致区域强化地图构建，过时部分被抑制

Result: 在nuScenes基准测试中，使用OpenStreetMap道路线作为条件，在100米感知范围内实现21.4%的相对改进，更大感知范围改进更显著，同时保持实时处理能力

Conclusion: 扩散模型为地图融合提供了鲁棒框架，能够融合低保真先验和高保真传感器数据，生成准确且最新的环境表示，推动更安全可靠的自动驾驶
...
TL;DR: 该论文提出了一种新的分步分层设计生成方法SLEDGE，将设计过程建模为基于指令的逐步分层修改，并引入了相应的评估套件。
...
Method: 提出SLEDGE（Step-by-step LayEred Design GEnerator），利用多模态LLMs将每次设计更新建模为基于先前状态的原子化分层变化，同时以指令为基础。

Result: 通过全面的实验分析和与现有方法的比较，证明了该方法的有效性。同时引入了包含数据集和基准测试的新评估套件。

Conclusion: 该工作填补了分步设计生成的研究空白，希望吸引更多关注这一实用但未被充分探索的研究领域。
...
<details>
  <summary>Details</summary>
Motivation: 射血分数是评估心脏功能的关键指标，传统方法需要手动追踪且依赖专家经验，耗时且存在观察者间差异。现有深度学习模型多为黑盒模型，缺乏透明度，降低了临床信任度。虽然有一些事后解释方法，但这些解释不指导模型内部推理，在临床应用中可靠性有限。

Method: 提出ProtoEFNet，一个基于视频的原型学习模型，用于连续射血分数回归。模型学习动态时空原型，捕捉临床有意义的心脏运动模式。同时提出原型角度分离损失，在连续射血分数谱上强制区分性表示。

Result: 在EchonetDynamic数据集上的实验表明，ProtoEFNet在保持与不可解释对应模型相当准确性的同时，提供临床相关洞察。消融研究显示，提出的损失函数将F1分数从77.67±2.68提升到79.64±2.10，性能提升2%。

Conclusion: ProtoEFNet通过原型学习方法实现了连续射血分数回归，在保持准确性的同时提供临床可解释性，解决了黑盒模型在医疗应用中的信任问题。提出的原型角度分离损失有效提升了模型性能。
...
TL;DR: HalluGen：一个基于扩散的框架，用于生成可控的幻觉，并创建首个大规模幻觉数据集，用于安全关键图像恢复的评估


<details>
  <summary>Details</summary>
Motivation: 生成模型在图像恢复中容易产生幻觉（看似合理但错误的伪影），这在医疗影像等安全关键领域尤其危险。然而，评估幻觉存在循环依赖问题：需要标注数据但标注成本高且主观。

Method: 提出HalluGen框架，基于扩散模型合成具有可控类型、位置和严重程度的真实幻觉。使用该框架构建了包含4,350张标注图像的大规模幻觉数据集，源自1,450张脑部MR图像。

Result: HalluGen能生成感知真实但语义错误的输出（分割IoU从0.86降至0.36）。开发了SHAFE指标，通过特征评估和软注意力池化提高幻觉敏感性。训练的无参考幻觉检测器能泛化到真实恢复失败场景。
...
TL;DR: 该研究系统比较了16种深度学习架构在3D OCT图像中检测亚临床圆锥角膜的性能，发现分层注意力模型在稀疏异常检测方面优于2D/3D CNN和ViT，具有更高的敏感性和特异性。


<details>
  <summary>Details</summary>
Motivation: 检测体积医学成像中微弱、空间分布异常的挑战：早期疾病信号的细微、非相邻特性常因次优的架构归纳偏置而丢失。2D/3D CNN强加局部性，而ViT扩散无约束的全局注意力，导致稀疏体积模式识别的最优归纳结构未解决。

Method: 对16种现代深度学习架构进行对照比较，涵盖2D/3D卷积、混合和体积Transformer家族，用于从3D前段OCT体积中检测亚临床圆锥角膜。通过注意力距离测量、表征相似性和辅助年龄/性别预测任务进行机制分析。
...
Conclusion: 分层注意力为3D医学成像中的早期病理变化分析提供了原则性和有效的方法，为未来体积异常检测系统提供了设计指导，确立了分层注意力在稀疏异常检测中的优势地位。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉理解、预测和生成直接在2D观测上进行，导致性能不佳。需要学习连续4D动态来生成未见视觉内容

Method: 1) 从稀疏单目2D帧重建4D世界(2D→4D)；2) 在低秩表示和物理约束下学习连续4D动态(离散4D→连续4D)；3) 时间推进世界，在采样时间和视点重新投影回2D，基于时空上下文感知生成未见区域(4D→2D)

Result: 通过4D动态建模，实现连续且物理一致的新视觉生成，在未见时间生成、未见空间生成和视频编辑等多个任务中展现强大潜力

Conclusion: SeeU通过2D→4D→2D框架学习连续4D动态，相比直接在2D上操作的方法，能实现更优的视觉内容生成和理解
...
TL;DR: FireSentry是一个省级规模的多模态野火数据集，具有亚米级空间分辨率和亚秒级时间分辨率，用于细粒度野火蔓延预测。基于该数据集提出了FiReDiff双模态范式，先在红外模态预测未来视频序列，再基于生成的动态精确分割火掩膜，在视频质量和掩膜精度上均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注粗时空尺度，依赖低分辨率卫星数据，只能捕捉宏观火灾状态，限制了高精度局部火灾动态建模能力。需要填补细粒度野火预测的数据和方法空白。

Method: 1. 构建FireSentry数据集：使用同步无人机平台收集可见光和红外视频流、现场环境测量数据和人工验证的火掩膜；2. 建立包含物理模型、数据驱动模型和生成模型的综合基准；3. 提出FiReDiff双模态范式：先在红外模态预测未来视频序列，然后在掩膜模态基于生成的动态精确分割火掩膜。

Result: FiReDiff在生成模型上取得最先进性能：视频质量提升39.2% PSNR、36.1% SSIM、50.0% LPIPS、29.4% FVD；掩膜精度提升3.3% AUPRC、59.1% F1分数、42.9% IoU、62.5% MSE。FireSentry基准数据集已公开可用。
...
Conclusion: ShelfGaussian成功整合了高斯模型的优势与视觉基础模型的能力，实现了开放词汇的多模态3D场景理解，在感知和规划任务中表现出色。
...
Method: 提出MOS框架，包含两个核心组件：1) 模态一致表示学习(MCRL)：应用去噪SAR图像处理和类级模态对齐损失来对齐跨模态的类内特征分布；2) 跨模态数据生成与特征融合(CDGF)：利用布朗桥扩散模型合成跨模态样本，在推理时与原始特征融合以增强对齐性和判别性

Result: 在HOSS ReID数据集上的实验表明，MOS在所有评估协议中都显著超越了最先进方法，在ALL to ALL、Optical to SAR和SAR to Optical设置下分别实现了R1准确率+3.0%、+6.2%和+16.4%的显著提升

Conclusion: MOS框架通过模态一致表示学习和跨模态数据生成与特征融合，有效缓解了光学-SAR模态差异，实现了模态一致的特征学习，为跨模态船舶重识别提供了有效解决方案
...
Result: 对19个代表性多模态模型的实验显示，它们在比较描述和差异感知能力上存在显著性能差距，表明当前模型在这方面的能力有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有的可负担性学习方法大多只关注"如何使用"物体，忽略了"是什么"和"在哪里"的问题。其他方法将物体检测和可负担性学习作为独立任务处理，缺乏有效交互和实时能力。

Method: YOLOA采用轻量级检测器，包含物体检测和可负担性学习分支，通过LLM适配器进行精炼。训练时，LLM适配器与物体和可负担性初步预测交互，生成更准确的类别先验、边界框偏移和可负担性门控。
...
TL;DR: 提出基于冻结扩散模型的标签高效高光谱图像分类框架，利用自然图像预训练的扩散模型提取空间特征，通过轻量级FiLM融合模块整合光谱信息，在稀疏标注下实现优于现有方法的性能。
...
Method: 1) 利用在自然图像上预训练的冻结扩散模型提取空间特征，从早期去噪时间步的高分辨率解码器层获取低层表示；2) 引入轻量级FiLM融合模块，使用光谱线索自适应调制冻结的空间特征，实现多模态学习。

Result: 在两个最新高光谱数据集上的实验表明，该方法仅使用稀疏训练标签就超越了现有最先进方法。消融研究进一步验证了扩散模型特征和光谱感知融合的有效性。
...
TL;DR: LM-CartSeg是一个全自动膝关节MRI分割和影像组学分析流程，通过几何规则改进分割质量，提供稳定的内外侧分区，并证明影像组学特征包含超出简单形态测量的判别信息。


<details>
  <summary>Details</summary>
Motivation: 膝关节MRI影像组学研究需要稳健且具有解剖学意义的感兴趣区域（ROI），现有方法多依赖手动标注且缺乏质量控制。本研究旨在开发一个全自动的软骨/骨分割、几何分区和影像组学分析流程。

Method: 训练两个3D nnU-Net模型，通过零样本预测融合和几何规则（连通分量清洗、10mm软骨下骨带构建、基于PCA和k-means的胫骨内外侧分割）进行后处理。从10个ROI提取4650个非形状影像组学特征，研究特征与体积/厚度的相关性。

Result: 后处理显著改善分割质量（ASSD从2.63降至0.36mm，HD95从25.2降至3.35mm，DSC 0.91）。几何分区规则在不同数据集上产生稳定分区。仅6-12%的特征与体积或厚度强相关，影像组学模型在排除尺寸相关特征后仍保持分类性能。

Conclusion: LM-CartSeg提供了自动化的质量控制ROI和影像组学特征，这些特征包含超出简单形态测量的判别信息，为多中心膝关节骨关节炎影像组学研究提供了实用基础。
...
Method: 从点云数据中无监督学习空间结构化的3D关键点，将其作为紧凑可解释的表征，用于条件化Elucidated Diffusion Model (EDM)来重建完整形状

Result: 学习到的关键点在不同物体实例间表现出可重复的空间结构，支持关键点空间的平滑插值，表明其能捕捉几何变化；在多样物体类别上表现优异，关键点一致性比先前方法提高6个百分点

Conclusion: 提出的框架成功弥合了无监督关键点学习与无条件生成设置之间的差距，为3D生成提供了结构化的可解释表征
...
TL;DR: 该论文提出在视频生成中引入几何正则化损失，通过增强潜在扩散模型实现逐帧深度预测，以解决现有方法缺乏3D结构建模导致的时序不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要在2D像素空间操作，缺乏明确的3D结构建模机制，导致时序几何不一致、运动不自然和结构伪影等问题。

Method: 通过增强潜在扩散模型实现逐帧深度预测，提出多视角几何损失函数，在共享3D坐标系中对齐跨帧的预测深度图，强制实现时间上的结构一致性。

Result: 在多个数据集上的实验表明，该方法相比现有基线能产生显著更稳定和几何一致的结果，改善了时空连贯性、形状一致性和物理合理性。

Conclusion: 该方法弥合了外观生成与3D结构建模之间的差距，通过几何正则化损失提高了视频生成的质量和一致性。
...
Method: 提出ThinkDeeper框架，核心是空间感知世界模型(SA-WM)，将当前场景蒸馏为指令感知的潜在状态并展开未来潜在状态序列；配合超图引导解码器分层融合多模态输入；同时构建DrivePilot数据集，使用RAG和CoT提示的LLM管道生成语义标注。
...
Conclusion: ThinkDeeper通过前瞻性空间推理有效解决了自动驾驶中自然语言指令的模糊性问题，为视觉定位提供了更鲁棒的解决方案。
...
TL;DR: TPI（文本打印图像）方法通过将文本直接渲染到白色画布上生成合成图像，实现低成本文本中心训练，提升视觉语言模型性能
...
Method: 提出文本打印图像（TPI）方法，将给定文本描述直接渲染到纯白色画布上生成合成图像。这种方法将文本投影到图像模态，可低成本集成到现有LVLM训练流程中

Result: 在4个模型和7个基准测试中，TPI比扩散模型生成的合成图像更有效地支持文本中心训练。TPI还作为低成本数据增强策略展示实用价值

Conclusion: 文本中心训练具有显著潜力，TPI方法为LVLM的完全自动化数据生成开辟了新路径，提供了一种可扩展且低成本的训练范式
...
TL;DR: NAS-LoRA是一种新的参数高效微调方法，通过在LoRA的编码器和解码器之间加入轻量级神经架构搜索块，动态优化先验知识，以弥合预训练SAM与特定领域之间的语义鸿沟。


<details>
  <summary>Details</summary>
Motivation: SAM作为强大的视觉基础模型，在适应特定下游任务（如医学和农业成像）时面临挑战。虽然LoRA及其变体被广泛用于增强SAM的适应性能，但Transformer编码器缺乏图像块内的空间先验，可能阻碍高级语义信息的获取。
...
Conclusion: NAS-LoRA通过结合神经架构搜索和参数高效微调，有效弥合了预训练SAM与特定领域之间的语义鸿沟，为视觉基础模型的领域适应提供了高效解决方案。
...
TL;DR: 提出Action Effect Modeling (AEM)框架，通过联合建模动作执行和动作效果来改进程序性任务中的错误检测，在EgoPER和CaptainCook4D基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要分析动作执行方式，忽视了动作产生的效果（如物体状态变化、空间排列等）。许多错误并不体现在执行过程中，而是体现在结果上，因此需要同时考虑执行和效果。

Method: 提出AEM统一框架：1）基于语义相关性和视觉质量选择最具信息量的效果帧；2）从视觉定位和符号场景图中提取互补线索，在共享潜在空间中对齐；3）设计基于提示的检测器，结合任务特定提示并将动作片段与预期执行语义对齐。

Result: 在EgoPER和CaptainCook4D基准测试的单类分类(OCC)设置下取得最先进的性能，证明同时建模执行和效果能实现更可靠的错误检测。

Conclusion: 建模动作执行和效果能显著提升程序性任务中的错误检测性能，效果感知的表征有潜力应用于更广泛的下游任务。
...
TL;DR: PRIS提出了一种在推理时自适应修改提示词的方法，通过分析生成视觉内容中的重复失败模式来重新设计提示词，从而提高文本到视觉生成的对齐质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要通过增加采样步数或种子数量来扩展视觉生成过程，但这很快会达到质量瓶颈，因为提示词在生成过程中保持不变。用户意图与生成视觉内容之间的精确对齐仍然是文本到视觉生成的核心挑战。
...
Conclusion: PRIS框架通过自适应修改提示词来响应扩展的视觉生成，解决了文本到视觉生成中对齐质量瓶颈的问题。元素级事实校正验证器提供了更准确和可解释的对齐评估，为推理时扩展提供了新的方向。
...
TL;DR: 提出公平感知的低秩适配方法，通过可微分的MaxAccGap损失函数优化医疗视觉语言模型在不同人口群体间的诊断准确性差异，在保持参数效率的同时显著减少公平性差距。


<details>
  <summary>Details</summary>
Motivation: 医疗视觉语言模型在医学影像任务上达到专家级性能，但在不同人口群体间存在显著的诊断准确性差异。现有方法缺乏在保持参数效率的同时优化公平性的有效机制。

Method: 提出公平感知的低秩适配方法，包括三种具体方法：FR-LoRA将MaxAccGap正则化整合到训练目标中；GR-LoRA应用逆频率加权平衡梯度贡献；Hybrid-LoRA结合两种机制。核心是引入可微分的MaxAccGap损失函数，实现跨人口群体准确性平等的端到端优化。

Result: 在10,000张青光眼眼底图像上评估，GR-LoRA将诊断准确性差异减少69%，同时保持53.15%的整体准确性。消融研究表明，强正则化强度在最小准确性代价下实现最优公平性，种族特定优化实现60%差异减少。方法仅需0.24%可训练参数。

Conclusion: 该方法结合参数效率与显式公平优化，能够在资源受限的医疗环境中实际部署公平的医疗AI系统，为减少医疗AI中的公平性差距提供实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在文本到图像生成方面表现出色，但难以处理像菜谱插图这样的结构化多步骤场景。现有的菜谱插图方法无法适应菜谱长度的自然变化，无论实际指令结构如何都生成固定数量的图像。
...
Conclusion: CookAnything框架在生成连贯、语义分明的图像序列方面表现出色，支持任意长度的烹饪指令，在指导性媒体和程序化内容创作方面具有广泛应用潜力。
...
### [31] [Towards Object-centric Understanding for Instructional Videos](https://arxiv.org/abs/2512.03479)
*Wenliang Guo,Yu Kong*

Main category: cs.CV

TL;DR: 该论文提出从动作中心范式转向物体中心范式，将动作视为驱动状态转换的机制，并引入Object-IVQA基准测试来评估物体中心推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于动作的方法难以处理真实过程中步骤顺序随物体状态变化而灵活调整的问题，需要转向物体中心范式来更好地理解程序性活动。

Method: 提出物体中心智能体框架，整合物体中心规划、感知、分析和生成工具，支持显式证据检索和跨不连续片段的多次推理。

Result: 实验表明现有大型视觉语言模型在物体级识别和推理方面表现不佳，而提出的框架取得了显著改进。

Conclusion: 物体中心范式为理解程序性活动提供了更有效的框架，Object-IVQA基准测试和提出的智能体框架为未来辅助AI的发展奠定了基础。
...
TL;DR: V-ITI是一个轻量级视觉推理时干预框架，通过检测视觉忽视并仅在需要时进行干预，有效减少多模态大语言模型的幻觉问题，同时保持通用任务性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但存在幻觉问题，产生与输入视觉内容不一致的输出，这在精度敏感领域中影响可靠性。现有方法通常通过干预注意力分数或输出logits来缓解幻觉，但只关注"如何干预"而忽略了"何时干预"的前提，导致"过度干预"问题，引入新的幻觉和不必要的计算开销。
...
Conclusion: V-ITI通过解决"何时干预"的关键问题，有效解决了多模态大语言模型的视觉忽视和幻觉问题，提供了一个轻量级且高效的推理时干预框架。
...
TL;DR: 提出了一种用于大规模用户生成视频平台的混合内容审核框架，结合监督分类和基于参考的相似性匹配，能够及时检测已知违规内容和新型边缘案例。


<details>
  <summary>Details</summary>
Motivation: 大规模用户生成视频平台（特别是直播环境）的内容审核面临挑战，需要及时、多模态且能应对不断演化的不良内容。传统方法难以同时处理已知违规和新型边缘案例。

Method: 采用混合审核框架：1) 监督分类管道处理已知违规；2) 基于参考的相似性匹配管道处理新型或微妙案例。多模态输入（文本、音频、视觉）通过两个管道处理，使用多模态大语言模型将知识蒸馏到每个管道中以提高准确性同时保持推理轻量。

Result: 分类管道在80%精确率下达到67%召回率，相似性管道在80%精确率下达到76%召回率。大规模A/B测试显示用户观看不良直播的比例减少了6-8%。

Conclusion: 该混合框架提供了一种可扩展且适应性强的多模态内容治理方法，能够同时处理明确违规和新兴对抗行为，适用于生产环境的大规模部署。
...
TL;DR: DPMFormer是一个用于领域泛化语义分割的新框架，通过域感知提示学习和对比学习来解决视觉-文本语义对齐问题，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的知识蒸馏方法在领域泛化语义分割中存在视觉与文本上下文语义不对齐的问题，这源于在单一源域上学习的固定上下文提示的僵化性。
...
Conclusion: DPMFormer通过域感知提示学习、对比学习和一致性学习，有效解决了视觉-文本语义对齐问题，为领域泛化语义分割提供了更鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索利用现代计算机视觉技术实现舞蹈动作的自动化分析，为舞蹈研究提供定量化指标，减少对专业设备或标记的依赖。

Method: 提出概念验证框架：1) 使用YOLOv8和v11进行舞者检测；2) 利用Segment Anything Model (SAM)进行精确像素级分割；3) 基于检测和分割结果跟踪舞者动作，量化舞蹈步数、空间覆盖模式和节奏一致性。

Result: 在49秒加纳AfroBeats舞蹈视频测试中：检测精度约94%，召回率89%；SAM分割IoU约83%；主要舞者比次要舞者多执行23%的舞步，运动强度高37%，使用空间多42%。

Conclusion: 该框架展示了技术可行性，为舞蹈定量分析提供了新方向，但存在单视频验证、缺乏系统标注和姿态估计方法对比等局限性，为未来系统验证研究奠定了基础。
...
TL;DR: ToG-Bench：首个面向任务的时空视频定位基准，针对第一人称视角视频，强调任务导向推理而非简单描述性定位


<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要局限于物体中心和描述性指令，缺乏任务导向推理能力，而这对于具身智能体完成目标导向交互至关重要

Method: 基于ScanNet视频构建包含100个标注片段和2,704个任务导向定位指令的基准，采用基础模型标注与人工精修的半自动化流程，引入任务级评估指标

Result: 基准测试揭示了任务导向时空视频定位的内在挑战，在显式-隐式和多对象定位方面存在显著性能差距，突显了感知与交互在具身场景中的融合难度

Conclusion: ToG-Bench填补了任务导向时空视频定位基准的空白，为具身智能研究提供了重要评估工具，揭示了当前模型在任务推理和上下文理解方面的局限性
...
### [37] [FloodDiffusion: Tailored Diffusion Forcing for Streaming Motion Generation](https://arxiv.org/abs/2512.03520)
*Yiyi Cai,Yuhan Wu,Kunhang Li,You Zhou,Bo Zheng,Haiyang Liu*

Main category: cs.CV

TL;DR: FloodDiffusion是一个用于文本驱动的流式人体运动生成的新框架，能够在实时延迟下生成与文本对齐的无缝运动序列。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于分块或带扩散头的自回归模型，无法有效处理时间变化的文本提示下的流式运动生成任务。

Method: 采用扩散强制框架，通过三个关键改进：使用双向注意力而非因果注意力、实现下三角时间调度器而非随机调度器、采用连续时间变化的方式引入文本条件。

Result: 在HumanML3D基准测试中达到0.057的FID分数，首次证明基于扩散强制的框架在流式运动生成任务上达到最先进性能。

Conclusion: FloodDiffusion通过改进的扩散强制框架成功解决了文本驱动的流式人体运动生成问题，实现了高质量、实时的运动生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗扰动方法多为白盒且架构特定，缺乏对攻击机制的可解释性。需要一种能够理解攻击原理、帮助开发者分析和防御的黑盒攻击方法。

Method: 提出BlackCAtt黑盒算法，结合因果像素和目标检测器的边界框，创建导致边界框丢失、修改或新增的对抗攻击。该方法将检测器视为黑盒，使用最小因果充分像素集构建攻击。
...
TL;DR: CartoMapQA是一个专门评估视觉语言模型理解地图能力的基准数据集，包含2000多个样本，涵盖从符号识别到路径推理的多层次地图解读技能。评估发现现有模型在地图语义理解、地理空间推理和OCR方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在视觉-语言融合方面取得了进展，但其在地图解读方面的能力尚未得到充分探索。地图理解对于导航、地理搜索和城市规划等实际应用至关重要，因此需要专门的评估工具来推动模型在这一领域的发展。

Method: 研究者创建了CartoMapQA基准数据集，包含2000多个样本，每个样本由地图图像、问题（开放式或多选题）和真实答案组成。任务涵盖低、中、高三个层次的地图解读技能：符号识别、信息提取、比例尺解读和路径推理。使用该数据集对开源和专有视觉语言模型进行了系统评估。

Result: 评估发现当前视觉语言模型在地图理解方面存在持续挑战：模型经常难以理解地图特定语义，地理空间推理能力有限，且容易受到OCR相关错误的影响。这些弱点在不同类型的模型中普遍存在。

Conclusion: CartoMapQA为评估和改进视觉语言模型的地图理解能力提供了有价值的工具。通过识别模型的弱点，该基准有助于指导未来模型架构的改进，最终支持开发更适合依赖可靠地图理解的实际应用的模型。数据集和源代码已向研究社区开放。
...
TL;DR: 该研究质疑DeepSeek-OCR提出的视觉压缩方法对语言建模的有效性，发现简单替代方法在相同压缩率下表现相当或更好


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR展示了从少量视觉token重建渲染文本的能力，引发了视觉压缩在语言模型上下文压缩中的兴奋。但现有评估仅停留在重建层面，未测试这些表示是否真正有助于语言建模。本研究旨在验证视觉压缩叙事的两个隐含假设

Method: 将DeepSeek-OCR的视觉编码器与两种简单替代方法进行比较：参数无关的平均池化和学习型分层编码器。在匹配的压缩率下评估文本重建能力，并测试这些表示对语言建模任务的实际帮助

Result: 简单方法（平均池化和分层编码器）在文本重建方面与视觉方法相当或更优；对于语言建模任务，视觉压缩方法甚至无法超越简单的截断方法，表现不佳

Conclusion: 对光学上下文压缩的兴奋超过了实际证据支持。视觉压缩在语言建模中并未显示出独特优势，简单替代方法表现更好。代码和检查点已开源
...
TL;DR: CodeVision：一个基于代码作为通用工具的MLLM框架，通过两阶段训练（SFT+RL）提升模型对图像方向变化和自然损坏的鲁棒性，支持灵活工具组合和错误恢复。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）在工具推理方面存在局限性：1）依赖有限工具集，缺乏现实必要性和可扩展性；2）即使最先进的MLLMs在图像简单方向变化或自然损坏时性能显著下降，显示出鲁棒性不足的问题。

Method: 提出CodeVision框架，将代码作为通用接口来调用任何图像操作，超越固定工具注册表。采用两阶段训练方法：1）在高质量数据集上进行监督微调（SFT），该数据集针对复杂多轮工具组合和错误恢复而构建；2）使用新颖密集过程奖励函数进行强化学习（RL），鼓励战略性和高效的工具使用。

Result: 在Qwen2.5-VL和Qwen3-VL系列上的实验表明，该方法显著提升模型性能，并培养出新兴能力：灵活工具组合、高效链式执行、以及从运行时反馈中实现鲁棒的错误恢复。同时构建了新的SFT和RL数据集以及评估基准套件。

Conclusion: CodeVision通过代码作为通用工具的框架，解决了MLLMs在工具推理中的可扩展性和鲁棒性问题，为更灵活、稳健的多模态推理系统提供了有效解决方案。
...
TL;DR: 本文提出Stable Signer模型，通过简化传统手语生成流程，将SLP任务重新定义为包含文本理解和Pose2Vid的端到端分层生成任务，显著提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统手语生成(SLP)流程复杂，包含多个阶段（Text2Gloss、Gloss2Pose、Pose2Vid等），各阶段存在文本转换不准确、姿态生成错误、视频渲染质量差等问题，导致误差累积，进展缓慢。

Method: 1. 简化传统冗余结构，将SLP重新定义为端到端分层生成任务，仅包含文本理解（Prompt2Gloss、Text2Gloss）和Pose2Vid两个主要阶段；2. 提出SLUL（Sign Language Understanding Linker）用于文本理解，使用新开发的SAGM Loss（Semantic-Aware Gloss Masking Loss）进行训练；3. 设计SLP-MoE手语渲染专家模块生成手势动作。

Result: 提出的Stable Signer模型相比当前SOTA生成方法性能提升了48.6%，能够端到端生成高质量、多风格的手语视频。

Conclusion: 通过简化传统SLP流程并引入SLUL和SLP-MoE模块，Stable Signer实现了更高效、准确的端到端手语视频生成，显著提升了生成质量和性能。
...
TL;DR: GLASTE：一种全局-局部感知的场景文本编辑框架，通过结合全局上下文和局部特征解决现有方法的不一致性和长度不敏感问题


<details>
  <summary>Details</summary>
Motivation: 现有场景文本编辑方法存在两个主要挑战：1）编辑后的局部块与周围区域不一致；2）对编辑前后文本长度差异不敏感。这些方法难以保持文本样式一致性，也无法处理长度显著变化的文本编辑。

Method: 提出GLASTE端到端框架，包含：1）全局-局部组合结构，同时整合高层全局上下文信息和精细局部特征；2）联合全局和局部损失函数；3）增强文本图像特征；4）将文本样式表示为与图像大小无关的向量；5）使用仿射融合保持目标文本图像的长宽比不变。

Result: 在真实世界数据集上的大量实验验证，GLASTE在定量指标和定性结果上都优于先前方法，有效缓解了不一致性和长度不敏感两个挑战。

Conclusion: GLASTE通过全局-局部感知的方法成功解决了场景文本编辑中的关键问题，实现了更一致、更灵活的文本编辑效果，能够处理不同长度的文本替换。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注静态高质量图像，忽略了时间退化和错误传播这两种关键失效模式——瞬态视觉损坏引发的幻觉会在后续帧中持续存在。在自动驾驶等安全关键应用中，视觉语言模型必须处理不完美条件下的连续视觉流。

Method: 1. 提出DIQ-H基准：应用基于物理的视觉退化（运动模糊、传感器噪声、压缩伪影）；2. 通过多轮问答任务测量幻觉持续性、错误恢复和时间一致性；3. 提出不确定性引导迭代优化（UIR）方法，使用轻量级VLM进行不确定性过滤生成可靠的伪地面真值。

Result: 1. UIR方法实现了15.3%的准确率提升；2. 在16个最先进的VLM上实验显示显著鲁棒性差距：GPT-4o仅达到78.5%的恢复率，开源模型的时间一致性低于60%；3. 揭示了现有模型在真实世界部署中的可靠性问题。

Conclusion: DIQ-H为评估视觉语言模型在真实世界部署中的可靠性提供了全面平台，揭示了现有模型在处理动态视觉退化时的严重不足，为未来模型改进提供了重要基准。
...
Method: 引入信息独特性概念来衡量token间的内在冗余，设计三个模块：帧组融合、token分配和空间动态压缩，逐步进行语义帧分组、自适应资源分配和细粒度空间压缩。

Result: 大量实验表明，UniComp在有限计算预算下始终优于现有压缩方法，能更好地保留关键视觉token，证明了信息独特性在token压缩中的关键作用。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法主要关注搜索策略和奖励模型，但忽视了文本到图像扩散模型中噪声随机性对方法性能的影响。本文旨在探索文本嵌入扰动这种新的随机性形式，以提升生成效果。
...
Conclusion: 文本嵌入扰动与空间噪声在频域上表现出互补行为，能够增强生成多样性和质量，为文本到图像扩散模型的测试时缩放提供了新的随机性形式。
...
TL;DR: 该论文提出Cross-Stain Contrastive Learning (CSCL)框架，通过多染色对比学习提升全切片图像表示质量，解决多染色数据对齐问题。


<details>
  <summary>Details</summary>
Motivation: 计算病理学需要通用、可迁移的全切片图像表示。结合多种染色（如免疫组化）可以丰富H&E特征，但多染色数据集稀缺且存在染色间错位问题，阻碍了稳定特征提取。

Method: 1) 构建五染色对齐数据集（H&E、HER2、KI67、ER、PGR）；2) 提出CSCL两阶段预训练框架：第一阶段使用轻量适配器进行补丁级对比对齐，第二阶段使用多实例学习进行切片级表示学习，包含跨染色注意力融合模块和全局对齐模块。

Result: 在癌症亚型分类、IHC生物标志物状态分类和生存预测任务上均取得显著提升，生成了高质量、可迁移的H&E切片级表示。

Conclusion: CSCL框架有效解决了多染色数据对齐问题，通过对比学习提升了H&E特征的兼容性和表示质量，为计算病理学提供了更强大的表示学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在长视频理解中面临上下文长度限制和密集视频token处理成本高的问题。现有查询感知帧选择方法计算开销大，作者质疑这种复杂搜索机制是否普遍必要。

Method: 首先识别并验证查询类型学（全局查询vs局部查询），然后提出DIG框架：对全局查询使用高效均匀采样，对局部查询激活专门管道提取查询相关帧。
...
TL;DR: DOT-BI是一种利用人类运动感知来区分人类和机器人的动态光学测试方法，通过隐藏数字与背景的纹理相同但运动差异来识别人类


<details>
  <summary>Details</summary>
Motivation: 需要一种快速简便的方法来在调查和在线流程中区分人类受访者和自动化系统，解决传统验证方法被AI绕过的问题

Method: 使用隐藏数字与背景具有相同随机黑白像素纹理，但通过数字与背景之间的运动和尺度差异使数字对人类可见，而逐帧算法处理无法提取有意义信号

Result: 1) 最先进的视频多模态模型（GPT-5-Thinking和Gemini 2.5 Pro）无法提取正确值；2) 在线调查中99.5%参与者成功完成任务，平均完成时间10.7秒；实验室研究显示相对于对照组没有负面用户体验影响

Conclusion: DOT-BI是一种有效区分人类和机器人的方法，对用户友好且能抵抗先进AI模型的攻击，已发布代码和预渲染变体以促进采用
...
TL;DR: 研究探讨了草图表示学习中时间顺序的重要性，比较了绝对坐标与相对坐标、自回归与非自回归解码器的性能差异，发现时间重要性取决于具体顺序和任务。


<details>
  <summary>Details</summary>
Motivation: 尽管草图表示学习领域已有显著进展，但时间顺序对表示质量的实际重要性仍不明确。本研究旨在探究将草图视为序列是否合理，以及哪些内部顺序更为重要。

Method: 通过比较不同顺序表示方法（绝对坐标vs相对坐标）和解码器类型（自回归vs非自回归）来评估草图序列建模的有效性。

Result: 结果显示：1）传统位置编码对草图序列建模有效；2）绝对坐标始终优于相对坐标；3）非自回归解码器优于自回归解码器；4）时间重要性取决于具体顺序和评估任务。

Conclusion: 草图确实可以作为序列处理，但时间顺序的重要性具有条件性，需要根据具体顺序和任务来评估。绝对坐标和非自回归方法在草图表示学习中表现更优。
...
Method: 提出金字塔稀疏注意力（PSA），引入多级池化的KV表示，每个查询块动态分配较低的池化级别给关键KV块，较高的级别给次要KV块，在完全保留和完全剪枝之间创建信息插值。采用解耦的块-瓦片设计和硬件友好的原生内核实现高效执行。

Result: 在视频理解和生成基准测试中，PSA保持了上下文信息和视觉保真度，在效率和质量的权衡上优于或达到现有稀疏注意力基线的性能。

Conclusion: PSA通过多级池化KV表示有效缓解了高稀疏度下的信息损失问题，在保持计算效率的同时提供了更好的效率-质量权衡，适用于视频理解和生成任务。
...
TL;DR: HyperRNA是一个基于超图的生成模型，用于解决RNA逆折叠问题，通过编码器-解码器架构生成能够折叠成特定二级结构的RNA序列。


<details>
  <summary>Details</summary>
Motivation: RNA逆折叠问题是RNA设计中的关键挑战，涉及识别能够折叠成所需二级结构的核苷酸序列，这对确保分子稳定性和功能至关重要。该任务的复杂性源于序列与结构之间的复杂关系。

Method: HyperRNA框架包含三个主要组件：预处理阶段基于3-bead粗粒度表示构建RNA骨架原子坐标的图结构；编码阶段使用注意力嵌入模块和超图编码器处理这些图，捕捉高阶依赖和复杂生物分子相互作用；解码阶段以自回归方式生成RNA序列。

Result: 在PDBBind和RNAsolo数据集上进行的定量和定性实验表明，HyperRNA在RNA序列生成和RNA-蛋白质复合物序列生成的逆折叠任务中优于现有RNA设计方法。
...
Method: 使用带纹理的平面集合表示化身，包含低频和高频两套可学习纹理。系统根据相机与化身表面的距离自动切换纹理：近距离使用高频纹理，远距离逐渐减少其影响。

Result: 在ActorsHQ高分辨率数据集上的实验表明，CloseUpAvatar在广泛的相机位置下渲染质量优于现有方法，同时通过限制基元数量保持高帧率。

Conclusion: CloseUpAvatar通过自适应纹理切换机制实现了在更广泛相机运动范围内的真实渲染，平衡了渲染质量和计算效率。
...
TL;DR: 该论文在生成模型效率提升和计算机视觉应用两方面做出贡献：改进标准化流架构的六项创新，以及五个实际应用系统，包括农业质量评估、地质测绘、隐私保护和艺术修复。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决两个核心问题：1) 提高生成模型（特别是标准化流）的计算效率和架构优化；2) 将生成模型应用于实际计算机视觉挑战，如农业质量评估、地质测绘、隐私保护和艺术修复等现实世界问题。

Method: 第一部分：提出六项标准化流架构改进：可逆3x3卷积层（提供可逆性数学证明）、更高效的Quad-coupling层、kxk卷积层的并行反演算法、卷积逆的反向传播算法、使用卷积逆的Inverse-Flow前向传播、基于预训练权重和标准化流层的紧凑超分辨率模型Affine-StableSR。

第二部分：五个应用系统：1) 使用条件GAN的农产品质量评估系统；2) 基于堆叠自编码器的无监督地质测绘框架；3) 基于人脸检测和图像修复的自动驾驶数据集隐私保护方法；4) 使用Stable Diffusion图像修复替换人脸和车牌；5) 适应多种退化类型的艺术修复扩散模型。

Result: 在标准化流架构方面，提出的改进显著提升了计算效率和模型性能；在实际应用方面，农业质量评估系统在种子纯度测试中达到良好准确率，地质测绘框架相比传统方法改进了特征提取，隐私保护和艺术修复方法也展示了有效性和实用性。

Conclusion: 论文成功地在生成模型理论改进和实际应用两方面都做出了重要贡献：一方面通过六项创新显著提升了标准化流的效率和性能，另一方面开发了五个实用的计算机视觉系统，展示了生成模型在解决现实世界问题中的强大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 牙齿点云常因遮挡和有限扫描视角存在大范围缺失区域，这会偏置编码器的全局特征并迫使解码器产生幻觉结构
...
TL;DR: Motion4D：将2D基础模型先验整合到4D高斯泼溅表示中，解决动态场景分析中的3D不一致问题，实现时空一致的场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有的2D视觉基础模型在单目视频动态场景分析中取得了进展，但缺乏3D一致性，导致复杂3D环境中的空间错位和时间闪烁问题，限制了场景几何和运动理解能力。

Method: 提出Motion4D框架，采用两部分迭代优化：1）顺序优化，分阶段更新运动和语义场保持局部一致性；2）全局优化，联合优化所有属性实现长期一致性。引入3D置信度图动态调整运动先验，基于像素级RGB和语义误差的自适应重采样过程，以及通过交替优化语义场和更新SAM2提示的迭代细化过程增强语义一致性。

Result: Motion4D在多种场景理解任务（包括基于点的跟踪、视频对象分割和新视角合成）上显著优于2D基础模型和现有3D方法。

Conclusion: Motion4D成功将2D基础模型先验整合到统一的4D高斯泼溅表示中，有效解决了动态场景分析中的3D不一致问题，为复杂3D环境中的时空一致场景理解提供了有效解决方案。
...
TL;DR: LAMP利用大语言模型作为运动规划器，将自然语言描述转换为3D轨迹，实现视频生成中的运动控制


<details>
  <summary>Details</summary>
Motivation: 当前视频生成在运动控制方面存在局限，特别是对于复杂电影场景中的物体动态和相机轨迹控制，需要更自然直观的控制接口

Method: 提出LAMP框架，定义运动领域特定语言，利用LLM的程序合成能力将自然语言转换为结构化运动程序，再确定性地映射到3D轨迹，并构建大规模程序化数据集

Result: 相比现有方法，LAMP在运动可控性和用户意图对齐方面表现更好，首次建立了从自然语言直接生成物体和相机运动的框架

Conclusion: LAMP通过结合LLM的程序合成能力和运动DSL，为视频生成提供了更自然、精确的运动控制接口，推动了视频生成中运动控制技术的发展
...
TL;DR: ReCamDriving是一个纯视觉的相机控制新轨迹视频生成框架，利用密集的3DGS渲染进行几何引导，通过两阶段训练和跨轨迹数据策展策略，实现了精确的相机可控生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于修复的方法无法恢复复杂伪影，基于LiDAR的方法依赖稀疏且不完整的线索。需要一种能够实现精确相机控制且具有结构一致性的视频生成方法。

Method: 1. 利用密集且场景完整的3DGS渲染提供显式几何引导；2. 采用两阶段训练范式：第一阶段使用相机姿态进行粗略控制，第二阶段结合3DGS渲染进行细粒度视角和几何引导；3. 提出基于3DGS的跨轨迹数据策展策略，消除相机变换模式的训练-测试差距。

Result: 构建了ParaDrive数据集，包含超过11万对平行轨迹视频对。实验表明ReCamDriving在相机可控性和结构一致性方面达到了最先进水平。

Conclusion: ReCamDriving通过3DGS渲染的几何引导和创新的训练策略，成功实现了精确的相机可控视频生成，解决了现有方法的局限性，为从单目视频进行可扩展的多轨迹监督提供了有效方案。
...
TL;DR: Colon-X是一个推进结肠镜多模态智能的开源项目，包含ColonVQA数据集（110万+视觉问答条目）和ColonReason推理数据集，开发了ColonR1模型，在数据稀缺条件下比监督微调提升25.22%准确率。


<details>
  <summary>Details</summary>
Motivation: 推动结肠镜多模态智能发展，填补从多模态理解到临床推理的空白，解决当前多模态大语言模型在结肠镜应用中鲁棒性和可信度不足的问题。

Method: 1) 构建ColonVQA多模态数据集（76个临床发现，18个多模态任务）；2) 评估22个MLLM模型的泛化能力和鲁棒性；3) 通过多专家辩论流程标注ColonReason推理数据集；4) 开发ColonR1模型，采用任务自适应奖励和梯度稳定优化技术。

Result: 1) 当前领先MLLM的临床输出远未达到鲁棒可信；2) ColonR1在数据稀缺条件下达到56.61%总体准确率，比监督微调提升25.22%；3) 为多模态结肠镜分析建立了新的推理基准。

Conclusion: Colon-X项目成功构建了结肠镜领域最全面的多模态数据集和首个推理专用模型，填补了从多模态理解到临床推理的空白，为社区提供了重要的数据基础和新的推理基准。
...
TL;DR: 提出ConvRot方法，通过分组旋转量化技术解决扩散变换器内存占用大、推理延迟高的问题，实现W4A4推理，在FLUX.1-dev上获得2.26倍加速和4.05倍内存减少。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在生成高质量图像方面表现出色，但随着模型规模增大，内存占用和推理延迟显著增加，给实际部署带来挑战。现有旋转量化方法存在开销大、难以处理行向异常值的问题。

Method: 提出ConvRot方法，采用分组旋转量化技术，利用正则Hadamard变换抑制行向和列向异常值，将复杂度从二次降低到线性。在此基础上设计ConvLinear4bit模块，集成旋转、量化、GEMM和反量化，实现无需重新训练的W4A4推理。

Result: 在FLUX.1-dev上的实验结果显示，实现了2.26倍的推理加速和4.05倍的内存减少，同时保持了图像保真度。这是首次将旋转量化技术应用于扩散变换器的即插即用W4A4推理。

Conclusion: ConvRot方法有效解决了扩散变换器的部署挑战，通过创新的旋转量化技术实现了高效的低比特推理，为大规模扩散模型的实用化部署提供了可行方案。
...
TL;DR: GaussianBlender是一个前馈框架，通过从3D高斯中学习解耦的几何和外观潜在空间，结合潜在扩散模型实现文本驱动的3D风格化，无需逐实例优化即可获得多视角一致的结果。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D风格化方法通常从2D图像编辑器蒸馏而来，需要耗时的逐资产优化，并且由于当前文本到图像模型的限制存在多视角不一致问题，这使得它们不适用于大规模生产。
...
<details>
  <summary>Details</summary>
Motivation: 被动视觉系统在复杂环境中可能无法提供足够信息，而主动视觉感知通过动态交互能获取更丰富数据，在机器人、自动驾驶、人机交互和监控等领域具有重要应用价值。

Method: 本文采用综述分析方法，系统探讨主动视觉感知的概念、原理和应用，分析其核心机制包括注意力引导、传感器移动和物体交互等动态信息获取策略。

Result: 主动视觉感知在复杂环境中展现出显著优势，但面临实时处理复杂视觉数据、动态环境决策和多模态传感融合等关键挑战，需要进一步研究解决。

Conclusion: 主动视觉感知具有广阔应用前景，但要实现更广泛采用，需要克服当前的技术挑战，包括实时处理能力、动态决策机制和多模态集成等问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有感知相似度评分方法存在局限性：基于深度特征的方法（如LPIPS）虽然与人类视觉对齐良好，但依赖复杂的非线性特征且缺乏可解释性；手工设计的度量（如SSIM）虽然可解释但缺乏关键感知特性。需要一种既与人类感知对齐又具有可解释性的方法。

Method: 提出结构化不确定性相似度评分（SUSS）：1）将图像建模为一组感知组件，每个组件用结构化多元正态分布表示；2）通过生成式自监督训练，使模型为人类不可察觉的增强分配高似然；3）最终评分是组件对数概率的加权和，权重从人类感知数据集中学习；4）在像素空间学习图像特定的线性残差变换，实现透明检查。

Result: SUSS与人类感知判断高度对齐，在不同失真类型上表现出强大的感知校准能力，提供局部化、可解释的相似度评估解释。作为感知损失函数在下游成像任务中表现出稳定的优化行为和竞争性性能。

Conclusion: SUSS成功解决了现有感知相似度评分方法的局限性，提供了一种既与人类视觉对齐又具有可解释性的新方法，通过结构化概率建模和像素空间线性变换实现了透明性和可检查性。
...
TL;DR: DINO-RotateMatch是一个深度学习框架，通过结合数据集自适应图像配对策略和旋转感知关键点提取匹配，解决大规模3D重建中的图像匹配挑战。
...
Conclusion: 结合自监督全局描述符和旋转增强的局部匹配方法，为大规模3D重建提供了鲁棒且可扩展的解决方案。
...
### [67] [PosA-VLA: Enhancing Action Generation via Pose-Conditioned Anchor Attention](https://arxiv.org/abs/2512.03724)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在具身任务中虽然表现良好，但会产生冗余和不稳定的动作，限制了在时间敏感场景中的应用。作者认为这是由于现有VLA模型的空间均匀感知场导致其容易被目标无关物体分散注意力。

Method: 提出PosA-VLA框架，通过姿态条件监督锚定视觉注意力，引导模型感知任务相关区域。该机制使模型能更好地对齐指令语义和可操作的视觉线索，采用轻量级架构且无需辅助感知模块。

Result: 在多种机器人操作基准测试中，该方法能够以精确且时间高效的方式执行具身任务，在各种挑战性环境中展现出强大的泛化能力。

Conclusion: 通过姿态条件锚定注意力机制，PosA-VLA框架有效解决了VLA模型在复杂环境中产生冗余动作的问题，提高了动作生成的精确性和效率，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在大型数据集（如LAION-5B）上训练，这些数据集包含大量偏见，导致模型学习并复制这些偏见，产生刻板印象的输出。现有方法通常需要人工标注数据集或针对每个生成概念训练外部分类器，限制了其应用范围。

Method: SelfDebias在推理过程中识别图像编码器嵌入空间中的语义聚类，并使用这些聚类来引导扩散过程，最小化输出分布与均匀分布之间的KL散度。该方法完全无监督，不需要人工标注数据集或外部分类器，能够自动识别语义模式。

Result: 实验表明SelfDebias能够泛化到不同的提示词和扩散模型架构（包括条件模型和无条件模型）。它不仅有效地在关键人口统计学维度上去偏图像，同时保持生成图像的视觉保真度，还能处理更抽象的概念，这些概念的偏见识别同样具有挑战性。

Conclusion: SelfDebias提供了一种完全无监督的测试时去偏方法，能够有效减少扩散模型中的偏见，同时保持图像质量，适用于各种扩散模型架构和概念类型。
...
TL;DR: LSRS方法通过在推理过程中对潜在尺度进行渐进式拒绝采样，有效缓解VAR模型中的结构错误，显著提升生成质量且计算开销很小。
...
Method: 提出潜在尺度拒绝采样(LSRS)，在推理过程中使用轻量级评分模型评估每个尺度采样的多个候选token图，选择高质量图指导后续尺度生成，优先处理对结构一致性关键的前期尺度。

Result: LSRS显著提升VAR生成质量且计算开销很小：VAR-d30模型推理时间仅增加1%时FID从1.95降至1.78；推理时间增加15%时FID可进一步降至1.66。
...
TL;DR: 提出了一种从图像自动识别和翻译古埃及象形文字为英语的方法，使用分割、映射和翻译三阶段流程，在BLEU评分上达到42.2的显著结果。


<details>
  <summary>Details</summary>
Motivation: 古埃及象形文字完全由图画组成，翻译面临单个符号多义性等挑战。深度学习翻译应用发展迅速，能够显著影响生活，因此研究自动识别和翻译方法具有重要意义。
...
Conclusion: 提出的方法能够有效实现古埃及象形文字从图像到英语的自动识别和翻译，在翻译质量上取得了显著进展。
...
TL;DR: 提出一种更稳健的基于视频测量呼吸率的方法，在受试者运动干扰下仍能保持高精度，平均绝对误差仅0.57次呼吸/分钟。


<details>
  <summary>Details</summary>
Motivation: 现有视频呼吸率测量方法要么在理想条件下测试，要么精度不足，需要一种更稳健的方法来应对受试者运动等实际干扰。

Method: 结合数学变换的方法，硬件要求低，通过视频分析测量呼吸率，对运动失真具有更强抵抗力。

Result: 在14名志愿者超过2.5小时的视频测试中，相对地面真值偏差小于5%，平均绝对误差为0.57次呼吸/分钟，优于先前工作。

Conclusion: 该方法能远程测量呼吸率，对受试者行为限制少，在实际应用中具有更好的鲁棒性和准确性。
...
### [72] [CoDA: From Text-to-Image Diffusion Models to Training-Free Dataset Distillation](https://arxiv.org/abs/2512.03844)
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法面临两个根本限制：1）大多数方法需要在整个目标数据集上预训练的扩散模型，违背了数据集蒸馏的初衷且训练成本高昂；2）使用通用文本到图像模型的方法存在分布不匹配问题，无法准确捕捉目标特定语义。

Method: 提出核心分布对齐（CoDA）框架，首先通过鲁棒的基于密度的发现机制识别目标数据集的"内在核心分布"，然后引导生成过程使生成的样本与该核心分布对齐，从而弥合通用生成先验与目标语义之间的差距。

Result: 在不依赖目标数据集特定训练生成模型的情况下，CoDA在所有基准测试（包括ImageNet-1K及其子集）上达到或超过了依赖此类模型的方法性能，在ImageNet-1K的50-images-per-class设置下取得了60.4%的最新SOTA准确率。

Conclusion: CoDA通过核心分布对齐有效解决了现有数据集蒸馏方法的局限性，无需目标数据集特定训练即可生成高质量蒸馏数据集，为数据集蒸馏提供了更实用和高效的解决方案。
...
TL;DR: 该研究发布了来自伊拉克埃尔比勒的339张前列腺穿刺活检全切片图像数据集，包含185名患者的数据，旨在解决当前病理AI数据集主要代表西方人群、缺乏中东地区数据的问题。


<details>
  <summary>Details</summary>
Motivation: 当前公开的病理学数据集稀缺且主要代表西方人群，AI模型对中东等数字化程度较低地区的泛化能力未知，因此需要发布多样化的数据集来支持全球不同人群的病理AI模型开发和验证。

Method: 收集了伊拉克埃尔比勒185名患者的339张前列腺穿刺活检全切片图像，由三位病理学家独立分配Gleason评分和国际泌尿病理学会分级。使用两种高通量扫描仪（Leica和Hamamatsu）和一种紧凑型扫描仪（Grundium）进行扫描，所有切片都经过去标识化处理并以原生格式提供。

Result: 创建了一个包含339张前列腺活检全切片图像的数据集，来自中东地区（伊拉克）的185名患者，包含三位病理学家的独立分级标注，支持分级一致性分析、颜色归一化和跨扫描仪鲁棒性评估。

Conclusion: 该数据集填补了中东地区病理学数据的空白，有助于开发更具全球代表性的病理AI模型，促进分级一致性分析、颜色归一化和跨扫描仪鲁棒性评估等研究，数据将在Bioimage Archive中公开并采用CC BY 4.0许可。
...
Method: 使用三种不同的预训练、中间微调和下游数据集及训练目标，在小型5M参数视觉Transformer上进行实验，探索这些训练策略的边际效益。

Result: 研究发现：1）预训练和微调总是有助于模型性能，但存在收益递减现象；2）中间微调可能对下游性能产生有害影响，这可能是由于任务机制的不相似性导致的；3）预训练和中间微调的组合效果表明，不加选择地堆叠中间任务可能浪费计算资源甚至降低性能。

Conclusion: 小型视觉Transformer最受益于有针对性的预训练和谨慎的数据选择，而不加选择地堆叠中间任务可能浪费计算资源并降低性能。这为资源受限环境下的模型训练提供了重要指导。
...
TL;DR: CaravelMetrics是一个用于自动脑血管分析的计算框架，通过骨架化图表示建模血管形态，可提取15种形态、拓扑、分形和几何特征，支持全局和区域多尺度分析。
...
Result: 应用于IXI数据集的570个3D TOF-MRA扫描（年龄20-86岁），产生可重复的血管图，捕获了年龄和性别相关变异，以及教育相关的血管复杂性增加，与文献报道一致。
...
TL;DR: 开发了基于Siamese Swin Transformer的双交叉注意力模型（SSDCA），用于结合直肠癌患者治疗前后的纵向内窥镜图像，准确区分完全缓解与局部复发，实现早期检测。


<details>
  <summary>Details</summary>
Motivation: 直肠癌患者在接受新辅助治疗后，对于临床完全缓解的患者采用"观察等待"策略已成为趋势。然而，在随访期间需要客观准确的方法从内窥镜图像中早期检测局部复发，以管理护理并预防远处转移。
...
Result: SSDCA在62名患者的测试集上表现最佳：平衡准确率81.76%±0.04，敏感性90.07%±0.08，特异性72.86%±0.05。对血液、粪便、毛细血管扩张和图像质量差等伪影具有稳定性能。UMAP聚类显示最大化的簇间分离和最小化的簇内分散。

Conclusion: SSDCA模型能够有效结合纵向内窥镜图像，准确区分直肠癌患者的完全缓解与局部复发，为"观察等待"策略提供了可靠的早期检测工具，具有临床应用的潜力。
...
TL;DR: FRESCO通过整合帧内和帧间对应关系来增强时空一致性约束，在零样本视频编辑任务中显著提升视觉连贯性


<details>
  <summary>Details</summary>
Motivation: 现有的零样本方法主要关注在注意力机制中集成帧间对应关系，但软约束不足以识别有效特征，可能导致时间不一致性

Method: 提出FRESCO框架，整合帧内对应关系和帧间对应关系，形成更鲁棒的时空约束，确保语义相似内容在帧间一致转换，并显式优化特征

Result: 在视频到视频转换和文本引导视频编辑两个零样本任务上验证，实验表明FRESCO能生成高质量、连贯的视频，显著优于现有零样本方法

Conclusion: 通过整合帧内和帧间对应关系，FRESCO实现了更好的时空一致性，为零样本视频编辑任务提供了更有效的解决方案
...
Conclusion: 这项工作挖掘了LLMs融合多样化数据类型的能力，为将人体中心信息集成到现有模型中铺平了道路，并可能实现人体、物体和场景的多模态、可控联合建模。
...
<details>
  <summary>Details</summary>
Motivation: 增强多模态大语言模型（MLLMs）的时序理解能力对于推进长视频分析至关重要，包括时序定位、动作检测和时间敏感问答等任务。现有基于强化学习的方法通常局限于有限的任务类型和数据，难以泛化到多样化的时序理解场景。

Method: 提出TempR1框架：1）构建多任务语料库，让模型接触多样化的时序结构和语义；2）基于分组相对策略优化（GRPO）算法实现稳定有效的跨任务优化；3）将时序任务分为预测区间与真实实例之间的三种对应类型，并为每种类型设计定制的定位奖励，使模型能够捕捉细粒度时序依赖并适应不同的时序模式。

Result: 大量实验表明，TempR1在多个基准测试中达到了最先进的性能。更重要的是，其在互补任务上的联合优化产生了强大的协同效应，既增强了泛化能力，又提升了单任务性能。

Conclusion: TempR1为MLLMs中的时序推理建立了一个可扩展且原则性的范式，通过多任务强化学习框架系统性地增强了模型的时序理解能力。
...
TL;DR: UniID提出了一种无需微调的人脸个性化统一框架，通过整合文本嵌入和适配器两种方法，实现了高身份保真度和灵活文本控制性的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有无需微调的人脸个性化方法分为文本嵌入和适配器两种范式，但都难以同时实现高身份保真度和灵活文本控制性。需要一种统一框架来协同整合这两种方法的优势。

Method: 提出UniID框架，采用原则性的训练-推理策略：训练时使用身份聚焦学习方案，引导两个分支专门捕获身份特征；推理时引入归一化重缩放机制，恢复基础扩散模型的文本控制性，同时让互补的身份信号相互增强。

Result: 在六个最先进方法的广泛实验中，UniID在身份保持和文本控制性方面都表现出优越性能。

Conclusion: UniID通过统一整合文本嵌入和适配器两种范式，实现了无需微调的高保真人脸个性化，同时保持了灵活的文本控制能力。
...
Result: 尽管无需手动掩码或提示，DirectDrag在图像质量上优于现有方法，同时在拖拽精度上保持竞争力，在DragBench和真实场景实验中表现出有效性

Conclusion: DirectDrag提供了一种高质量、交互式的图像操作方法，通过最小化用户输入实现精确高效的图像编辑，同时保持高图像保真度和准确的点对齐
...
### [82] [Emergent Outlier View Rejection in Visual Geometry Grounded Transformers](https://arxiv.org/abs/2512.04012)
*Jisang Han,Sunghwan Hong,Jaewoo Jung,Wooseok Jang,Honggyu An,Qianqian Wang,Seungryong Kim,Chen Feng*

Main category: cs.CV

TL;DR: 研究发现现有前馈3D重建模型VGGT即使没有显式离群点拒绝机制，也能内在区分干扰图像，通过特定层的内部表征实现有效的噪声过滤。


<details>
  <summary>Details</summary>
Motivation: 在野外图像集合中进行可靠的3D重建常受"噪声"图像干扰，这些图像与其他图像几乎没有视角重叠。传统SfM流水线通过几何验证和离群点拒绝处理这种情况，但前馈3D重建模型缺乏这些显式机制，导致在野外条件下性能下降。

Method: 通过在不同比例合成干扰物下的深入分析，识别出VGGT模型中自然表现出离群点抑制行为的特定层。进一步探究发现该层编码了判别性内部表征，能够实现有效的噪声过滤能力，无需额外微调或监督即可用于前馈3D重建中的离群视图拒绝。

Result: 在受控和野外数据集上的大量实验表明，这种隐式过滤机制具有一致性，并能很好地泛化到多样化场景中。

Conclusion: 前馈3D重建模型即使没有显式离群点拒绝机制，也具备内在的干扰图像区分能力，通过利用特定层的内部表征可以实现有效的噪声过滤，提升在野外条件下的重建性能。
...
Conclusion: 提出的框架能够自动学习潜在图像表示中的群作用，无需人工干预，为可控图像变换提供了新方法。
...
TL;DR: NVRC-Lite：轻量级神经视频压缩框架，通过多尺度特征网格和八叉树上下文模型，在保持低复杂度同时提升性能并加速编码


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示（INR）的视频压缩方法中，NVRC虽然性能优异但计算复杂，而轻量级INR虽然复杂度低但性能有限。同时，现有INR通常使用自回归模型进行熵编码，虽然有效但编码速度慢，不实用。
...
Conclusion: NVRC-Lite成功扩展了NVRC框架到轻量级表示，通过多尺度特征网格和八叉树上下文模型，在保持低计算复杂度的同时显著提升了压缩性能并大幅加速了编码过程。
...
TL;DR: C3G提出了一种前馈框架，通过可学习token引导生成紧凑的3D高斯，减少冗余并提升多视图特征聚合，在无姿态新视图合成和3D场景理解任务中表现优异。
...
Conclusion: 紧凑但几何意义明确的表示足以实现高质量场景重建和理解，证明了该方法在内存效率和特征质量方面的优势。
...
<details>
  <summary>Details</summary>
Motivation: 构建真正交互式世界模型需要三个关键要素：实时长时程流式生成、一致的空间记忆和精确的用户控制。现有方法通常只能单独解决其中一个方面，同时实现这三个目标极具挑战性，例如长时记忆机制往往会降低实时性能。

Method: 基于自回归视频扩散蒸馏技术，使用高度压缩的历史潜在token（包含相对动作和绝对相机姿态）在KV缓存中表示长时程记忆。通过微调双向教师视频模型生成超出原始5秒训练时长的序列，并采用新的内存高效自强制范式将其转化为因果学生生成器，实现全上下文蒸馏。

Result: RELIC作为140亿参数模型，在精心策划的Unreal Engine渲染数据集上训练，实现了16FPS的实时生成，相比先前工作表现出更准确的动作跟随、更稳定的长时程流式生成和更鲁棒的空间记忆检索。

Conclusion: RELIC的能力为下一代交互式世界建模奠定了坚实基础，统一解决了实时长时程流式生成、空间记忆和用户控制这三个关键挑战。
...
TL;DR: PosterCopilot是一个基于大型多模态模型的图形设计框架，通过渐进式三阶段训练策略提升布局几何准确性和审美推理能力，支持分层可控的迭代编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于大型多模态模型的自动化图形设计方法存在几何布局不准确、缺乏专业工作流程所需的分层迭代编辑能力等问题，需要开发更精确可控的设计框架。
...
Conclusion: PosterCopilot通过创新的训练策略和工作流程，解决了现有自动化图形设计方法的局限性，实现了专业级的分层可控迭代编辑能力。
...
TL;DR: SimFlow通过固定VAE编码器的方差为常数，简化了归一化流与变分自编码器的联合训练，避免了复杂的噪声添加和去噪流程，在ImageNet生成任务上取得了SOTA性能。
...
Method: 提出SimFlow方法，将VAE编码器的方差固定为常数（如0.5），而不是让编码器预测方差。这种方法允许编码器输出更广泛的token分布，解码器学习从增强的token分布中重建干净图像，同时简化了VAE证据下界，使NF与VAE能够稳定地联合训练。
...
Conclusion: 通过固定VAE方差这一简单修改，SimFlow有效解决了现有NF方法的两个关键限制，实现了更简单的训练流程和更好的生成质量，为归一化流与变分自编码器的联合训练提供了有效解决方案。
...
TL;DR: 该研究将伦理熵作为衡量LLM安全性的状态变量，开发了分类器来监测伦理熵动态变化，发现基础模型熵持续增长而调优模型能降低约80%伦理熵，并建立了实时监控管道来预警价值漂移。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型安全评估主要依赖静态基准测试，但关键失效模式是动态的：分布漂移下的价值漂移、越狱攻击以及部署中安全对齐的缓慢退化。需要建立动态监控框架来应对这些实时风险。

Method: 基于智能第二定律将伦理熵作为状态变量，定义了五维行为分类法，训练分类器从模型转录中估计伦理熵S(t)，在四个前沿模型的基础版和指令调优版上进行压力测试，测量熵动态变化，并估计有效对齐工作率gamma_eff。

Result: 基础模型显示持续的伦理熵增长，而调优变体能抑制漂移并将伦理熵降低约80%。从这些轨迹中估计出有效对齐工作率，并将S(t)和gamma_eff嵌入监控管道，当熵漂移超过稳定阈值时发出警报。

Conclusion: 该研究使伦理熵框架可操作化，为LLM提供了动态安全监控方法，能够实时监督价值漂移，解决了静态基准测试无法捕捉的动态失效问题，为部署中的模型安全提供了运行时的监督机制。
...
TL;DR: 该论文研究了Embeddings-as-a-Service（EaaS）水印技术，揭示了现有水印易受改写攻击的漏洞，并提出了一种基于线性变换的新型水印方案WET来防御模仿攻击。


<details>
  <summary>Details</summary>
Motivation: 随着企业提供Embeddings-as-a-Service（EaaS），模型知识产权保护变得重要。现有EaaS水印技术存在安全漏洞，需要更强大的水印方案来防御模仿攻击。

Method: 首先通过改写攻击测试现有EaaS水印的脆弱性，然后提出WET水印技术，采用线性变换嵌入向量，通过反向变换和相似度比较进行水印验证。

Result: 改写攻击能有效绕过当前最先进的EaaS水印；提出的WET水印对改写攻击具有近乎完美的可验证性，并通过消融研究验证了各组件和超参数的重要性。

Conclusion: 现有EaaS水印存在改写攻击漏洞，WET水印技术能有效防御此类攻击，为EaaS提供商提供了更强大的知识产权保护方案。
...
TL;DR: 提出Reasoning Dependency Generation (RDG)框架，通过生成平衡推理数据来减轻大型语言模型中的选择支持性偏见，实验显示在记忆和评估任务上分别有81.5%和94.3%的改进。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明大型语言模型存在选择支持性偏见，这会损害AI辅助决策的客观性。现有的去偏见方法主要针对人口统计和社会偏见，而针对认知偏见的方法尚未充分探索。

Method: 提出Reasoning Dependency Generation (RDG)框架，通过自动构建平衡的推理问答对，显式地建模和解耦选择、证据和理由之间的依赖关系，生成大规模跨领域数据集用于微调。

Result: 使用RDG生成数据微调的LLM在记忆实验上改进81.5%，在评估实验上改进94.3%，同时在标准BBQ基准测试上保持相似性能。

Conclusion: RDG是首个解决LLM中认知偏见的方法，为开发更可靠的AI辅助决策支持系统做出了贡献，开创了处理LLM认知偏见的新途径。
...
TL;DR: InvertiTune框架通过受控数据生成和监督微调，实现高效的单次知识图谱构建，超越现有方法并展示更强的跨数据集泛化能力
...
Method: 提出InvertiTune框架：1）从大型知识库系统提取子图并过滤噪声；2）利用LLM生成对应的自然文本描述；3）通过监督微调训练轻量模型进行单次KG构建

Result: 在CE12k数据集上，InvertiTune超越更大的非微调LLM和SOTA Text2KG方法，在CrossEval-1200测试集上展示更强的跨数据集泛化能力

Conclusion: 高质量、现实的训练数据对推进高效高性能Text2KG系统至关重要，InvertiTune框架为此提供了有效解决方案
...
TL;DR: 本文提出了一种从政治文本中检测和解析解释的框架，使用轻量级因果语言模型提取因果主张，形成结构化数据集供下游分析使用。


<details>
  <summary>Details</summary>
Motivation: 解释是人们理解政治世界的基本要素，公民经常询问和回答关于事件原因、责任归属以及改进措施的问题。然而，尽管解释如此重要，在政治科学中它们仍然是系统分析中发展不足的对象，现有方法分散且通常是针对特定问题的。

Method: 作者训练了一个轻量级的因果语言模型，能够从政治文本中返回结构化数据集，以因果对的形式提取因果主张。该方法具有适度的标注要求、良好的泛化能力和相对于人工编码的准确性。

Result: 展示了如何在规模上研究因果解释，证明了该方法在标注要求、泛化能力和准确性方面的优势，能够有效地从政治文本中提取和分析因果主张。

Conclusion: 该框架为政治科学中解释的系统分析提供了有效工具，填补了现有方法的空白，使研究人员能够在更大规模上研究和理解政治文本中的因果解释。
...
TL;DR: 提出了一种名为随机掩码微调（RMFT）的新型隐私保护微调技术，可显著减少大型语言模型对个人身份信息（PII）的记忆，同时最小化性能影响。


<details>
  <summary>Details</summary>
Motivation: 当前自然语言模型（特别是大型语言模型）对训练数据中个人身份信息的记忆带来了严重的安全和隐私风险，需要有效的隐私保护技术来平衡隐私与模型性能。

Method: 提出了随机掩码微调（RMFT）技术，这是一种隐私保护的微调方法。同时开发了MaxTER评估框架，用于评估隐私-效用权衡，并使用AURC指标比较RMFT与去重方法的性能。

Result: 在Enron电子邮件数据集上的实验表明，RMFT相比基线微调实现了80.81%的总提取率降低和80.17%的已见提取率降低，性能优于去重方法，仅导致困惑度增加5.73%。

Conclusion: RMFT是一种有效的隐私保护微调技术，能显著减少LLM对PII的记忆，同时保持模型性能，为隐私-效用权衡提供了实用的解决方案。
...
TL;DR: 本研究提出了一个LLM辅助的标注流程，用于分析西班牙语-英语和西班牙语-瓜拉尼语两种类型不同语境下的双语话语的社会语言学和主题特征，通过自动标注发现了性别、语言优势和话语功能之间的系统性联系。


<details>
  <summary>Details</summary>
Motivation: 传统的社会语言学研究依赖人工标注，难以扩展到大规模语料分析。本研究旨在利用大语言模型自动标注双语话语的社会语言学和主题特征，为跨语言和低资源双语研究提供计算方法的支持。

Method: 使用大语言模型自动标注了3,691个语码转换句子的主题、体裁和话语语用功能，整合了迈阿密双语语料库的人口统计元数据，并为西班牙语-瓜拉尼语数据集增加了新的主题标注。

Result: 在迈阿密数据中发现了性别、语言优势和话语功能之间的系统性联系；在巴拉圭文本中观察到正式的瓜拉尼语和非正式的西班牙语之间的明显双语分工；这些发现用大规模定量证据复制和扩展了早期的互动和社会语言学观察。
...
TL;DR: PERCS数据集：针对四种用户角色（普通大众、医学生、非医学研究者、医学专家）的个性化生物医学摘要简化数据集，支持可控的摘要生成研究
...
Method: 创建PERCS数据集，包含生物医学摘要及其针对四种角色的个性化总结；由医生使用详细的错误分类法进行事实准确性和角色对齐审查；对四种大语言模型进行基准测试

Result: 技术验证显示不同角色总结在可读性、词汇和内容深度上存在明显差异；建立了未来研究的基线结果；数据集、标注指南和评估材料已公开

Conclusion: PERCS数据集填补了针对特定受众的生物医学摘要简化研究空白，支持角色特定沟通和可控生物医学摘要生成的研究，有助于提高健康素养
...
TL;DR: 提出Idea-Gated Transformer架构，通过分离语义规划和语法生成来解决自回归语言模型中的"主题漂移"问题，使用概念向量实时门控词汇选择，在保持困惑度相当的同时显著提升领域保持能力。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在下一词预测训练中容易出现"主题漂移"问题，生成内容会逐渐偏离初始提示，这是因为模型过度依赖局部关联而非全局规划。虽然增大模型规模可以缓解这一问题，但下一词预测目标固有的短视性仍然存在。

Method: 引入Idea-Gated Transformer架构，分离语义规划和语法生成。添加辅助的"Idea Head"来预测未来上下文窗口的词袋分布，生成潜在"概念向量"，在生成过程中主动门控主词汇选择。提出可微分门控机制，实时抑制语义不相关的标记，有效修剪搜索空间。

Result: 在WikiText-103上的实验表明，Idea-Gated模型在验证困惑度上与标准GPT-2基线相当，但表现出显著优越的领域保持能力。定性和定量分析显示，门控机制成功将生成锁定在特定语义簇（如金融、科学）中，并能抵抗关联漂移。

Conclusion: Idea-Gated Transformer为更可控的语言建模提供了一条参数高效的路径，通过分离语义规划和语法生成，有效解决了自回归语言模型中的主题漂移问题，实现了更好的内容一致性控制。
...
TL;DR: 本文提出了一种新颖的假设驱动向后逻辑推理框架（HBLR），通过置信感知的符号翻译和假设驱动的向后推理，解决了传统前向推理方法的冗余路径、幻觉步骤和语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 逻辑推理是自然语言理解的核心挑战和人工智能的基本能力，支撑着科学发现、数学定理证明和复杂决策。尽管大语言模型取得了显著进展，但当前大多数方法仍依赖前向推理范式，从前提逐步生成推理过程。然而，这些方法常存在推理路径冗余、幻觉步骤和语义漂移问题，导致推理效率低下且不可靠。

Method: HBLR框架包含两个核心阶段：1）置信感知符号翻译阶段：仅将高置信度的文本片段转换为逻辑形式（如一阶逻辑），不确定内容保留为自然语言；翻译反思模块通过评估符号输出并在必要时将其恢复为文本来确保语义保真度。2）假设驱动向后推理阶段：模拟人类演绎思维，假设结论为真并递归验证其前提；推理反思模块识别并纠正有缺陷的推理步骤，增强逻辑连贯性。

Result: 在五个推理基准测试上的广泛实验表明，HBLR在准确性和效率方面均持续优于强基线方法。

Conclusion: HBLR框架通过结合置信感知翻译和假设驱动向后推理，有效解决了传统前向推理方法的局限性，在逻辑推理任务中实现了更高效和可靠的性能。
...
TL;DR: 研究人员收集了Portal 2合作模式中的11.5小时语音对话语料库，包含24.5K条话语，用于研究复杂协作环境中的语言现象


<details>
  <summary>Details</summary>
Motivation: 合作视频游戏为研究复杂环境中不确定条件下的沟通和推理提供了丰富的语言数据，现有闲聊或任务导向对话语料库缺乏这类复杂协作场景的语言现象

Method: 收集Portal 2合作模式中的11.5小时人类语音对话，包含24,500条话语，记录玩家视频、音频、转录文本、游戏状态数据，并进行手动和自动语言标注

Result: 创建了Portal对话语料库，识别出复杂空间指代、澄清与修复、临时约定形成等现有语料库中罕见但重要的语言现象
...
<details>
  <summary>Details</summary>
Motivation: LoRA作为参数高效微调方法虽然流行，但由于其低秩假设，训练出的模型性能往往不令人满意。需要改进LoRA以更好地模拟基于梯度优化算法的全参数微调过程。

Method: 提出Dual LoRA方法，将低秩矩阵分为两组：幅度组控制是否更新参数及更新幅度，方向组决定参数向前还是向后移动。通过为幅度组添加ReLU函数，为方向组添加符号函数来实现。

Result: 在广泛的NLP任务（自然语言生成、理解和常识推理）上，使用GPT-2、RoBERTa、DeBERTa和LLaMA-1/2/3作为基线模型进行实验，结果显示Dual LoRA在相同可训练参数数量下始终优于LoRA及其最先进的变体。

Conclusion: Dual LoRA通过引入幅度和方向分离的归纳偏置，有效改进了LoRA的性能，能够更好地模拟全参数微调过程，在参数高效微调中表现出色。
...
TL;DR: PretrainZero是一个基于强化学习的主动学习框架，通过在预训练语料上进行强化学习，将RL从领域特定的后训练扩展到通用预训练，显著提升基础模型的通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的大思维模型虽然在特定领域（如软件和数学）展现出专家级能力，但严重依赖特定领域可验证的奖励，这限制了通用推理能力的扩展。人类能够主动从通用经验中学习，而现有方法无法实现这一点。

Method: 提出PretrainZero框架：1）主动预训练：学习统一的推理策略，主动从预训练语料中识别合理且信息丰富的内容，通过RL进行预测；2）自监督学习：无需可验证标签、预训练奖励模型或监督微调，直接在通用维基百科语料上使用RL预训练推理器；3）验证扩展：通过处理越来越困难的掩码跨度来增强通用推理能力。

Result: 在强化预训练中，PretrainZero将Qwen3-4B-Base在MMLU-Pro、SuperGPQA和数学平均基准上的性能分别提升了8.43、5.96和10.60分。预训练模型还可以作为下游RLVR任务的推理基础模型。

Conclusion: PretrainZero通过将强化学习扩展到通用预训练阶段，突破了可验证数据对通用推理能力的限制，显著提升了基础模型的通用推理性能，为人工通用智能的发展提供了新方向。
...
TL;DR: Top-k注意力机制在解码和训练阶段的研究：验证了精确Top-k解码在下游任务中能达到甚至超越全注意力的性能，探索了原生Top-k注意力训练策略，并提供了熵视角的理论解释。
...
Method: 研究Top-k注意力机制在解码和训练阶段的有效性和理论机制：1）验证精确Top-k解码的有效性；2）探索原生Top-k注意力训练策略；3）研究近似Top-k算法精度对下游任务的影响；4）从熵的角度提供理论解释。

Result: 实验表明：1）解码阶段仅保留与查询相似度最高的关键键作为上下文窗口，在HELMET和LongBench v2等下游任务上能达到甚至超越全注意力的性能；2）训练和推理阶段Top-k注意力操作的一致性有助于进一步释放Top-k解码潜力；3）下游任务性能与近似精度呈正相关；4）Top-k注意力SFT后的模型在下游任务中表现出明显的熵降低现象。

Conclusion: Top-k注意力机制能有效降低大语言模型的推理计算成本，同时保持甚至提升模型性能，低熵状态更适合Top-k解码，为长上下文建模提供了高效解决方案。
...
TL;DR: 该研究系统评估了大型语言模型在抽象摘要任务中的推理能力，发现推理并非通用解决方案，其效果高度依赖于具体策略和上下文，存在摘要质量与事实忠实度之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在数学和代码生成等分析任务中表现出色，但其在抽象摘要任务中的实用性被广泛假设但未经验证。研究旨在填补这一空白，评估推理策略对摘要质量和忠实度的影响。

Method: 首先将通用推理策略适配到摘要领域，然后对8种推理策略和3个大型推理模型在8个不同数据集上进行大规模比较研究，评估摘要质量和忠实度。

Result: 研究发现：1）推理效果高度依赖具体策略和上下文；2）存在摘要质量与事实忠实度的权衡：显式推理策略倾向于提高流畅性但牺牲事实基础，而隐式推理则呈现相反模式；3）增加大型推理模型的内部推理预算不会改善甚至可能损害事实一致性。

Conclusion: 有效的摘要需要忠实的压缩而非创造性的过度思考，推理不是通用解决方案，其效果取决于具体应用场景和策略选择。
...
TL;DR: 提出一个可解释的事实一致性评估框架，用于评估领域内和开放领域文本，通过分解文本为原子事实并引入加权度量来改进现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理任务中取得显著进展，但容易生成错误或误导性但看似合理的论点（幻觉问题）。这在临床等高风险领域尤为严重，现有评估指标无法充分评估事实一致性且缺乏可解释性，使得诊断和缓解错误变得困难。

Method: 提出一个可解释的事实一致性评估框架：1）将文本分解为原子事实；2）引入灵活、无模式的方法论；3）采用加权度量而非绝对度量；4）提出控制复杂领域评估复杂度的机制。

Result: 在流行的通用和临床数据集上进行了基准测试，并发布了代码以支持未来研究中事实感知模型的训练。

Conclusion: 该框架解决了现有事实一致性评估方法的局限性，提供了更准确、可解释的评估工具，特别适用于高风险领域如临床应用。
...
TL;DR: 意大利首次全面调查显示，生成式AI在意大利广泛采用，正取代其他技术成为主要信息来源，但存在数字素养低、性别鸿沟等问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是了解生成式AI在意大利的采用情况、使用模式和数字素养，评估其社会影响和潜在的数字鸿沟风险。尽管生成式AI具有社会潜力，但采用不均衡和对局限性认识不足可能导致数字鸿沟扩大。

Method: 基于新收集的1,906名意大利语成年人的调查数据，对意大利生成式AI的采用、使用模式和数字素养进行了首次全面实证映射。

Result: 研究发现：1) 生成式AI在工作和个人用途中广泛采用，包括情感支持和医疗建议等敏感任务；2) 生成式AI正取代其他技术成为主要信息来源，尽管用户数字素养低且难以识别错误信息；3) 存在显著的性别鸿沟，女性采用率仅为男性的一半，使用频率也较低，这种差异在年长群体中尤为明显；4) 数字素养是采用的关键预测因素，但只能部分解释性别差异。

Conclusion: 研究强调了针对性的教育倡议的必要性，并需要进一步调查数字素养无法完全解释的公平参与障碍。数据提供了生成式AI多用途使用的详细见解，揭示了在促进技术普及的同时需要关注数字素养和结构性障碍。
...
TL;DR: 该研究提出了Hydro-SE Bench基准测试，包含4000道选择题，用于评估大语言模型在水科学与工程领域的知识与应用能力，发现商业模型表现优于小参数模型，但模型在行业标准和水利结构等专业领域仍有不足。


<details>
  <summary>Details</summary>
Motivation: 水科学与工程是保障人类供水、生产清洁水电能源、减轻洪旱灾害的关键领域，需要多学科知识和专家协作。随着大语言模型的快速发展，其在Hydro-SE领域的应用潜力日益受到关注，但模型在该领域的知识和应用能力尚未得到充分评估。

Method: 研究提出了Hydro-SE Bench评估基准，包含4000道多项选择题，涵盖9个子领域，能够从基本概念知识、工程应用能力、推理计算能力三个方面评估大语言模型的表现。

Result: 评估结果显示：商业大语言模型准确率在0.74-0.80之间，小参数模型准确率在0.41-0.68之间。模型在与自然科学和物理科学密切相关的子领域表现良好，但在行业标准、水利结构等专业领域知识方面表现不佳。模型扩展主要提升了推理和计算能力。

Conclusion: 该研究揭示了LLMs在Hydro-SE任务中的优势和不足，为模型开发者提供了明确的训练目标，为Hydro-SE研究人员提供了应用LLMs的实用指导。模型在实际工程应用问题处理方面仍有很大提升潜力。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够评估AI导师教学质量的工具，为教育利益相关者和ACL社区提供支持学习、收集用户反馈和标注的平台。

Method: 使用语言技术构建应用程序，包含软件演示、评估功能、模型检查工具和数据可视化组件。

Result: 开发了AITutor-EvalKit工具，能够评估AI导师的教学质量，并提供演示、评估、模型检查和数据可视化功能。

Conclusion: AITutor-EvalKit是一个有价值的工具，既服务于教育利益相关者，也支持ACL社区的学习和研究需求。
...
TL;DR: 提出DZ-TDPO框架解决长对话系统中的状态惯性问题，通过冲突感知动态KL约束和可学习时序注意力偏置实现非破坏性对齐，在MSC数据集上达到SOTA胜率，同时保持零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 长对话系统存在状态惯性问题，即静态约束阻碍模型解决用户意图演变与历史上下文之间的冲突。现有方法通常采用破坏性的权重更新，会损害模型的通用能力。

Method: 提出DZ-TDPO框架，结合冲突感知的动态KL约束和可学习的时序注意力偏置，实现非破坏性的对齐方法，通过精确的注意力调节而非权重更新来缓解状态惯性。

Result: 在Multi-Session Chat数据集上，DZ-TDPO在Phi-3.5模型上达到86.2%的胜率，Qwen2.5-7B模型达到99.4%的胜率。发现"容量-稳定性权衡"现象：小模型需要付出"对齐税"（困惑度激增）来克服历史惯性，而大模型能以可忽略的困惑度开销实现近乎完美的对齐。

Conclusion: 状态惯性可以通过精确的注意力调节而非破坏性的权重更新来缓解，这能保持模型在不同规模下的通用能力（如MMLU表现）。DZ-TDPO框架为长对话系统的对齐提供了有效的非破坏性解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全对齐策略存在不足，攻击者可能通过操纵模型的内部表示来绕过安全防护。研究者希望揭示这种新的攻击面，展示现有安全对齐在表示层面的脆弱性。

Method: 通过系统性地将有害关键词（如"bomb"）替换为良性标记（如"carrot"）在多个上下文示例中，配合有害请求的前缀，使良性标记的内部表示逐渐收敛到有害标记的表示，实现语义覆盖。

Result: 攻击无需优化，在闭源和开源系统中都具有广泛的可转移性，在Llama-3.3-70B-Instruct上单句上下文覆盖达到74%的攻击成功率。语义覆盖逐层出现，早期层的良性含义在后期层收敛为有害语义。

Conclusion: Doublespeak揭示了LLMs潜在空间中的新攻击面，表明当前的对齐策略不足，需要在表示层面进行操作。这暴露了现有安全防护的脆弱性，需要开发更强大的表示级安全机制。
...
TL;DR: 将DoLa对比解码方法从仅解码器架构扩展到T5/FLAN-T5编码器-解码器架构，评估其对指令遵循能力的影响，发现对某些任务有益，对某些有害，并通过逐层分析解释原因。


<details>
  <summary>Details</summary>
Motivation: 现有对比解码方法如DoLa仅在解码器架构中实现，且主要研究其对事实性的改进。本研究旨在将DoLa扩展到编码器-解码器架构（T5/FLAN-T5），并评估其对指令遵循能力的影响，填补该领域的研究空白。

Method: 将DoLa对比解码策略适配到T5和FLAN-T5模型家族，在编码器-解码器架构中实现对比解码。通过逐层分析FLAN-T5模型的logit演化，量化DoLa对token输出概率的影响。

Result: DoLa对某些任务类别能提高文本生成的忠实度，但对其他任务有害。这是首次在编码器-解码器架构中实现对比解码策略。

Conclusion: DoLa在编码器-解码器架构中的效果具有任务依赖性，通过逐层logit分析可以解释其影响机制，为理解对比解码在不同架构中的表现提供了新见解。
...
TL;DR: 本文提出一个优化大语言模型在心理学文本分类任务中提示工程的实证框架，发现构建定义、任务框架和示例对性能影响最大，推荐结合人工和自动方法生成多种提示变体并基于训练集表现选择最佳方案。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在文本分类任务中表现良好，但其输出严重依赖提示的措辞。现有研究很少关注心理学等专业领域，这些领域的构念具有精确的理论定义，可能未在预训练数据中充分体现。需要系统优化LLM在识别文本中理论构念时的提示工程方法。

Method: 提出一个优化LLM提示性能的实证框架，实验评估五种提示策略：代码本引导的经验提示选择、自动提示工程、角色提示、思维链推理和解释性提示，结合零样本和少样本分类。在三个构念和两个模型上进行测试，通过训练集评估提示变体，在保留集上验证最终方法。

Result: 发现角色、思维链和解释性提示不能完全解决措辞不当提示带来的性能损失。影响最大的提示特征是构念定义、任务框架，以及较小程度上的示例。与专家判断最一致的分类结果来自结合代码本引导经验提示选择和自动提示工程的少样本提示。

Conclusion: 建议研究人员尽可能生成和评估多种提示变体（人工制作、自动生成或两者结合），基于训练集的实证表现选择提示和示例，并在保留集上验证最终方法。这为需要与专家判断对齐的关键场景提供了实用、系统和理论驱动的LLM提示优化方法。
...
TL;DR: 该研究提出通过微调LLMs遵循医学共识指南进行推理和预测，以解决医疗AI中解释可信度不足的问题。以Sepsis-3定义为例，小规模微调模型优于大型LLMs的单样本学习和基于医学文本训练的模型。


<details>
  <summary>Details</summary>
Motivation: 医疗早期预测的机器学习虽然性能突破，但过度关注预测准确性而忽视了获得医疗从业者信任所需的可信解释。医学共识指南在医疗领域普遍存在，但现有模型未能有效遵循这些指南进行推理。

Method: 将医学共识指南转化为电子健康记录中的语言化推理规则实例，用于微调LLMs学习共识规则及其例外情况。采用多模态方法，将时间序列预测模型的输出表示与LLM集成，以处理稀疏和不规则采样的临床变量预测。

Result: 在Sepsis-3共识定义实验中，小规模微调模型在未见患者数据上对规则（及例外）的推导正确性接近完美，显著优于使用明确定义提示的大型LLMs单样本学习模型和基于医学文本训练的模型。多模态集成进一步改善了时间序列预测结果。

Conclusion: 通过微调LLMs遵循医学共识指南进行逐步推理，可以有效提高医疗预测的解释可信度和推导正确性。早期预测的主要瓶颈不是分布外泛化，而是向未来泛化预测稀疏不规则采样的临床变量，多模态集成是解决这一问题的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: Transformer解码器在长序列任务中KV缓存内存需求过大，现有的跨层KV缓存共享方法（如YOCO、CLA）性能不如GQA等层内方法。研究发现顶层KV缓存中，values主要来自底层，keys则同时依赖底层和中层信息。

Method: 提出FusedKV方法，顶层KV缓存是底层和中层最有信息量的缓存的融合学习。直接在应用RoPE后的keys上进行融合，避免重新计算旋转嵌入。进一步提出FusedKV-Lite，顶层KV缓存直接使用底层values和中层keys，减少I/O开销。

Result: 在332M到4B参数的LLM实验中，FusedKV减少50%缓存内存，同时验证困惑度低于标准Transformer解码器，实现了内存高效的高性能架构替代方案。

Conclusion: FusedKV通过智能融合底层和中层信息来优化KV缓存，在显著减少内存占用的同时保持甚至提升模型性能，为长序列处理提供了有效的解决方案。
...
TL;DR: 研究构建了包含巴斯克语标准语、社交媒体和历史文本的多样化语料库，训练了BERnaT系列模型，发现结合多样化数据的模型在所有任务类型上表现更优，强调了语言多样性对构建包容性语言模型的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型依赖经过质量过滤的大规模文本语料库，但过滤过程可能无意中排除非标准语言变体（方言、历史、非正式等），这会降低模型鲁棒性并加剧表征偏见。研究认为语言模型应捕捉完整的语言变异谱系，而不仅仅依赖标准化文本。

Method: 聚焦巴斯克语（形态丰富、资源稀缺的语言），构建了结合标准语、社交媒体和历史来源的新语料库，预训练了BERnaT系列编码器模型（标准、多样化、组合三种配置）。提出了将自然语言理解任务分为标准和多样化子集的评估框架，以评估语言泛化能力。

Result: 结果显示，在标准和多样化数据上训练的模型始终优于仅在标准语料库上训练的模型，在所有任务类型上都有性能提升，且不影响标准基准测试的准确性。

Conclusion: 这些发现强调了语言多样性在构建包容性、可泛化语言模型中的重要性。语言模型应整合多样化语言变体以提高鲁棒性和代表性。
...
TL;DR: BRAND数据集聚焦南亚四大宗教，揭示多语言模型在宗教语境中的偏见，特别是对伊斯兰教的偏见，以及英语与孟加拉语之间的性能差异。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在偏见检测方面有所进步，但宗教等敏感话题仍面临挑战，因为细微错误可能导致严重误解。多语言模型在宗教语境中经常误判且准确性不足。

Method: 引入BRAND数据集，包含南亚四大宗教（佛教、基督教、印度教、伊斯兰教）的2400多个条目，使用英语和孟加拉语三种不同类型的提示进行测试。

Result: 模型在英语中的表现优于孟加拉语，即使回答宗教中立问题时也持续表现出对伊斯兰教的偏见，揭示了多语言模型在不同语言中处理相似问题时的持续偏见。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型适配到低资源语言面临数据稀缺和跨语言漂移的挑战，藏语作为形态丰富且代表性不足的语言，需要专门的适配方法来提升其语言理解和生成能力。

Method: 采用两阶段适配方法：1) 持续预训练(CPT)建立藏语语言基础；2) 监督微调(SFT)进行任务和翻译专业化。对Qwen3-4B的435层进行分层分析，研究适配过程中的变化模式。

Result: 困惑度从2.98降至1.54；汉藏翻译质量显著提升：BLEU从0.046提升至0.261，chrF从2.2提升至6.6。分析显示适配主要集中在嵌入层和输出头，中后期MLP投影编码领域特定变换。

Conclusion: CPT构建藏语语义流形，SFT以最小的表征干扰优化任务对齐。本研究首次定量探索了藏语LLM适配动态，为将多语言基础模型扩展到低资源环境提供了开放可复现的框架。
...
TL;DR: 本文提出两种tokenizer适应方法：继续BPE训练用于词汇扩展，以及基于叶子的词汇剪枝用于词汇缩减，以提高预训练语言模型在新领域或语言中的适应效果。


<details>
  <summary>Details</summary>
Motivation: 当前tokenizer适应方法存在两个问题：1）词汇扩展时，传统方法在新数据上训练新tokenizer并添加非重叠词汇，但很多新增词汇无法被访问或从未使用；2）需要词汇剪枝方法来移除冗余词汇同时保持模型质量。

Method: 1. 继续BPE训练：通过在现有预训练tokenizer基础上，继续在新数据上进行BPE合并学习过程来适应tokenizer，而不是从头训练新tokenizer。2. 基于叶子的词汇剪枝：识别并移除词汇表中冗余的token，同时保持模型性能。

Result: 在多语言和多种模型家族的实验中，继续BPE训练方法提高了tokenization效率，并更好地利用了新增词汇。词汇剪枝方法能有效移除冗余token而不损害模型质量。

Conclusion: 这两种方法为受控的词汇修改提供了实用工具，作者已将其作为开源包发布。继续BPE训练解决了词汇扩展中的低效问题，而词汇剪枝则提供了精简词汇表的能力。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在多语言视觉问答任务中表现出色，同时保持竞争力的纯文本性能的2B规模视觉语言模型

Method: 结合SigLIP2视觉编码器和Qwen3语言主干，通过注意力池化连接器实现任意分辨率图像的令牌高效处理

Result: 在标准VQA基准测试和多语言评估中，Jina-VLM超越了同类可比模型，同时保持了有竞争力的纯文本性能

Conclusion: Jina-VLM展示了通过精心设计的架构组合，可以在2B参数规模下实现卓越的多语言视觉问答性能，同时不牺牲文本处理能力
...
TL;DR: SkillFactory是一种在强化学习前通过监督微调让模型学习认知技能的方法，使用模型自身生成的"银"训练数据，帮助模型在RL后更好地泛化到更困难的任务


<details>
  <summary>Details</summary>
Motivation: 如何让模型学习基础模型不具备的认知技能（如验证答案、回溯、尝试替代方法等），而不依赖于从更强模型的蒸馏

Method: 在强化学习前进行监督微调，使用模型自身生成的样本重新排列成技能训练数据（"银"SFT轨迹），尽管不完美但能有效引导模型在RL阶段获得技能

Result: 1) SkillFactory SFT初始化帮助模型在RL后泛化到更困难的任务变体；2) 模型确实使用了认知技能；3) SkillFactory模型在领域外任务上比基础RL模型更稳健

Conclusion: 在强化学习前学习的归纳偏置有助于模型学习稳健的认知技能使用
...
TL;DR: 本文提出将AI对齐重新构想为通过过程化、多智能体、发展性机制来构建具有熵减和理性响应能力的智能体，而非编码固定的人类价值内容。


<details>
  <summary>Details</summary>
Motivation: 传统基于内容的价值规范方法存在结构不稳定性问题，包括"是-应该"鸿沟、价值多元主义和扩展框架问题，这构成了"规范陷阱"。需要新的理论框架来解决AI对齐的根本挑战。

Method: 提出三个哲学贡献：1) 阐述"规范陷阱"论证；2) 提出"熵减"作为理解多智能体对齐动态的信息论框架；3) 基于相容论的指导控制理论建立真实与模拟道德能力的功能区分，并设计具身实验范式和验证机制。

Result: 建立了新的AI对齐理论框架，生成了关于人工系统中价值涌现和道德能动性的具体可证伪预测，但实证验证仍在进行中。

Conclusion: AI对齐应转向过程导向、多智能体交互的发展性方法，通过熵减机制实现状态对齐，而非试图编码固定的价值内容。这为AI道德能力的发展提供了新的理论基础。
...
TL;DR: 本文提出了一种名为"权重计算主义"的新型认知架构，基于第一原理构建，通过逻辑原子和基本操作实现可解释的决策，为AGI提供了一条可行路径。


<details>
  <summary>Details</summary>
Motivation: 当前AI范式作为"体验架构师"面临可解释性和价值对齐的根本挑战，需要一种能够实现透明推理、可追溯价值对齐的认知架构。

Method: 将认知解构为不可分割的逻辑原子和两个基本操作（指向和比较），通过可解释的权重计算模型（权重=收益*概率）形式化决策，所有值都可追溯到可审计的初始权重集。

Result: 该架构实现了透明、类人推理，在全新场景中表现出稳健学习能力，通过图算法计算引擎和全局工作空间工作流实现，初步代码实现和场景验证支持其可行性。

Conclusion: 权重计算主义为构建可信赖、对齐的AGI奠定了实践和理论基础，提供了实现可解释性、内在通用性和可追溯价值对齐的可行路径。
...
TL;DR: 该论文探讨了大型推理模型在复杂推理任务中生成长思维链的局限性，研究了符号求解器集成方法何时能增强传统长思维链方法，发现符号求解器主要在问题需要有限隐式推理但涉及充足搜索空间时有效。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成长思维链在复杂推理任务中表现良好，但这种方法可能导致大量token开销，特别是当模型"过度思考"产生冗长推理链时，甚至会导致错误答案。需要探索符号求解器集成方法何时能增强传统长思维链方法。

Method: 采用符号求解器集成方法，利用LLM的代码生成能力将推理任务转换为可执行代码，然后使用符号求解器解决。通过实验研究传统长思维链方法何时能被符号求解器增强。

Result: 实验结果显示：1）符号求解器集成方法仅在问题需要有限隐式推理但涉及充足搜索空间时有效；2）最新LLM（如GPT-4o）在推理深度较浅的演绎问题上表现更好；3）符号求解器集成方法显著提高了LLM在需要重复回溯的约束满足问题上的性能；4）提供声明性示例时，即使是CodeLlama-13B也能在困难的斑马谜题上超越GPT-4o。

Conclusion: 符号求解器集成方法在特定类型推理问题中能有效增强传统长思维链方法，特别是在需要大量搜索但有限隐式推理的约束满足问题上。这种方法为优化大型推理模型的推理效率提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 主动推理使用期望自由能作为规划决策目标来平衡学习智能体的利用和探索驱动力。然而，文献中对偏好分布应如何指定以及特定指定如何影响主动推理智能体的推理和学习几乎没有关注。本研究旨在填补这一空白。

Method: 考虑了四种定义偏好分布的方式：硬目标与软目标、涉及或不涉及目标塑造（即中间目标）。在网格世界导航任务中比较了四种智能体（每种使用一种偏好分布）的性能。
...
TL;DR: 本文提出了一种评估LLM智能体在零样本混合动机环境中合作能力的方法，使用Concordia多智能体模拟环境，揭示了当前智能体在需要说服和规范执行场景中的合作泛化能力存在显著差距。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在社会互动方面展现出强大能力，并越来越多地部署在与人类和人工智能体交互的场景中。然而，现有评估方法无法衡量这些能力在新颖社交情境中的泛化效果，这是LLM智能体发展的关键前沿问题。

Method: 引入基于Concordia自然语言多智能体模拟环境的评估方法，测试智能体在不同合作伙伴和情境中识别和利用互利机会的能力，通过NeurIPS 2024 Concordia竞赛在谈判到集体行动等多样化场景中进行实证评估。

Result: 研究发现当前智能体能力与实现可靠合作所需的稳健泛化之间存在显著差距，特别是在需要说服和规范执行的场景中，智能体难以有效实现互利共赢。
...
TL;DR: 提出了一种通信受限的多智能体强化学习框架，通过区分有损和无损消息，将通信影响量化到全局奖励中，提高在复杂动态环境中的可扩展性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中普遍存在有损通信问题，现有带通信的多智能体强化学习方法由于可扩展性和鲁棒性有限，难以应用于复杂动态的真实环境。

Method: 1) 提出通用通信约束模型统一描述不同场景的通信条件；2) 将其作为学习先验区分特定场景的有损和无损消息；3) 基于双重互信息估计器解耦有损和无损消息对分布式决策的影响；4) 引入通信约束的多智能体强化学习框架，将通信消息影响量化到全局奖励中。

Result: 在多个通信受限基准测试中验证了方法的有效性。

Conclusion: 提出的通信约束多智能体强化学习框架能够有效处理现实世界中的有损通信问题，提高了在复杂动态环境中的可扩展性和鲁棒性。
...
TL;DR: 提出PAN编程模型，分离智能体工作流逻辑与推理时策略，通过EnCompass框架实现，提高智能体可靠性和灵活性


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体开发方法通常将核心工作流逻辑与推理时策略（如树搜索）耦合在一起，这种耦合限制了开发效率和灵活性

Method: 引入"概率天使非确定性"（PAN）编程模型，分离工作流描述与推理策略；实现EnCompass框架，使用Python装饰器将智能体工作流程序编译为搜索空间

Result: 通过三个案例研究证明，该框架让程序员能够快速提高智能体可靠性，轻松切换不同推理时策略，且只需少量额外编码

Conclusion: PAN编程模型和EnCompass框架有效解决了智能体开发中工作流与推理策略的耦合问题，提高了开发效率和系统灵活性
...
TL;DR: DeepRule是一个用于零售品类和定价优化的自动化业务规则生成框架，通过三级架构解决理论模型与现实经济复杂性之间的系统性错位问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有理论模型与现实经济复杂性之间的系统性错位，具体包括三个关键差距：1) 非结构化文本源导致的数据模态不匹配；2) 非线性价格弹性和时变属性建模中的动态特征纠缠挑战；3) 多层业务约束导致的操作不可行性。

Method: 采用三级架构：1) 混合知识融合引擎，使用大语言模型深度语义解析非结构化文本；2) 博弈论约束优化机制，通过双边效用函数动态协调供应链利益；3) 可解释决策蒸馏接口，利用LLM引导的符号回归优化定价策略和可审计业务规则。

Result: 在真实零售环境中验证了该框架，相比系统性B2C基线实现了更高的利润，同时确保了操作可行性。

Conclusion: 建立了一个闭环管道，统一了非结构化知识注入、多智能体优化和可解释策略合成，实现了真实的经济智能。
...
TL;DR: RoCo是一个基于多智能体角色协作的自动启发式设计系统，通过四个专门角色（探索者、利用者、批评者、整合者）的协作来生成高质量启发式方法，在组合优化问题上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的自动启发式设计研究通常只考虑单一角色，限制了启发式方法的多样性和质量。需要一种能够通过多角色协作来增强启发式设计多样性和质量的系统。

Method: 提出RoCo多智能体角色协作系统，包含四个专门角色：探索者（促进长期潜力，创造性思维）、利用者（关注短期改进，保守性优化）、批评者（评估进化步骤并提供反馈）、整合者（综合提案平衡创新与利用）。这些智能体通过结构化的多轮过程进行交互，包括反馈、精炼和精英突变。

Result: 在五个不同的组合优化问题上，在白盒和黑盒设置下进行评估。实验结果表明RoCo实现了优越性能，生成的启发式方法在两种场景下都优于现有方法（包括ReEvo和HSEvo）。

Conclusion: 这种基于角色的协作范式为稳健和高性能的自动启发式设计建立了新标准，通过多角色协作有效提升了启发式设计的多样性和质量。
...
TL;DR: Omni-AutoThink：一种自适应推理框架，根据任务难度动态调整Omni模型的推理深度，解决现有系统推理行为僵化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有Omni模型在统一多模态感知和生成方面取得进展，但推理行为仍然僵化——要么对简单问题过度推理，要么在需要推理时无法有效推理。需要一种能根据任务难度自适应调整推理深度的框架。

Method: 提出两阶段框架：1）自适应监督微调阶段，使用大规模推理增强数据赋予Omni模型基本推理能力；2）自适应强化学习阶段，基于任务复杂度和奖励反馈优化推理行为。同时构建了涵盖文本、文本-音频、文本-视觉、文本-音频-视觉模态的综合自适应推理基准。

Result: 实验结果表明，相比之前的基线方法，提出的框架显著提升了自适应推理性能。所有基准数据和代码将公开。

Conclusion: Omni-AutoThink通过自适应调整推理深度，有效解决了多模态模型推理行为僵化的问题，为构建更智能的自适应推理系统提供了新方向。
...
TL;DR: 该研究提出了一个基于Blocksworld问题的基准测试框架，通过Model Context Protocol标准化工具接口，用于系统评估LLM智能体在工业自动化中的规划与执行能力。
...
Method: 引入包含可执行仿真环境的基准测试，基于Blocksworld问题提供五个复杂度类别，通过集成Model Context Protocol作为标准化工具接口，使不同智能体架构能够无需特定实现修改即可连接和评估。

Result: 通过单智能体实现展示了基准测试的适用性，建立了用于比较基于LLM的规划和执行方法的定量指标。

Conclusion: 该基准测试框架为系统评估和比较LLM智能体在工业自动化中的自适应规划与执行能力提供了标准化工具，填补了该领域缺乏统一评估标准的空白。
