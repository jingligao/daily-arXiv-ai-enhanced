...
TL;DR: 本文展示了如何使用拓扑数据分析中的持久同调方法来客观分析艺术风格，能够统计显著地区分不同艺术家、同一流派内的艺术家，以及区分艺术家作品与AI生成的风格模仿作品。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的视觉直觉和经验，缺乏客观性。本文旨在利用数学工具为艺术分析提供结构化、客观的视角。

Method: 使用拓扑数据分析中的持久同调方法，这是一种能够捕捉数据拓扑特征的技术。

Result: 持久同调方法能够以统计显著性区分不同艺术家、同一艺术流派内的艺术家，并能有效区分艺术家原作与AI生成的风格模仿作品。

Conclusion: 持久同调为艺术风格分析提供了客观、可解释的数学工具，能够有效识别和区分艺术风格特征。
...
TL;DR: 提出了一种使用StyleGAN2生成猕猴面部表情的方法，通过数据增强、样本选择和损失函数优化来解决训练数据不足的问题，成功生成了多样化的面部表情并实现了基于风格的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 由于可用于训练的动物面部图像在数量和变化上都很有限，特别是在不同个体的面部表情方面，因此生成动物面部表情具有挑战性。本研究专注于在系统神经科学和进化研究中广泛研究的猕猴。

Method: 1) 使用计算机图形学的运动转移技术合成新的面部表情图像进行数据增强；2) 基于初始训练的StyleGAN2模型的潜在表示进行样本选择，确保训练数据集的多样性和均匀采样；3) 优化损失函数以确保准确再现细微运动（如眼球运动）。

Result: 该方法能够为多个猕猴个体生成多样化的面部表情，表现优于仅使用原始静态图像训练的模型。此外，该模型在基于风格的图像编辑方面也很有效，特定风格参数对应不同的面部运动。
...
TL;DR: 本文提出了PairHuman数据集，这是首个专为高质量双人肖像生成设计的大规模基准数据集，包含超过10万张图像，并开发了DHumanDiff基线方法，在双人肖像生成中实现增强的面部一致性和个性化生成与场景创建的平衡。


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制具有重要应用价值，如保存情感记忆和婚礼摄影规划，但缺乏基准数据集阻碍了高质量双人肖像定制的发展。

Method: 构建PairHuman数据集（包含10万+图像，涵盖多样场景、服装和双人互动，附带丰富元数据），并开发DHumanDiff基线方法，专注于增强面部一致性和平衡个性化人物生成与语义驱动场景创建。

Result: 实验结果表明，所提出的数据集和方法能够生成高度定制化、视觉质量优越且符合人类偏好的双人肖像。

Conclusion: PairHuman数据集和DHumanDiff方法为双人肖像生成提供了有效的基准和解决方案，显著提升了定制化肖像的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 中子成像在惯性约束聚变分析中至关重要，但图像常被高斯和泊松混合噪声退化，传统滤波和阈值方法难以有效去除这些重叠噪声，需要开发能保持图像保真度的去噪技术。

Method: 采用无监督自编码器，在潜在空间中结合Cohen-Daubechies-Feauveau (CDF 97)小波变换，用于混合高斯-泊松噪声去噪。

Result: 该方法成功去噪中子成像数据，与BM3D等非机器学习滤波方法相比，表现出更低的重建误差和更优的边缘保持指标。

Conclusion: 该方法为中子图像噪声减少和ICF实验三维重建分析提供了有前景的进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型如CLIP通过微调提升安全性时，往往导致泛化性能显著下降，这种权衡源于将不安全概念强制对齐到单一预定义安全目标的刚性对齐策略。

Method: 提出邻近感知方法，将不安全概念重定向到语义上最接近的安全替代品，以最小化表示变化。开发SaFeR-CLIP微调框架应用最小干预原则。

Result: SaFeR-CLIP成功调和了安全性和性能，相比先前方法恢复了高达8.0%的零样本准确率，同时保持强大的安全性。还贡献了NSFW-Caps基准，包含1000个高度对齐的测试对。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单视图SVG生成多视图一致SVG的挑战，填补生成建模与结构化向量表示之间的空白，支持资产创建和语义向量编辑等应用。

Method: 三阶段框架：1) 将栅格化输入提升为3D表示并渲染多视图图像；2) 扩展SAM2的时间记忆机制到空间域，建立跨视图部件级对应关系；3) 路径整合和结构优化以减少冗余并保持边界和语义。

Result: 生成的SVG在多个视图间展现出强大的几何和颜色一致性，显著减少冗余路径，同时保留精细结构细节。

Conclusion: 该工作成功连接了生成建模和结构化向量表示，为单输入、对象级多视图SVG生成提供了可扩展的途径。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成方法改进自回归3D网格生成模型，解决传统方法在质量、速度和编辑性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 传统3D网格制作依赖人工艺术家，耗时且难以扩展。现有自回归模型存在质量与速度的权衡问题，且序列依赖限制了增量编辑能力。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，结合了LLM场景布局推理、程序生成、扩散式3D生成和对象感知场景分解技术。
...
Result: 系统能够生成几何一致、视觉丰富且实时渲染高效的世界，支持对布局、规模和风格的细粒度控制，可在标准游戏引擎中立即探索或编辑。

Conclusion: 这项工作代表了向可扩展、可访问的生成式世界构建迈出的一步，推动了3D生成AI在游戏、模拟和沉浸式社交环境应用的前沿发展。
...
Method: 整合配准的Sentinel-2和ALOS-2影像，通过混合人机流程生成和验证结构化文本标注，建立从人类活动到地上生物量回归的认知逻辑链。
...
TL;DR: 本文提出了BOP-ASK数据集，用于评估和训练视觉语言模型在细粒度物体交互推理方面的能力，包括精确3D定位、物理兼容性、物体可用性和多步空间规划等任务。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现良好，但缺乏对物体交互的细粒度理解能力，如精确3D定位、物理兼容性等，这些对实际应用至关重要。

Method: 利用BOP数据集的6D物体姿态，开发数据生成流程，获得细粒度标注，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系等。

Result: 构建了包含15万张图像和3300万个问答对的数据集，涵盖6个任务（其中4个是新颖的）。实验表明，在BOP-ASK上训练的模型优于基线，并展现出精确物体姿态估计、轨迹规划等新兴能力。

Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，显著提升了模型在细粒度物体交互推理方面的性能，并展示了泛化能力。
...
TL;DR: 提出Parts-Mamba模型，一种混合GCN-Mamba架构，用于解决骨架动作识别中因遮挡或通信质量问题导致的骨架不完整问题，通过捕捉远距离关节的上下文信息，在NTU RGB+D数据集上取得了最高12.9%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现实场景中捕获的骨架常因人体部位遮挡或通信质量问题而不完整，现有GCN模型在缺失局部上下文时性能较差，需要增强对远距离关节上下文信息的捕捉能力。

Method: 提出Parts-Mamba混合模型，结合GCN和Mamba架构，通过部分特定扫描特征捕捉部位特定信息，并通过部分-身体融合模块保留非邻近关节的上下文信息。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下进行评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能有效处理骨架不完整问题，通过增强远距离关节上下文信息的捕捉，显著提升了骨架动作识别在遮挡场景下的性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein (JGW)目标，将传统的GW距离扩展到支持多对多对象匹配，解决了传统GW只能进行成对匹配的限制。
...
Method: 通过引入JGW目标，扩展GW框架以实现对象集合的同步匹配，采用最优传输中的传统算法（包括熵正则化）来求解点云对象表示。

Result: 与GW的其他变体相比，该方法在准确性和计算效率方面表现出优越性能，在合成和真实数据集上的实验证明了其在多形状匹配中的有效性。
...
TL;DR: 该论文提出了一种称为表示对齐（REPA）的方法，将扩散或基于流的生成模型与预训练的自监督视觉编码器（如DINOv2）在表示空间中对齐，以指导逆问题中的重建过程，显著提高了重建质量和感知真实性。


<details>
  <summary>Details</summary>
Motivation: 在逆问题中，预训练的生成模型作为先验被使用，但缺乏真实信号。通过将生成模型的内部表示与预训练的自监督编码器对齐，可以提供强大的归纳偏置，改善收敛性和样本质量。

Method: 提出REPA方法，在推理时通过对齐扩散或基于流模型与预训练自监督视觉编码器（如DINOv2）的表示来指导重建过程，即使没有真实信号，也能通过近似目标特征对齐来增强重建保真度。

Result: 理论分析显示REPA正则化与DINOv2嵌入空间中的散度度量相关，且REPA更新将模型内部表示导向干净图像的表示。实验证明该方法在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提高重建质量，同时通过减少离散化步骤提供效率增益。

Conclusion: REPA方法为逆问题提供了一种有效的表示对齐机制，能够显著提升重建的感知质量，并在多个先进求解器中展示出通用性和效率优势。
...
TL;DR: 提出NFGlassNet方法，利用闪光/无闪光图像中的反射动态特性进行玻璃表面检测，通过反射对比挖掘模块和反射引导注意力模块实现准确检测，并在构建的3.3K图像对数据集上验证了方法的优越性。


<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，未能充分利用玻璃本身的固有特性。观察到真实场景中玻璃两侧光照强度差异导致反射变化的现象，提出利用闪光/无闪光图像对中的反射动态进行检测。

Method: 提出NFGlassNet方法，包含反射对比挖掘模块(RCMM)用于提取反射特征，以及反射引导注意力模块(RGAM)用于融合反射和玻璃表面特征。构建了3.3K闪光/无闪光图像对数据集进行训练。

Result: 大量实验表明，该方法在玻璃表面检测任务上优于现有最先进方法。

Conclusion: 利用闪光/无闪光图像对中的反射动态特性能够有效提升玻璃表面检测的准确性，所提出的NFGlassNet方法在该任务上表现出色。
...
TL;DR: 本文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视觉时空推理任务。R-AVST包含5K个未剪辑视频和27K个对象，覆盖100种音频-视觉事件；AVST-Zero采用强化学习方法，通过多维度奖励直接优化模型行为。
...
Method: 1) 构建R-AVST数据集：采用基于LLM的关键对象提取、自动空间标注和人工质量检查的流程；2) 提出AVST-Zero模型：基于强化学习，避免中间监督，通过精心设计的多维度奖励直接优化行为。

Result: 广泛实验验证了R-AVST在推进音频-视觉时空推理方面的有效性，AVST-Zero相比现有模型表现出竞争力。
...
TL;DR: 本文提出Warm Diffusion方法，通过结合热扩散（仅噪声）和冷扩散（仅模糊）的优势，构建统一的模糊-噪声混合扩散模型（BNMD），利用图像频谱依赖性简化评分模型估计。


<details>
  <summary>Details</summary>
Motivation: 热扩散未能利用图像高频细节与低频结构之间的强相关性，导致生成早期步骤的随机行为；冷扩散虽然利用图像相关性进行预测，但忽略了噪声在塑造数据流形中的作用，导致流形外问题。需要结合两者的优势。

Method: 提出Warm Diffusion方法，使用模糊-噪声混合扩散模型（BNMD）联合控制模糊和噪声，采用分治策略利用图像频谱依赖性，通过解耦去噪和去模糊过程简化评分模型估计。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成方面的有效性。

Conclusion: Warm Diffusion成功整合了热扩散和冷扩散的优势，通过模糊-噪声混合模型实现了更好的图像生成性能，并通过频谱分析研究了模型学习动态与数据流形变化之间的权衡关系。
...
TL;DR: 本文提出了Q-Real数据集，用于对AI生成图像的真实性和合理性进行细粒度评估，包含3,088张图像及其标注，并构建了Q-Real Bench基准来评估多模态大语言模型在判断和定位推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成内容质量评估方法通常只提供单一质量分数，过于粗糙，无法为改进生成模型提供有针对性的指导。真实性和合理性是AI生成图像的两个关键维度，需要细粒度评估来有效提升生成性能。

Method: 构建了包含3,088张文本到图像模型生成图像的Q-Real数据集，为每张图像标注主要实体的位置，并提供了真实性和合理性维度的判断问题和归因描述。设计了Q-Real Bench基准来评估多模态大语言模型在判断和定位推理任务上的能力。

Result: 实验结果表明数据集质量高且具有重要意义，基准评估全面。通过设计的微调框架在多个多模态大语言模型上进行实验，验证了方法的有效性。

Conclusion: Q-Real数据集为AI生成图像的细粒度质量评估提供了有效工具，能够为改进生成模型提供有针对性的指导，数据集和代码将在发表后发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决无需真实异常样本或训练的情况下的异常生成问题。

Method: 提出Delta-Denoising方法：1）使用最小提示对驱动两个扩散分支进行对比；2）通过累积每步去噪差异生成图像特定的定位图；3）使用该定位图指导潜在修复过程；4）进行token级提示优化和空间注意力偏置来改进稳定性和控制。

Result: 在公共数据集上的实验表明，DeltaDeno实现了良好的生成效果、真实性和下游检测性能的持续提升。

Conclusion: DeltaDeno作为一种无需训练的零样本异常生成方法，能够有效定位和生成局部缺陷，同时保持周围上下文，在异常检测下游任务中表现出色。
...
TL;DR: 本文提出了一种基于扩散模型的视频超分辨率方法DGAF-VSR，通过光学引导变形模块和特征域时序条件模块，解决了现有方法中的错误累积、空间伪影以及感知质量与保真度之间的权衡问题。
...
Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下进行变形能更好地保留高频信息，但这一优势并非单调递增。DGAF-VSR通过精心设计的对齐和补偿机制，显著提升了视频超分辨率的性能。
...
<details>
  <summary>Details</summary>
Motivation: CBCT图像中的牙齿分割在邻牙粘连情况下面临严重挑战，因为粘连会导致解剖形状严重失真，需要开发能够保持形状完整性的分割方法。
...
TL;DR: 本文提出了OmniGround基准测试，包含3,475个视频和81个类别，用于解决时空视频定位中的类别偏见和推理简化问题。同时提出了PG-TAF训练免费框架，在复杂真实场景中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在时空视频定位任务中与现实需求存在显著差距，主要由于基准测试范围有限，导致模型存在类别偏见、推理过于简化以及语言鲁棒性差的问题。

Method: 提出了Forward-Backward-Refinement标注流程，结合多方向跟踪和智能错误校正；开发了DeepSTG系统评估框架；提出了PG-TAF训练免费两阶段框架，将STVG分解为高层时间定位和细粒度时空传播。

Result: 在复杂真实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系中。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试中均取得一致增益。

Conclusion: OmniGround基准测试揭示了当前STVG模型的局限性，PG-TAF框架通过分解任务显著提升了在复杂真实场景中的性能，为时空视频定位研究提供了新的方向。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理风险的基准，通过隐私感知与推理框架和双语多模态数据集，揭示现有模型存在重大但未被测量的推理型隐私风险。


<details>
  <summary>Details</summary>
Motivation: 现有隐私基准主要评估隐私感知能力，无法应对视觉语言模型从分散信息推断和链接构建个体档案的新威胁，存在关键评估空白。

Method: 提出隐私感知与推理框架，构建包含合成个体档案的双语多模态数据集，设计9个挑战性任务评估从属性检测到跨图像重识别和链式推理的全谱系能力。

Result: 对50多个基础模型和商业模型的大规模评估显示：许多模型存在重大推理型隐私风险；感知级指标无法预测推理风险；现有安全对齐措施对此类攻击无效。
...
TL;DR: 提出一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光流方程作为物理约束，无需预估计运动场即可同时恢复时间相干图像和运动场。
...
Method: 使用两个隐式神经表示分别参数化时空图像内容和光流场，通过光流方程耦合两者作为物理约束，结合数据一致性损失进行联合优化。

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更好的重建质量、准确运动估计和改进的时间保真度。

Conclusion: 基于流正则化约束的隐式联合建模在动态MRI重建中具有重要潜力，能够同时获得高质量图像重建和准确运动估计。
...
TL;DR: 本文提出了FingerCap任务，用于生成捕捉手部动作详细手指级语义的文本描述，并构建了包含4万对手部运动视频和字幕的大规模数据集FingerCap-40K。为解决视频MLLMs的时间稀疏性问题，提出了FiGOP方法，通过结合RGB关键帧和后续手部关键点来恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部运动对视觉感知、具身智能和多模态通信至关重要。当前视频MLLMs由于时间稀疏性无法捕捉细微的手指动态，需要解决这一瓶颈问题。

Method: 提出了FiGOP方法，将每个RGB关键帧与后续手部关键点配对，通过轻量级时间编码器将关键点转换为运动嵌入并与RGB特征集成，在不增加RGB密度的情况下恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，现有的强开源和闭源Video-MLLMs在手指级推理方面仍有困难，而FiGOP增强模型在HandJudge和人工评估中都取得了持续提升。

Conclusion: FiGOP方法有效解决了视频MLLMs的时间稀疏性问题，能够更好地捕捉手指级运动语义，为细粒度手部运动理解提供了有效解决方案。
...
TL;DR: 本文提出了一种基于点监督的面部表情识别方法P-FES，仅需每个表情实例的单时间戳标注。通过双分支框架实现实例自适应强度建模和表情分类，在多个数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情识别方法主要依赖完全监督学习，需要昂贵的时间边界标注。本文旨在通过点监督方法减少标注成本，仅使用每个表情实例的单时间戳进行训练。

Method: 提出双分支框架：1）基于高斯的实例自适应强度建模模块用于软伪标签生成；2）类别感知的顶点分类分支区分宏表情和微表情。引入强度感知对比损失增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的大量实验证明了所提框架的有效性。

Conclusion: 提出的点监督面部表情识别框架通过实例自适应强度建模和双分支设计，在减少标注成本的同时保持了良好的性能，为面部表情分析提供了实用解决方案。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR属性两个相互依赖的潜在变量，实现了统一的材料生成和处理框架。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在变量；将其表述为5帧序列并使用视频扩散架构；在MatHybrid-410K混合数据集上训练。

Result: MatPedia实现了原生1024×1024分辨率合成，在质量和多样性方面显著超越现有方法，能够处理文本到材料生成、图像到材料生成和本征分解等多个任务。

Conclusion: MatPedia通过统一的联合表示成功桥接了RGB外观和PBR属性，为材料生成和处理提供了强大的基础模型，克服了现有方法的局限性。
...
TL;DR: 该论文提出了首个基于烤箱的烹饪过程数据集，并开发了一种边缘设备高效的食谱和烹饪状态引导生成器，用于从原始食物图像合成逼真的烹饪食物图像。
...
Method: 引入首个带有厨师标注熟度级别的烤箱烹饪过程数据集，提出边缘高效的食谱和烹饪状态引导生成器，该生成器以原始食物图像为条件合成逼真食物图像，并引入领域特定的烹饪图像相似性(CIS)指标作为训练损失和进度监控信号。

Result: 该模型在现有基准上表现优异，FID分数显著降低（在自建数据集上提升30%；在公共数据集上提升60%）。

Conclusion: 该方法能够实现用户偏好的视觉目标而非固定预设，确保了时间一致性和烹饪合理性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的方法在处理域偏移和未知类别时面临结构风险与开放空间风险的权衡困境，特别是难以区分与已知类别具有细粒度视觉相似性的'困难未知'样本。

Method: 提出语义增强的CLIP框架，包括语义感知提示增强模块、双工对比学习和语义引导的扩散模块，通过分解图像为判别性语义标记并生成伪未知样本来学习更精细的决策边界。

Result: 在五个基准测试上的广泛实验显示，该方法在准确率和H-score上分别比最先进方法提升了3%和5%。
...
TL;DR: 提出受自然选择启发的3DGS剪枝框架，通过优化梯度自主决定高斯原语的保留或剪枝，无需人工干预，在15%预算下获得超过0.6 dB PSNR增益。


<details>
  <summary>Details</summary>
Motivation: 3DGS使用大量高斯原语拟合场景，导致存储和计算开销大。现有剪枝方法依赖人工设计标准或引入额外可学习参数，效果不理想。

Method: 提出自然选择启发的剪枝框架，将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度自主决定高斯原语的保留或剪枝。引入具有有限不透明度先验的不透明度衰减技术加速选择过程。

Result: 与3DGS相比，在15%预算下获得超过0.6 dB PSNR增益，在紧凑3DGS中达到最先进性能。

Conclusion: 该框架完全可学习且无需人工干预，实现了高效的高斯原语剪枝，显著提升了3DGS的紧凑性和性能。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份来实现人脸去识别，采用化学替换机制的思想，在潜在h空间中进行语义位移来编辑身份。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间难以达到良好平衡，需要开发一种能够有效保留面部属性同时充分抑制身份信息的方法。

Method: 通过优化引导发现身份编辑方向，使用新颖的试剂损失函数监督属性保留和身份抑制，提出线性和测地线（基于切线）两种编辑方案来导航潜在流形。

Result: 在CelebA-HQ和FFHQ数据集上的实验结果表明，FLUID在身份抑制和属性保留之间实现了优越的平衡，在定性和定量指标上都优于最先进的去识别方法。

Conclusion: FLUID框架通过潜在空间中的身份替换，为人脸去识别提供了一种有效的解决方案，在保持面部属性的同时实现了良好的身份抑制效果。
...
Method: 引入DepthFocus，一个可操控的视觉Transformer，通过标量深度偏好进行条件化，动态调整计算以聚焦于目标深度。训练主要使用新构建的50万多层合成数据集，捕捉多样透射效果。

Result: DepthFocus在BOOSTER等传统单深度基准测试中达到最先进性能，并在新提出的多深度数据集上定量展示意图对齐估计。在未见过的透射场景中表现出强大的泛化能力。

Conclusion: DepthFocus作为向主动和类人3D感知迈出的重要一步，展现了其稳健性，能够处理复杂场景中的选择性感知。
...
TL;DR: RacketVision是一个用于体育分析的新型数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度的球拍姿态标注，支持球跟踪、球拍姿态估计和轨迹预测三个任务。


<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉研究，通过提供首个大规模细粒度球拍姿态标注数据集，研究复杂的人-物交互。

Method: 构建包含乒乓球、网球和羽毛球的数据集，提供球拍姿态和球位置的细粒度标注，采用CrossAttention机制进行多模态融合。

Result: 评估显示，简单拼接球拍姿态特征会降低性能，而CrossAttention机制能有效利用球拍姿态信息，在轨迹预测任务上超越单模态基线。
...
TL;DR: 本文提出了ConfusedTourist评估套件，用于测试视觉语言模型在混合文化线索下的稳定性，发现即使是最先进的模型也会因文化概念混合而性能大幅下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常每个图像只包含单一文化概念，忽略了多个不相关文化线索共存的情况，而模型在这种文化混合场景下的稳定性对于支持多元文化社会至关重要。

Method: 开发了ConfusedTourist文化对抗鲁棒性评估套件，通过图像堆叠和基于图像生成的扰动来测试VLMs对地理线索变化的稳定性。

Result: 实验显示VLMs在简单图像堆叠扰动下准确率大幅下降，基于图像生成的变体效果更差；可解释性分析表明失败源于系统性的注意力转移至干扰线索。
...
TL;DR: 提出OmniPT统一行人跟踪框架，结合LVLM进行语义理解，通过RL-Mid Training-SFT-RL训练策略解决跟踪任务建模和格式化输出问题。


<details>
  <summary>Details</summary>
Motivation: LVLM在图像级任务表现优异，但在实例级任务如行人跟踪中仍存在性能差距。新兴的Referring MOT等任务强调高级语义理解，这正是LVLM的优势所在。

Method: 采用RL-Mid Training-SFT-RL训练策略：基于预训练LVLM权重，先进行RL使模型输出可监督的边界框格式，然后用大量行人数据集进行中期训练，在多个行人跟踪数据集上进行监督微调，最后进行RL提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，该方法性能优于先前方法。

Conclusion: OmniPT框架成功将LVLM应用于行人跟踪任务，实现了跟踪、基于参考的跟踪以及跟踪对象的语义理解交互，在基准测试中表现出色。
...
TL;DR: 提出了ReBrain框架，通过检索增强扩散模型从稀疏CT扫描重建完整脑部MRI，解决了低剂量CT分辨率差导致的MRI合成挑战。
...
Method: 使用Brownian Bridge扩散模型合成2D MRI切片，同时通过微调检索模型从先验数据库中检索结构和病理相似的CT切片，通过ControlNet分支指导中间MRI切片生成，并使用球面线性插值处理检索失败情况。

Result: 在SynthRAD2023和BraTS数据集上的实验表明，ReBrain在稀疏条件下的跨模态重建中达到了最先进的性能。

Conclusion: ReBrain框架能够有效解决稀疏CT到MRI的重建问题，为无法进行MRI检查的患者提供替代方案。
...
TL;DR: STAR建模通过使用图像块位置网格中均匀生成树的遍历顺序，在保持采样性能的同时提供灵活的序列顺序，以支持图像编辑。


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中暴露随机排列序列顺序时，要么性能下降，要么在推理时序列顺序选择灵活性受限。需要一种既能保持性能又能提供灵活序列顺序的方法。

Method: STAR利用图像块位置网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保图像连接部分观测作为序列前缀出现。

Result: STAR通过定制化但结构化的随机策略，相比随机排列，保持了后缀补全能力，同时维持了采样性能，无需对语言AR建模中广泛采用的模型架构进行重大更改。

Conclusion: STAR建模能够结合图像先验知识（如中心偏差和局部性），在保持采样性能的同时提供足够灵活的序列顺序，以适应推理时的图像编辑需求。
...
Conclusion: REArtGS++通过时间几何约束和平面高斯泼溅，实现了更通用的铰接物体重建和关节参数估计。
...
TL;DR: SketchVerify是一个无需训练、基于草图验证的运动规划框架，通过测试时采样和验证循环来生成更动态连贯的轨迹，从而提高视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要使用单次规划或迭代优化，前者局限于简单运动，后者计算成本高昂。需要一种既能生成高质量运动规划又高效的解决方案。

Method: 提出草图验证框架：预测多个候选运动规划，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量视频草图来高效评分，迭代优化直到找到满意规划。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。扩展轨迹候选数量能持续提升性能。

Conclusion: SketchVerify通过测试时采样和验证循环，在保持高效的同时显著提升了运动规划质量，为视频生成提供了更优的运动控制方案。
...
TL;DR: 本文提出DiverseVAR方法，通过抑制模型输入中的关键特征分量并在输出中放大该分量，有效解决VAR模型的多样性崩溃问题，无需额外训练即可显著提升生成多样性。
...
Method: 提出DiverseVAR方法，分析发现特征图中的关键分量控制早期尺度的多样性形成，通过在模型输入中抑制关键分量并在输出中放大该分量来恢复生成多样性。

Result: 实验结果表明，该方法能显著增强生成多样性，且对性能影响可忽略不计。

Conclusion: DiverseVAR有效解锁了VAR模型的固有生成潜力，同时保持高保真度合成，为解决VAR模型多样性问题提供了简单有效的解决方案。
...
TL;DR: 本文提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像-输入文本、图像-输出文本和文本-文本三个路径的相互作用，并开发了针对不同问答对齐格式的干预方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在各种任务中表现出色，但它们仍然容易产生幻觉。本研究旨在深入理解幻觉产生的机制并开发有效的干预方法。

Method: 提出了与transformer因果架构对齐的综合干预框架，分析不同干预路径对幻觉的影响，并针对判别式和生成式格式开发了识别和干预关键幻觉头的方法。

Result: 实验表明，该方法在多个基准测试中能够持续减少各种对齐类型的幻觉。

Conclusion: LVLMs的幻觉并非来自单一因果路径，而是多个路径相互作用的结果，且模型依赖的路径取决于问答对齐格式，提出的干预方法能有效减少幻觉。
...
<details>
  <summary>Details</summary>
Motivation: 现有铰接物体重建方法通常需要多阶段、多视角等昂贵输入，存在局限性。
...
TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像，利用扩散模型和语义掩码生成新颖的合成LiDAR点云，以解决自动驾驶系统训练数据不足的问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统训练需要大量多样的点云数据，特别是边缘场景数据难以获取。现有方法依赖手工制作的3D虚拟环境，成本高且难以完全模拟真实世界复杂性。
...
Method: 提出ReCoVAD框架，包含两个核心通路：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中规模视觉语言模型生成文本事件描述和精炼异常分数，通过集成大语言模型定期审查描述以识别未见异常。
...
TL;DR: 本文分析了多语言文本到图像模型在跨语言文化一致性方面的问题，发现模型倾向于产生文化中立或英语偏见的结果。作者提出了一种定位文化敏感神经元的方法，并开发了两种对齐策略来增强文化一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多语言文本到图像模型虽然视觉真实性和语义对齐方面进步迅速，但由于语言承载文化内涵，模型在跨语言提示下应保持文化一致性。研究发现现有模型往往产生文化中立或英语偏见的结果，这源于文化相关表征激活不足而非知识缺失。

Method: 提出一种探测方法，将文化敏感信号定位到少数固定层中的一小部分神经元。基于此发现，引入两种互补的对齐策略：(1)无需主干微调的推理时文化激活，放大已识别的神经元；(2)仅更新文化相关层的层定向文化增强。

Result: 在CultureBench上的实验表明，与强基线相比，该方法在文化一致性方面持续改进，同时保持了保真度和多样性。

Conclusion: 通过定位文化敏感神经元并应用针对性的对齐策略，可以有效提升多语言文本到图像模型的文化一致性，而不会损害其他性能指标。
...
Conclusion: ChainV通过动态整合视觉提示显著提升了多模态推理的准确性和效率，特别是在数学密集型任务中效果显著。
...
TL;DR: 本文提出MuM模型，通过多视图掩码自编码扩展MAE方法，专门针对3D视觉任务进行特征学习，在多个下游任务中超越了DINOv3和CroCo v2等最先进视觉编码器。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习方法主要优化语义理解而非几何推理，CroCo虽然针对3D理解进行了改进，但仍有局限性。本文旨在继续CroCo的研究路径，专注于学习适合3D视觉的特征。

Method: 将MAE扩展到同一场景的任意多个视图，通过统一掩码所有视图并使用具有帧间注意力的轻量级解码器，构建比CroCo更简单、更可扩展的方法。

Result: 在包括前向重建、密集图像匹配和相对姿态估计在内的下游任务中，MuM模型的表现超过了最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: 通过多视图掩码自编码方法，MuM成功学习了专门针对3D视觉任务的特征表示，在多个几何推理任务中取得了优越性能。
...
Method: 提出了一个统一的三级监督方案，通过加速度约束强制物理合理性，利用事件流进行高时间分辨率指导，并使用卡尔曼正则化器融合多源观测。还设计了基于实时运动状态自适应调度训练过程的运动感知模拟退火策略。
...
TL;DR: 本文提出了一种使用生成模型创建多样化合成人脸图像的方法，并发布了包含27,780张图像、926个身份的DIF-V数据集，以解决现有数据集存在的种族、性别等偏见问题。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸图像数据集在种族、性别等人口统计特征上存在显著偏见，限制了人脸验证系统的有效性和公平性。

Method: 提出了一种综合方法，集成先进的生成模型来创建多样化、高质量合成人脸图像，强调代表各种面部特征，并确保符合身份证照片特征。

Result: 分析显示现有验证模型对某些性别和种族存在偏见，身份风格修改会对模型性能产生负面影响。

Conclusion: 通过解决现有数据集中的固有不平等问题，这项工作不仅丰富了人工智能多样性和伦理的讨论，还为开发更包容和可靠的人脸验证技术奠定了基础。
...
TL;DR: 该论文提出从传统的1D相关色温(CCT)映射转向2D色度空间，并使用轻量级多层感知机(MLP)替代传统CCT插值，以提升非普朗克光源(如LED)下的色度测量精度。
...
Method: 使用基于2D色度特征的轻量级MLP替代传统CCT插值，通过包含代表性LED光源的光箱校准程序训练MLP模型。

Result: 在多种LED照明下验证，该方法平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署。

Conclusion: 从1D CCT到2D色度空间的转变显著提高了非普朗克光源下的色度测量精度，MLP方法提供了稳健且计算高效的解决方案。
...
TL;DR: 提出了一个多阶段优化框架，将高性能浮点图像压缩模型转换为高效的FPGA硬件友好型整数实现，包括动态范围感知量化、混合精度搜索和通道剪枝技术。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习图像压缩模型在资源受限FPGA上部署的挑战，弥合高性能浮点模型与高效整数实现之间的差距。

Method: 1. 动态范围感知量化(DRAQ)：使用统计校准的激活裁剪和权重正则化处理极端数据异常值和动态范围问题；2. 渐进混合精度搜索算法：为每层分配最优非均匀位宽；3. 针对GDN层的通道剪枝方法。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%；硬件感知优化进一步降低20%以上计算复杂度，对率失真性能影响可忽略，最终模型在效率和性能上均优于现有FPGA实现。

Conclusion: 该框架成功实现了高性能深度学习图像压缩模型在FPGA上的高效部署，在保持压缩质量的同时显著提升了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散式真实世界图像超分辨率中保真度与可控性之间的平衡问题：多步扩散方法存在生成多样性和随机性导致保真度低，而一步方法因保真度特定微调而失去控制灵活性。

Method: 提出噪声混合视觉流设计：新引入的视觉流接收带可调噪声的低质量图像，原始视觉流接收带一致噪声的低质量图像；采用保真度感知对抗训练实现一步推理。

Result: 在通用真实世界图像超分辨率任务上达到最先进性能，并在中文字符真实场景文本图像超分辨率等挑战性场景中实现提示可控性，无需特定数据集训练。
...
Method: 提出CC-DiceCE损失函数，基于CC-Metrics框架，并与现有的blob损失在nnU-Net框架中进行基准测试。

Result: CC-DiceCE损失提高了病灶检测（召回率），分割性能几乎没有下降，但会略微增加假阳性。在多数据集研究中，CC-DiceCE通常优于blob损失。

Conclusion: CC-DiceCE损失函数能有效改善小病灶检测性能，在医学图像分割中具有实用价值。
...
TL;DR: 本文介绍了RS-FMD数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD包含150多个遥感基础模型，REMSA是基于LLM的智能体，能够从自然语言查询中自动选择合适模型。


<details>
  <summary>Details</summary>
Motivation: 遥感领域基础模型越来越多，但由于文档分散、格式异构和部署约束不同，选择合适的模型变得困难。

Method: 构建RS-FMD结构化数据库，开发基于LLM的REMSA智能体，使用上下文学习对候选模型进行排序，并提供透明解释。

Result: REMSA在75个专家验证的查询场景基准测试中表现优于多个基线方法，包括朴素智能体、密集检索和非结构化RAG-based LLM。
...
TL;DR: DiffRefiner是一个两阶段轨迹预测框架，第一阶段使用基于transformer的Proposal Decoder生成粗略轨迹预测，第二阶段应用Diffusion Refiner迭代去噪和细化这些初始预测，通过结合判别性轨迹提议模块来增强基于扩散的规划性能。


<details>
  <summary>Details</summary>
Motivation: 生成式方法（如扩散模型）学习未来运动的底层分布，实现更灵活的轨迹预测，但现有方法通常依赖于去噪人工制作的轨迹锚点或随机噪声，仍有很大改进空间。

Method: 提出DiffRefiner：1）第一阶段使用transformer-based Proposal Decoder通过预定义轨迹锚点从传感器输入回归生成粗略轨迹预测；2）第二阶段应用Diffusion Refiner迭代去噪和细化初始预测；3）设计细粒度去噪解码器增强场景合规性。

Result: 在NAVSIM v2上达到87.4 EPDMS，在Bench2Drive上达到87.1 DS和71.4 SR，在两个公共基准测试中都创下了新记录。

Conclusion: DiffRefiner通过结合判别性轨迹提议模块和生成式细化过程，实现了最先进的轨迹预测性能，各组件有效性通过消融研究得到验证。
...
<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，影响固定黑盒下游推理模型的性能。现有无配对图像转换方法往往忽视类别特定的语义对齐，造成内容-类别映射错位，损害诊断准确性。

Method: 提出UI-Styler框架，采用模式匹配机制将目标图像的纹理模式转移到源图像上，同时保持源图像的结构内容；引入基于目标域伪标签的类别感知提示策略，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的大量实验表明，UI-Styler在分布距离和下游任务（如分类和分割）上始终优于现有UIT方法，达到最先进的性能水平。

Conclusion: UI-Styler通过有效的风格转换和语义对齐机制，成功解决了超声图像跨设备域适应问题，为医学图像分析提供了可靠的解决方案。
...
TL;DR: FireScope是一个基于视觉语言模型的推理生成框架，通过整合哨兵2号影像、气候数据和专家定义的风险栅格，预测野火风险图，并能在跨大陆场景中实现可靠泛化。


<details>
  <summary>Details</summary>
Motivation: 现有野火风险预测方法缺乏因果推理和多模态理解能力，难以实现可靠的泛化。需要开发能够整合视觉、气候和地理因素，并具备推理能力的野火风险预测模型。

Method: 提出FireScope-Bench数据集和基准，结合哨兵2号影像、气候数据和专家定义的风险栅格；开发FireScope框架，基于视觉语言模型，通过强化学习和视觉监督学习预测风险栅格并生成推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope取得显著性能提升；专家反馈和自动分析确认其推理轨迹忠实且语义有意义。

Conclusion: 基于语言的推理能够提升视觉生成模型的泛化能力；FireScope是首个展示语言推理能改善视觉生成泛化性的框架，也是首个可跨大陆应用的高分辨率野火风险模型。
...
TL;DR: 本文系统评估了自监督表征在音频-视觉深度伪造检测中的应用潜力，发现这些特征能捕获相关检测信息且具有互补性，但无法可靠地跨数据集泛化。


<details>
  <summary>Details</summary>
Motivation: 探索自监督表征在音频-视觉深度伪造检测中的潜力，此前研究要么孤立使用这些特征，要么将其嵌入复杂架构中，缺乏系统性评估。

Method: 在多种模态（音频、视频、多模态）和领域（唇部运动、通用视觉内容）上系统评估自监督特征，从检测效果、编码信息可解释性和跨模态互补性三个维度进行分析。

Result: 大多数自监督特征能捕获深度伪造相关信息且具有互补性，模型主要关注语义相关区域而非虚假伪影，但无法可靠地跨数据集泛化，泛化失败源于数据集特性而非特征本身。

Conclusion: 自监督表征在深度伪造检测中具有潜力，能学习有意义的模式，但实现稳健的跨域性能仍面临根本性挑战。
...
Method: 首先引入INTSD大规模夜间交通标志数据集，包含41个类别；其次提出LENS-Net方法，集成自适应图像增强检测器进行联合光照校正和标志定位，以及结构化多模态CLIP-GCNN分类器，利用跨模态注意力和图基推理。

Result: 该方法超越了现有框架，消融研究证实了其关键组件的有效性。

Conclusion: LENS-Net通过集成图像增强和多模态分类，显著提升了夜间交通标志识别的性能，数据集和代码已公开用于研究。
...
TL;DR: PostCam是一个用于动态场景中新颖视角视频生成的框架，支持拍摄后编辑相机轨迹。通过查询共享交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，采用两阶段训练策略，在相机控制精度和视频生成质量上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频重拍摄方法存在相机运动注入策略不理想的问题，限制了相机控制精度，且生成的视频无法保留源视频的精细视觉细节。

Method: 提出查询共享交叉注意力模块，将6-DoF相机位姿和2D渲染视频帧两种控制信号融合到共享特征空间中；采用两阶段训练策略：先学习粗粒度相机控制，再结合视觉信息优化运动精度和视觉保真度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性上比现有最优方法提升超过20%，同时达到最高的视频生成质量。

Conclusion: PostCam框架通过创新的控制信号融合和两阶段训练策略，实现了更精确灵活的相机运动操控，显著提升了动态场景中新颖视角视频的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在嵌入3D位置到视觉表示时难以实现动作执行的时间一致性控制，需要解决时空一致的精细操作问题。

Method: 采用4D感知视觉表示（将1D时间嵌入3D位置形成4D嵌入）和时空动作表示（扩展传统空间动作表示包含时间信息），通过跨注意力机制融合多模态表示到LLM中进行时空动作预测。

Result: 在多个机器人操作任务中验证了方法的优越性，实现了空间平滑和时间一致的操作性能。

Conclusion: VLA-4D模型通过统一的4D感知框架，有效解决了机器人操作中的时空一致性问题，为通用VLA模型提供了新的解决方案。
...
TL;DR: 本文提出了一种针对医学图像分割的持续学习方法CA-SAM，通过轻量级的对齐层模块来平衡SAM模型的计算效率和性能，在九个医学分割数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的隐私政策差异使得联合训练不可行，需要从数据流中进行持续学习以避免灾难性遗忘。虽然SAM模型具有强大的零样本先验，但其大参数量和计算开销限制了实际部署。

Method: 提出了对齐层（Alignment Layer）作为轻量级即插即用模块，用于对齐编码器-解码器特征分布，有效适配SAM到特定医学图像。基于此构建了CA-SAM持续学习策略，自动适配对齐层来缓解灾难性遗忘，同时利用SAM的零样本先验保持对未见数据集的强性能。

Result: 在九个医学分割数据集的持续学习场景下进行实验，CA-SAM实现了最先进的性能表现。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM通过轻量级对齐层和持续学习策略，有效解决了医学图像分割中的持续学习挑战。
...
TL;DR: SPECTRE是一个基于Transformer的3D CT基础模型，通过自监督和跨模态预训练学习通用CT表示，解决了体素CT的令牌扩展、几何各向异性和弱监督等挑战，在多个CT基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 体素CT面临令牌扩展、几何各向异性和弱临床监督等独特挑战，使得标准Transformer和对比学习方法难以直接应用，需要开发专门的基础模型。

Method: 使用3D视觉Transformer架构，结合DINO自蒸馏和SigLIP视觉语言对齐进行预训练，同时优化局部Transformer进行高分辨率特征提取和全局Transformer进行全扫描上下文建模。

Result: 在多个CT基准测试中，SPECTRE在零样本和微调设置下均优于现有CT基础模型，证明无需私有数据即可获得高性能通用表示。

Conclusion: SPECTRE是一个可扩展、开源且完全基于Transformer的3D医学影像基础模型，能够学习几何一致且临床有意义的特征表示。
...
TL;DR: 提出了一种针对鱼眼图像的BEV语义分割框架，通过几何反投影和像素级深度分布估计处理非线性失真，使用高斯参数化建模几何不确定性，无需去失真或透视校正即可生成连续的BEV语义图。


<details>
  <summary>Details</summary>
Motivation: 鱼眼图像存在极端非线性失真、遮挡和深度模糊，使得准确的BEV语义分割具有挑战性。

Method: 使用多相机高分辨率鱼眼图像，通过校准的几何反投影和像素级深度分布估计，将每个像素通过高斯参数化提升到3D空间，预测空间均值和各向异性协方差来显式建模几何不确定性，然后通过可微分splatting将投影的3D高斯融合到BEV表示中。

Result: 在复杂停车和城市驾驶场景中表现出强大的分割性能，在严重鱼眼失真和多样化环境条件下，可行驶区域的IoU达到87.75%，车辆的IoU达到57.26%。

Conclusion: 该框架能够有效处理鱼眼图像的几何失真，生成连续的不确定性感知语义图，在挑战性场景中实现准确的BEV语义分割。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接学习连续3D语义占据，在保持恒定内存的同时实现长距离监督和推理，在Occ3D-nuScenes基准测试中比先前方法提升26%的语义RayIoU。


<details>
  <summary>Details</summary>
Motivation: 由于大规模3D标注成本高昂，现有方法要么依赖2D渲染一致性导致3D结构仅隐式出现，要么使用离散体素网格限制了空间精度和可扩展性。

Method: 使用独立的4D时空查询在相邻帧间采样，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入压缩场景表示在保持近场细节的同时平滑压缩远距离区域。

Result: 在自监督Occ3D-nuScenes基准测试中语义RayIoU比先前相机方法提升26%，运行速度为11.6 FPS。
...
TL;DR: 提出了一种用于彩色图像盲去卷积的新方法，使用四元数保真项来处理颜色通道间的关系，避免传统方法将彩色图像转为灰度或单独处理各颜色通道的问题。


<details>
  <summary>Details</summary>
Motivation: 现有彩色图像盲去卷积方法通常将彩色图像转为灰度或单独处理每个颜色通道，忽略了颜色通道之间的相互关系，导致去模糊效果不佳。

Method: 设计了一个新颖的四元数保真项，利用四元数卷积核的特性，包含四个核：一个用于捕捉整体模糊的非负卷积核，以及三个分别对应红、绿、蓝通道的无约束卷积核来建模它们之间的未知相互依赖关系。在盲去卷积过程中使用归一化的四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的广泛实验表明，该方法有效消除了伪影，显著提高了去模糊效果。

Conclusion: 该方法展示了作为彩色图像去卷积强大工具的潜力，能够有效处理颜色通道间的相互关系，提升去模糊性能。
...
TL;DR: 本文提出了一种基于相关性反馈的机制来改进视觉语言模型在推理时的检索性能，包括四种反馈策略：伪相关性反馈、生成相关性反馈、注意力反馈总结器和显式反馈，实验表明这些方法能显著提升检索效果。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言查询的视觉搜索，但提升性能通常需要微调或使用更大模型。本文受传统文本搜索启发，旨在通过相关性反馈机制在推理时改进检索性能，避免对模型进行微调。

Method: 提出了四种相关性反馈策略：1）伪相关性反馈：基于排名靠前的结果优化查询嵌入；2）生成相关性反馈：使用合成标题进行查询优化；3）注意力反馈总结器：基于Transformer的多模态细粒度特征整合模型；4）显式反馈：使用真实标题作为上限基准。

Result: 在Flickr30k和COCO数据集上的实验显示，生成相关性反馈、注意力反馈总结器和显式反馈相比无反馈检索，在MRR@5指标上对小型视觉语言模型提升3-5%，对大型模型提升1-3%。注意力反馈总结器与显式反馈类似，能缓解查询漂移并在多轮检索中比生成相关性反馈更稳健。

Conclusion: 相关性反馈能持续提升不同视觉语言模型的检索性能，为交互式和自适应视觉搜索开辟了新机会。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段学习框架，通过预训练、多粒度微调和强化学习优化，特别解决了立体化学信息识别的挑战，在立体分子识别方面达到最先进性能。
...
Method: 采用三阶段训练范式：1) 在大规模但嘈杂数据集上进行预训练；2) 使用具有更丰富监督信号的数据集进行多粒度微调，探索化学键分类和原子定位等辅助任务；3) 使用强化学习进行后训练优化，并引入新的立体化学结构数据集。

Result: 实验结果表明，即使MolSight参数规模相对紧凑，GRPO算法仍能进一步提升模型在立体分子识别上的性能，在多样化数据集上实现了最先进的(立体)化学光学结构识别性能。

Conclusion: MolSight框架通过创新的三阶段训练方法，成功解决了OCSR中立体化学信息识别的关键挑战，为化学信息学、药物发现和LLM应用提供了有效的自动化工具。
...
TL;DR: BiFingerPose是一种基于双模态的手指姿态估计算法，通过结合电容图像和指纹补丁，能够准确预测完整的手指姿态信息，包括之前无法可靠估计的滚转角。
...
Method: 提出双模态输入方法，包括来自触摸屏的电容图像和来自屏下指纹传感器的指纹补丁。通过这种双模态融合来提升姿态估计性能。

Result: BiFingerPose相比之前SOTA方法在预测性能上提升超过21%，任务完成效率提高2.5倍，用户操作准确率提升23%。能够可靠估计滚转角，其他姿态参数的预测性能也大幅提升。

Conclusion: 该方法在增强认证安全性和改善交互体验方面具有应用潜力，通过开发的原型展示了交互可能性。
...
TL;DR: 本文提出了一种基于几何和语义特征分层融合的空间感知视觉编码器SpatialGeo，以提升多模态大语言模型在三维空间推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在空间推理能力上存在局限，主要源于视觉编码器（如CLIP）的空间模糊性缺陷，仅关注实例级语义特征而忽略了空间几何信息。

Method: 通过分层适配器将自监督学习的几何特征与CLIP的语义特征融合，采用随机特征丢弃策略避免仅依赖CLIP编码器的平凡解，高效训练预训练LLaVA模型。

Result: SpatialGeo在空间推理任务中显著提升准确率，在SpatialRGPT-Bench上比现有最优模型至少提高8.0%，推理时内存成本减少约50%。

Conclusion: 提出的空间感知视觉编码器有效增强了MLLMs的空间基础能力，为三维空间推理任务提供了高效解决方案。
...
TL;DR: NoPe-NeRF++是一种无需姿态先验的局部到全局优化算法，通过特征匹配初始化、局部联合优化和全局捆绑调整，显著提升了NeRF训练中的姿态估计精度和新视角合成质量。
...
TL;DR: 本文提出了一种改进生成图像中透明物体折射效果的方法，通过同步物体边界内外像素并使用斯涅尔折射定律进行扭曲和合并，从而生成更符合光学物理约束的图像。


<details>
  <summary>Details</summary>
Motivation: 生成图像模型在合成透明物体时表现不佳，特别是折射效果，因为折射像素光线经常与图像其他部分的表面相交，对颜色产生约束。现有模型未能充分学习光学定律来准确渲染折射物体。

Method: 在生成过程的每一步，使用斯涅尔折射定律对物体边界内外像素进行扭曲和合并来实现同步。对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的第二个生成全景图像同步来恢复其外观。

Result: 该方法生成了更符合光学物理约束的图像，折射效果更加真实可信。

Conclusion: 提出的方法能够显著提高生成图像中透明物体折射效果的物理准确性，通过像素同步和光学定律的应用实现了更逼真的折射渲染。
...
TL;DR: 提出了一个统一的框架，用于生成多媒体绘画过程，通过语义驱动的风格控制机制和多媒介风格增强，实现跨媒介的一致纹理演化和过程转移。
...
Result: 在LPIPS、DINO和CLIP指标上取得了强劲结果，构建了大规模真实绘画过程数据集，并通过感知距离剖面曲线定量建模创作序列。

Conclusion: 该框架能够生成跨媒介一致、时间连贯的绘画过程，有效模拟人类艺术创作流程，为个性化绘画教学提供了新途径。
...
TL;DR: DSeq-JEPA提出了一种基于人类视觉感知的顺序预测架构，通过识别主要判别性区域并按重要性顺序预测，结合了JEPA的潜在预测和GPT风格的顺序推理。


<details>
  <summary>Details</summary>
Motivation: 现有I-JEPA方法对所有区域进行均匀独立预测，缺乏对预测顺序和重要性的显式建模，而人类视觉感知具有选择性注意和顺序处理的特点。

Method: 首先基于Transformer衍生的显著性图识别主要判别性区域，然后按判别性顺序预测后续区域，形成从主要到次要线索的课程式语义进展。

Result: 在图像分类、细粒度视觉分类、检测分割和低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体更关注判别性和泛化性表示。

Conclusion: DSeq-JEPA通过整合潜在预测和顺序推理，实现了更有效的视觉表示学习，在多个任务上表现优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占据估计中广泛使用，但大量高斯基元显著增加内存需求，不适合实时推理。超二次曲面允许减少基元数量和内存需求，但缺乏超二次曲面光栅化器阻碍了模型监督实现。

Method: 提出SuperQuadricOcc方法，采用基于超二次曲面的场景表示。通过多层级球面细分的高斯近似超二次曲面，在训练期间实现高斯光栅化监督。

Result: 在Occ3D数据集上，相比之前高斯方法，内存占用减少75%，推理速度提升124%，mIoU提升5.9%，无需时间标签。基元数量减少84%，首次实现实时推理同时保持竞争力性能。

Conclusion: SuperQuadricOcc是首个实现实时推理的占据模型，通过超二次曲面表示显著减少了内存需求和基元数量，同时保持竞争性性能，并提供了快速超二次曲面体素化模块用于评估。
...
TL;DR: 提出了ATAC方法，通过在CLIP嵌入空间中计算增强引起的漂移向量来推断语义恢复方向，从而在测试时对抗性防御中实现高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像文本匹配方面表现出色，但对图像对抗扰动高度脆弱。由于对抗性微调成本过高，现有测试时防御方法的鲁棒性仍然有限。

Method: 直接在CLIP嵌入空间中操作，计算增强引起的漂移向量来推断语义恢复方向，并基于这些潜在漂移的角度一致性来校正嵌入。

Result: 在广泛基准测试中，ATAC始终实现极高的鲁棒性，平均比先前最先进方法高出近50%，同时计算开销最小。在非常规和极端设置下保持最先进的鲁棒性，甚至对自适应攻击也表现出非平凡的鲁棒性。
...
TL;DR: SVRecon将稀疏体素光栅化与符号距离函数(SDF)结合用于高保真表面重建，通过几何初始化和空间平滑损失解决稀疏体素优化中的局部最小值问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素与邻居空间解耦且边界锐利，在优化过程中容易陷入局部最小值，而SDF提供的平滑连续几何场在独立参数化的稀疏体素间保持平滑性具有挑战。

Method: 采用视觉几何模型进行鲁棒几何初始化，并通过空间平滑损失在父子体素组和兄弟体素组间强制一致关系，促进连贯平滑的体素结构。

Result: 在多个基准测试中的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛速度。

Conclusion: SVRecon通过结合稀疏体素和SDF，成功解决了稀疏体素优化中的局部最小值问题，实现了高保真表面重建。
...
TL;DR: 本文提出非参数概率鲁棒性(NPPR)作为更实用的鲁棒性度量，它不依赖预定义的扰动分布，而是直接从数据中学习优化的扰动分布，在分布不确定性下实现保守的鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型对微小输入扰动敏感，现有概率鲁棒性(PR)方法假设固定已知的扰动分布，这在实践中不现实。

Method: 基于高斯混合模型(GMM)和多层感知机(MLP)头以及双三次上采样开发NPPR估计器，涵盖输入相关和输入无关的扰动场景。

Result: 在CIFAR-10/100和Tiny ImageNet数据集上的实验表明，NPPR比现有方法提供更保守的PR估计，降低幅度可达40%。

Conclusion: NPPR被验证为更实用的鲁棒性度量，建立了对抗鲁棒性(AR)、概率鲁棒性(PR)和NPPR之间的理论关系。
...
TL;DR: MorphSeek提出了一种细粒度表示级别的策略优化范式，将可变形图像配准重新定义为潜在特征空间中的空间连续优化过程，通过随机高斯策略头实现高效探索和从粗到细的优化。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像配准中高维变形空间和体素级监督稀缺的挑战，现有强化学习方法将空间投影为粗糙低维表示，限制了捕捉空间变异变形的能力。

Method: 在编码器顶部引入随机高斯策略头来建模潜在特征的分布，集成无监督预热和弱监督微调，通过多轨迹采样稳定训练并提高标签效率。

Result: 在三个3D配准基准测试（OASIS脑MRI、LiTS肝脏CT和腹部MR-CT）中，相比竞争基线实现了持续的Dice改进，同时保持高标签效率和低参数成本。

Conclusion: MorphSeek推进了表示级别策略学习范式，实现了空间一致和数据高效的变形优化，为高维设置中的可扩展视觉对齐提供了原则性、骨干无关和优化器无关的解决方案。
...
TL;DR: 本文提出了一种名为MCMoE的新框架，用于解决多模态动作质量评估中模态缺失的问题。该框架通过自适应门控模态生成器和专家混合机制，在单阶段训练中统一了单模态和联合表示学习。


<details>
  <summary>Details</summary>
Motivation: 多模态动作质量评估在实际推理阶段经常面临部分模态不可用的问题，导致现有多模态模型无法运行，且由于跨模态交互中断而引发灾难性性能下降。

Method: 提出MCMoE框架，包含自适应门控模态生成器动态融合可用信息重建缺失模态，设计模态专家学习单模态知识并动态混合所有专家知识提取跨模态联合表示，通过专家混合进一步细化和补充缺失模态。

Result: 在三个公共AQA基准测试上的广泛实验表明，MCMoE在完整和不完整多模态学习中都达到了最先进的结果。
...
TL;DR: 提出了MMT-ARD多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet和零样本基准测试中显著提高了鲁棒精度和训练效率。
...
Conclusion: MMT-ARD在增强多模态大模型对抗鲁棒性方面具有有效性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行前向模拟，无法回答反事实查询（如"如果移除这个物体会发生什么？"），而这类能力对于评估AI在不同条件下的行为越来越重要。

Method: 1) 构建观察场景的数字孪生，以结构化文本形式显式编码对象及其关系；2) 使用大语言模型推理这些表示，预测反事实干预如何随时间传播改变观察场景；3) 用修改后的表示条件化视频扩散模型以生成反事实视觉序列。
...
TL;DR: Radar2Shape是一个去噪扩散模型，能够从部分可观测的高频雷达信号中重建任意3D形状，通过将雷达频率与多分辨率形状特征相关联来实现这一目标。
...
Method: 采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以雷达信号频率为条件，以从粗到细的方式扩散到这个潜在空间中。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI序列图像来估计脊柱年龄，并研究脊柱年龄差距与退行性病变和生活方式因素的关联。


<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，易受年龄相关退化的影响。通过MRI识别这些退化对于评估脊柱健康至关重要，需要开发自动化的脊柱年龄估计方法。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退化聚类，构建深度学习模型进行脊柱年龄估计，通过消融研究确定最佳模型配置。

Result: 发现脊柱年龄差距与椎间盘突出、骨赘、椎管狭窄、骨折等退行性病变相关，也与吸烟和体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差距可作为衡量整体脊柱健康的有用生物标志物，对临床评估脊柱退行性病变具有重要价值。
...
TL;DR: 本文分析了多模态模型中缩小LLM规模对视觉能力的影响，发现视觉能力下降比推理能力更严重，提出了Extract+Think方法来解决这个问题。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的规模化发展，实际应用需要更小、更高效的模型。本文旨在研究缩小大型语言模型(LLM)规模如何影响多模态能力，特别是视觉能力。

Method: 通过视觉提取调优(visual extraction tuning)训练模型提取任务相关的视觉细节，然后应用逐步推理生成答案，形成Extract+Think方法。

Result: 研究发现LLM规模缩小对视觉能力的影响比对推理能力的影响更大，Extract+Think方法在效率和性能方面达到了新的标准。

Conclusion: Extract+Think方法通过分离视觉提取和推理步骤，有效解决了多模态模型规模缩小时的视觉能力瓶颈问题。
...
TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大信息区域、重新编码像素并更新推理状态，在文本丰富的视频问答任务中取得SOTA效果。
...
Method: 提出视觉反刍：迭代选择帧、放大信息区域、重新编码像素、更新推理状态。构建两个数据集，采用多阶段反刍学习框架，通过SFT和GRPO-based RL训练7B LMM。
...
Result: 标记连接方法更具参数效率且性能更优；在生成质量、3D一致性和指令保真度方面优于现有2D提升方法。

Conclusion: 该方法为指令引导的3D编辑设定了新的基准，实现了高效、一致且高质量的3D内容编辑。
...
TL;DR: 提出SPI（语义金字塔索引）框架，通过多分辨率向量索引和查询自适应分辨率控制，解决现有向量数据库检索管道在语义粒度适应性方面的不足，显著提升检索速度和内存效率。
...
Result: 在多个RAG任务中，SPI实现了最高5.7倍的检索加速和1.8倍的内存效率提升，端到端QA F1分数提升最高2.5分。

Conclusion: SPI框架与现有向量数据库基础设施兼容，可轻松部署到生产RAG系统中，为检索质量和延迟提供理论保证。
...
TL;DR: Bench360是一个用于本地大语言模型推理的综合性基准测试框架，允许用户自定义任务、数据集和指标，自动测试不同LLM、推理引擎和量化级别在各种使用场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试工具设计目标狭窄，无法将系统指标和任务特定指标整合到统一的易用框架中，用户难以找到平衡功能和非功能需求的最优配置。

Method: 开发Bench360框架，支持用户定义自定义任务、数据集和任务特定指标，自动测试不同LLM、推理引擎和量化级别在单流、批处理和服务器等使用场景下的性能。

Result: 在四个常见LLM任务、三个硬件平台和四个推理引擎上的测试结果显示，任务性能和系统效率之间存在多个有趣的权衡，不同推理引擎和模型存在显著差异。

Conclusion: 本地推理没有单一最佳设置，这强烈表明需要像Bench360这样的框架来帮助用户找到最优配置。
...
TL;DR: 本研究评估了大型语言模型对突尼斯阿拉伯语的理解能力，创建了包含突尼斯方言、标准突尼斯阿拉伯语和英语的平行数据集，并在转写、翻译和情感分析三个任务上测试了多个流行模型。
...
Method: 创建包含平行翻译和情感标签的新数据集，在转写、翻译和情感分析三个任务上对多个流行LLM进行基准测试。

Result: 结果显示模型之间存在显著差异，揭示了它们在理解和处理突尼斯方言方面的优势和局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类别呈各向同性分布，将边界限制为球形，忽略了不同特征方向上的分布方差，限制了开放意图检测的性能。

Method: 1) 使用监督对比学习获得判别性特征空间；2) 应用可学习矩阵参数化椭圆体作为已知类边界；3) 通过双损失函数优化边界，平衡经验风险和开放空间风险。

Result: 在多个文本意图基准和问题分类数据集上达到最先进性能，椭圆体边界的灵活性展现出优越的开放意图检测能力和泛化潜力。

Conclusion: EliDecide方法通过椭圆体决策边界有效解决了文本开放意图分类问题，在复杂开放世界场景中具有广泛的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在需要与人类价值观对齐的应用中日益重要，但传统的模型微调方法是静态的，无法适应涉及动态价值观和偏好的日常情境。

Method: 开发了一个实用、可重复且模型无关的程序，使用Schwartz基本人类价值观理论和对话数据集进行结构化评估，比较基线提示与明确基于价值观的提示。

Result: 实验表明，即使不改变模型或动态优化提示，价值观引导也是可能的。

Conclusion: 该方法提供了一种在不修改模型的情况下实现价值观引导的有效途径。
...
TL;DR: 该论文研究了AI检测模型对经过AI轻微润色的人类撰写文章的误判问题，特别是在阿拉伯语领域。研究创建了两个数据集来评估14个大型语言模型和商业AI检测器的性能，发现轻微润色会显著降低检测准确率。


<details>
  <summary>Details</summary>
Motivation: 解决AI检测模型对经过AI轻微润色的人类撰写文章的误判问题，这种误判可能导致错误指控作者使用AI抄袭，并损害AI检测器的可信度。

Method: 生成两个阿拉伯语数据集：第一个包含800篇文章（半AI生成半人类撰写），用于评估14个LLM和商业AI检测器；第二个Ar-APT数据集包含400篇人类撰写文章，由10个LLM使用4种润色设置处理，共16400个样本，用于评估8个最佳模型对轻微润色文章的检测能力。

Result: 所有AI检测器都错误地将大量文章归因于AI。最佳LLM Claude-4 Sonnet准确率为83.51%，但对LLaMA-3轻微润色的文章准确率降至57.63%。最佳商业模型originality.AI准确率为92%，但对Mistral或Gemma-3轻微润色的文章准确率骤降至12%。

Conclusion: AI检测模型对经过AI轻微润色的人类撰写文章存在严重误判问题，需要开发更鲁棒的检测方法来应对这种挑战。
...
TL;DR: 本文复现了基于最近邻的测试时训练方法，通过在推理时对检索到的最近邻序列进行微调，显著降低了语言模型的困惑度和比特数指标。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在大型语言模型中的有效性，特别是通过最近邻检索来提升模型在推理时的性能，并探索该方法在不同规模模型和架构上的通用性。

Method: 使用预训练的RoBERTa嵌入和Faiss索引，为每个测试输入检索20个最近邻，然后对每个邻居应用一次梯度更新，在GPT-2、GPT-Neo和R1-Distilled-Qwen2.5-1.5B模型上进行实验。

Result: 测试时训练显著降低了困惑度和比特数指标，在GitHub和EuroParl等结构化或专业数据集上改进最大。未在The Pile上预训练的模型受益更多，小型模型可接近大型模型的性能。
...
TL;DR: 本文通过系统分析多语言大语言模型内部的语言信息结构，发现语言信息在第一个Transformer块就实现高度分离，且在整个模型深度中保持线性可分。研究发现语言方向与词汇嵌入的对齐程度与训练数据的语言组成密切相关，揭示了训练语料对模型语言表征的结构性印记效应。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言LLM在多语言任务上表现出色，但对其内部语言信息如何组织和在不同层中如何涌现的系统性分析仍然有限。本文旨在深入理解多语言LLM中语言信息的结构特性和形成机制。

Method: 在六个多语言LLM上进行全面的探测研究，覆盖所有268个Transformer层，使用线性和非线性探针结合新的Token-Language Alignment分析，量化语言编码的层间动态和几何结构。

Result: 语言信息在第一个Transformer块就实现显著分离（从第0层到第1层提升76.4±8.2个百分点），且在整个模型深度中几乎完全线性可分。中文包含模型的中文匹配峰值达到16.43%，而英语中心模型仅为3.90%，显示出4.21倍的结构性印记效应。

Conclusion: 多语言LLM不是通过表面脚本特征来区分语言，而是通过训练语料塑造的潜在表征结构来区分。这些发现为多语言表示学习中的数据组成策略和公平性提供了实用见解。
...
TL;DR: 本文提出了一种基于语言模型的嵌入方法，用于解决SNOMED CT中词汇外查询的概念检索问题，在直接上位词和祖先概念检索任务上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: SNOMED CT作为大规模生物医学本体，其知识检索面临语言歧义、同义词、多义词等挑战，特别是当查询是词汇外(OOV)时，问题更加严重。

Method: 采用基于语言模型的本体嵌入方法，构建了针对SNOMED CT的OOV查询标注数据集，测试直接上位词和祖先概念的检索性能。

Result: 该方法在SNOMED CT概念检索任务上优于SBERT和两种词汇匹配基线方法，证明了方法的有效性。

Conclusion: 所提出的基于语言模型的嵌入方法能够有效处理SNOMED CT中的OOV查询检索问题，且方法具有通用性，可扩展到其他本体。
...
TL;DR: 研究发现共情行为（牺牲任务效率来满足人类需求）在LLM激活空间中呈现线性方向，通过对比提示在三个模型上测试检测和操控能力，发现不同模型在检测精度和操控稳定性方面存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中编码的共情行为是否可以被检测和操控，以及安全训练对共情行为编码和操控性的影响。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上测试共情行为的检测和操控能力，包括AUROC评估和双向操控实验。

Result: 所有模型在最佳层都能高精度检测共情行为（AUROC 0.996-1.00），但跨模型探针一致性有限。操控成功率：Qwen 65.3%、Phi-3 61.7%、Dolphin在亲共情方向94.4%但反共情方向崩溃。安全训练不影响共情编码但可能影响操控稳健性。

Conclusion: 共情行为在LLM中编码为线性方向，检测精度高但操控能力因模型而异。安全训练主要影响操控稳健性而非防止操控，Dolphin显示非对称操控性，亲共情方向稳健但反共情方向易崩溃。
...
### [99] [Interpretable dimensions support an effect of agentivity and telicity on split intransitivity](https://arxiv.org/abs/2511.16824)
*Eva Neu,Brian Dillon,Katrin Erk*

Main category: cs.CL

TL;DR: 本研究使用基于种子词的可解释维度重新检验了非作格/非宾格动词与施事性/终结性之间的关联，发现两者确实存在联系，且可解释维度与人类判断结合能有效评估语义属性。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为施事性动作动词更倾向非作格句法，终结性事件动词更倾向非宾格句法，但Kim等人(2024)发现人类对施事性和终结性的评分不能很好预测不及物动词的句法行为。

Method: 使用基于施事性和终结性量表对立极的种子词计算可解释维度。

Result: 研究结果支持非作格/非宾格与施事性/终结性之间的关联。
...
### [100] [PEPPER: Perception-Guided Perturbation for Robust Backdoor Defense in Text-to-Image Diffusion Models](https://arxiv.org/abs/2511.16830)
*Oscar Chew,Po-Yi Lu,Jayden Lin,Kuan-Hao Huang,Hsuan-Tien Lin*

Main category: cs.CL

TL;DR: 本文提出PEPPER防御方法，通过重写输入提示词来破坏文本到图像扩散模型中的后门攻击触发器，在保持生成质量的同时显著降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 针对文本到图像扩散模型易受后门攻击的问题，需要一种能有效防御触发器的保护机制。

Method: PEPPER通过将输入提示词重写为语义不同但视觉相似的描述，并添加不显眼的元素来稀释触发器的影响。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，能大幅降低攻击成功率并保持生成质量，且能与现有防御方法结合使用。

Conclusion: PEPPER提供了一种有效的后门防御方案，具有强鲁棒性和通用性，优于单独的防御方法。
...
### [101] [ConCISE: A Reference-Free Conciseness Evaluation Metric for LLM-Generated Answers](https://arxiv.org/abs/2511.16846)
*Seyed Mohssen Ghafari,Ronny Kol,Juan C. Quiroz,Nella Luan,Monika Patial,Chanaka Rupasinghe,Herman Wandabwa,Luiz Pizzato*

Main category: cs.CL

TL;DR: 本文提出了一种无参考的评估指标，用于量化大型语言模型生成回复的简洁性，通过计算原始回复与摘要之间的压缩比率来识别冗余内容。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常生成冗长、包含冗余细节的回复，这会降低清晰度和用户满意度，并增加模型开发者的成本，特别是基于输出token数量收费的专有模型。

Method: 该方法通过三个计算的平均值来量化非必要内容：1）原始回复与LLM抽象摘要之间的压缩比；2）原始回复与LLM抽取摘要之间的压缩比；3）LLM在保持意义的前提下尽可能移除非必要词语的单词移除压缩。

Result: 实验结果表明，所提出的指标能够有效识别LLM输出中的冗余内容，为对话AI系统中回复简洁性的自动化评估提供了实用工具。

Conclusion: 该方法提供了一种无需人工标注的自动化评估方案，能够有效评估LLM生成回复的简洁性，有助于提高对话系统的效率和质量。
...
### [102] [Improving Latent Reasoning in LLMs via Soft Concept Mixing](https://arxiv.org/abs/2511.16885)
...
Method: 提出SCM训练方案，构建软概念向量（通过概率加权平均嵌入形成），将其混合到模型隐藏状态中，并使用强化学习优化整个潜在推理过程。

Result: 在五个推理基准测试上的实验表明，SCM提高了LLMs的推理性能，同时保持了稳定的训练动态。

Conclusion: SCM通过直接在训练中暴露软表示，有效提升了语言模型的推理能力，是弥合软概念推理与离散标记训练差距的有效方法。
...
### [103] [Deep Improvement Supervision](https://arxiv.org/abs/2511.16886)
*Arip Asadulaev,Rayan Banerjee,Fakhri Karray,Martin Takac*

Main category: cs.CL

TL;DR: 提出了一种新的训练方法，通过将TRMs的潜在推理框架化为无分类器引导和隐式策略改进算法，显著提高了训练效率，减少了18倍的前向传播次数并消除了停止机制，同时在ARC-1上达到24%准确率。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过最小化改变来进一步提高Tiny Recursive Models (TRMs)在复杂推理任务上的效率。
...
Conclusion: 提出的训练方法能够显著提高TRMs的训练效率，同时保持与标准TRMs相当的质量，在参数效率方面表现出色。
...
### [104] [Predicting the Formation of Induction Heads](https://arxiv.org/abs/2511.16893)
*Tatsuya Aoyama,Ethan Gotlieb Wilcox,Nathan Schneider*

Main category: cs.CL

TL;DR: 本文研究了训练数据的统计特性与归纳头形成的关系，发现批量大小和上下文长度的组合可以预测归纳头形成点，表面二元重复频率和可靠性强烈影响归纳头形成，并确定了精确的帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 虽然专门的注意力头（称为归纳头）被认为是现代语言模型卓越的上下文学习能力的基础，但其形成的精确机制仍不清楚。本研究旨在探究训练数据的统计特性与归纳头形成之间的关系。

Method: 通过分析自然和合成数据的统计特性，研究批量大小、上下文长度、表面二元重复频率和可靠性等因素对归纳头形成的影响，并建立预测模型和帕累托前沿。

Result: 发现批量大小和上下文长度的简单组合可以预测归纳头形成点；表面二元重复频率和可靠性强烈影响归纳头形成，并确定了精确的帕累托前沿；当频率和可靠性较低时，类别性和边际分布形状变得重要。

Conclusion: 局部依赖性与高二元重复频率和可靠性足以促进归纳头形成，但在频率和可靠性较低的情况下，数据的类别性和分布形状对归纳头形成具有重要影响。
...
### [105] [ARQUSUMM: Argument-aware Quantitative Summarization of Online Conversations](https://arxiv.org/abs/2511.16985)
*An Quang Tang,Xiuzhen Zhang,Minh Ngoc Dinh,Zhuang Li*

Main category: cs.CL

TL;DR: 本文提出了一种新颖的论点感知定量摘要任务，旨在揭示对话中论点的声明-理由结构，并通过ARQUSUMM框架实现该任务，在论点结构表示和定量准确性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议性话题需要不仅总结多样化论点，还要揭示其理由和论证结构。现有对话摘要方法虽然考虑了句子间的论证关系，但未能深入分析句子内部的论点结构。

Method: 提出ARQUSUMM框架：1）利用LLM少样本学习识别句子中的命题及其声明-理由关系；2）采用论点结构感知的聚类算法聚合论点并量化支持度。

Result: 实验表明ARQUSUMM在论点结构表示、文本质量和量化准确性方面优于现有对话和定量摘要模型，生成的摘要对用户更有帮助。

Conclusion: 论点感知定量摘要是对话摘要的重要方向，ARQUSUMM框架通过揭示深层论点结构和量化论证强度，显著提升了摘要质量和使用价值。
...
### [106] [Supervised Fine Tuning of Large Language Models for Domain Specific Knowledge Graph Construction:A Case Study on Hunan's Historical Celebrities](https://arxiv.org/abs/2511.17012)
*Junjie Hao,Chun Wang,Ying Qiao,Qiuyue Zuo,Qiya Song,Hua Ma,Xieping Gao*

Main category: cs.CL

TL;DR: 该研究提出了一种监督微调方法来增强领域特定信息提取，针对湖南历史名人领域设计了细粒度的模式引导指令模板，并在四个公开大语言模型上进行参数高效指令微调，显著提升了信息提取性能。
...
### [107] [Do Vision-Language Models Understand Visual Persuasiveness?](https://arxiv.org/abs/2511.17036)
*Gyuwon Park*

Main category: cs.CL

TL;DR: 本文构建了视觉说服力判断数据集，引入视觉说服因素分类法，探索认知引导和知识注入策略，发现VLMs存在召回偏向，高层面语义对齐是最强预测因子，对象基础推理能显著提升性能。
...
Method: 构建二元说服力判断数据集，提出视觉说服因素分类法（包括低层感知、中层构图和高层语义线索），探索认知引导和知识注入策略。

Result: VLMs存在召回偏向（过度预测高说服力），对低/中层特征判别能力弱，高层语义对齐是最强预测因子；简单指令或非引导推理效果有限，而对象基础推理能显著提升精度和F1分数。
...
### [108] [Principled Design of Interpretable Automated Scoring for Large-Scale Educational Assessments](https://arxiv.org/abs/2511.17069)
*Yunsung Kim,Mike Hardy,Joseph Tey,Candace Thille,Chris Piech*

Main category: cs.CL

TL;DR: 本文提出了一个可解释的自动评分框架AnalyticScore，通过四个可解释性原则(FGTI)来解决大规模评估中自动评分系统的透明度和可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的自动评分系统虽然具有可扩展性和高效性，但缺乏透明度和可解释性，无法满足大规模真实世界评估的需求。

Method: 开发AnalyticScore框架，包括三个步骤：(1)提取可识别的回答要素，(2)使用LLM将回答转化为人类可解释的特征值，(3)应用直观的有序逻辑回归模型进行评分。

Result: AnalyticScore在评分准确性上优于许多不可解释的评分方法，在ASAP-SAS数据集的10个项目上平均仅比不可解释的SOTA方法低0.06 QWK。特征化行为与人类标注者高度一致。

Conclusion: 提出的FGTI原则和AnalyticScore框架为实现大规模真实世界评估中的可解释自动评分提供了可行方案，在保持高准确性的同时确保了可解释性。
...
### [109] [MUCH: A Multilingual Claim Hallucination Benchmark](https://arxiv.org/abs/2511.17081)
*Jérémie Dentan,Alexi Canesse,Davide Buscaldi,Aymen Shabou,Sonia Vanier*

Main category: cs.CL

TL;DR: MUCH是首个面向实际部署条件的声明级不确定性量化基准，包含4,873个多语言样本和24个token生成logits，并提出高效确定性分割算法，评估显示现有方法在性能和效率上仍有很大改进空间。
...
Method: 构建多语言基准数据集，发布token级生成logits支持白盒方法开发，提出仅需0.2%生成时间的确定性分割算法，适用于实时监控。

Result: 创建了包含4种欧洲语言和4个开源LLM的基准，分割算法效率显著提升，评估表明现有UQ方法在性能和效率方面仍需大幅改进。

Conclusion: MUCH基准为声明级不确定性量化研究提供了现实部署条件下的评估标准，当前方法在准确性和效率上仍有明显不足，需要进一步研究改进。
...
### [110] [Learning to Compress: Unlocking the Potential of Large Language Models for Text Representation](https://arxiv.org/abs/2511.17129)
*Yeqin Zhang,Yizheng Zhao,Chen Hu,Binxing Jiao,Daxin Jiang,Ruihang Miao,Cam-Tu Nguyen*

Main category: cs.CL

TL;DR: 本文探索使用上下文压缩作为无监督适应LLM的预训练任务，通过生成紧凑记忆token替代完整上下文进行序列预测，显著提升文本表示性能，优于基于token级预测任务的方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要针对因果建模和下一token预测优化，不适合生成整体文本表示。虽然已有研究引入预训练任务来适应LLM，但大多依赖token级预测目标，存在局限性。

Method: 提出使用上下文压缩作为预训练任务，模型学习生成紧凑记忆token来替代完整上下文进行下游序列预测。结合对比学习进一步优化表示模型（LLM2Comp）。

Result: 实验表明，精心设计的压缩目标能显著提升LLM文本表示性能，优于基于token级预训练任务的模型。LLM2Comp在广泛任务上优于当代基于LLM的文本编码器，且样本效率更高，所需训练数据显著减少。

Conclusion: 上下文压缩是一种有效的预训练任务，能够显著提升LLM在文本表示任务上的性能，同时提高样本效率。
...
### [111] [LangMark: A Multilingual Dataset for Automatic Post-Editing](https://arxiv.org/abs/2511.17153)
*Diego Velazquez,Mikaela Grace,Konstantinos Karageorgos,Lawrence Carin,Aaron Schliem,Dimitrios Zaikis,Roger Wechsler*

Main category: cs.CL

TL;DR: 本文介绍了LangMark，一个用于英语到7种语言翻译的多语言自动后编辑数据集，包含206,983个三元组，并证明大型语言模型通过少量样本提示可以有效执行自动后编辑任务。


<details>
  <summary>Details</summary>
Motivation: 解决神经机器翻译领域缺乏针对NMT输出的大规模多语言自动后编辑数据集的问题，促进APE系统的发展。

Method: 构建并发布LangMark数据集，包含源文本、NMT输出和人工后编辑翻译的三元组，由专业语言学家标注；使用大型语言模型进行少量样本提示的自动后编辑实验。

Result: LLMs通过少量样本提示能够有效执行APE，改进领先的商业甚至专有机器翻译系统的翻译质量。
...
### [112] [The PLLuM Instruction Corpus](https://arxiv.org/abs/2511.17161)
...
TL;DR: 本文介绍了PLLuM项目中用于微调波兰语大语言模型的指令数据集，包括有机、转换和合成指令的功能分类，并分享了人工编写与合成指令数据集在语言适应方面的观察。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目中的波兰语大语言模型开发提供指令数据集，并探讨不同类型指令数据集在语言模型适应中的影响。

Method: 使用有机、转换和合成三种类型的指令数据集来微调基于transformer的大语言模型，并分析比较这些数据集的效果。

Result: 开发了PLLuM指令语料库（PLLuMIC）的代表性子集，该数据集可为其他LLM的类似数据集开发提供指导和规划。

Conclusion: PLLuMIC数据集对于指导其他语言模型的指令数据集开发具有实用价值，同时揭示了人工编写与合成指令在语言适应方面的不同影响。
...
### [113] [Hallucinate Less by Thinking More: Aspect-Based Causal Abstention for Large Language Models](https://arxiv.org/abs/2511.17170)
*Vy Nguyen,Ziqi Xu,Jeffrey Chan,Estrid He,Feng Xia,Xiuzhen Zhang*

Main category: cs.CL

TL;DR: 提出ABCA框架，通过因果推理分析LLM内部知识多样性，实现早期弃权决策，提高可靠性。
...
Method: 基于方面的因果弃权框架，分析LLM知识内部多样性，通过条件因果效应评估知识可靠性。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，达到最先进性能，增强了弃权决策的可解释性。

Conclusion: ABCA框架通过因果推理实现早期弃权，有效处理知识冲突和知识不足情况，提升LLM响应可靠性。
...
### [114] [Attention-Guided Feature Fusion (AGFF) Model for Integrating Statistical and Semantic Features in News Text Classification](https://arxiv.org/abs/2511.17184)
*Mohammad Zare*
...
<details>
  <summary>Details</summary>
Motivation: 传统新闻分类方法依赖统计特征但缺乏上下文理解，而现代深度学习方法使用语义特征但可能忽略简单有效的统计指标。需要结合两种特征类型的优势来提升分类性能。

Method: 提出注意力引导特征融合（AGFF）模型，在统一框架中结合统计特征（如词频、TF-IDF）和语义特征，使用基于注意力的机制动态确定各特征类型的相对重要性。

Result: 在基准新闻数据集上的评估显示，AGFF模型性能优于传统统计模型和纯语义深度学习模型。消融研究验证了融合过程中各组成部分的贡献。

Conclusion: 策略性地整合多样化特征类型能显著提高分类准确性。该模型能够平衡并利用统计和语义表示的互补优势，为实际新闻分类任务提供实用有效的解决方案。
...
### [115] [E$^3$-Pruner: Towards Efficient, Economical, and Effective Layer Pruning for Large Language Models](https://arxiv.org/abs/2511.17205)
*Tao Yuan,Haoli Bai,Yinfei Pan,Xuyang Cao,Tianyu Zhang,Lu Hou,Ting Hu,Xianzhi Yu*

Main category: cs.CL

TL;DR: 提出E3框架，通过可微分掩码优化和熵感知知识蒸馏，实现高效、经济、快速的层剪枝，在保持高性能的同时显著降低训练成本和提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有层剪枝方法难以同时解决性能下降、训练成本高和加速有限等实际部署挑战，需要一种更有效的压缩方案。

Method: 使用Gumbel-TopK采样器的可微分掩码优化方法实现高效精确的剪枝掩码搜索；采用熵感知自适应知识蒸馏策略提升任务性能。

Result: 在Qwen3-32B模型上剪枝25%层，MATH-500准确率达96%，仅比原模型下降0.8%，优于现有SOTA方法；推理速度提升1.33倍，仅消耗0.5B tokens（后训练数据量的0.5%）。

Conclusion: E3框架在多种模型架构和基准测试中表现出优越性，能够有效平衡性能保持、训练经济性和推理效率。
...
### [116] [Parrot: Persuasion and Agreement Robustness Rating of Output Truth -- A Sycophancy Robustness Benchmark for LLMs](https://arxiv.org/abs/2511.17220)
*Yusuf Çelebi,Mahmoud El Hussieni,Özay Ezerceli*

Main category: cs.CL

TL;DR: PARROT框架用于测量大语言模型在权威和说服压力下的准确性退化，通过双盲评估、置信度跟踪和行为分类来量化模型对错误信息的顺从现象。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在权威压力下过度顺从（奉承）的现象，评估模型在真实世界部署中对错误信息的抵抗能力。

Method: 使用双盲评估比较中性问题和权威性错误问题，通过基于对数似然的校准跟踪量化置信度变化，采用八状态行为分类法系统分类失败模式。

Result: 先进模型（如GPT-5、GPT-4.1）跟随率低（≤11%），准确性损失小；而较老/较小模型显示严重认知崩溃（GPT-4:80%，Qwen 2.5-1.5B:94%）。国际法和全球知识领域脆弱性高，基础数学相对稳健。
...
### [117] [Lost in Translation and Noise: A Deep Dive into the Failure Modes of VLMs on Real-World Tables](https://arxiv.org/abs/2511.17238)
*Anshul Singh,Rohan Chaudhary,Gagneet Singh,Abhay Kumary*

Main category: cs.CL

TL;DR: MirageTVQA是一个新的基准测试，包含近6万个多语言QA对，专门评估视觉语言模型在处理视觉噪声表格和多语言场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有表格QA数据集主要是英语且格式完美，与现实场景存在显著差距，需要评估模型在视觉噪声和多语言环境下的鲁棒性。

Method: 创建包含24种语言、近60,000个QA对的MirageTVQA基准，其中表格包含模拟扫描文档的视觉噪声。

Result: 领先VLM模型在视觉噪声下性能下降超过35%，且存在英语优先偏见，推理能力无法有效迁移到其他语言。

Conclusion: MirageTVQA为测量和推动更鲁棒的表格推理VLM模型提供了基准，揭示了当前模型在现实场景中的主要缺陷。
...
### [118] [Estonian WinoGrande Dataset: Comparative Analysis of LLM Performance on Human and Machine Translation](https://arxiv.org/abs/2511.17290)
...
Method: 通过专业翻译人员进行人工翻译，并设计针对爱沙尼亚语特点和WinoGrande数据集翻译挑战的详细提示进行机器翻译实验。

Result: 人工翻译的爱沙尼亚语数据集上模型性能略低于原始英语测试集，机器翻译数据性能明显更差，提示工程对翻译质量和模型准确性的改进有限。
...
### [119] [A new kid on the block: Distributional semantics predicts the word-specific tone signatures of monosyllabic words in conversational Taiwan Mandarin](https://arxiv.org/abs/2511.17337)
*Xiaoyun Jin,Mirjam Ernestus,R. Harald Baayen*

Main category: cs.CL

TL;DR: 本文基于语料库研究普通话单音节词在自发对话中的音高实现，发现词义对音高轮廓有显著影响，即使控制多种变量后，词义仍是音高实现的重要预测因子。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索普通话单音节词在自然对话中的音高实现模式，特别关注词义对音高轮廓的影响，挑战传统普通话声调理论。

Method: 使用广义加性模型分解观察到的音高轮廓，将其分解为与不同控制变量和语义预测因子相关的组成音高轮廓。

Result: 研究发现词义对音高实现有显著影响：词义比词本身是更好的预测因子，异形同音词具有不同的音高轮廓，且通过上下文嵌入可以准确预测单个词标记的音高轮廓。

Conclusion: 研究结果挑战了传统的普通话声调理论，但符合判别性词汇模型的理论框架，表明分布语义学在语音学中具有重要意义。
...
### [120] [Don't Learn, Ground: A Case for Natural Language Inference with Visual Grounding](https://arxiv.org/abs/2511.17358)
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态表示来增强自然语言推理，通过将语言在视觉上下文中进行基础化，以克服文本偏见和表面启发式问题。
...
### [121] [Selective Rotary Position Embedding](https://arxiv.org/abs/2511.17388)
*Sajad Movahedi,Timur Carstensen,Arshia Afzal,Frank Hutter,Antonio Orvieto,Volkan Cevher*

Main category: cs.CL

TL;DR: 本文提出了选择性RoPE，这是一种输入依赖的旋转位置嵌入机制，可推广RoPE并为线性和softmax变换器实现任意角度的旋转。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中输入依赖门控改善语言相关任务的启发，作者希望将这种选择性机制引入到位置编码中，以改进位置信息的表示。

Method: 引入选择性RoPE，这是一种输入依赖的旋转嵌入机制，通过任意角度的旋转来编码位置信息，并揭示了softmax注意力中隐藏的旋转结构。

Result: 在语言建模和困难序列任务（如复制、状态跟踪和检索）上，选择性RoPE的输入依赖旋转机制显著提升了性能。

Conclusion: 选择性RoPE成功地将选择性机制引入位置编码，为变换器模型提供了更灵活和有效的位置表示方法。
...
### [122] [Beyond Multiple Choice: A Hybrid Framework for Unifying Robust Evaluation and Verifiable Reasoning Training](https://arxiv.org/abs/2511.17405)
...
TL;DR: ReVeL框架将多项选择题重写为开放式问题，通过LLM进行验证，解决了MCQA中选项泄露信号导致评估不准确的问题，提升了模型训练和评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 多项选择题(MCQA)的选项可能泄露可利用信号，导致准确率指标不能真实反映模型能力，并在强化微调(RFT)中鼓励答案猜测行为。

Method: 提出ReVeL框架，将MCQA问题重写为开放式问题，根据答案类型应用不同的重写和验证方案，使用GRPO对Qwen2.5-VL模型进行微调。

Result: 在ReVeL-OpenQA上训练的模型在多项选择基准上匹配MCQA准确率，并将OpenQA准确率提高约6个百分点；ReVeL评估显示MCQA基准中存在高达20个百分点的分数膨胀。
...
### [123] [Masked-and-Reordered Self-Supervision for Reinforcement Learning from Verifiable Rewards](https://arxiv.org/abs/2511.17473)
*Zhen Wang,Zhifeng Gao,Guolin Ke*

Main category: cs.CL

TL;DR: MR-RLVR通过掩码填充和步骤重排序构建过程级自监督奖励，在仅结果可验证的数学推理任务中显著提升了RLVR的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR在数学推理（特别是定理证明）中可扩展性有限，因为中间推理过程至关重要而最终答案难以直接可靠验证。同时，token级SFT容易退化为死记硬背而非诱导长链思维。

Method: 提出MR-RLVR方法，通过"掩码后填充"和"步骤重排序"构建过程级自监督奖励。训练流程包括两个阶段：首先在采样的数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调。

Result: 在Qwen2.5-3B和DeepSeek-R1-Distill-Qwen-1.5B上实现MR-RLVR，并在AIME24、AIME25、AMC23和MATH500上评估。在固定采样和解码预算下，相比原始RLVR，平均相对增益为+9.86% Pass@1、+5.27% Pass@5和+4.00% Pass@8。

Conclusion: 引入过程感知的自监督信号可以有效增强RLVR在仅结果可验证场景中的可扩展性和性能。
...
### [124] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
TL;DR: 研究发现视觉创造力存在明显梯度：视觉艺术家最富创造力，其次是非艺术家，然后是受人类启发的人工智能，最后是自我引导的人工智能。增加人类指导能显著提升AI的创造力输出。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在发散思维任务中已能与人类创造力相媲美，但视觉创造力领域仍未被充分探索。本研究旨在比较人类参与者（视觉艺术家和非艺术家）与图像生成AI模型在视觉创造力方面的表现差异。

Method: 研究比较了人类参与者（视觉艺术家和非艺术家）与图像生成AI模型（两种提示条件：高人类输入的"人类启发"和低人类输入的"自我引导"）的图像生成能力。由255名人类评估者和GPT4o对生成的图像进行创造力评估。

Result: 发现了清晰的创造力梯度：视觉艺术家最具创造力，其次是非艺术家，然后是受人类启发的生成AI，最后是自我引导的生成AI。增加人类指导显著改善了生成AI的创造力输出，使其产出接近非艺术家的水平。人类和AI评估者在创造力判断模式上也表现出显著差异。
...
### [125] [Cognitive BASIC: An In-Model Interpreted Reasoning Language for LLMs](https://arxiv.org/abs/2511.16837)
*Oliver Kramer*

Main category: cs.AI

TL;DR: Cognitive BASIC是一种基于BASIC风格的提示语言和模型内解释器，将大语言模型的推理结构化为显式的逐步执行轨迹。


<details>
  <summary>Details</summary>
Motivation: 受到复古BASIC简单性的启发，重新利用编号行和简单命令作为可解释的认知控制层，使现代LLM能够可靠地模拟此类短程序，实现模型内部透明的多步推理。
...
Conclusion: Cognitive BASIC为LLM推理提供了一种结构化的、可解释的方法，能够有效支持多步推理和知识管理。
...
### [126] [Fantastic Bugs and Where to Find Them in AI Benchmarks](https://arxiv.org/abs/2511.16842)
...
TL;DR: 提出一个利用响应模式统计分析来识别基准测试中无效问题的框架，通过专家审查和LLM初审来提高基准测试的可靠性。
...
Method: 基于统计分析方法，假设平均分足以总结模型性能，构建单维潜在结构。当项目统计值超出预期范围时，标记为潜在问题。结合专家审查和LLM初审来减少人工工作量。

Result: 在9个广泛使用的基准测试中，该方法指导专家审查识别问题问题的精度高达84%。LLM初审进一步减少了人工工作量。

Conclusion: 该框架为系统化基准测试修订提供了高效且可扩展的解决方案，能够可靠识别无效问题并减少人工审查负担。
...
### [127] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
...
Method: 提出混合差分奖励机制，包含两个互补组件：基于全局势函数的时序差分奖励和直接测量动作边际效用的动作梯度奖励。将协同驾驶问题建模为具有时变智能体集的多智能体部分可观测马尔可夫博弈，并提供完整的HDR实例化方案。

Result: 使用在线规划和多智能体强化学习算法的广泛实验表明，HDR机制显著提高了收敛速度和策略稳定性，引导智能体学习到能有效平衡交通效率和安全的优质协同策略。

Conclusion: HDR机制成功解决了传统奖励信号失效的问题，为多车辆协同驾驶任务提供了有效的奖励设计框架。
...
Method: 提出MirrorMind分层认知架构：个体层面构建研究者的高保真认知模型；领域层面将集体知识映射为结构化学科概念图；跨学科层面作为正交编排引擎。架构将记忆存储与智能执行分离。
...
TL;DR: 本文研究了在明确工具调用预算下如何有效扩展工具增强智能体，提出了预算跟踪器和BATS框架，通过预算意识实现更好的成本-性能扩展曲线。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强智能体在扩展工具调用预算时缺乏预算意识，导致性能很快达到瓶颈，无法充分利用增加的预算资源。

Method: 提出预算跟踪器插件提供持续预算意识，并开发BATS框架动态调整规划和验证策略，基于剩余资源决定深入挖掘还是转向新路径。

Result: 预算感知方法产生更有利的扩展曲线，推动了成本-性能帕累托前沿，实现了更透明和原则化的智能体扩展理解。

Conclusion: 预算意识是工具增强智能体有效扩展的关键，本文为理解智能体扩展提供了系统性的实证见解。
...
TL;DR: 本文重新解释扩散模型在逆问题求解中的作用，提出DAPS++方法将扩散阶段与数据驱动细化完全解耦，实现更高的计算效率和鲁棒重建性能。
...
Method: 将扩散重新解释为期望最大化框架中的初始化阶段，提出DAPS++方法，使似然项更直接地指导推理，同时保持数值稳定性。通过减少函数评估次数和测量优化步骤来提高效率。

Result: DAPS++在计算效率和重建性能方面表现优异，在多种图像恢复任务中实现鲁棒重建，同时需要更少的函数评估和优化步骤。

Conclusion: 扩散在逆问题求解中的主要作用是提供初始化，DAPS++通过解耦扩散和数据驱动细化，解释了统一扩散轨迹在实践中保持有效的原因，并实现了高效稳定的重建。
...
TL;DR: 本文提出了一种名为MIR的互惠内在奖励方法，用于解决多智能体强化学习中稀疏奖励问题，特别是在情节奖励场景下。该方法通过激励智能体探索影响队友的行为，结合原始策略有效促进团队探索并提升算法性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的情节奖励面临两大挑战：联合动作轨迹的指数级稀疏性，以及现有方法未能充分考虑影响团队状态的联合动作。这些因素导致在稀疏奖励环境下算法性能受限。

Method: 提出了互惠内在奖励（MIR）方法，通过激励个体智能体探索能够影响队友的行为，与原始策略结合使用，有效刺激团队探索。同时创建了MiniGrid-MA环境用于验证。

Result: 在MiniGrid-MA环境中的实验结果表明，该方法相比现有最先进方法表现出更优越的性能，验证了MIR在多智能体稀疏奖励场景下的有效性。

Conclusion: MIR是一种简单而有效的增强策略，能够显著改善多智能体强化学习在极端稀疏奖励环境下的表现，为相关研究提供了新的解决方案。
...
TL;DR: 本文提出了一个基于分层任务抽象机制(HTAM)的多智能体框架EarthAgent，专门用于解决复杂地理空间分析任务，相比通用框架在专业领域表现更优。


<details>
  <summary>Details</summary>
Motivation: 通用LLM驱动的智能体在需要严格结构化工作流程的专业领域(如遥感)中表现不佳，因为这些领域需要专门工具和多步骤程序。

Method: 提出了分层任务抽象机制(HTAM)，将多智能体系统构建为反映领域内在任务依赖关系的逻辑层次结构，确保程序正确性并将复杂问题分解为顺序层。

Result: 构建了GeoPlan-bench基准测试和评估指标，实验表明EarthAgent显著优于现有的单智能体和多智能体系统。
...
<details>
  <summary>Details</summary>
Motivation: 为实现智能AI系统的持续自主性、推理和交互能力，需要将关于智能体的假设与明确的认知、合作和治理模型相结合。

Method: 利用AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具，将自适应数据驱动方法与结构化推理协调模型对齐。

Result: 提出了一个连接形式理论和实践自主性的智能体视角，为构建能力强大且灵活、同时具有透明度、合作性和问责制的智能系统指明了路径。

Conclusion: AAMAS社区的概念工具为实现智能AI系统提供了坚实基础，通过结合自适应方法和结构化模型，可以构建既强大又负责任的智能系统。
...
TL;DR: 本文系统评估了使用合成和离策略数据对LLM行为探测泛化能力的影响，发现响应生成策略显著影响探测性能，且训练数据域偏移比策略偏移造成更大性能下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然样本稀少，研究者被迫依赖合成或离策略数据进行探测训练，但这种方法对探测泛化能力的影响尚不明确。

Method: 在8种不同LLM行为上测试线性和注意力探测，比较不同响应生成策略（合成、离策略、同策略）对探测性能的影响，并分析域偏移和策略偏移的效应。

Result: 响应生成策略显著影响探测性能，但效果因行为而异；离策略数据到同策略数据的成功泛化可预测真实监控场景的泛化能力；域偏移比策略偏移造成更大性能下降。

Conclusion: 在缺乏同策略数据时，使用同域离策略数据比不同域同策略数据产生更可靠的探测，强调需要更好处理LLM监控中分布偏移的方法。
...
Result: 与通用CP相比，安全关键对象平均精度损失小于1%，仅使用20%通信带宽；相比现有选择性CP方法，感知性能提升15%。
...
<details>
  <summary>Details</summary>
Motivation: 传统学术论文评估方法耗时且存在评估者主观差异，需要更高效、一致的评估解决方案。
...
Conclusion: RubiSCoT框架能够通过一致、可扩展和透明的评估来优化学术评估流程。
