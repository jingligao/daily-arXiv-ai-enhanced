...
Motivation: AI生成图像的真实性日益增强，给视觉真实性验证带来挑战，需要开发能够分类、定位和解释图像伪影的可解释检测系统。

Method: 使用轻量级卷积分类器"Faster-Than-Lies"与视觉语言模型Qwen2-VL-7B相结合，通过基于自编码器的重建误差图生成伪影定位热图，并将70种视觉伪影类型分为8个语义组。

Result: 在扩展的CiFAKE数据集上达到96.5%的准确率，在8核CPU上推理时间为175ms，能够部署在本地或边缘设备上，并实现了可解释的文本生成。

Conclusion: 这项工作证明了在低分辨率图像中结合视觉和语言推理进行可解释真实性检测的可行性，并概述了在取证、工业检测和社交媒体审核等领域的跨领域应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够轻松计数各种对象，而现有计数模型在对象形状复杂、内部对称或重叠时容易出错，需要开发能够感知视觉重复和结构关系的计数方法。

Method: 基于CounTR架构，用自监督基础模型DINOv2替换视觉编码器，引入位置嵌入融合来保持几何关系，通过轻量级卷积解码器将特征解码为密度图。
...
<details>
  <summary>Details</summary>
Motivation: 河流中漂浮人为垃圾的扩散已成为紧迫的环境问题，对生物多样性、水质以及人类活动如航行和娱乐产生不利影响，需要有效的监测方法。

Method: 使用固定原位相机采集数据，应用深度学习模型进行漂浮垃圾检测和量化，测试不同环境条件和学习配置，包括数据泄露相关偏差实验，并实现几何模型从2D图像估计物体实际尺寸。

Result: 研究发现数据集构建协议的重要性，特别是负样本图像的整合和时间泄露的考虑，证明了使用投影几何结合回归校正进行度量物体估计的可行性。

Conclusion: 该方法为开发稳健、低成本、自动化的城市水环境监测系统铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像超分辨率在分布外条件下经常失败，产生视觉合理但物理不准确的结果，特别是在处理由不同传感器捕获的稀有地貌特征时。

Method: 采用双重条件架构：门控ControlNet保持低分辨率输入的几何保真度，文本提示提供复杂特征的语义指导；引入多面损失函数确保光谱和辐射一致性；使用随机前向传播方法量化预测不确定性。

Result: 在盲评估中，地球物理专家认为模型输出接近地面真实图像的质量，在感知指标上取得显著提升，FID降低了近40%。

Conclusion: RareFlow为数据稀缺科学领域提供了高保真合成的稳健框架，并为严重领域偏移下的受控生成提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景生成方法通常局限于单对象生成、需要领域特定训练或缺乏完整360度视角支持，限制了应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 教学视频中的场景级字幕可以通过理解视觉线索和时间结构来增强学习效果，但缺乏这种结构的字幕可能缺乏连贯性和质量，影响教育效果。

Method: 使用自适应帧采样和多模态窗口捕捉场景内关键转换，采用多模态思维链过程生成多个动作-对象对，通过动态步长窗口选择算法平衡时间上下文和冗余，最终生成融合视觉语义和时间推理的教学字幕。

Result: 在N-gram指标（BLEU、METEOR）和语义相似度度量（BERTScore、CLIPScore）上均优于强基线模型，定性分析显示生成的字幕在时间连贯性和信息量方面表现更佳。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人像生成图像到3D模型存在视觉伪影、细节缺失和身份保持不完整的问题，而图像扩散模型虽然质量高但计算昂贵且缺乏3D一致性。

Method: 采用前馈图像到头像生成管道获得初始3D表示和噪声渲染，然后使用单步扩散模型进行多视角一致的精炼，并通过合成多视角数据预训练和高质量真实图像微调的训练策略。

Result: 该方法在定性和定量上都优于当前最先进的人像新视角合成方法，同时保持时间效率。

Conclusion: TurboPortrait3D成功地将图像扩散模型用于提升图像到头像方法的生成质量，实现了低延迟、高质量的3D人像合成。
...
<details>
  <summary>Details</summary>
Motivation: 在室内场景中，大面积的低纹理区域使得基于光度损失的3D高斯泼溅方法难以恢复高保真3D表面，导致几何模糊。

Method: 设计了语言提示平面先验管道，使用预训练的视觉语言分割模型并通过跨视图融合和几何先验检查来优化区域建议；在3D高斯优化中增加了平面先验监督和几何先验监督项。

Result: 在标准室内基准测试中，PlanarGS重建出准确且详细的3D表面，大幅优于现有最先进方法。

Conclusion: PlanarGS通过引入平面和几何先验监督，有效解决了室内场景重建中的几何模糊问题，显著提升了3D表面重建质量。
...
Method: 借鉴计算机图形学中的USD标准，引入Neural USD框架，采用结构化分层表示场景和对象，应用微调方法确保控制信号的解耦。

Result: 评估了框架的多个设计考虑，展示了Neural USD如何支持迭代和增量式工作流程。

Conclusion: Neural USD框架为精确的对象级控制提供了有效解决方案，支持对外观、几何和姿态的独立控制。
...
Method: 采用有效的数据收集和生成框架、策略遵循训练流程和定制损失函数，提出多样化QA生成和训练策略以增强学习效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学图像分析中缺乏透明推理过程，无法提供临床医生依赖的逐步推理，限制了其在医疗实践中的应用和可信度。

Method: 采用两阶段训练方法：首先进行推理风格的监督微调，然后使用可验证奖励的强化学习，模型结合高保真视觉编码输出系统化推理过程。

Result: 在分布外评估中达到竞争性的多标签分类性能，同时提升可解释性。专家放射科医生研究表明完整推理轨迹提高了信心，支持错误审计并减少报告完成时间。

Conclusion: 该方法推动了医学影像AI向可信赖、可解释的方向发展，在胸部放射学等推理质量与预测质量同等重要的医疗任务中具有重要意义。
...
Method: 构建包含工作与学习、生活方式与日常、社交活动、外出与文化四个领域的同步自我中心视频、音频和文本数据集，所有数据在统一全局时间线上对齐，并通过人工精炼获得高质量视觉叙述和语音转录。定义了12个诊断子任务，涵盖记忆、理解和跨记忆推理三个核心能力。

Result: TeleEgo提供了超过14小时/参与者的多模态数据，包含3,291个人工验证的QA项目，支持多种问题格式（单选、二元、多选和开放式），在严格的流式设置下进行评估。

Conclusion: TeleEgo为实用AI助手的开发提供了现实和全面的评估框架，通过实时准确性和记忆持久时间两个指标联合评估正确性、时间响应性和长期记忆保持能力。
...
Method: AdvBlur方法将对抗性模糊图像集成到数据集中，并采用双损失函数框架来处理领域泛化问题，有效减轻未见分布变化的影响。

Result: 在多个数据集上的综合评估表明，该方法在未见外部数据集上取得了与最先进的领域泛化糖尿病视网膜病变模型相竞争的性能。
...
<details>
  <summary>Details</summary>
Motivation: 主动脉血管树自动分析具有巨大临床潜力，但缺乏共享高质量数据阻碍了其发展。SEG.A挑战赛旨在通过引入大型公开多机构数据集来推动该领域进展。

Method: 挑战赛在隐藏测试集上评估自动算法，后续可选任务包括用于计算模拟的表面网格生成。主要采用3D U-Net架构的深度学习方法和定制化后处理步骤。

Result: 顶级算法集成显著优于单个模型，性能与算法设计（特别是定制化后处理步骤）和训练数据特征密切相关。3D U-Net架构在顶级提交中占主导地位。

Conclusion: 该计划不仅建立了新的性能基准，还为推动未来创新提供了持久资源，目标是开发稳健且可临床转化的工具。
...
Method: 构建包含20个数据集的标准化基准，使用轨道和表面图像，涵盖分类、分割和检测任务，并提供预训练模型的基础评估。

Result: 所有分析结果表明，火星特定的基础模型可能比通用领域模型具有优势，激励了领域自适应预训练的进一步探索。
...
Method: 提出交替优化-微调流水线，在对抗性后缀优化和利用优化后缀微调LLM之间交替进行；集成双重规避策略：通过辅助LLM困惑度评分约束生成人类可读提示，引入禁止标记惩罚抑制黑名单中显式标记的生成。

Result: 广泛的实验表明，该方法生成的人类可读、抗过滤的对抗性提示具有优秀的红队测试性能，以及卓越的零样本迁移能力，能够即时适应未见过的提示，并在商业API中暴露关键漏洞。

Conclusion: APT框架成功解决了现有方法的局限性，提供了一种高效、黑盒、人类可读的对抗性提示生成方法，显著提升了文本到图像模型安全评估的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏调优方法存在两个问题：1) 两阶段方法在定位任务相关权重时忽略了微调过程中的参数调整；2) 梯度稀疏掩码导致高内存使用。

Method: 提出单阶段方法SNELLA：1) 通过两个低秩可学习矩阵的合并选择性更新权重矩阵，引入非线性核函数提高合并矩阵秩；2) 自适应双层稀疏分配机制，基于重要性分数在层间和层内进行权重竞争。

Result: 在分类、分割和生成任务上，SNELLA在FGVC基准上比SPT-LoRA获得1.8%更高的Top-1准确率(91.9% vs 90.1%)，内存使用减少31.1%-39.9%。

Conclusion: SNELLA在保持低内存使用的同时实现了最先进的性能，为参数高效微调提供了有效的单阶段解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注对抗性微调或提示优化，但忽视了CLIP编码特征中文本和图像特征之间的差距，这种不对齐在对抗扰动下会被显著放大，导致分类性能严重下降。

Method: COLA框架包含两个步骤：(1)将对抗性图像嵌入投影到类别文本特征张成的子空间，过滤非语义失真；(2)将图像和文本建模为多个增强视图上的离散分布，通过最优传输细化对齐，并将子空间投影无缝集成到成本计算中。

Result: 在14个零样本分类基准上的广泛评估证明了COLA的有效性，特别是在PGD对抗攻击下，ImageNet及其变体的平均改进达到6.7%，同时在干净样本上保持高精度。

Conclusion: COLA是一种无需训练且与现有微调模型兼容的框架，能够在对抗条件下确保稳定的跨模态对齐，显著提升视觉语言模型的鲁棒性。
...
Method: 通过研究交叉注意力和自注意力机制的两个关键特性来实现文本移除和样式控制。使用自注意力反转来减少文本幻觉，重新分配交叉注意力以减少文本幻觉。在潜在优化框架中引入新的损失函数：交叉注意力内容损失来提高文本渲染准确性，自注意力样式损失来促进样式定制。

Result: OmniText在多个任务和指标上实现了最先进的性能，与其他文本修复方法相比表现优异，与专门方法相当。还提出了OmniText-Bench基准数据集来评估多样化的文本图像操作任务。

Conclusion: OmniText是第一个能够执行多样化文本图像操作任务的通才方法，在文本移除、重新缩放、重新定位以及具有各种样式的插入和编辑等应用中表现出色。
...
Method: 提出IIS评分，通过评估解释过程中的信息损失来量化表示的可解释性，测量可解释语义的比例。

Result: 发现可解释性与分类能力呈正相关，高分类能力的表示包含更多可解释语义；通过可解释性最大化微调可进一步提升分类性能。

Conclusion: 实践者可以统一改进预训练视觉模型的可解释性和分类能力，两者相互促进而非相互冲突。
...
Method: 在频域中利用中间特征进行跨架构传输：应用傅里叶变换捕获全局特征信息；特征转换模块（FTM）生成紧凑的频域表示；可学习的特征对齐模块（FAM）投影学生特征并通过多级匹配进行对齐；结合中间特征的均方误差和logits的KL散度的联合目标函数

Result: 在CIFAR-100和ImageNet-1K数据集上分别比最新方法提升5.59%和0.83%

Conclusion: UHKD是统一异构表示并有效利用视觉知识的高效方法
...
<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法加速扩散过程时忽略了去噪趋势，缺乏对模型特定容忍度的误差控制，导致多步重用下的轨迹偏差和生成结果不一致。

Method: 1. 一致性趋势预测器：利用扩散轨迹的平滑连续性，将历史去噪模式投影到稳定的未来方向，并在多个近似步骤中渐进分布；2. 模型特定误差容忍度搜索机制：通过识别从波动语义规划到稳定质量精炼的过渡点来推导校正阈值。

Result: ETC在FLUX基础上实现了2.65倍加速，一致性退化可忽略（-0.074 SSIM分数）。
...
Result: 通过将显式布局基础与基于自优化的推理时间扩展相结合，该框架在场景与提示对齐方面比现有文本到图像模型表现更好。

Conclusion: 该训练免费框架通过布局基础和自优化实现了更好的文本到图像生成质量，代码已开源。
...
Method: 从T2V角度分析字幕内容，将视频重建所需元素分解为多个维度，提出原则性的字幕设计方法，并构建VC4VG-Bench基准进行多维度评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在自动驾驶场景理解中面临的多任务干扰、空间推理不足和输出质量不稳定的问题，提升模型在感知、预测、规划和异常检测等关键任务上的表现。

Method: 1) 混合提示路由器分类问题并分发到任务特定专家提示；2) 任务特定提示嵌入坐标系统、空间推理规则、角色扮演和思维链推理；3) 视觉组装模块组合多视角图像、对象裁剪和自适应历史帧；4) 按任务配置模型推理参数。

Result: 在Qwen2.5-VL-72B上实现，Phase-1（干净数据）平均准确率达到70.87%，Phase-2（异常数据）达到72.85%，证明了结构化提示和空间基础化能显著提升VLM在安全关键自动驾驶任务上的性能。

Conclusion: 通过系统化的提示工程和空间推理增强，可以有效提升视觉语言模型在复杂自动驾驶场景理解任务中的表现，为安全关键应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: SAM2作为SAM的继任者，在视频分割方面表现出强大的泛化能力，但其鲁棒性尚未被探索。现有针对SAM的攻击方法无法直接迁移到SAM2，主要面临两个关键挑战：来自提示的方向性引导和跨连续帧的语义纠缠。

Method: 提出UAP-SAM2方法，采用目标扫描策略将每帧划分为k个区域并随机分配提示以减少提示依赖性，同时设计双语义偏差框架，通过扭曲当前帧语义和破坏跨帧语义一致性来优化通用对抗扰动。

Result: 在六个数据集上的两个分割任务实验表明，UAP-SAM2方法有效攻击SAM2，比较结果显示其显著优于现有最先进攻击方法，优势明显。

Conclusion: UAP-SAM2成功解决了SAM2特有的架构挑战，实现了高效的跨提示通用对抗攻击，为评估分割基础模型的鲁棒性提供了重要工具。
...
TL;DR: MC-SJD是一种无需训练、无损的并行解码框架，通过扩展Speculative Jacobi Decoding来加速自回归视觉生成。该方法通过耦合技术最大化连续迭代中采样相同草稿token的概率，显著提升了接受率，实现了图像生成4.2倍和视频生成13.3倍的加速效果。


<details>
  <summary>Details</summary>
Motivation: 自回归建模在视觉生成中表现出潜力，但其逐token生成方式导致推理速度极慢，通常需要数千步才能生成单个样本，这严重限制了实际应用。

Method: 提出MC-SJD框架，基于信息论中的耦合方法，通过最大化连续迭代中采样相同草稿token的概率来提升接受率。该方法只需对现有算法进行单行修改，即可实现显著性能提升。

Result: MC-SJD实现了图像生成约4.2倍加速和视频生成约13.3倍加速，相比标准自回归解码，且不降低输出质量。

Conclusion: MC-SJD通过简单的算法修改有效解决了自回归视觉生成中的推理速度瓶颈，为实际应用提供了可行的加速方案。
...
Conclusion: ID²Face提供了一种有效的训练中心化人脸匿名化方法，通过显式解耦身份和非身份信息实现高质量匿名化
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法主要基于注意力分数选择最显著的token，导致所选token语义不完整。需要一种能同时考虑显著性和覆盖度的剪枝策略。

Method: 提出SCOPE方法，定义token集合覆盖度，计算未选token的覆盖增益，结合显著性得分提出SCOPE分数，迭代选择最高分token。

Result: 在多个视觉语言理解基准测试中使用LLaVA-1.5和LLaVA-Next模型进行实验，结果表明该方法持续优于先前方法。

Conclusion: SCOPE方法通过联合建模显著性和覆盖度，有效减少了计算开销，同时更好地保持了语义完整性，在多模态大语言模型中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的深度阴影去除模型由于固定的注意力模式，容易混合来自无关区域的照明线索，导致结构扭曲和颜色不一致。本文从序列建模角度重新审视阴影去除问题，探索使用Mamba这一选择性状态空间模型。

Method: 提出DeshadowMamba方法：1）使用Mamba模型通过方向状态转换传播全局上下文；2）设计CrossGate方向调制机制，在Mamba输入门中注入阴影感知相似性；3）引入ColorShift正则化，通过对比学习目标基于全局颜色统计来抑制颜色污染。

Result: 在公共基准测试上的广泛实验表明，DeshadowMamba实现了最先进的视觉质量和强大的定量性能。

Conclusion: 通过将序列建模适应于阴影去除所需的结构完整性和色彩一致性要求，DeshadowMamba方法在阴影去除任务中表现出色，证明了选择性状态空间模型在该领域的潜力。
...
Method: 训练免费的one-shot归因方法：1）为待分析图像生成描述提示；2）使用所有候选源重合成图像；3）在特征空间中比较重合成图像与原图的相似度，选择最接近的模型作为归因结果。

Result: 提出的重合成方法在只有少量训练样本时优于现有最先进的少样本方法和其他基线方法。新数据集被证明具有挑战性，是开发评估未来少样本和零样本方法的宝贵基准。

Conclusion: 基于重合成的方法在数据稀缺条件下表现出色，新数据集为合成图像归因研究提供了有价值的测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在细粒度视觉感知方面的瓶颈问题，现有方法存在数据稀缺、监督微调损害通用能力、强化微调偏重文本推理等问题。

Method: 设计两阶段任务将视觉感知学习构建为从粗到细的渐进过程，开发ViPER自举框架，通过自我批判和自我预测实现迭代演进，整合图像级和实例级重建与两阶段强化学习策略。

Result: 应用于Qwen2.5-VL系列产生Qwen-Viper模型，在7个综合基准上平均提升1.7%，细粒度感知任务上最高提升6.0%，在不同视觉语言场景中保持优越性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 遥感应用中深度学习性能受限于标注数据稀缺和跨域标注成本高，现有视觉语言模型直接应用于遥感领域存在显著域差距和任务特定语义适应需求。

Method: 评估了多种代表性提示学习方法：上下文优化、条件上下文优化、多模态提示学习和自约束提示学习，并与零样本CLIP和线性探针基线进行对比。

Result: 在多个遥感基准数据集上的实验表明，提示学习方法在少样本场景下始终优于基线，其中自约束提示学习在跨域泛化方面表现最稳健。
...
<details>
  <summary>Details</summary>
Motivation: 解决nnU-Net等深度学习框架在临床部署中的困难，包括依赖项过多和整体化设计问题，使高性能研究管道能够转化为便携的临床可用工具。

Method: 采用模块化设计，预处理使用Anima工具箱生成BIDS兼容输出，推理使用ONNX Runtime和Float16量化，提供图形和命令行界面，支持Python脚本和独立Windows可执行文件。

Result: 在300名亚急性和慢性卒中患者的保留测试集上，分割性能与原始PyTorch管道相当（Dice差异<10^-3），模型大小减少约50%。

Conclusion: StrokeSeg成功展示了高性能研究管道可以转化为便携、临床可用的工具，为脑卒中病灶分割的临床部署提供了可行解决方案。
...
Result: 研究发现：(1) 在预训练阶段利用报告对标签在文本中明确表示的下游分类任务有益，但通过显式图像-文本对齐的预训练在标签与文本弱关联的设置中可能有害；(2) 使用报告进行微调能带来显著改进，在某些设置中比预训练方法影响更大。
...
<details>
  <summary>Details</summary>
Motivation: 卒中后MRI不仅显示局灶性病变，还揭示继发性结构变化如萎缩和脑室扩大，这些异常作为恢复和预后的影像生物标志物，但现有监督分割方法难以有效捕捉。

Method: 使用基于流的生成模型REFLECT进行无监督异常检测，在ATLAS数据上使用双专家中央切片标注，通过自由响应ROC分析评估异常图性能，比较了在卒中患者无病灶切片和健康对照数据上训练的两种模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪中目标数量未知且随时间变化、目标身份一致性保持困难、非线性动态处理以及弱噪声检测器下的鲁棒跟踪问题。

Method: 提出混合跟踪方法，结合随机和确定性策略；使用粒子群优化（PSO）引导粒子朝向目标分布模式；集成目标间社交交互来增强PSO引导粒子；建立基于空间一致性、外观、检测置信度、轨迹惩罚和社交分数的综合状态观测模型。

Result: 在标准基准测试和实际场景中，GenTrack相比最先进的跟踪器表现出优越性能，减少了ID切换和轨迹丢失，特别是在遮挡情况下。

Conclusion: GenTrack提供了一种有效的多目标跟踪解决方案，通过混合方法和社交交互显著提升了跟踪性能，并提供了公开的源代码实现便于复现和比较。
...
<details>
  <summary>Details</summary>
Motivation: 解决在非线性动态下未知且随时间变化的目标数量跟踪问题，确保标识一致性，特别是在目标交互和长时间遮挡情况下。

Method: 使用随机粒子滤波处理非线性动态和非高斯噪声，结合粒子群优化引导粒子；采用确定性关联通过成本矩阵确保标识一致性；提出平滑状态更新方案；使用速度回归提供趋势种子速度。

Result: 实验结果表明该方法在性能上优于最先进的跟踪器，适用于预录制视频和实时摄像头流。

Conclusion: 所提出的跟踪器在保持标识一致性和处理复杂场景方面表现出色，为多目标跟踪提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: Qaraaoun水库是黎巴嫩最大的地表水体，但由于传感器频繁故障和维护能力有限，需要可靠的存储量监测方法来实现可持续管理。

Method: 处理Sentinel-2和Landsat影像，使用新提出的水域分割指数划定水面，基于支持向量回归的机器学习模型训练包含水面面积、水位和体积计算的数据集。

Result: 水域分割与地面真实数据在95%以上的海岸线处一致，超参数调优后SVR性能优化，误差低于水库总容量的1.5%，决定系数超过0.98。
...
TL;DR: 提出了一种基于功能分解的深度条件图像压缩框架（DCIC-sgp），通过自生成先验来分离图像的结构主干和细节纹理，有效解决了传统方法在低码率下几何变形严重的问题。
...
Method: 采用功能分解框架，首先编码自生成先验来捕获图像结构主干，然后利用该先验深度调节整个压缩流程，特别是分析变换，使其专注于残差高熵细节。

Result: 在Kodak、CLIC和Tecnick数据集上，相比VVC测试模型VTM-12.1分别实现了14.4%、15.7%和15.1%的BD-rate降低，视觉分析显示显著减轻了几何变形伪影。

Conclusion: 所提出的深度条件图像压缩框架通过分层依赖驱动的方法有效解耦信息流，在低码率下保持更好的几何结构完整性，同时实现优异的率失真性能。
...
TL;DR: 视频扩散模型（VDMs）在视觉任务中表现出比语言模型更高的数据效率，表明视频预训练为视觉基础模型提供了有益的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言领域取得了成功，但在视觉领域仍面临组合理解、样本效率和通用问题解决的挑战。研究旨在探索视频扩散模型是否能弥补这一差距。

Method: 设计受控评估，为预训练的语言模型和视频扩散模型配备轻量级适配器，在自然模态中测试任务表现，包括ARC-AGI、ConceptARC、视觉游戏、路径规划和元胞自动机等基准。
...
TL;DR: 该论文提出了一种基于深度学习的解决方案，用于分类三种茶树病害（红锈病、Helopeltis和红蜘蛛螨），并检测叶片受损区域。评估了SSD MobileNet V2和Faster R-CNN ResNet50 V1两种目标检测模型，其中Faster R-CNN表现更好（mAP 25%）。还使用Mask R-CNN进行实例分割来计算叶片受损面积。


<details>
  <summary>Details</summary>
Motivation: 开发能够自动识别茶树病害并量化叶片受损面积的深度学习系统，以帮助茶农及时检测和处理病害问题，提高茶叶产量和质量。

Method: 使用SSD MobileNet V2和Faster R-CNN ResNet50 V1进行目标检测，比较两种模型的性能；使用Mask R-CNN进行实例分割，并开发自定义方法来计算叶片受损面积。

Result: Faster R-CNN ResNet50 V1在IOU 0.50:0.95范围内获得25%的mAP，优于SSD MobileNet V2的20.9%。成功实现了三种茶树病害的分类和受损区域的检测。

Conclusion: Faster R-CNN ResNet50 V1在茶树病害检测方面表现优于SSD MobileNet V2，结合Mask R-CNN的实例分割技术可以有效识别病害并量化受损面积，为茶树病害管理提供了有效的自动化解决方案。
...
TL;DR: Kineo是一个完全自动化的无标定多视角运动捕捉系统，可从非同步、未标定的消费级RGB相机视频中重建3D关键点和密集场景点云，无需精确相机标定，大幅提升了无标定方法的精度和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的无标定运动捕捉方法虽然减少了对精确相机标定的依赖，但存在计算成本高和重建精度低的问题，限制了非专业用户和野外场景的应用。

Method: 利用现成的2D关键点检测器同时标定相机（包括Brown-Conrady畸变系数）和重建3D关键点及密集场景点云；采用置信度驱动的时空关键点采样策略和基于图的全局优化，确保鲁棒标定且计算成本与序列长度无关；引入成对重投影共识评分来量化3D重建可靠性。

Result: 在EgoHumans和Human3.6M数据集上的评估显示，相比现有无标定方法，Kineo将相机平移误差降低约83-85%，相机角度误差降低86-92%，世界平均关节误差降低83-91%；在实际场景中处理效率高，特定配置下处理速度超过视频时长。

Conclusion: Kineo显著提升了无标定运动捕捉的精度和效率，为实际应用提供了可行的解决方案，代码已开源以促进可复现性和实际采用。
...
TL;DR: 提出Decoupled MeanFlow方法，将流模型转换为流映射模型而无需架构修改，实现1-4步高质量图像生成，在ImageNet 256×256和512×512上达到SOTA性能。
...
Result: 在ImageNet 256×256和512×512上，1步FID分别为2.16和2.12，4步FID分别为1.51和1.68，比流模型快100倍以上。

Conclusion: 训练流模型后转换比从头训练流映射更高效有效，实现了高质量快速图像生成。
...
Method: 构建在现有MLLMs基础上，集成两个组件：上下文感知视觉头自回归生成视觉表示，预训练草图解码器将这些表示渲染为人类可解释的图像，实现文本推理与视觉潜在生成的交替进行。
...
<details>
  <summary>Details</summary>
Motivation: 病理学家对千兆像素全切片图像的解释准确率约为70%，增加第二位病理学家并不能显著提高决策一致性，该领域缺乏足够的行为数据来解释诊断错误和不一致性。

Method: 使用名为PTAH的应用基础测试平台收集数据，记录了19位病理学家解释397张WSI的18.69小时数据，包括171,909次注视、263,320次扫视和1,867,362次鼠标交互事件。

Result: 创建了包含眼动追踪、鼠标交互、刺激追踪、视口导航和诊断决策数据的综合行为数据集，所有实验已在https://osf.io/hj9a7预注册。

Conclusion: 该数据集可用于解释诊断错误和不一致性，并改进病理学家和AI系统的训练，完整数据集和分析代码可在https://go.osu.edu/pathogaze获取。
...
TL;DR: 提出了一种名为Group Relative Attention Guidance（GRAG）的新方法，通过重新加权DiT模型中不同token的delta值来调节模型对输入图像相对于编辑指令的关注度，实现无需调优的连续细粒度编辑强度控制。
...
Method: 研究DiT模型中的MM-Attention机制，发现Query和Key token共享一个仅层依赖的偏置向量。将该偏置解释为模型固有的编辑行为，而每个token与其对应偏置之间的delta编码了内容特定的编辑信号。基于此提出GRAG方法，重新加权不同token的delta值来调节模型关注度。

Result: 在现有图像编辑框架上的广泛实验表明，GRAG只需四行代码即可集成，持续提升编辑质量。与常用的Classifier-Free Guidance相比，GRAG实现了更平滑和精确的编辑程度控制。
...
TL;DR: 提出SAGE方法，通过结合结构引导和生成合成，实现零样本的视频过渡，在多样视频片段间产生平滑、语义一致的过渡效果。


<details>
  <summary>Details</summary>
Motivation: 传统视频过渡方法在处理具有大时间间隔或显著语义差异的视频片段时存在困难，需要内容感知和视觉连贯的过渡解决方案。

Method: 基于艺术工作流程，提出SAGE方法，结合线图和运动流提供的结构引导与生成合成，实现零样本视频过渡。

Result: 在广泛实验中，SAGE在定量指标和用户研究中均优于传统和生成基线方法。

Conclusion: SAGE方法能够有效处理多样视频片段间的过渡，产生高质量、语义一致的过渡效果。
...
Result: 在M2I合成数据集和RoScenes真实数据集上的实验表明，MIC-BEV在3D目标检测中达到最先进性能，并在极端天气和传感器退化等挑战条件下保持鲁棒性。
...
TL;DR: 研究发现预训练的视觉Transformer（ViT）中自然涌现出物体绑定能力，能够判断两个图像块是否属于同一物体，这种能力在自监督模型中显著强于ImageNet监督模型。


<details>
  <summary>Details</summary>
Motivation: 探索视觉Transformer是否自然具备物体绑定能力，即判断图像块是否属于同一物体的能力，这对于理解ViT如何处理物体表示具有重要意义。

Method: 使用相似性探针从ViT各层的图像块嵌入中解码IsSameObject属性，比较不同预训练目标（DINO、MAE、CLIP、ImageNet监督）下该能力的表现。

Result: IsSameObject解码准确率超过90%，在自监督ViT中可靠涌现，但在ImageNet监督模型中明显较弱；该信号编码在物体特征的低维子空间中并主动引导注意力。

Conclusion: ViT确实具备物体绑定能力，这种能力不是架构的副产品，而是通过特定预训练目标获得的，挑战了ViT缺乏物体绑定的观点。
...
TL;DR: ProMoE是一个专门为扩散变换器设计的MoE框架，通过两阶段路由机制和显式路由指导来促进专家专业化，解决了视觉MoE中因视觉令牌的空间冗余和功能异质性导致的专家专业化不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有的MoE方法在扩散变换器中效果有限，主要原因是语言令牌和视觉令牌存在根本差异：语言令牌语义密集且变化显著，而视觉令牌存在空间冗余和功能异质性，阻碍了视觉MoE中的专家专业化。

Method: 提出ProMoE框架，包含两阶段路由器：1）条件路由根据功能角色将图像令牌划分为条件集和无条件集；2）原型路由通过可学习原型基于语义内容细化条件图像令牌的分配。同时引入路由对比损失来增强原型路由过程。

Result: 在ImageNet基准测试上的广泛实验表明，ProMoE在Rectified Flow和DDPM训练目标下均超越了最先进的方法。

Conclusion: ProMoE通过显式路由指导和原型路由机制有效解决了视觉MoE中的专家专业化问题，为扩散变换器的MoE应用提供了有效解决方案。
...
TL;DR: URSA是一个离散视频生成框架，通过迭代全局优化离散时空token，结合线性化度量路径和分辨率相关时间步长偏移机制，实现高效高分辨率图像合成和长视频生成，性能媲美连续扩散方法。
...
Method: URSA将视频生成任务构建为离散时空token的迭代全局优化，采用线性化度量路径和分辨率相关时间步长偏移机制，并引入异步时间微调策略统一多种任务。

Result: 在具有挑战性的视频和图像生成基准测试中，URSA持续优于现有离散方法，性能与最先进的连续扩散方法相当。

Conclusion: URSA证明了离散生成建模在视频生成领域的潜力，能够有效缩小与连续方法的差距，实现高效可扩展的视频生成。
...
TL;DR: 提出Generative View Stitching (GVS)方法，通过并行采样整个序列解决自回归视频扩散模型在相机引导生成中的碰撞问题，实现稳定、无碰撞且帧间一致的视频生成。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型虽然能够进行长序列生成，但无法利用未来信息来指导当前生成，在预定义相机轨迹的场景中容易导致碰撞和生成崩溃。

Method: 提出GVS采样算法，基于Diffusion Forcing框架，引入Omni Guidance技术同时利用过去和未来信息增强时间一致性，并实现闭环机制保证长距离连贯性。

Result: GVS实现了稳定、无碰撞、帧间一致的相机引导视频生成，能够处理各种预定义相机路径，包括著名的'不可能楼梯'等复杂场景。

Conclusion: GVS方法成功解决了自回归视频生成中的碰撞问题，为相机引导的视频生成提供了稳定可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索极端量化和动态路由等复杂技术在大型语言模型中的组合效应，这些方法的组合效果目前了解甚少。

Method: 引入BitSkip混合架构框架，系统研究量化与Hadamard变换的相互作用，比较不同位宽和变换配置的性能。

Result: 8位量化模型（BitSkip-V1）的困惑度为1.13，优于全精度基线的1.19；Hadamard变换在8位精度下导致性能下降超过37,000%；BitSkip-V1在第18层提供32.5%速度增益，仅损失4%质量。

Conclusion: 简单的8位量化模型在质量和效率上表现最佳，Hadamard变换在低精度下存在根本性的训练不稳定问题，BitSkip-V1提供了优越的早期退出特性。
...
TL;DR: 该论文评估了大语言模型处理文化相关语言的能力，特别是理解和使用包含文化细微差别的比喻表达。研究发现LLMs在理解阿拉伯语谚语和埃及阿拉伯语习语方面表现较差，且在语用使用和内涵解读方面存在挑战。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型处理文化基础语言的能力，特别是理解和使用包含当地知识和文化细微差别的比喻表达，以比喻语言作为文化推理的诊断工具。

Method: 设计了针对阿拉伯语和英语的上下文理解、语用使用和内涵解读评估任务，评估了22个开源和闭源LLMs在埃及阿拉伯语习语、多方言阿拉伯语谚语和英语谚语上的表现。

Result: 结果显示：阿拉伯语谚语准确率比英语低4.29%，埃及习语比阿拉伯谚语低10.28%；语用使用任务准确率比理解任务下降14.07%；模型在内涵意义解读上最多只能达到85.58%的人类标注一致性。

Conclusion: 比喻语言是文化推理的有效诊断工具：虽然LLMs通常能解释比喻意义，但在适当使用方面面临挑战。作者发布了首个用于比喻理解和语用使用评估的埃及阿拉伯语习语数据集Kinayat。
...
TL;DR: 本文收集了美国手语STEM对话的动作捕捉数据集，分析了对话中手语与孤立词汇的差异，发现对话中的手势持续时间比孤立手势短24.6%-44.6%，并评估了手语嵌入模型识别STEM手势的能力。


<details>
  <summary>Details</summary>
Motivation: 现有手语模型主要基于翻译员或孤立词汇数据，忽略了自然对话中的变异性。人类交流会根据语境和对话者动态调整时空特征和发音风格，这在教育环境中尤其明显。

Method: 收集ASL STEM对话的动作捕捉数据集，使用连续运动学特征分析对话特有的趋同现象与个体努力减少，比较双人互动手语、单人讲座和翻译文章的差异。

Result: 对话手势的平均持续时间比孤立手势短24.6%-44.6%，在独白语境中未观察到显著减少。通过评估手语嵌入模型，发现参与者随时间推移的趋同程度。

Conclusion: 研究连接了语言分析和计算建模，理解语用学如何塑造手语发音及其在手语技术中的表示，揭示了对话中手语的自然变异性特征。
...
TL;DR: CRADLE BENCH是一个用于多维度心理健康危机检测的基准测试，涵盖7种临床标准定义的心理危机类型，并首次纳入时间标签。该基准提供600个临床医生标注的评估样本和420个开发样本，以及约4K个通过多数投票集成方法自动标注的训练样本。


<details>
  <summary>Details</summary>
Motivation: 检测心理健康危机情况（如自杀意念、强奸、家庭暴力、儿童虐待和性骚扰）是语言模型面临的关键但未被充分探索的挑战。当这些情况在用户与模型交互中出现时，模型必须可靠地标记它们，因为未能检测可能带来严重后果。

Method: 构建了一个包含7种危机类型的基准测试，使用临床医生标注的评估样本和开发样本，以及通过多数投票集成多个语言模型自动标注的训练样本。进一步在基于共识和一致集成协议定义的子集上微调了六个危机检测模型。

Result: 多数投票集成方法显著优于单模型标注。提供了在不同协议标准下训练的互补模型，为心理健康危机检测提供了全面的评估框架。

Conclusion: CRADLE BENCH为心理健康危机检测提供了一个全面且具有时间维度的基准测试，通过集成标注和多模型训练策略，提高了危机检测的可靠性和准确性。
...
TL;DR: 论文研究了大型语言模型在对话中的时间盲点问题，提出了TicToc-v1测试集来评估模型的时间感知能力，发现现有模型在没有时间信息时表现仅略优于随机，添加时间戳后改进有限，需要专门的后训练对齐。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多轮对话中存在时间盲点，无法感知消息间的时间流逝，导致工具调用决策不当——要么过度依赖旧上下文而跳过必要调用，要么不必要地重复调用。

Method: 引入TicToc-v1测试集，包含34个不同时间敏感度的场景；在对话消息中添加时间戳；收集人类偏好数据分为prefer-noTool和prefer-Tool两个子集；评估LLM工具调用决策与人类偏好的对齐程度。

Result: 无时间信息时，大多数模型表现仅略优于随机，最佳对齐率仅略高于60%；添加时间戳后略有改进，特别是对较大模型，但改进有限，峰值约65%；基于提示的对齐方法效果有限。
...
TL;DR: 提出了一种轻量级框架，利用冻结的大型语言模型分析纵向临床数据，无需微调即可生成准确预测，在阿尔茨海默病早期监测中表现良好。


<details>
  <summary>Details</summary>
Motivation: 探索如何在不进行模型微调的情况下，利用冻结的大型语言模型有效分析纵向临床数据，特别是针对神经心理学评估和阿尔茨海默病早期监测的需求。

Method: 开发轻量级框架，将患者病史和上下文信息整合到语言模型空间中，直接利用冻结的LLM生成预测，无需额外训练。

Result: 该方法在神经心理学评估中实现了准确可靠的性能，即使在训练数据极少的情况下也能保持良好表现。

Conclusion: 该框架展示了在临床数据分析和阿尔茨海默病早期监测方面的潜力，为利用预训练语言模型进行医疗预测提供了有效途径。
...
TL;DR: 本文介绍了AfriMTEB，一个针对非洲语言的文本嵌入基准测试，包含59种语言、14个任务和38个数据集，以及AfriE5模型，通过跨语言对比蒸馏在非洲语言上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 非洲语言在现有的多语言文本嵌入基准测试中代表性不足，现有任务多从翻译基准中重新利用，缺乏针对非洲语言的专门评估。

Method: 1. 构建AfriMTEB基准测试，扩展MMTEB覆盖59种非洲语言；2. 开发AfriE5模型，通过跨语言对比蒸馏适配mE5模型到非洲语言。

Result: AfriE5在性能评估中超越了Gemini-Embeddings和mE5等强基线模型，实现了最先进的性能。

Conclusion: AfriMTEB填补了非洲语言在文本嵌入评估中的空白，AfriE5模型为非洲语言处理提供了有效的解决方案。
...
TL;DR: 本文提出了一个新颖的增强框架，通过三个即插即用步骤来评估大型语言模型的公平性，发现即使是最先进的模型也容易受到输入扰动的影响，表现出更高的刻板印象倾向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型因其训练数据的判别性质而表现出刻板印象偏见，尽管已有方法试图避免使用刻板信息，但现有的偏见对齐方法存在脆弱性。

Method: 提出了一个包含三个即插即用步骤的增强框架，适用于多个公平性评估基准，通过在Bias Benchmark for Question Answering (BBQ)数据集上应用增强来测试模型。

Result: 发现包括最先进的开源和闭源模型在内的大型语言模型都容易受到输入扰动的影响，更可能表现出刻板印象行为，特别是当目标人口属于文献研究较少的社区时。

Conclusion: 需要将公平性和安全性研究扩展到更多样化的社区，以解决模型对较少研究社区的偏见行为问题。
...
TL;DR: 提出一种基于智能体的自动化索赔匹配方法，使用指令跟随LLMs构建两步流程：先用LLMs生成提示，再用LLMs进行索赔匹配的二元分类任务。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在自动化索赔匹配任务中的应用潜力，特别是研究LLM生成的提示是否能超越人工生成的提示，以及如何优化计算资源使用。

Method: 采用两步流水线方法：第一步使用LLMs生成提示，第二步使用LLMs进行索赔匹配的二元分类。研究不同规模LLMs在提示生成中的表现，以及使用不同LLMs组合的效果。

Result: LLM生成的提示能够超越最先进的人工生成提示；较小的LLMs在提示生成过程中表现与较大模型相当，可以节省计算资源；使用不同LLMs组合（一个用于提示生成，另一个用于索赔匹配）具有有效性。

Conclusion: 该方法不仅实现了高效的自动化索赔匹配，还揭示了LLMs对索赔匹配任务的理解机制，为优化LLMs应用提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决在复杂工业目录中弥合通用语言理解与领域特定知识之间差距的问题，同时减少对训练数据和模型微调的依赖

Method: 基于人工编写提示种子，通过级联系统自动生成和优化指令，满足特定目录需求，无需训练标签或模型微调

Result: 相比传统思维链提示，精度和召回率提升8-10%，专家工作量从每属性5.1小时减少到3分钟（减少99%），在五种语言和多个质量评估任务中保持性能增益

Conclusion: 该自动提示级联系统在电商产品质量评估中实现了高效、可扩展的解决方案，显著降低了领域专家参与需求，同时保持跨语言和任务的强泛化能力
...
TL;DR: 提出了一种基于连接组信息的LLM框架，将动态fMRI连接性编码为时间序列，应用稳健归一化，并将这些数据映射到适合冻结预训练LLM的表示形式，用于临床预测。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够敏感检测早期阿尔茨海默病的方法，实现错误率远低于临床公认阈值的预测，为及时干预提供可能。

Method: 使用连接组信息LLM框架，将动态fMRI连接性编码为时间序列，应用稳健归一化技术，并将数据映射到适合冻结预训练LLM的表示形式。
...
TL;DR: 本文研究了多语言大语言模型在遗忘学习过程中出现的语言混淆现象，发现仅用英语数据进行知识擦除对多语言模型效果不佳，并提出N-Mix评分来量化这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明仅用英语数据擦除多语言LLM的知识不够充分，但分析过于关注性能指标。本文从评估角度出发，发现当多语言LLM使用并行多语言数据集进行全微调后再进行遗忘学习时，会出现语言混淆问题，导致基于参考的评估指标失效。

Method: 通过三个步骤处理语言混淆现象：(1)引入基于N-gram的语言混合(N-Mix)评分来量化语言混淆；(2)证明当N-Mix评分高时，基于参考的指标会产生假阴性；(3)建议需要能够直接评估生成句子内容的新型遗忘学习评估方法。

Result: 研究发现语言混淆在多语言LLM中普遍且一致存在，基于参考的评估指标在N-Mix评分高时会失效，需要基于语义的评估指标来准确评估遗忘学习效果。

Conclusion: 多语言LLM的遗忘学习评估需要新的语义基础指标，因为传统的基于参考的指标在语言混淆情况下无法准确评估模型性能，N-Mix评分可以有效量化语言混淆程度。
...
TL;DR: 提出M-Eval方法，通过异质性分析检测RAG医疗问答系统中的事实错误，提高回答准确性达23.31%。


<details>
  <summary>Details</summary>
Motivation: 当前RAG医疗问答系统存在生成错误信息（如幻觉）和未能正确使用外部知识的问题，需要一种有效的事实核查方法。

Method: 基于循证医学的异质性分析方法，从外部知识库提取额外医学文献，检索RAG系统生成的证据文档，通过异质性分析检查证据是否支持回答中的不同观点。

Result: 该方法在各种LLM上实现了高达23.31%的准确率提升，能够有效检测RAG系统中的错误。

Conclusion: M-Eval方法能够提高LLM应用的可靠性，减少诊断错误，使基于RAG的医疗系统更加可信。
...
TL;DR: 提出了PICOs-RAG方法，通过将用户查询扩展和规范化成专业格式，利用EBM中的PICO搜索策略提取关键信息，显著提升检索效率和相关性，相比基线方法提升8.8%。
...
Method: 使用PICOs-RAG方法将用户查询扩展和规范化成专业格式，利用EBM中的PICO格式提取最重要信息用于检索。
...
TL;DR: 本文提出了一种基于元分析思想的新方法，用于重新排序和过滤医学证据，以提高基于检索增强生成（RAG）的循证医学（EBM）任务中证据质量，实验显示准确率提升达11.4%。


<details>
  <summary>Details</summary>
Motivation: 循证医学对证据质量有严格要求，但现有RAG应用在EBM任务中难以有效区分高质量证据，导致误诊风险增加。

Method: 采用模拟元分析的方法，结合可靠性分析、异质性分析和外推分析等多个EBM方法，对医学证据进行重新排序和过滤。

Result: 实验结果表明，该方法能够从PubMed数据集中提取更高质量和更可靠的证据，准确率提升最高达11.4%。

Conclusion: 该方法成功提升了RAG在EBM任务中的证据质量，减少了错误知识注入回答的可能性，帮助用户获得更有效的回复。
...
TL;DR: 本文提出了TEXT2DB任务，强调信息抽取输出与目标数据库的集成，并开发了OPAL框架来处理用户指令驱动的数据库更新。
...
TL;DR: 本文提出了一种基于细粒度语义置信度奖励的强化学习框架，通过语义聚类引导大语言模型在超出知识范围时拒绝回答，从而减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用粗粒度信号（如整体置信度或不确定性分数）来指导LLMs拒绝回答超出知识范围的问题，这可能导致模型对自身知识边界认知不精确，无法有效缓解幻觉问题。

Method: 提出基于细粒度语义置信度奖励的强化学习框架：采样多个候选答案并进行语义聚类，然后训练LLM保留高置信度聚类中的答案，丢弃低置信度聚类中的答案，从而实现准确的事后拒绝。

Result: 该方法在领域内和领域外基准测试中显著提高了可靠性。

Conclusion: 提出的细粒度语义置信度奖励框架能够有效提升大语言模型的可靠性，特别是在处理超出知识范围问题时表现出色。
...
TL;DR: 该论文提出了一种训练大型多模态模型形成语言约定的方法，通过模拟参考游戏让模型学会高效沟通，在人类交互中能减少41%的消息长度同时提高15%的成功率。


<details>
  <summary>Details</summary>
Motivation: 人类通过共享对话上下文形成临时语言约定来实现高效沟通，但现有大型多模态模型缺乏这种能力。研究旨在让模型学会形成约定以实现更高效的沟通。

Method: 使用模拟参考游戏在模型之间进行训练，无需额外人工数据。在涉及照片和七巧板图像的重复参考游戏中训练模型形成约定。

Result: 模型与人类交互时消息长度减少41%，成功率提高15%。人类听众与形成约定的模型交互时响应更快。仅基于成功或成本的训练都不足以引发约定形成。

Conclusion: 该方法成功训练模型形成语言约定，实现高效沟通，证明成功和成本两个因素对约定形成都是必要的。
...
TL;DR: 提出了HalloMTBench多语言幻觉基准测试框架，用于诊断大型语言模型在机器翻译中的幻觉问题，包含5435个高质量实例，覆盖11个英语到其他语言的翻译方向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在机器翻译中容易产生幻觉，但现有的MT基准测试无法充分暴露多语言LLM的失败情况，需要专门的诊断框架来揭示幻觉问题。

Method: 构建了包含指令脱离和源文脱离的分类法，使用4个前沿LLM生成候选翻译，通过LLM评委集成和专家验证的方式筛选高质量实例，创建了HalloMTBench基准测试。

Result: 评估了17个LLM，发现了独特的"幻觉触发因素"，包括模型规模、源文长度敏感性、语言偏见以及强化学习放大的语言混合问题。
...
TL;DR: Global PIQA是一个覆盖100多种语言和文化的常识推理基准，由来自65个国家的335名研究人员手动构建，包含116种语言变体，其中超过50%的示例涉及当地食物、习俗等文化特定元素。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏覆盖大量语言和文化的文化特定评估基准，需要构建一个全面的多语言常识推理基准来评估LLM在不同文化背景下的表现。

Method: 采用参与式方法，由来自65个国家的335名研究人员手动构建，涵盖116种语言变体、14个语系和23种文字系统，包含平行和非平行数据集。

Result: 最先进的LLM在总体上表现良好，但在低资源语言中表现较弱（准确率差距达37%），开源模型普遍不如专有模型，许多语言文化中的日常知识仍是改进领域。

Conclusion: Global PIQA不仅为LLM评估提供了工具，还展示了人类语言所嵌入的广泛文化多样性，强调日常知识评估与复杂推理能力同等重要。
...
TL;DR: 该研究旨在记录和分析孟加拉语方言的语音和形态特征，探索为区域方言构建计算模型（特别是自动语音识别系统）的可行性，以促进方言多样性保护和包容性数字工具的发展。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语在语音和发音基础上存在显著的方言多样性，但关于孟加拉语方言的计算处理研究仍然有限。该研究旨在填补这一空白，通过记录和分析方言特征，为虚拟助手等语言技术应用提供支持。

Method: 研究通过文档化和分析孟加拉语方言的语音和形态属性，探索构建针对区域变体的计算模型，特别是自动语音识别系统，并创建了公开可用的数据集。

Result: 研究创建了孟加拉语方言数据集并公开发布，为后续计算模型开发提供了基础资源。

Conclusion: 该研究为孟加拉语方言的计算处理奠定了基础，有助于方言多样性保护和包容性数字工具的发展，对孟加拉语社区的虚拟助手和语言技术应用具有潜在价值。
...
TL;DR: Squrve是一个统一的、模块化的Text-to-SQL框架，通过标准化执行范式和多参与者协作机制，将研究成果与实际应用相结合，在基准测试中表现优于原始方法。


<details>
  <summary>Details</summary>
Motivation: 尽管Text-to-SQL技术在学术上取得了显著进展，但在实际系统部署中仍面临集成工具有限的挑战，需要将研究进展与实际应用相结合。

Method: 首先建立通用执行范式来标准化调用接口，然后基于七个抽象的有效原子参与者组件提出多参与者协作机制。

Result: 在广泛采用的基准测试中，协作工作流持续优于原始个体方法。

Conclusion: Squrve为处理复杂现实世界查询开辟了一条新的有效途径，代码已在GitHub开源。
...
TL;DR: 本文提出了两种模式感知的过滤方法：模式感知行级去重（PLD）和模式感知尾随标点过滤（PTF），通过考虑行级信号及其在文档中的顺序分布来保留结构重要内容，相比传统过滤方法提升了语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统行级过滤技术（如行级去重和尾随标点过滤）有时会丢弃有价值内容，对下游性能产生负面影响，需要改进这些基本方法以保留更多有用信息。

Method: 提出了两种增强型过滤方法：PLD考虑行级重复模式及其在文档中的分布，PTF分析标点模式在文档中的上下文分布，两者都结合了行级信号和顺序分布信息。

Result: 在英语和韩语的小型语言模型（10亿参数）训练中，新方法在多项选择题基准上持续提升性能，并在SQuAD v1和KorQuAD v1的生成式问答任务中显著提高准确率。

Conclusion: 模式感知的过滤方法通过考虑行级信号的顺序分布，能够有效保留结构重要内容，相比传统过滤技术显著提升了语言模型的性能表现。
...
TL;DR: Ko-MuSR是首个全面评估韩语长文本中多步软推理能力的基准数据集，通过构建完全韩语的叙事、推理链和多选题，最小化数据污染，为韩语NLP发展提供系统评估基础。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对韩语长文本多步推理的评估基准，需要构建一个能够系统评估韩语推理能力的数据集，同时避免数据污染问题。

Method: 基于MuSR框架构建Ko-MuSR，包含完全韩语的叙事文本、推理链和人工验证的多选题，评估四个大语言模型（两个多语言模型和两个韩语专用模型）的推理能力。

Result: 多语言模型在韩语推理任务中表现优于韩语专用模型，表明推理能力具有跨语言泛化特性；精心设计的提示策略（结合少样本示例、推理轨迹和任务特定提示）显著提升准确率，接近人类水平。

Conclusion: Ko-MuSR为韩语NLP发展提供了坚实基础，能够系统评估长文本推理能力和提示策略，推动韩语自然语言处理技术的进步。
...
TL;DR: 本文介绍了MuSaG，这是首个德语多模态讽刺检测数据集，包含33分钟来自德国电视节目的手动选择和人工标注的语句。该数据集提供对齐的文本、音频和视频模态，并比较了9个开源和商业模型在单模态和多模态设置下的性能。


<details>
  <summary>Details</summary>
Motivation: 讽刺是一种复杂的比喻语言形式，其字面意义与真实意图相矛盾。在社交媒体和流行文化中的普遍存在对自然语言理解、情感分析和内容审核构成持续挑战。随着多模态大语言模型的出现，讽刺检测需要整合来自音频和视觉的线索。

Method: 构建了MuSaG数据集，包含33分钟来自德国电视节目的语句，每个实例提供对齐的文本、音频和视频模态，并由人类分别标注。基准测试了9个开源和商业模型，涵盖文本、音频、视觉和多模态架构。

Result: 结果显示，人类在对话环境中严重依赖音频线索，而模型在文本上表现最佳。这突显了当前多模态模型在整合音频信息方面的差距。
...
TL;DR: 该研究探讨了外部知识整合对自然语言生成的影响，通过创建KITGI基准测试，比较了在完整外部知识和过滤知识条件下使用T5-Large模型生成句子的表现。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索外部知识在自然语言生成中的作用，特别是常识生成任务中知识整合的重要性，旨在设计可解释的知识增强NLG系统。

Method: 采用三阶段方法：识别并移除关键知识、重新生成句子、人工评估输出的常识合理性和概念覆盖度。使用CommonGen数据集扩展创建KITGI基准，包含从ConceptNet检索的语义关系和人工标注输出。

Result: 结果显示，使用完整知识生成的句子在两个标准上达到91%的正确率，而过滤知识后性能急剧下降至6%。

Conclusion: 相关外部知识对于保持NLG的连贯性和概念覆盖至关重要，需要设计可解释的知识增强NLG系统，并开发超越表面指标的评估框架来捕捉底层推理。
...
TL;DR: 该论文提出了一种基于潜在空间语义对齐的跨规模大语言模型知识迁移方法，通过激活值而非直接使用层参数来实现有效的知识转移。


<details>
  <summary>Details</summary>
Motivation: 由于不同规模LLM之间存在神经不兼容性（架构和参数差异），现有直接重用层参数的方法受到严重限制，需要找到有效的跨规模知识迁移方法以实现更大的灵活性和更广泛的应用。

Method: 以激活值作为层间知识迁移的媒介，利用潜在空间中的语义对齐来实现跨规模知识转移，而不是直接使用层参数。

Result: 在四个基准测试上的评估证明了该方法的有效性，优于先前工作，能更好地在不同规模模型之间对齐模型行为。

Conclusion: 潜在空间语义对齐是实现LLM跨规模知识转移的基本前提，进一步分析揭示了缓解跨规模知识转移的关键因素，并提供了对潜在语义对齐本质的见解。
...
TL;DR: 提出基于知识和确定性两个维度的幻觉分类框架，通过模型特定的数据集构建区分不同类型的幻觉，并验证了知识轴分类的有效性，发现不同模型具有不同的知识和幻觉模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常基于外部属性对幻觉进行分类，忽视了幻觉可能需要基于其底层机制采取针对性缓解策略。需要从LLMs内部属性出发对幻觉进行更有效的分类。

Method: 提出基于知识和确定性两个维度的幻觉分类框架，包括模型特定的数据集构建过程，应用引导缓解技术验证知识轴分类，并引入新的评估指标来衡量缓解方法在关键案例上的效果。

Result: 验证了知识轴分类的有效性，显示两种幻觉类型存在显著差异；发现不同模型具有不同的知识和幻觉模式；揭示某些缓解方法在平均表现良好但在关键案例上失败。
...
TL;DR: 该论文研究了如何通过推理和训练时选择来提高大型语言模型在医疗等敏感领域解释的忠实性，发现few-shot示例的数量和质量、提示设计以及指令调优阶段对模型忠实性有显著影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生不忠实反映其预测驱动因素的解释，在医疗环境中这种不忠实性尤为危险，可能削弱临床医生的信任并导致不安全的决策支持。

Method: 评估了三种LLM（GPT-4.1-mini、LLaMA 70B、LLaMA 8B）在两个数据集（BBQ和MedQA）上的表现，操纵了few-shot示例的数量和类型、提示策略以及训练过程。

Result: few-shot示例的数量和质量显著影响模型忠实性；忠实性对提示设计敏感；指令调优阶段提高了MedQA上的测量忠实性。
...
TL;DR: 该论文提出了一种多模态方法，结合文本和语音信息来解决阿拉伯语方言的变音符号恢复任务，通过两种集成策略融合CATT文本编码器和Whisper语音编码器，在训练中随机禁用语音输入以提高模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语方言句子中的变音符号恢复任务，通过结合文本和语音信息的多模态方法来提高恢复准确性。

Method: 使用CATT预训练模型的编码器处理文本模态，OpenAI Whisper基础模型的编码器处理语音模态。采用两种集成策略：早期融合（将语音标记与文本标记合并）和交叉注意力融合。在训练中随机禁用语音输入以增强模型鲁棒性。

Result: 在开发集上，模型实现了0.25的词错误率和0.9的字符错误率；在测试集上，实现了0.55的词错误率和0.13的字符错误率。
...
TL;DR: 本研究比较了5种大语言模型在生成适合挪威儿童（5岁和9岁）对话方面的表现，发现大多数模型生成的语言比目标年龄组更复杂，GPT-4和NorBloom-7b表现相对较好。


<details>
  <summary>Details</summary>
Motivation: 大语言模型主要基于成人对话数据训练，在生成真实儿童对话方面面临挑战，特别是在低资源语言环境中开发适合儿童的专门应用。

Method: 通过盲评方式，由11位教育专业人士评估真实儿童访谈数据和LLM生成的文本样本，评估真实性和发展适宜性，并计算评分者间信度。

Result: 评估者表现出较高的评分者间信度（ICC=0.75），对年幼儿童（5岁）的年龄预测准确率高于年长儿童（9岁）。大多数模型生成的语言被认为比目标年龄组更复杂。

Conclusion: 研究结果突显了在开发涉及儿童的专业LLM系统时面临的关键数据相关挑战，特别是在低资源语言环境中，缺乏全面的适龄词汇资源。
...
TL;DR: 本文提出了一种自动生成自然语言推理问题变体的方法MERGE，通过替换开放类词汇但保留推理逻辑，评估模型在最小变化问题上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有NLI模型在泛化基准测试中表现不佳，但手动创建新基准成本高，自动生成高质量变体困难。

Method: 提出MERGE方法，自动生成原始NLI问题的变体，通过替换开放类词汇但保持底层推理逻辑不变。

Result: NLI模型在变体上的性能下降4-20%，表明即使在最小变化的问题上泛化能力也较低。

Conclusion: 模型在推理保留的变体上表现不佳，揭示了当前NLI模型的泛化局限性，并分析了替换词类别、词概率和合理性对性能的影响。
...
TL;DR: 本文提出LATR方法解决强化学习中轨迹多样性不足的问题，通过前瞻树搜索策略在不确定性高的生成步骤分支，模拟不同候选路径并修剪相似分支，显著提升策略学习效率和最终性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习管道中，组rollout期间采样的轨迹多样性有限，同质化轨迹和相关奖励会削弱策略更新的回报信号，阻碍有效策略学习。这种多样性不足主要源于token级随机采样，局部变化容易坍缩为几乎相同的推理路径。

Method: 提出LATR（前瞻树基rollout）策略，通过强制分支到可能产生不同延续的候选token来显式促进轨迹级多样性。具体包括三个阶段：(1)在高不确定性生成步骤分支，(2)为每个新分支执行前瞻模拟，(3)修剪模拟过程中表现出持续相似性的分支。

Result: 与随机采样相比，LATR平均加速策略学习131%，在GRPO和DAPO算法上，不同推理任务的最终pass@1性能提升4.2%。

Conclusion: LATR通过增强轨迹多样性有效解决了强化学习中的瓶颈问题，显著提升了策略学习效率和模型推理能力。
...
Method: 采用双玩家范式：演员生成响应，批判者提供反馈，演员据此优化响应。通过两阶段优化：第一阶段用基于规则的直接奖励信号强化批判者的判别能力；第二阶段引入基于演员优化的间接奖励提升批判者的帮助性，同时通过正则化保持判别能力。
...
TL;DR: 本文提出了一种评估大语言模型在文化接地和方言内容上表现的综合方法，通过将现代标准阿拉伯语多选题翻译成英语和多种阿拉伯语方言，并转换为开放式问题，对多种模型进行基准测试。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在回答日常问题中的应用日益增多，但它们在文化接地和方言内容上的表现存在语言间的不均衡，特别是在阿拉伯语方言方面存在知识差距。

Method: 将现代标准阿拉伯语多选题翻译成英语和多种阿拉伯语方言，转换为开放式问题，对零样本和微调模型进行基准测试，并生成思维链推理来微调模型进行逐步推理。

Result: 模型在阿拉伯语方言上表现不佳，阿拉伯语中心模型在多选题上表现良好但在开放式问题上表现不佳，思维链推理提高了判断正确性但n-gram指标表现不一。

Conclusion: 研究揭示了模型在文化接地和方言特定知识方面的持续差距，开发的数据集将公开发布以支持文化和语言包容性评估的进一步研究。
...
TL;DR: LongWeave是一个新的长文本生成基准，通过约束验证评估（CoV-Eval）方法平衡真实性和可验证性，评估LLM在复杂现实约束下的长文本生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有长文本生成基准要么使用难以验证的真实世界查询，要么使用忽略现实复杂性的合成设置，需要一种既能反映现实复杂性又能客观评估的方法。

Method: 采用约束验证评估（CoV-Eval）方法，首先在真实场景中定义可验证目标，然后系统生成相应查询、文本材料和约束，支持自定义输入/输出长度（最高64K/8K tokens）的七种任务。

Result: 对23个LLM的评估显示，即使最先进的模型在现实复杂性增加和输出长度增加时，长文本生成仍面临显著挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于知识蒸馏的情感分析方法存在两个主要问题：人工编写指令的多样性和数量有限，无法确保蒸馏知识的全面覆盖；大规模用户文本导致高计算成本，影响方法实用性。

Method: 提出COMPEFFDIST框架，包含两个关键模块：基于属性的自动指令构建和基于难度的数据过滤，分别解决指令多样性不足和数据效率低的问题。

Result: 在多个模型系列（Llama-3、Qwen-3、Gemma-3）上应用该方法，3B学生模型在大多数任务上能够达到20倍大小教师模型的性能水平，并且在数据效率上显著优于基线方法，仅需10%数据即可达到相同性能。

Conclusion: COMPEFFDIST框架有效解决了情感分析知识蒸馏中的指令多样性和数据效率问题，实现了高性能且实用的轻量级模型开发。
...
### [93] [Fine-tuning Large Language Models with Limited Data: A Survey and Practical Guide](https://arxiv.org/abs/2411.09539)
*Marton Szep,Daniel Rueckert,Rüdiger von Eisenhart-Rothe,Florian Hinterwimmer*

Main category: cs.CL

TL;DR: 本文对数据稀缺场景下大语言模型微调方法进行了系统性综述，重点介绍了参数高效微调、领域和跨语言适应、模型专业化策略以及偏好对齐方法。
...
Method: 系统回顾参数高效微调技术以降低训练和部署成本，涵盖领域和跨语言适应方法（包括编码器和解码器模型），以及模型专业化策略。同时研究使用有限人类或合成反馈指导模型行为的偏好对齐方法。

Result: 强调了经验权衡、选择标准和最佳实践，为基于任务约束选择合适技术提供指导，包括模型扩展、数据扩展和灾难性遗忘缓解。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以清晰区分语言模型的推理能力和事实知识记忆，因为基准测试性能往往反映的是事实回忆而非真正的推理。

Method: 构建两个结构相同但内容不同的平行世界语料库：真实映射世界（可利用参数知识）和合成映射世界（参数知识无意义），并在其上设计镜像任务（多跳问答和页面导航）。

Result: 实验显示模型在参数知识和知识增强设置下都存在持续的知识优势差距，知识获取和整合机制可以减少但无法消除这一差距。
...
TL;DR: LuxIT是一个针对卢森堡语的新型单语指令调优数据集，旨在解决低资源语言环境中高质量训练数据不足的问题。通过使用DeepSeek-R1-0528从原生卢森堡语文本合成数据，并采用LLM作为评判的质量保证流程。尽管在多个小型LLM上的微调结果显示性能表现不一，但该数据集为卢森堡语自然语言处理做出了重要贡献。


<details>
  <summary>Details</summary>
Motivation: 解决指令调优大型语言模型在低资源语言环境中因缺乏高质量训练数据而效果受限的问题，特别是针对卢森堡语这一资源稀缺的语言。

Method: 1. 从原生卢森堡语文本语料库合成LuxIT数据集；2. 使用DeepSeek-R1-0528进行数据生成；3. 采用LLM作为评判的质量保证流程；4. 在多个小型LLM上对数据集进行微调；5. 在卢森堡语语言能力考试上对微调后的模型进行基准测试。

Result: 在卢森堡语语言能力考试上的基准测试结果好坏参半，不同模型的性能表现差异显著，表明数据集的实际效用存在不确定性。

Conclusion: LuxIT为卢森堡语自然语言处理做出了重要贡献，并提供了一个可复制的单语方法学，但研究结果强调了需要进一步优化其应用的深入研究。
...
TL;DR: 本文引入了一种新的对抗性改写任务，通过生成语法正确且语义保留的文本改写来降低多模态大语言模型在推理分割任务中的性能，并提出了SPARTA方法在语义潜在空间中进行优化。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注图像输入的扰动，而语义等效的文本改写在实际应用中至关重要但尚未充分探索，需要评估模型对多样化用户表达方式的鲁棒性。

Method: 提出了SPARTA方法——一种黑盒、句子级优化方法，在文本自动编码器的低维语义潜在空间中操作，通过强化学习指导生成对抗性改写。

Result: SPARTA在ReasonSeg和LLMSeg-40k数据集上取得了显著更高的成功率，比先前方法高出最多2倍，揭示了先进推理分割模型在严格语义和语法约束下仍然容易受到对抗性改写攻击。

Conclusion: 多模态大语言模型在推理分割任务中对对抗性文本改写存在脆弱性，SPARTA方法有效暴露了这种漏洞，为模型鲁棒性评估提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力的不断提升，现有基准测试主要关注英语，而在非英语语言评估方面仍缺乏系统性研究。本文旨在填补这一空白，特别关注欧洲语言的评估需求。

Method: 首先回顾了LLM基准测试的最新发展，然后提出了专门针对多语言或非英语使用场景的基准测试分类法。进一步提出了一套最佳实践和质量标准建议。

Result: 提出了一个系统化的基准测试分类框架，强调评估方法需要更高的语言和文化敏感性。为欧洲语言基准测试的协调发展提供了具体指导原则。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法在个性化文本生成中容易产生语调、话题或风格的漂移，需要一种能够持续保持个性化一致性的解决方案。

Method: 采用训练免费的批判-精炼框架，包含LLM生成器基于检索配置文件生成草稿，批判LLM提供结构化反馈，生成器进行修订，并通过淘汰策略保留更强草稿。

Result: 在Yelp、Goodreads和Amazon数据集上，PerFine相比PGraphRAG在GEval指标上提升了7-13%，在3-5次精炼迭代中持续改进，并随批判模型规模增大而扩展。

Conclusion: 基于配置文件的反馈为个性化LLM生成提供了强大范式，既无需训练又模型无关。
...
TL;DR: 本文综述了RAG和推理增强在缓解LLM幻觉方面的协同作用，提出了基于知识和逻辑的幻觉分类，并建立了统一框架。


<details>
  <summary>Details</summary>
Motivation: 幻觉是LLM可靠部署的主要障碍，RAG和推理增强是最有效的缓解策略，但它们的协同机制尚未系统研究。

Method: 采用应用导向的能力增强视角，分析RAG、推理增强及其在智能体系统中的整合如何缓解幻觉，提出幻觉分类法。

Result: 建立了统一框架，通过实际应用、评估和基准测试支持，系统检验了RAG和推理增强对知识型和逻辑型幻觉的缓解效果。
...
TL;DR: 本文提出了从科学演讲中预测参考文献（RPT）的新任务，并发布了首个大规模数据集Talk2Ref，包含6,279个演讲和43,429篇引用论文。通过评估最先进的文本嵌入模型和提出的双编码器架构，证明了在Talk2Ref上微调能显著提高引用预测性能。
...
Method: 提出了RPT任务，构建了Talk2Ref数据集，评估了零样本检索场景下的最先进文本嵌入模型，提出了双编码器架构，并探索了处理长文本转录和领域适应的策略。

Result: 在Talk2Ref数据集上微调显著提高了引用预测性能，证明了该任务具有挑战性，同时验证了从口语科学内容中学习语义表示的有效性。

Conclusion: Talk2Ref数据集和训练模型以开放许可发布，旨在促进未来将口语科学交流整合到引用推荐系统中的研究。
...
TL;DR: 本文提出使用自然语言批评来增强大语言模型的置信度校准，研究了批评内容（不确定性vs置信度）和方法（自我批评vs批评校准训练），并提出了CritiCal训练方法，在复杂推理任务中显著优于现有方法。
...
Method: 提出自然语言批评作为解决方案：1）研究批评内容：不确定性（问题导向）或置信度（答案特定）；2）提出自我批评方法和CritiCal批评校准训练方法，利用自然语言批评改进置信度校准。

Result: 实验表明CritiCal显著优于自我批评和其他基线方法，在复杂推理任务中甚至超过其教师模型GPT-4o，并在分布外设置中表现出鲁棒的泛化能力。

Conclusion: 自然语言批评是增强大语言模型置信度校准的有效方法，CritiCal方法在复杂任务中表现出色，提升了LLM的可靠性。
...
TL;DR: 该论文提出了一种面向零静默率目标的词汇消歧方法，该方法在Silberztein的INTEX系统中实现，重点研究了局部消歧语法的验证问题。


<details>
  <summary>Details</summary>
Motivation: 解决词汇消歧中词性标注的歧义问题，特别是在需要保证零静默率（正确标签从不被丢弃）的应用场景中，传统唯一标注方法无法满足这一要求。

Method: 在INTEX系统中实现局部消歧语法，通过转换器路径的交互验证和组合应用来处理词汇歧义，确保正确标签始终保留。

Result: 研究发现单独考虑转换器路径不足以验证局部消歧语法，需要考虑路径间的交互作用；组合多个转换器时结果无法通过单独分析预测；直觉制定的消歧规则可能因未预见的构造或歧义而不准确。

Conclusion: 为实现零静默率目标，局部语法需要仔细测试，必须详细规范语法在文本应用中的行为，考虑转换器间的交互影响。
...
### [103] [Dark & Stormy: Modeling Humor in the Worst Sentences Ever Written](https://arxiv.org/abs/2510.24538)
...
### [104] [Open Korean Historical Corpus: A Millennia-Scale Diachronic Collection of Public Domain Texts](https://arxiv.org/abs/2510.24541)
*Seyoung Song,Nawon Kim,Songeun Chae,Kiwoong Park,Jiho Jin,Haneul Yoo,Kyunghyun Cho,Alice Oh*

Main category: cs.CL

TL;DR: 本文介绍了开放韩语历史语料库，这是一个大规模、开放许可的数据集，涵盖1300年历史、6种语言和多种书写系统，包含1800万文档和50亿标记，用于量化分析韩语的主要语言变迁。


<details>
  <summary>Details</summary>
Motivation: 韩语历史中存在口语和书面语形式之间的差异以及从汉字到韩文字母的关键转变，但由于缺乏可访问的历史语料库，这种语言演变在NLP领域尚未得到充分探索。

Method: 构建开放韩语历史语料库，这是一个大规模、开放许可的数据集，跨越1300年和6种语言，包含韩国式汉语（吏读）和韩汉混合文字等代表性不足的书写系统。语料库包含来自19个来源的1800万文档和50亿标记。

Result: 利用该资源定量分析主要语言变迁：(1) 吏读使用在1860年代达到顶峰后急剧下降；(2) 从汉字到韩文的转变始于1890年左右，是一个快速转型；(3) 朝鲜的词汇差异导致现代分词器产生高达51倍的词汇外率。

Conclusion: 这项工作通过捕捉韩语历史，为定量历时分析提供了基础资源。此外，它可作为大型语言模型的预训练语料库，可能提高其对现代韩文中的汉韩词汇以及古旧书写系统的理解。
...
### [105] [BEST-RQ-Based Self-Supervised Learning for Whisper Domain Adaptation](https://arxiv.org/abs/2510.24570)
...
Result: 在ATCO2航空交通管制语料库上，使用5000小时未标注语音和2小时标注语音微调，相比基线模型和微调模型获得12%的相对改进。
...
### [106] [ReForm: Reflective Autoformalization with Prospective Bounded Sequence Optimization](https://arxiv.org/abs/2510.24592)
...
Method: 提出ReForm方法，将语义一致性评估紧密集成到自动形式化过程中，采用前瞻有界序列优化(PBSO)训练模型，在不同序列位置使用不同奖励以确保准确的形式化和语义验证。

Result: 在四个自动形式化基准测试中，ReForm比最强基线平均提升17.2个百分点。ConsistencyCheck基准显示人类专家在38.5%的情况下也会产生语义错误。

Conclusion: ReForm通过反射式方法显著提升自动形式化性能，同时揭示了自动形式化本身的内在难度，即使人类专家也难以避免语义错误。
...
### [107] [Diffusion LLM with Native Variable Generation Lengths: Let [EOS] Lead the Way](https://arxiv.org/abs/2510.24605)
...
<details>
  <summary>Details</summary>
Motivation: 解决当前扩散大语言模型生成长度固定的问题，传统方法需要预先确定生成长度作为超参数，导致效率和灵活性不足。
...
Conclusion: 该方法实现了更快的推理速度和更高的准确性，使扩散大语言模型超越了单纯的学术新颖性，支持在实际应用中的实用化。
...
### [108] [Long-Context Modeling with Dynamic Hierarchical Sparse Attention for On-Device LLMs](https://arxiv.org/abs/2510.24606)
*Siheng Xiong,Joe Zou,Faramarz Fekri,Yae Jee Cho*

Main category: cs.CL

TL;DR: DHSA是一种动态分层稀疏注意力框架，通过数据驱动方式在线预测注意力稀疏性，无需重新训练。它将序列自适应分割为变长块，计算块级表示，然后上采样到token级重要性分数，在保持精度的同时显著降低计算成本和内存使用。


<details>
  <summary>Details</summary>
Motivation: 现有静态稀疏方法（如滑动窗口或全局token）无法适应注意力模式的内容相关变化，而动态方法依赖预定义模板或启发式机制，限制了通用性并可能剪枝重要token。需要一种更灵活、数据驱动的稀疏注意力方法来解决长上下文建模的可扩展性问题。

Method: DHSA框架：1）自适应将序列分割为变长块；2）使用长度归一化聚合计算块表示（按块大小的平方根缩放平均嵌入）；3）将块级相似性分数上采样到token级相似性，计算重要性分数以确定保留哪些token级交互。

Result: 在Gemma2上的实验表明：DHSA在精度上与密集注意力相当，同时将预填充延迟降低20-60%，峰值内存使用减少35%。相比块稀疏注意力等基线方法，DHSA实现了6-18%的相对精度提升，且成本相当或更低。
...
TL;DR: 本文研究了基于前缀的方法在零样本跨语言迁移中的表现，发现这些方法在Llama 3.1 8B和Mistral v0.3 7B模型上比LoRA基线提升高达6%，在低资源多语言环境中具有显著优势。


<details>
  <summary>Details</summary>
Motivation: 随着Llama和Mistral等大型语言模型的发布，零样本跨语言迁移变得可行，但将这些仅解码器LLM适应跨语言新任务仍具挑战性。前缀方法在仅解码器模型中的零样本迁移潜力尚未充分探索。

Method: 对三种基于前缀的方法（软提示调优、前缀调优和Llama Adapter）进行综合研究，评估从英语到35+种高资源和低资源语言的零样本跨语言迁移，分析语言家族和文字的影响，以及模型规模从1B到24B的扩展效应。

Result: 在Llama 3.1 8B上，前缀方法在Belebele基准上比LoRA基线高出6%，Mistral v0.3 7B也观察到类似改进。前缀调优仅使用1.23M学习参数，但在多样化基准上实现了一致的改进。

Conclusion: 基于前缀的技术是LoRA的有效且可扩展的替代方案，特别是在低资源多语言环境中具有巨大潜力。
...
TL;DR: 本文提出相对缩放定律，研究不同测试分布之间的性能差距如何随模型规模变化，发现缩放并非万能均衡器，不同领域的性能轨迹各异。


<details>
  <summary>Details</summary>
Motivation: 传统的缩放定律在聚合测试集上测量，掩盖了不同子群体间的性能差异。本文旨在揭示缩放过程中不同测试分布间的相对性能变化。

Method: 使用255个解码器Transformer模型，在匹配计算预算（10^18-10^20 FLOPs）下训练，分析MMLU学术领域、英语方言和AI风险行为等不同测试分布的性能轨迹。

Result: 发现多样化的性能轨迹：学术领域趋于平等；英语方言表现取决于人口规模；AI风险行为分裂，能力和影响力相关风险增加而对抗性风险不变。

Conclusion: 缩放虽然提升整体性能，但不是普遍均衡器。发布所有模型检查点以支持进一步研究相对缩放定律与传统缩放定律的对比分析。
...
Result: 在三个新收集的数据集和两个广泛使用的基准测试上，OpenRM显著优于现有的奖励建模方法。将其集成到推理时响应选择和训练时数据选择中，在下游LLM对齐任务中带来一致的收益。

Conclusion: 工具增强的奖励模型具有扩展可靠长文本评估的潜力，能够有效解决现有奖励模型在知识密集型任务中的局限性。
...
TL;DR: 本研究通过眼动追踪分析发现，阅读障碍读者对词汇长度、频率和可预测性三个特征都表现出更强的敏感性，其中可预测性影响最大。通过反事实操作这些特征，阅读障碍与控制组的差距可缩小约三分之一。


<details>
  <summary>Details</summary>
Motivation: 探究在自然阅读环境中，阅读障碍读者的额外阅读成本在什么条件下产生，以及词汇特征如何影响这些成本。

Method: 使用眼动追踪技术，结合词汇层面的特征（词汇长度、频率和可预测性），建模这些特征对阅读障碍读者阅读时间的影响。

Result: 所有三个词汇特征都显著影响典型读者和阅读障碍读者的阅读时间，阅读障碍读者对每个特征都表现出更强的敏感性，特别是对可预测性。通过反事实操作这些特征，阅读障碍与控制组的差距可缩小约三分之一。

Conclusion: 研究结果支持阅读障碍理论中关于语言工作记忆和语音编码需求增加的假设，为干预措施和计算模型提供了可操作的指导，并建议进一步研究词汇复杂性和副中央凹预览效益来解释剩余差距。
...
<details>
  <summary>Details</summary>
Motivation: 随着检索增强生成(RAG)的普及，信息检索的角色从为人类用户检索信息转变为为AI系统检索上下文知识，其中相关性难以预先定义或标注。

Method: 提出R3框架，通过试错反馈强化对比学习，让检索结果与环境交互产生对比信号，自动引导检索器的自我改进。

Result: 在多样化任务上的广泛实验表明，R3比原始检索器提升RAG性能5.2%，比最先进检索器提升4.9%，且与基于后训练或指令调优LLM的LLM增强检索和RAG系统效果相当。

Conclusion: R3既高效又实用，仅需4个GPU且能在一天内完成训练，为RAG系统提供了有效的检索优化解决方案。
...
TL;DR: 本文提出了一种两阶段的机器翻译评估方法MQM重标注，通过让标注者重新审查已有的MQM标注来提升评估质量，发现重标注能发现更多被遗漏的错误。
...
Method: 采用两阶段的MQM评估范式，让标注者重新审查已有的MQM标注（可能来自自己、其他人类标注者或自动标注系统）。

Result: 重标注行为符合预期目标，重标注能产生更高质量的标注，主要原因是发现了初次标注时遗漏的错误。

Conclusion: MQM重标注是一种有效的机器翻译评估改进方法，能够提升评估质量并捕捉更多翻译错误。
...
TL;DR: 该研究评估了基于提示的角色扮演对大型语言模型医学推理能力的影响，发现角色提示主要影响表面语言特征而非核心推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究基于提示的角色扮演是否能在LLMs中引发不同的、角色特定的认知过程，还是仅仅修改语言风格。

Method: 采用RP-Neuron-Activated评估框架，在三个医学问答数据集上使用神经元消融和表示分析技术来评估推理路径的变化。

Result: 结果显示角色提示并未显著增强LLMs的医学推理能力，主要影响表面语言特征，没有证据表明不同临床角色之间存在不同的推理路径或认知差异。

Conclusion: 当前基于提示的角色扮演方法无法复制真实世界医学实践中的认知复杂性，需要开发能够模拟真实认知过程而非语言模仿的模型。
...
TL;DR: SPICE是一个强化学习框架，通过让单个模型扮演挑战者和推理者两个角色，从大型语料库中挖掘文档生成多样化推理任务，实现持续自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有无基础的自对弈方法改进效果有限，需要环境交互来实现持续适应。SPICE旨在通过语料库基础提供丰富的外部信号，实现持续自我改进。

Method: SPICE框架中，单个模型扮演两个角色：挑战者从大型语料库中挖掘文档生成多样化推理任务，推理者解决这些任务。通过对抗动态，挑战者在推理者能力前沿创建自动课程。

Result: SPICE在数学推理基准上提升8.9%，在通用推理基准上提升9.8%，在多个模型系列上实现一致增益。
...
TL;DR: AgentFold是一种新的智能体范式，通过主动上下文管理解决长视野任务中的上下文饱和问题，采用多尺度折叠操作来动态管理历史轨迹，在BrowseComp基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决基于ReAct的智能体在长视野任务中面临的上下文饱和问题，以及固定总结方法可能导致关键细节丢失的挑战，受人类回顾性认知过程启发。

Method: 引入AgentFold范式，将上下文视为动态认知工作空间，通过监督微调学习执行多尺度折叠操作，包括细粒度压缩和深度整合。

Result: AgentFold-30B-A3B在BrowseComp上达到36.2%，在BrowseComp-ZH上达到47.3%，超越了规模大得多的开源模型和领先的专有智能体。

Conclusion: AgentFold通过主动上下文管理有效解决了长视野任务中的上下文饱和问题，证明了简单监督微调即可实现显著性能提升。
...
TL;DR: 本文提出了agent数据协议（ADP），一种轻量级表示语言，用于统一异构格式的智能体训练数据，并通过实验验证了其在多种智能体任务上的有效性。


<details>
  <summary>Details</summary>
Motivation: 大规模智能体监督微调的研究成果相对稀少，主要因为智能体训练数据收集面临独特挑战。作者认为瓶颈不在于缺乏底层数据源，而在于数据分散在异构格式、工具和接口中。

Method: 引入agent数据协议（ADP）作为异构格式智能体数据集与下游统一训练流程之间的中间语言。ADP设计足够表达以捕捉多种任务，同时保持简单易解析和训练。

Result: 将13个现有智能体训练数据集统一为ADP格式，并在多个智能体框架上进行SFT。实验显示平均性能提升约20%，在标准编码、浏览、工具使用和研究基准上达到SOTA或接近SOTA性能。

Conclusion: ADP有助于降低标准化、可扩展和可复现智能体训练的门槛，所有代码和数据已公开发布。
...
TL;DR: 论文提出了ComboBench基准测试，评估大语言模型将语义动作转换为VR设备操作序列的能力，在4款VR游戏的262个场景中测试了7个LLM，发现顶级模型在任务分解方面表现良好，但在程序推理和空间理解方面仍落后于人类。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型是否能够像人类一样基于常识和具身理解，将高级语义动作精确转换为VR设备操作序列，这一能力在VR游戏中至关重要但尚未充分研究。

Method: 创建ComboBench基准测试，包含来自4款流行VR游戏（Half-Life: Alyx, Into the Radius, Moss: Book II, Vivecraft）的262个场景，评估7个LLM（GPT-3.5, GPT-4, GPT-4o, Gemini-1.5-Pro, LLaMA-3-8B, Mixtral-8x7B, GLM-4-Flash）的表现，并与标注的真实数据和人类表现进行比较。

Result: Gemini-1.5-Pro等顶级模型在任务分解方面表现强劲，但在程序推理和空间理解方面仍落后于人类；不同游戏间性能差异显著，表明对交互复杂性敏感；少量示例能显著提升性能。

Conclusion: 大语言模型在VR设备操作序列生成方面具有潜力，但需要改进程序推理和空间理解能力；针对性的增强训练可以提升LLM在VR操作方面的能力。
...
TL;DR: 本文介绍了为WMT25翻译评估共享任务提交的两个系统：MetricX-25用于质量分数预测，GemSpanEval用于错误跨度检测，均基于Gemma 3模型进行微调。
...
Conclusion: 基于Gemma 3的微调方法在翻译质量评估任务中表现优异，编码器架构适合质量分数预测，解码器架构适合错误跨度检测任务。
...
TL;DR: Game-TARS是一个通用游戏智能体，使用统一、可扩展的动作空间进行训练，基于人类对齐的键盘鼠标输入。通过大规模持续预训练在异构领域（操作系统、网页、模拟游戏）上，实现了在多种游戏任务中的优异表现。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够跨异构领域（操作系统、网页、模拟游戏）工作的通用游戏智能体，通过统一、可扩展的动作空间实现大规模持续预训练，克服API或GUI方法的局限性。

Method: 采用基于人类对齐键盘鼠标输入的统一动作空间；使用超过500B token的多样化轨迹和多模态数据进行预训练；关键技术创新包括衰减持续损失减少因果混淆，以及高效的稀疏思维策略平衡推理深度和推理成本。

Result: 在开放世界Minecraft任务中成功率比之前最优模型提高约2倍；在未见过的网页3D游戏中接近新手的通用性；在FPS基准测试中超越GPT-5、Gemini-2.5-Pro和Claude-4-Sonnet；训练时间和测试时间的扩展结果证实统一动作空间在跨游戏和多模态数据扩展时持续改进。
...
TL;DR: 本文探讨AI在科学问题解决中的作用，重点关注其对学科创造力的影响。通过区分创造性方法和创造性产品，提出学科创造力的概念，并通过数学案例表明AI可能取代而非扩展学科创造力。


<details>
  <summary>Details</summary>
Motivation: 研究AI在科学问题解决中的角色，特别是其对学科创造力的潜在影响，探讨AI可能如何改变科学追求的价值。
...
Conclusion: AI在科学问题解决中具有双重作用：既能扩展学科创造力，也可能通过某些方法取代它，这对科学追求的价值产生潜在影响。
...
TL;DR: 本文提出了多环境POMDPs（ME-POMDPs）的概念，扩展了标准POMDPs以处理离散模型不确定性。作者将ME-POMDPs推广为具有初始信念集合的对抗性信念POMDPs（AB-POMDPs），并开发了精确和近似算法来计算鲁棒策略。


<details>
  <summary>Details</summary>
Motivation: 当多个领域专家对问题建模存在分歧时，需要一种能够处理模型不确定性的框架。ME-POMDPs旨在找到对集合中任何POMDP都鲁棒的单一策略，最大化最坏情况下的奖励。

Method: 首先将ME-POMDPs推广为AB-POMDPs，然后证明任何ME-POMDP都可以简化为仅在其转移和奖励函数或仅在其观测和奖励函数上变化的ME-POMDP。开发了精确和近似（基于点）的算法来计算AB-POMDPs的鲁棒策略。

Result: 作者展示了能够在标准POMDP基准测试的多环境设置扩展中计算策略，验证了所提出方法的有效性。

Conclusion: ME-POMDPs和AB-POMDPs为处理模型不确定性提供了有效的框架，所开发的算法能够计算对模型变化具有鲁棒性的策略。
...
<details>
  <summary>Details</summary>
Motivation: 当前串联质谱分析方法依赖数据库匹配或需要中间片段预测的多步骤流程，难以识别参考数据库中不存在的化合物，需要一种能够直接从质谱数据生成分子结构的端到端方法。

Method: 利用测试时调优增强预训练transformer模型，直接从串联质谱和分子式进行端到端分子结构生成，绕过人工注释和中间步骤。

Result: 在NPLIB1和MassSpecGym两个基准测试上分别超越DiffMS方法100%和20%，测试时调优在实验质谱上的相对性能提升达62%。

Conclusion: 该方法能够动态适应新质谱数据，即使预测与真实值有偏差，生成的分子候选结构仍保持结构准确性，为人工解释和更可靠的识别提供有价值指导。
...
TL;DR: 研究探讨生成式AI在象棋谜题领域的创造力，开发了一个能生成具有美学吸引力、新颖性、反直觉和独特解法的象棋谜题AI系统，并邀请三位世界级专家进行评估。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，需要验证其是否能产生真正具有创造性和新颖性的输出，特别是在象棋谜题这种需要高度创造力的领域。

Method: 开发了一个专门用于生成象棋谜题的AI系统，该系统能产生具有美学吸引力、新颖性、反直觉和独特解法的谜题，并邀请三位世界级象棋专家对生成的谜题进行评价和选择。

Result: 三位世界级象棋专家（国际象棋排局大师Amatzia Avni、特级大师Jonathan Levitt和Matthew Sadler）对AI生成的谜题进行了评估，选择了他们最喜欢的谜题并解释了其吸引力所在。

Conclusion: 研究表明AI系统能够生成具有创造力、挑战性和美学设计的象棋谜题，得到了领域专家的认可，证明了生成式AI在创造性任务中的潜力。
...
TL;DR: 病理学基础模型在癌症诊断和预后方面表现不佳，存在准确性低、鲁棒性差、计算需求大等根本性弱点，源于通用AI基础建模假设与人体组织内在复杂性之间的概念不匹配。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在计算机视觉和语言处理领域取得了革命性突破，但在计算病理学中的快速应用并未带来预期的突破，反而暴露了根本性弱点，需要深入分析这些不足的原因。

Method: 通过系统评估识别病理学基础模型的七个相互关联的根本问题：生物复杂性、无效的自监督、过度泛化、过度架构复杂性、缺乏领域特定创新、数据不足以及与组织切片大小相关的根本设计缺陷。

Result: 研究发现当前病理学基础模型在诊断准确性、鲁棒性、几何稳定性、计算效率和安全漏洞方面存在严重问题，无法有效处理人体组织的形态学复杂性。

Conclusion: 当前病理学基础模型在概念上与组织形态学本质存在错位，需要对整个范式进行根本性的重新思考，而不是简单套用通用AI基础建模方法。
...
TL;DR: ReCAP是一个用于大型语言模型的递归上下文感知推理和规划框架，通过计划提前分解、结构化重新注入父计划和内存高效执行等机制，显著提升了长时程任务的推理和规划能力。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在多步推理和动态重新规划任务中面临的上下文漂移、目标信息丢失和循环失败等问题，同时避免分层提示方法导致的跨级连续性减弱或运行时开销过大。

Method: 结合三种关键机制：计划提前分解（生成完整子任务列表，执行第一项并优化剩余）、结构化重新注入父计划（在递归返回时保持多级上下文一致性）、内存高效执行（限制活动提示，使成本随任务深度线性扩展）。

Result: 在多个长时程推理基准测试中显著改善了子目标对齐和成功率，在严格pass@1协议下，同步Robotouille提升32%，异步Robotouille提升29%。

Conclusion: ReCAP框架通过将高层目标与低层动作对齐、减少冗余提示以及保持跨递归的连贯上下文更新，有效提升了大型语言模型在复杂长时程任务中的推理和规划能力。
...
TL;DR: 该论文研究了多智能体路径规划中的去中心化目标分配问题，比较了贪婪启发式、最优分配和基于大语言模型的方法，发现LLM智能体在适当提示下能实现接近最优的性能。


<details>
  <summary>Details</summary>
Motivation: 解决去中心化条件下多智能体在共享环境中的协调挑战，特别是在目标分配问题中探索语言模型的潜力。

Method: 智能体基于环境结构化表示独立生成目标偏好排序，然后交换排序信息，通过固定冲突解决规则确定分配，不进行协商或迭代协调。系统比较了贪婪启发式、最优分配和LLM智能体在完全可观测网格世界中的表现。

Result: LLM智能体在精心设计的提示和定量信息支持下，能够实现接近最优的完工时间，并持续优于传统启发式方法。
...
TL;DR: 本文提出了一种基于强化学习的框架，通过设计基于国际象棋引擎搜索统计的新型奖励机制，显著提高了反直觉棋谜的生成质量，生成的反直觉棋谜比例从0.22%提升到2.5%，超过了现有数据集和最佳模型。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在多个领域快速发展，但在生成真正具有创造性、美学价值和反直觉的输出方面仍面临挑战。本文旨在解决国际象棋棋谜生成中的这些困难。

Method: 首先对生成式AI架构进行基准测试，然后引入基于国际象棋引擎搜索统计的强化学习框架，设计奖励机制来增强棋谜的独特性、反直觉性、多样性和真实性。

Result: 强化学习方法将反直觉棋谜生成比例从0.22%大幅提升至2.5%，超过了现有数据集(2.1%)和最佳Lichess训练模型(0.4%)。生成的棋谜满足新颖性和多样性标准，保留了美学主题，并被人类专家评为比传统棋谜更具创造性、趣味性和反直觉性，甚至接近经典作品水平。

Conclusion: 最终成果是一本经过精心策划的AI生成棋谜集，其创造性得到了三位世界知名专家的认可，证明了该方法在生成高质量创意内容方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索数字孪生在动态系统建模和控制中的集成应用，比较物理基础、数据驱动和混合方法与传统及AI驱动控制器的效果。

Method: 使用微型温室作为测试平台，开发了四种预测模型（线性、物理基础建模PBM、长短期记忆LSTM、混合分析与建模HAM）和三种控制策略（模型预测控制MPC、强化学习RL、基于大语言模型LLM的控制），在插值和外推场景下进行比较评估。

Result: 结果显示：在建模方面，HAM在准确性、泛化性和计算效率方面表现最均衡，LSTM精度高但资源成本更大；在控制方面，MPC提供稳健可预测性能，RL展示强适应性，LLM控制器与预测工具结合时提供灵活的人机交互。

Conclusion: HAM模型在建模中提供最佳平衡性能，MPC控制器在控制中表现稳健，RL具有强适应性，LLM控制器在结合预测工具时支持灵活的人机交互，为数字孪生系统的设计和选择提供了实用指导。
...
### [132] [Latent Chain-of-Thought for Visual Reasoning](https://arxiv.org/abs/2510.23925)
*Guohao Sun,Hang Hua,Jian Wang,Jiebo Luo,Sohail Dianat,Majid Rabbani,Raghuveer Rao,Zhiqiang Tao*

Main category: cs.AI

TL;DR: 本文提出了一种基于变分推理的思维链训练方法，通过稀疏奖励函数和贝叶斯推理扩展策略，提升大型视觉语言模型在推理任务中的性能、泛化能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有思维链训练方法（如SFT、PPO、GRPO）在未见推理任务上泛化能力不足，且过度依赖有偏见的奖励模型，需要更有效的训练算法。

Method: 将LVLMs中的推理重新表述为后验推理，提出基于摊销变分推理的可扩展训练算法，使用多样性强化学习算法设计稀疏奖励函数，并采用贝叶斯推理扩展策略替代昂贵的搜索方法。

Result: 在七个推理基准测试中，所提方法在有效性、泛化性和可解释性方面均优于现有最先进的LVLMs。

Conclusion: 基于变分推理的思维链训练方法能够有效提升大型视觉语言模型的推理能力，克服确定性采样限制和奖励黑客问题，实现更好的泛化性能。
...
### [133] [Decentralized Causal Discovery using Judo Calculus](https://arxiv.org/abs/2510.23942)
*Sridhar Mahadevan*

Main category: cs.AI

TL;DR: 本文提出了一种基于直觉主义分散化框架的因果发现理论——柔道演算，使用j-稳定因果推断和j-do-演算在层拓扑中形式化定义，能够处理现实应用中因果效应随环境变化的问题。


<details>
  <summary>Details</summary>
Motivation: 在生物学、医学和社会科学等实际应用中，因果效应往往依赖于具体环境（如年龄、国家、剂量、基因型或实验协议），需要一种能够形式化处理这种环境依赖性的因果发现方法。

Method: 使用柔道演算，通过Lawvere-Tierney模态算子j选择相关环境，j-稳定性确保因果主张在相关环境族中构造性和一致性地成立。结合基于分数、约束和梯度的标准因果发现方法，实现算法和实现框架。

Result: 在合成和真实世界数据集上的实验结果表明，基于层理论的分散化因果发现具有计算效率优势，并且性能优于经典因果发现方法。

Conclusion: 柔道演算提供了一种有效的框架来处理因果效应的环境依赖性，通过分散化方法提高了计算效率并改善了性能。
...
### [134] [The Sign Estimator: LLM Alignment in the Face of Choice Heterogeneity](https://arxiv.org/abs/2510.23965)
*Aymane El Gadarri,Ali Aouad,Vivek F. Farias*

Main category: cs.AI

TL;DR: 提出了一种名为符号估计器的新方法，通过用二元分类损失替换交叉熵损失，解决了传统LLM对齐方法在人类偏好异质性下的不一致性问题，实现了可证明的一致性和高效估计。


<details>
  <summary>Details</summary>
Motivation: 传统LLM对齐方法对人类偏好的异质性很脆弱，拟合简单的概率模型到成对比较数据会产生对群体平均效用的不一致估计，这是社会福利的规范度量。
...
### [135] [Learning Individual Movement Shifts After Urban Disruptions with Social Infrastructure Reliance](https://arxiv.org/abs/2510.23989)
...
<details>
  <summary>Details</summary>
Motivation: 预测破坏性事件前个体移动模式的变化具有挑战性，因为缺乏衡量个体异质性社会基础设施韧性的指标，传统特征有限且难以大规模获取，个体移动模式与空间上下文的复杂交互未被充分捕捉，以及个体级移动数据稀疏且不适合传统预测方法。

Method: 开发了一个条件深度学习模型，将个体的社会基础设施韧性(SIR)整合到模型中，利用大规模稀疏的个体级数据捕捉个体移动模式与局部空间上下文之间的复杂关系。

Result: 实验表明，整合个体的SIR和空间上下文可以增强模型预测事件后个体移动模式的能力。条件模型能够捕捉到具有相似事件前移动模式但SIR不同的个体之间移动模式的差异变化。

Conclusion: 通过将个体的社会基础设施韧性纳入条件深度学习框架，可以更有效地预测破坏性事件后个体移动模式的变化，为社区资源需求规划提供重要见解。
...
### [136] [OneCast: Structured Decomposition and Modular Generation for Cross-Domain Time Series Forecasting](https://arxiv.org/abs/2510.24028)
*Tingyue Pan,Mingyue Cheng,Shilong Zhang,Zhiding Liu,Xiaoyu Tao,Yucong Luo,Jintao Zhang,Qi Liu*

Main category: cs.AI

TL;DR: OneCast是一个跨域时间序列预测框架，通过将时间序列分解为季节性和趋势组件，分别使用轻量级投影模块和语义感知分词器进行建模，在八个领域实验中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决跨域时间序列预测中由于领域特定趋势变化和不一致周期性模式导致的泛化能力不足问题，现有方法将时间序列视为未分化的序列而未显式解耦其固有结构组件。

Method: 提出OneCast框架：1）将时间序列分解为季节性和趋势组件；2）季节性组件通过轻量级投影模块使用可解释基函数重建周期性模式；3）趋势组件通过语义感知分词器编码为段级离散标记，使用掩码离散扩散机制进行推断；4）两分支输出结合生成最终预测。
...
TL;DR: 本研究比较了经典模型和机器学习模型在电动汽车跟驰行为建模中的表现，发现随机森林回归器在所有场景下都优于经典物理模型。
...
Method: 使用经典模型（IDM、OVM、OVRV、简化CACC）和随机森林回归器，基于真实世界电动汽车跟随内燃机车辆的数据集进行建模和参数校准。
...
TL;DR: HistoLens是一个透明的AI助手，让病理学家能用自然语言提问组织切片问题，提供结构化报告和可视化证据，保持医生主导地位。
...
Method: 开发HistoLens系统，将自然语言问题转换为精确AI查询，提供结构化报告和热图可视化证据，并训练AI专注于患者组织而忽略背景噪声。

Result: 创建了一个透明协作的工作流程，病理学家保持专家主导地位，使用可信AI助手验证见解并做出更快、更自信的诊断。

Conclusion: 通过提供透明推理和可视化证据，HistoLens成功建立了医生对AI的信任，实现了人机协作的病理诊断工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 手动事件管理在面临海量异构可观测数据时劳动密集且容易出错，现有自动化方法难以跨系统泛化、可解释性有限且部署成本高，阻碍了实际应用。

Method: 采用免训练数据处理器处理异构可观测数据，多智能体协作框架进行诊断推理，并引入双自演进机制集成内部模型更新和外部经验积累。

Result: 在OPENRCA基准测试中表现出最先进的性能，证明其具有可泛化性、可解释性、成本效益和自演进能力。
...
### [140] [BLM$_1$: A Boundless Large Model for Cross-Space, Cross-Task, and Cross-Embodiment Learning](https://arxiv.org/abs/2510.24161)
...
TL;DR: 本文提出了BLM₁模型，这是一个多模态空间基础模型，能够在数字和物理空间之间无缝操作，并实现跨具身化和任务的泛化。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在数字-物理空间和不同具身化之间的泛化能力差，视觉-语言-动作模型缺乏鲁棒的高层具身推理能力，而大多数具身大语言模型仅限于数字空间。因此，需要统一的模型来跨越数字和物理空间，同时泛化到不同的具身化和任务。

Method: BLM₁通过两阶段训练范式实现：第一阶段通过精选的数字语料库向MLLM注入具身知识，同时保持语言能力；第二阶段通过意图桥接接口训练策略模块，从MLLM中提取高层语义来指导控制，而无需微调MLLM主干。该方法得到了一个自收集的跨具身化演示套件的支持，涵盖四种机器人具身化和六个逐步挑战性的任务。

Result: 在数字和物理基准测试中，单个BLM₁实例在数字任务中实现了约6%的性能提升，在物理任务中实现了约3%的性能提升，优于MLLMs、ELLMs、VLAs和GMLMs四个模型家族。

Conclusion: BLM₁模型成功实现了跨空间转移、跨任务学习和跨具身化泛化，为数字和物理空间中的统一多模态推理和控制提供了有效的解决方案。
...
### [141] [Investigating Intra-Abstraction Policies For Non-exact Abstraction Algorithms](https://arxiv.org/abs/2510.24297)
*Robin Schmöcker,Alexander Dockhorn,Bodo Rosenhahn*

Main category: cs.AI

TL;DR: 本文针对MCTS中抽象技术的使用提出了改进，特别是当同一抽象节点包含多个动作时，随机平局决胜规则的不足。作者提出并评估了多种替代的抽象内策略，其中多个策略在大多数环境和参数设置下优于随机策略。


<details>
  <summary>Details</summary>
Motivation: MCTS的样本效率问题可以通过状态和动作抽象来解决，但现有方法如pruned OGA在处理同一抽象节点包含多个动作时，采用随机平局决胜规则，这可能导致性能不佳。

Method: 提出并评估了多种替代的抽象内策略，以改进当多个动作属于同一抽象节点时的选择机制。

Result: 实验表明，提出的多个抽象内策略在大多数环境和参数设置下都优于随机策略。

Conclusion: 通过改进抽象内策略，可以显著提升MCTS在抽象环境中的性能，为抽象技术的应用提供了更好的解决方案。
...
### [142] [Verifying Large Language Models' Reasoning Paths via Correlation Matrix Rank](https://arxiv.org/abs/2510.24299)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然推理能力强，但容易产生错误和幻觉。现有验证方法依赖外部资源，计算开销大且适用范围有限。

Method: 发现输入问题与输出推理路径之间相关矩阵的秩是推理正确性的稳健指标，基于此设计了一个简单即插即用的Self-Indicator方法来重新加权候选推理路径。

Result: 该方法在区分正确与错误推理路径上达到超过75%的准确率，并在三个推理基准上将准确率提高了8%以上。

Conclusion: LLM的内部行为确实暗示了其推理路径的可信度，Self-Indicator方法能有效验证模型输出且计算开销极小。
...
### [143] [Retrieval and Argumentation Enhanced Multi-Agent LLMs for Judgmental Forecasting](https://arxiv.org/abs/2510.24303)
*Deniz Gorur,Antoni Rago,Francesca Toni*

Main category: cs.AI

TL;DR: 本文提出了一种用于声明验证的多智能体框架，通过不同智能体对声明真实性产生分歧并提供正反证据，构建定量双极论证框架(QBAFs)，结合多种基于大语言模型的智能体方法，实验表明多智能体组合能提高预测准确性并提供可解释的证据组合。
...
Method: 提出多智能体声明验证框架，使用三种基于LLM的智能体：ArgLLM(生成和评估QBAFs)、RbAM(基于关系从外部源挖掘论证)、RAG-ArgLLM(结合检索增强生成和ArgLLM)。在标准判断性预测数据集上实验，比较不同LLM和智能体数量的效果。

Result: 实验结果显示，组合多个智能体的证据能够提高预测准确性，特别是在三个智能体的情况下效果更佳，同时提供了可解释的证据组合过程。

Conclusion: 多智能体框架通过整合不同视角的证据，有效提升了声明验证的准确性和可解释性，为判断性预测任务提供了新的解决方案。
...
### [144] [Generative Large Language Models (gLLMs) in Content Analysis: A Practical Guide for Communication Research](https://arxiv.org/abs/2510.24337)
*Daria Kravets-Meinke,Hannah Schmid-Petri,Sonja Niemann,Ute Schmid*

Main category: cs.AI

TL;DR: 本文探讨了生成式大语言模型在传播研究内容分析中的应用，提出了应对七个关键挑战的最佳实践指南，旨在使基于gLLM的内容分析更易用并符合学科质量标准。
...
Method: 综合新兴研究，提出全面的最佳实践指南，涵盖代码本开发、提示工程、模型选择、参数调优、迭代优化、模型可靠性验证和性能增强七个方面。

Result: 通过系统梳理gLLM辅助定量内容分析的研究现状，为传播研究者提供了应对关键挑战的实用框架。

Conclusion: 该指南旨在使基于gLLM的内容分析对更广泛的传播研究者更加易用，并确保符合有效性、可靠性、可重复性和研究伦理等既定的学科质量标准。
...
### [145] [VDSAgents: A PCS-Guided Multi-Agent System for Veridical Data Science Automation](https://arxiv.org/abs/2510.24339)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的数据科学系统仅依赖LLM内部推理，缺乏科学和理论原则指导，限制了其在处理噪声和复杂现实数据集时的可信度和鲁棒性。
...
### [146] [An N-of-1 Artificial Intelligence Ecosystem for Precision Medicine](https://arxiv.org/abs/2510.24359)
*Pedram Fard,Alaleh Azhir,Neguine Rezaii,Jiazi Tian,Hossein Estiri*

Main category: cs.AI

TL;DR: 提出了一种多智能体生态系统用于N-of-1决策支持，旨在解决传统医疗AI系统对边缘患者（罕见变异、多病症、代表性不足人群）表现不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 传统医疗AI系统通过最小化大数据集上的误差来提供强大的总体准确性，但在边缘患者（罕见变异、多病症、代表性不足人群）上表现不佳，这种平均患者谬误侵蚀了公平性和信任。

Method: 设计了一个多智能体生态系统，智能体按器官系统、患者群体和分析模式进行聚类，共享模型库和证据合成工具。结果在协调层中汇聚，权衡可靠性、不确定性和数据密度，然后向临床医生提供决策支持包。

Result: 验证从群体平均值转向个体可靠性，通过低密度区域的误差、小规模校准和风险-覆盖权衡来衡量。
...
### [147] [Improving LLM Reasoning via Dependency-Aware Query Decomposition and Logic-Parallel Content Expansion](https://arxiv.org/abs/2510.24390)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM推理方法在计算效率低下的顺序生成和僵化推理策略的限制下，难以同时满足现代Web平台对高质量复杂推理和低延迟高吞吐量的双重需求。

Method: Orion将单个查询推理分解为两个协同阶段：关键点生成（通过检索增强的少样本提示提取逻辑结构化的关键点）和内容并行扩展（基于依赖图并发扩展这些点以确保逻辑一致性），并引入跨查询的流水线调度机制。

Result: 实验显示，Orion相比基线方法实现了最高4.33倍的token生成速度和3.42倍的延迟降低，同时通过显式建模点间依赖关系将推理质量提升了18.75%。

Conclusion: Orion框架成功解决了LLM在实时Web应用中的效率与质量平衡问题，通过创新的两阶段分解和并行化策略显著提升了推理性能。
...
### [148] [APTBench: Benchmarking Agentic Potential of Base LLMs During Pre-Training](https://arxiv.org/abs/2510.24397)
*Jiarui Qin,Yunjia Xi,Junjie Huang,Renting Rui,Di Yin,Weiwen Liu,Yong Yu,Weinan Zhang,Xing Sun*

Main category: cs.AI

TL;DR: APTBench是一个为预训练阶段设计的基准测试框架，将真实世界智能体任务转换为适合基础模型的多选题或文本补全问题，用于评估模型在预训练阶段的智能体潜力。
...
### [149] [Human-Level Reasoning: A Comparative Study of Large Language Models on Logical and Abstract Reasoning](https://arxiv.org/abs/2510.24435)
*Benjamin Grando Moreira*

Main category: cs.AI

TL;DR: 本研究比较了GPT、Claude、DeepSeek、Gemini、Grok、Llama、Mistral、Perplexity和Sabi'a等多个大型语言模型的逻辑和抽象推理能力，使用8个定制推理问题，并与人类表现进行基准测试。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型的推理能力对于推进人工智能发展至关重要，因为这超越了单纯的语言任务表现，涉及理解这些模型是否真正理解信息、进行推理以及以逻辑有效的方式得出结论。

Method: 使用8个定制设计的推理问题，比较多个LLM的逻辑和抽象推理技能，并将LLM结果与人类在相同任务上的表现进行基准测试。

Result: 研究揭示了显著差异，表明LLM在演绎推理方面存在困难。

Conclusion: 大型语言模型在推理能力方面与人类存在明显差距，特别是在演绎推理方面表现不佳，这为模型改进提供了重要方向。
...
### [150] [Law in Silico: Simulating Legal Society with LLM-Based Agents](https://arxiv.org/abs/2510.24442)
*Yiding Wang,Yuxuan Chen,Fanxu Meng,Xifan Chen,Xiaolei Yang,Muhan Zhang*

Main category: cs.AI

TL;DR: 本文提出了Law in Silico框架，利用大语言模型模拟法律社会，通过实验验证了该框架能够复现宏观犯罪趋势，并揭示透明适应性法律系统对弱势群体权利的保护作用。


<details>
  <summary>Details</summary>
Motivation: 由于现实法律实验成本高或不可行，需要一种有效的替代方法来验证和发展法律理论。大语言模型具备世界知识和角色扮演能力，适合作为法律社会模拟的基础。

Method: 开发了Law in Silico框架，基于LLM的智能体模拟法律场景，包含个体决策和立法、裁决、执法等制度机制。

Result: 实验表明LLM智能体能够很大程度上复现宏观犯罪趋势，微观模拟显示功能良好、透明且适应性强的法律系统能更好地保护弱势个体权利。

Conclusion: LLM为基础的法律社会模拟是可行的，能够提供与现实观察一致的见解，并为法律系统优化提供参考。
...
### [151] [From Cross-Task Examples to In-Task Prompts: A Graph-Based Pseudo-Labeling Framework for In-context Learning](https://arxiv.org/abs/2510.24528)
*Zihan Chen,Song Wang,Xingbo Fu,Chengshuai Shi,Zhenyu Lei,Cong Shen,Jundong Li*

Main category: cs.AI

TL;DR: 提出了一种成本效益高的两阶段流程，通过跨任务示例和基于图的标签传播方法，减少对LLM数据标注的依赖，构建用于上下文学习的演示数据。
...
Conclusion: 该流水线结合了跨任务监督的灵活性和无LLM传播的可扩展性，为上下文学习提供了有效的解决方案。
...
### [152] [FunReason-MT Technical Report: Overcoming the Complexity Barrier in Multi-Turn Function Calling](https://arxiv.org/abs/2510.24645)
...
Method: 采用环境-API图交互收集多样化高质量轨迹、高级工具查询合成简化困难查询构建、引导迭代链生成复杂思维链，解决多轮函数调用数据的结构缺陷。

Result: 基于FunReason-MT生成数据构建的4B模型在BFCLv3基准上取得了同类尺寸模型中最先进的性能，超越了大多数闭源模型，在BFCLv4上的进一步性能改进证实了其可靠性。

Conclusion: FunReason-MT为智能体学习提供了可靠且鲁棒的数据源，能够有效解决真实世界多轮工具使用的数据合成挑战。
...
### [153] [Advancing site-specific disease and pest management in precision agriculture: From reasoning-driven foundation models to adaptive, feedback-based learning](https://arxiv.org/abs/2510.24650)
*Nitin Rai,Daeun,Choi,Nathan S. Boyd,Arnold W. Schumann*

Main category: cs.AI

TL;DR: 本文综述了基于基础模型的作物定点病害管理研究进展，重点分析了视觉语言模型和大型语言模型在自适应学习、强化学习和数字孪生框架中的应用现状与发展趋势。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习和深度学习在计算机视觉领域的快速发展，作物定点病害管理从手工特征提取演进到大规模自动化特征学习。基础模型的出现为处理作物病害数据集提供了全新方法，能够整合视觉和文本数据，解释症状文本，推理症状与管理的关系，并为种植者和教育者提供交互式问答支持。

Method: 本文筛选了约40篇关于基础模型在定点病害管理中应用的文献，重点分析大型语言模型和视觉语言模型，并讨论它们在自适应学习、强化学习和数字孪生框架中的作用。

Result: 主要发现包括：基础模型在2023-24年文献激增；视觉语言模型发展速度远超大型语言模型；强化学习和自适应学习在智能喷洒领域仍处于起步阶段；基于强化学习的数字孪生可以虚拟模拟定向喷洒；解决仿真到现实的差距对实际部署至关重要；人机协作仍然有限；多模态基础模型与实时反馈将推动下一代定点病害管理。

Conclusion: 基础模型正在改变作物定点病害管理的方式，特别是在整合视觉和语言信息方面展现出巨大潜力。未来需要重点关注仿真到现实的迁移、人机协作的深化以及多模态实时反馈系统的开发，以实现更有效的田间病害管理。
...
### [154] [OrchDAG: Complex Tool Orchestration in Multi-Turn Interactions with Plan DAGs](https://arxiv.org/abs/2510.24663)
...
Method: 引入OrchDAG合成数据生成管道，将工具执行建模为有向无环图，并提出基于图的奖励结合GRPO风格算法进行训练。
...
### [155] [Bridging Tool Dependencies and Domain Knowledge: A Graph-Based Framework for In-Context Planning](https://arxiv.org/abs/2510.24690)
...
<details>
  <summary>Details</summary>
Motivation: 为了揭示和利用工具与文档之间的依赖关系，从而提升示例工件的生成质量。

Method: 从工具模式构建工具知识图谱，同时从内部文档和标准操作程序构建补充知识图谱，然后将两者融合，采用深度-稀疏集成策略对齐结构工具依赖与程序知识。
