...
TL;DR: 提出了一种新颖的语义3D运动转移方法，能够实现无绑定、跨类别的运动转移，通过运动嵌入和动态3D高斯泼溅重建技术，在保持结构一致性的同时实现高质量运动转移。


<details>
  <summary>Details</summary>
Motivation: 为了解决多视角视频中语义3D运动转移的挑战，特别是实现跨类别对象间的语义对应运动转移，同时避免复杂的绑定过程。

Method: 基于隐式运动转移技术，通过条件反演从源视频提取运动嵌入，将其应用于静态目标形状的渲染帧，并使用生成的视频监督动态3D高斯泼溅重建。引入了基于锚点的视角感知运动嵌入机制和鲁棒的4D重建流程。

Result: 建立了首个语义3D运动转移基准测试，相比适应基线方法展现出更优的运动保真度和结构一致性。

Conclusion: 该方法在语义3D运动转移任务中表现出色，为跨类别对象运动转移提供了有效的解决方案，并发布了代码和数据供进一步研究使用。
...
TL;DR: 提出了InstructMix2Mix框架，通过将2D扩散模型的编辑能力蒸馏到预训练的多视图扩散模型中，实现了稀疏输入视图下的多视图图像编辑，在保持单帧编辑质量的同时显著提升了跨视图一致性。


<details>
  <summary>Details</summary>
Motivation: 解决从稀疏输入视图进行多视图图像编辑的任务，现有基于逐场景神经场或时间注意力机制的方法在此设置下往往产生伪影和不一致的编辑结果。

Method: 提出InstructMix2Mix框架，用多视图扩散学生替代SDS中的传统神经场整合器，采用跨时间步的增量学生更新、专门的教师噪声调度器防止退化，以及无需额外成本的注意力修改来增强跨视图一致性。

Result: 实验表明I-Mix2Mix在保持高单帧编辑质量的同时，显著改善了多视图一致性。

Conclusion: I-Mix2Mix通过将2D扩散编辑能力蒸馏到多视图扩散模型中，有效解决了稀疏视图下的多视图编辑问题，实现了更好的跨视图一致性。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在临床推理中的三个主要限制：数据异质性、缺乏可靠的诊断推理依据，以及有限的可扩展性和泛化能力。

Method: 1) 设计基于教科书的推理生成器合成高质量诊断轨迹；2) 使用监督微调赋予模型基础推理能力；3) 开发结合疾病层次结构的强化学习范式，将推理模式扩展到大规模稀疏数据。

Result: 在多个皮肤病数据集上的广泛实验表明，SkinR1实现了优越的诊断准确性。消融研究证明了监督微调建立的推理基础的重要性。

Conclusion: SkinR1通过统一的端到端框架有效解决了皮肤病诊断中的关键挑战，结合了深度推理和广泛泛化能力，为临床可信赖的视觉语言模型提供了有前景的解决方案。
...
TL;DR: 该论文提出了FarSLIP框架，通过构建多粒度遥感图像-文本数据集MGRS-200k，采用patch-to-patch蒸馏和CLS token区域类别对齐方法，解决了CLIP在遥感领域细粒度对齐的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有遥感专用CLIP变体仍继承有限的空间感知能力，主要由于当前数据集生成全局标注而未能充分利用对象级监督，以及直接应用区域-文本对齐方法会导致性能下降。
...
Conclusion: FarSLIP通过改进的细粒度视觉-语言对齐方法，显著提升了遥感领域的空间感知能力，并在多个任务上取得了优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前病理学基础模型能够从全切片图像中提取丰富的补丁级特征，但将这些特征聚合为切片级预测的方法存在设计限制，影响了泛化性和可靠性。

Method: nnMIL引入补丁级和特征级的随机采样，实现大批量优化、任务感知采样策略，以及跨数据集和模型架构的高效可扩展训练。轻量级聚合器执行滑动窗口推断生成集成预测，并支持原则性不确定性估计。

Result: 在35个临床任务和四个病理基础模型上，nnMIL在疾病诊断、组织学分型、分子生物标志物检测和泛癌预后预测方面持续优于现有MIL方法，展示了强大的跨模型泛化能力、可靠的不确定性量化和稳健的生存分层。
...
TL;DR: X-WIN是一个新颖的CXR世界模型，通过从胸部CT学习预测其2D投影来提取体积知识，解决了CXR结构叠加限制3D解剖结构捕获的问题。


<details>
  <summary>Details</summary>
Motivation: CXR作为2D投影图像，受限于结构叠加，无法捕获3D解剖结构，这给表示学习和疾病诊断带来了挑战。
...
Conclusion: X-WIN通过内部化3D解剖结构知识，能够预测3D空间中的各种变换下的CXR，有效提升了CXR表示学习和疾病诊断能力。
...
TL;DR: CPSL是一种紧凑的2.5D视频表示方法，通过深度和内容显著性引导将2D内容分解为几何一致的层，实现视差校正的新视角合成，避免了昂贵的3D重建。
...
Method: 使用每帧深度和内容显著性指导，将帧分解为具有软alpha带和边缘深度缓存的几何一致层，通过深度加权变形和前向后alpha合成实现新视角合成，并采用运动引导传播和逐层编码保持时间一致性。

Result: 在多个基准测试中，CPSL相比基于层和神经场的基线方法实现了更优的感知质量和边界保真度，同时将存储和渲染成本降低数倍。

Conclusion: CPSL提供了一条从2D视频到可扩展2.5D沉浸式媒体的实用路径。
...
TL;DR: 该论文提出了首个包含580个多模态人类活动序列的长周期工作流基准，支持无监督周期工作流检测、任务完成跟踪和过程异常检测三个任务，并提出了一个轻量级、无需训练的基线方法。


<details>
  <summary>Details</summary>
Motivation: 填补长周期工作流研究的空白，传统方法主要关注短周期活动，而长周期工作流具有低对比度模式，在制造、体育和日常生活中普遍存在但研究不足。

Method: 提出了一个轻量级、无需训练的基线方法，用于建模多样化的周期工作流模式，支持多模态人类活动序列分析。

Result: 实验表明：该基准对现有方法构成显著挑战；提出的基线方法在所有评估任务中大幅优于竞争方法；在真实应用中，基线方法达到与传统监督方法相当的部署优势，无需标注和重新训练。

Conclusion: 该研究为长周期工作流分析提供了首个基准和有效的基线方法，在多个实际应用场景中展现出优越性能，为相关领域研究奠定了基础。
...
TL;DR: 提出一种低成本、通用的多视角视频同步方法，使用LED时钟编码时间信息，实现毫秒级时间对齐，支持RGB和红外相机，在手术等实际应用中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 在异构相机系统（专业与消费级设备、可见光与红外传感器、有无音频系统）中，硬件同步能力往往不可用，需要一种通用的时间同步方法。

Method: 使用定制的LED时钟，通过红色和红外LED编码时间信息，从录制的帧中视觉解码曝光窗口（开始和结束时间）实现毫秒级同步。

Result: 与硬件同步相比，残差误差为1.34ms RMSE；在实验中优于基于光、音频和时间码的同步方法；提升了下游计算机视觉任务（多视角姿态估计和3D重建）的性能；在超过25个异构相机的大规模手术记录中验证了系统。

Conclusion: 该方法简化并优化了同步流程，扩展了在无约束环境（包括工业和临床应用）中基于视觉的高级传感技术的应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决激光切割中能耗高和环境影响的挑战，针对当前CO2激光吸泵缺乏自适应控制和开环运行的问题。

Method: 采用闭环配置，结合基于无透镜散斑传感的定制CNN和基于USB摄像头与VGG16迁移学习的材料分类方法，以及独立的烟雾检测深度学习模型。

Result: 实验证明吸泵能耗显著降低20%-50%，吸泵在空闲时自动停止，运行时动态调整功率。

Conclusion: 该研究为制造业可持续发展做出重要贡献，通过智能控制实现显著的能源节约。
...
TL;DR: 本文提出了一种边缘引导空间注意力机制和渐进式训练策略，用于改善透明物体的深度估计和语义分割性能。
...
Result: 在Syn-TODD和ClearPose基准测试中，EGSA持续提高了深度估计精度，在透明区域改进最大，同时保持了有竞争力的分割性能。

Conclusion: 边缘引导融合是一种稳健的方法，能够显著改善透明物体感知性能。
...
TL;DR: 提出了一种仅使用特征知识蒸馏的框架，无需logit损失函数，通过知识质量指标选择最有效的教师层进行蒸馏，在图像分类任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的特征知识蒸馏方法通常结合logit损失和中间层特征损失，本文旨在探索仅使用特征损失进行知识蒸馏的可能性，并识别哪些教师层能提供最有效的知识。

Method: 提出基于特征的知识蒸馏框架，引入知识质量指标来评估教师层的重要性，仅使用特征损失而不依赖logit损失（如交叉熵）来训练学生模型。

Result: 在三个图像分类数据集和四种不同的师生模型对上实验，该方法实现了最先进的性能，相比标准方法top-1准确率提升高达15%。

Conclusion: 仅使用特征损失的知识蒸馏方法是有效的，通过精心选择教师层可以显著提升学生模型的性能，为知识蒸馏提供了新的方向。
...
TL;DR: Kandinsky 5.0是一个用于高分辨率图像和10秒视频合成的最先进基础模型家族，包含三个核心模型系列：6B参数的图像生成模型、2B参数的轻量级视频模型和19B参数的高质量视频生成模型。
...
Method: 采用多阶段训练流程，包括广泛预训练，并结合质量增强技术如自监督微调(SFT)和基于强化学习(RL)的后训练。包含新颖的架构、训练和推理优化，实现高生成速度。

Result: 在人类评估中展示了最先进的性能，能够实现高生成速度，在各种任务中表现出色。

Conclusion: Kandinsky 5.0作为一个大规模公开生成框架，通过发布开源代码和训练检查点，将显著推进研究社区高质量生成模型的开发和可访问性。
...
<details>
  <summary>Details</summary>
Motivation: 金融文档具有视觉密集和表格繁重的布局，其中数值和时间信息与结构紧密耦合。传统OCR指标仅捕获表面文本相似性，而小的OCR错误可能导致重大误解。

Method: 开发了基于LLM的评估流程，执行结构化事实提取和上下文验证，用于视觉复杂的金融文档。所有标注由金融专家创建和验证，严格控制符号、数量和时间表达的质量。

Result: 基准测试显示，尽管最强的专有模型达到最高的事实准确性，但在视觉复杂的数值和时间上下文中仍存在显著错误。
...
TL;DR: 本文提出了一种名为CKDA的跨模态知识解耦与对齐方法，用于解决可见光-红外终身行人重识别中的知识遗忘问题。该方法通过分离和平衡保存模态特定知识和模态通用知识，避免了知识获取与抗遗忘之间的相互干扰。


<details>
  <summary>Details</summary>
Motivation: 现有的可见光-红外终身行人重识别方法通常使用跨模态知识蒸馏来缓解旧知识的灾难性遗忘，但这些方法忽略了模态特定知识获取和模态通用知识抗遗忘之间的相互干扰，导致协同遗忘问题。

Method: 提出了CKDA方法，包含模态通用提示模块和模态特定提示模块来显式解耦和纯化不同模态的判别信息，避免知识间的相互干扰。同时设计了跨模态知识对齐模块，在双模态原型的基础上，在两个相互独立的模态间和模态内特征空间中对齐解耦的新旧知识。

Result: 在四个基准数据集上的广泛实验验证了CKDA方法的有效性和优越性，相比最先进的方法取得了更好的性能。

Conclusion: CKDA方法通过显式分离和平衡保存模态特定知识和模态通用知识，有效解决了可见光-红外终身行人重识别中的协同遗忘问题，为跨模态终身学习提供了新的解决方案。
...
TL;DR: 提出基于结构化复值2D高斯图元的新全息图表示方法，将参数搜索空间减少10倍，实现更低的VRAM使用、更快的优化速度和更高保真度的重建。
...
Method: 使用结构化复值2D高斯图元作为全息图表示，开发可微分光栅化器，并与GPU优化的自由空间光传播核集成，支持端到端训练。
...
TL;DR: 该研究探讨计算机视觉模型在训练过程中是否表现出与儿童发展过程相似的发展对齐现象，特别是在几何和数字概念方面。研究发现ResNet-50模型在某些几何概念类别上表现出发展对齐，并在数字概念中形成了类似人类"心理数字线"的表征。


<details>
  <summary>Details</summary>
Motivation: 基于先前研究发现计算机视觉模型在图像分类任务中学习到的几何和数字概念表征与成人相似，本研究旨在探索这些模型是否也表现出发展对齐，即其训练过程中的性能改善是否与儿童的发展进程相匹配。

Method: 使用ResNet-50模型作为详细案例研究，分析其在几何和数字概念学习过程中的发展轨迹，并与儿童发展数据进行对比。

Result: 在几何方面，模型对某些概念类别（欧几里得几何、几何图形、度量属性、拓扑学）表现出发展对齐，但对其他类别（手性图形、几何变换、对称图形）则未表现出对齐。在数字方面，模型随着经验积累形成了类似人类"心理数字线"的表征。
...
TL;DR: UniHOI是一个统一模型，通过共享的token空间同时处理人-物交互检测和生成任务，实现了双向的图像与交互语义映射，在长尾检测和开放词汇生成任务上均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统方法将人-物交互检测和生成作为独立任务处理，限制了交互理解的全面发展。需要统一框架来促进知识共享和提升泛化能力。

Method: 引入对称的交互感知注意力模块和统一的半监督学习范式，在有限标注下实现图像与交互语义的有效双向映射。

Result: 在HOI检测上准确率提升4.9%，在开放词汇生成任务上交互指标提升42.0%，在两个任务上都达到最先进水平。
...
TL;DR: 提出DLRRF模型解决高光谱和多光谱图像融合中的图像间变异问题，通过建模光谱退化算子变化和分解目标图像为低秩与残差分量来提升融合性能。
...
Method: 将光谱变异建模为光谱退化算子的变化；将目标高光谱图像分解为低秩和残差分量，残差分量用于捕捉丢失的空间细节；利用光谱相关性进行降维；引入隐式正则化器利用空间先验信息；采用PAO算法在PnP框架下求解。

Result: 广泛的数值实验证明DLRRF在存在图像间变异的情况下能够实现优越的高光谱和多光谱图像融合性能。

Conclusion: DLRRF模型通过建模光谱退化变化和图像分解方法，有效解决了高光谱和多光谱图像融合中的图像间变异问题，显著提升了融合性能。
...
Conclusion: ProPL为通用超声图像分割建立了新的基准，展示了在临床环境中的实际应用潜力。
...
TL;DR: 评估现有多模态大语言模型在垂直书写的日文文本上的阅读能力，发现现有模型在垂直日文文本上的表现不如水平文本，并通过合成数据集训练提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 由于一些日文文档采用垂直书写方式，而现有研究对垂直书写日文文本的关注有限，因此需要评估和改进多模态大语言模型对垂直日文文本的理解能力。

Method: 生成合成的日文OCR数据集，包含水平和垂直书写的日文文本，用于模型微调和评估；同时创建来自真实文档图像的评估数据集。

Result: 现有MLLMs在垂直书写的日文文本上表现比水平文本差；使用合成数据集训练后，原本无法处理垂直书写的模型性能得到提升。

Conclusion: 支持垂直书写对日文文档理解至关重要，通过合成数据集训练可以有效改善MLLMs对垂直日文文本的处理能力。
...
TL;DR: 本文提出了VR-Bench基准测试，用于评估视频模型通过视频生成进行空间推理的能力，特别是在迷宫求解任务中。研究发现视频模型在空间感知方面优于视觉语言模型，且测试时多样化采样能提升推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 类比语言模型从文本生成发展到文本推理的发展路径，探索视频模型是否能够通过视频生成进行推理。视频在空间布局和时间连续性方面的优势使其成为空间推理的理想载体。

Method: 构建VR-Bench基准测试，包含7,920个程序生成的视频，涵盖五种迷宫类型和多样化视觉风格。通过监督微调(SFT)激发视频模型的推理能力，并研究测试时多样化采样对推理性能的影响。

Result: 视频模型在空间感知方面表现优于领先的视觉语言模型(VLMs)，能够很好地泛化到不同场景、任务和复杂度级别。测试时多样化采样可将推理可靠性提升10-20%。
...
Conclusion: BokehFlow能够生成视觉上吸引人的散景效果并提供精确控制，无需深度输入，在质量和效率上均表现优异。
...
TL;DR: MambaTrack3D是一个基于状态空间模型Mamba的新型3D单目标跟踪框架，针对高时间变化动态户外环境设计，通过帧间传播和分组特征增强模块解决了现有方法的计算复杂度和时间冗余问题。


<details>
  <summary>Details</summary>
Motivation: 解决高时间变化动态户外环境中现有基于内存的跟踪器存在的二次计算复杂度、时间冗余和几何先验利用不足的问题。

Method: 提出Mamba-based Inter-frame Propagation模块实现帧间传播替代传统单帧特征提取，以及Grouped Feature Enhancement Module在通道层面分离前景和背景语义。

Result: 在KITTI-HTV和nuScenes-HTV基准测试中优于现有方法，在标准KITTI数据集上保持竞争力，实现了优异的精度-效率权衡。

Conclusion: MambaTrack3D在专业HTV和常规跟踪场景中均表现出强大的泛化能力和鲁棒性能。
...
Method: 提出TiCAL框架，利用伪单模态情感标签和典型性估计动态评估训练样本的一致性，在双曲空间中嵌入特征以捕捉情感类别的细粒度差异。

Result: 在CMU-MOSEI和MER2023等基准数据集上的实验验证了TiCAL的有效性，特别是在模态不一致性高的样本上表现更好，相比最先进的DMD方法有约2.6%的提升。

Conclusion: TiCAL框架能够有效缓解模态间情感冲突，提高整体识别准确率，为多模态情感识别提供了新的解决方案。
...
TL;DR: 本文研究了离散扩散多模态大语言模型(dMLLMs)的效率优化问题，重点关注视觉令牌冗余在不同架构和任务中的演化规律，以及视觉令牌剪枝对模型响应和效率的影响。


<details>
  <summary>Details</summary>
Motivation: 现有dMLLMs在推理时由于需要在每个去噪步骤中进行全序列注意力计算，导致显著的计算开销。先前研究从模态无关的角度尝试解决此问题，但大多忽略了模态特定的视觉令牌冗余问题。

Method: 通过系统研究不同dMLLM架构和任务中视觉令牌冗余的演化规律，分析视觉令牌剪枝对模型响应和效率的影响，探索针对不同dMLLM类型的优化策略。

Result: 研究发现：视觉冗余仅出现在从头训练的dMLLMs处理长答案任务时；视觉令牌剪枝会引入不可忽视的信息损失，但只有从头训练的dMLLMs能在后期去噪步骤中逐步恢复丢失的信息；层跳跃适用于加速AR到扩散的dMLLMs，而渐进或后期剪枝对从头训练的dMLLMs更有效。
...
TL;DR: 本文提出高斯混合(Gaussian Blending)方法替代传统alpha混合，解决了3D高斯泼溅在未见采样率下渲染时的模糊和阶梯伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在训练时未见采样率下合成视图时会出现明显的视觉差异：放大时产生侵蚀引起的模糊伪影，缩小时产生扩张引起的阶梯伪影。作者推测这些伪影源于alpha混合的基本限制。

Method: 提出高斯混合方法，将alpha和透射率视为空间变化的分布而非标量。通过考虑像素区域内alpha值的空间分布来更新透射率，允许附近背景泼溅对最终渲染做出贡献。

Result: 高斯混合保持了实时渲染速度，无需额外内存成本，可作为即插即用模块集成到现有3DGS框架中。在各种未见采样率下有效捕捉细节，在未见和已见采样率下均优于现有新视图合成模型。

Conclusion: 高斯混合通过将alpha和透射率建模为空间分布，成功解决了3DGS在未见采样率下的渲染伪影问题，在保持实时性能的同时显著提升了渲染质量。
...
TL;DR: 提出了一种与SAM集成的无偏语义解码(USD)策略，通过同时从支持和查询集中提取目标信息，利用CLIP模型的语义指导进行一致预测，解决了SAM在少样本分割中对准确提示的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要从支持集中提取提示来激活SAM的泛化能力，但这种方式在适应未知类别时容易导致有偏的解码过程。需要一种能够同时利用支持和查询集信息的方法来增强SAM的无偏语义判别能力。

Method: 1. 设计了两种特征增强策略：全局补充（图像级别）和局部引导（像素级别），利用CLIP的语义对齐能力丰富原始SAM特征；2. 提出可学习的视觉-文本目标提示生成器，通过交互目标文本嵌入和CLIP视觉特征生成目标聚焦的提示嵌入。

Result: 该方法在不重新训练视觉基础模型的情况下，通过具有语义判别能力的特征和富含目标信息的提示引导，将注意力集中在目标区域。

Conclusion: USD策略有效解决了SAM在少样本分割中的提示依赖问题，通过结合CLIP的语义指导实现了更准确和无偏的分割性能。
...
TL;DR: WaveFuse-AL是一种新颖的多策略主动学习框架，通过自适应融合BALD、BADGE、Entropy和CoreSet四种采集策略，结合周期性和性能驱动的适应机制，在医学影像任务中显著提升性能并降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 传统主动学习中的单一采集策略在不同学习阶段表现不一致，需要一种能够自适应调整策略重要性的方法来提高医学影像标注的效率。
...
Conclusion: WaveFuse-AL通过自适应融合多策略和周期性调整，能够有效提升医学影像主动学习的性能，最大化有限标注预算的效用。
...
TL;DR: 该论文提出了一种结构感知的管道，从真实的手术培训师-学员对话中学习手术行动本体，并利用它来指导反馈生成。通过挖掘工具-行动-目标三元组、微调视频到IAT模型，以及使用GPT-4o生成基于IAT的反馈，显著提高了反馈生成的质量和临床相关性。
...
Method: 1) 从真实世界反馈文本中挖掘工具-行动-目标三元组，并将表面形式聚类为规范化类别；2) 微调视频到IAT模型，利用手术程序和任务上下文以及细粒度时间工具运动；3) 使用IAT三元组表示指导GPT-4o生成临床基础、培训师风格的反馈。

Result: 在视频到IAT识别任务中，上下文注入和时间跟踪带来一致的AUC提升。在反馈文本生成任务中，仅使用视频的GPT-4o得分为2.17，而IAT条件化达到2.44，将可接受生成的比例从21%提高到42%。传统文本相似性指标也有改善。
...
TL;DR: SceneEdited是首个专门为通过3D点云更新支持高精地图维护研究而设计的大规模城市级数据集，包含800多个最新场景，覆盖73公里驾驶区域，模拟了超过23,000个城市环境变化。


<details>
  <summary>Details</summary>
Motivation: 高精地图对城市规划、基础设施监控和自动驾驶至关重要，但环境变化导致地图快速过时。现有技术主要关注变化检测而非实际更新3D地图，特别是在依赖2D图像变化检测时存在明显差距。

Method: 创建了包含800多个最新场景的数据集，覆盖73公里驾驶区域和约3平方公里城市区域，通过手动和自动方式合成了超过23,000个对象变化，模拟真实城市修改。提供基于图像的结构从运动流程的基线方法用于更新过时场景。

Result: 数据集包含校准的RGB图像、LiDAR扫描和详细变化掩码，支持训练和评估。提供了全面的工具包支持可扩展性、可追踪性和可移植性，便于未来数据集扩展和过时对象标注的统一。

Conclusion: SceneEdited数据集和工具包为3D地图更新研究建立了标准化基准，填补了变化检测与实际地图更新之间的技术空白，推动了高精地图维护技术的发展。
...
TL;DR: 本文提出了一种新的多模态持续指令调优方法，通过将灾难性遗忘概念化为新任务学习中旧任务梯度缺失的问题，利用参数空间的几何特性来近似缺失梯度，有效缓解灾难性遗忘。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人体运动预测评估指标存在缺陷，它们简单赞赏广泛分布的运动，即使这些运动属于单一模式或在运动学上无效。需要一种能够同时评估预测运动的覆盖范围（多模态分布）和有效性（运动合理性）的指标。

Method: 提出MMCM指标，通过聚类将运动空间划分为多个模式，利用这些模式显式评估预测运动是否分布在多个模式中。同时从运动数据集中收集可能的未来运动来识别有效模式，确保运动在运动学上的合理性。

Result: 实验验证了聚类方法能够产生合理的模式定义，并且MMCM能够准确评估多模态预测的质量。

Conclusion: MMCM是一种有效的人体运动预测评估指标，能够同时考虑预测运动的覆盖范围和有效性，解决了现有指标的不足。
...
TL;DR: 该论文提出了指令引导的病变分割（ILS）新范式，构建了首个大规模胸部X光病变分割指令-答案数据集MIMIC-ILS，并开发了ROSALIA视觉语言模型，能够基于简单指令分割多种病变类型并提供文本解释。


<details>
  <summary>Details</summary>
Motivation: 解决当前胸部X光病变分割模型存在的目标标签少、依赖专家级详细文本输入的问题，降低实际使用门槛。
...
Conclusion: ILS范式有效解决了胸部X光病变分割的实用性限制，MIMIC-ILS数据集为像素级病变定位提供了基础资源，ROSALIA模型展示了该方法的有效性。
...
Method: 使用ViT编码器在辅助年龄和性别分类任务上预训练学习切片级特征，然后将冻结的编码器应用于所有矢状切片生成嵌入向量2D矩阵，输入到残差CNN回归器中，在最终全连接层整合受试者性别信息来估计连续脑年龄。

Result: 在11个MRI数据集（超过130个采集点）验证中MAE为3.34年（Pearson r=0.98，Spearman ρ=0.97，R²=0.95），在4个独立队列中MAE在3.77-5.04年之间，优于基线和最先进模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于参考的对象组合方法在将真实世界对象插入风格化域时失败，实用方法缺乏生成保真度，而生成方法需要不切实际的在线微调。
...
Conclusion: Insert In Style是第一个既实用又高保真的零样本生成框架，无需文本提示，通过解耦表示和专门架构解决了概念干扰问题。
...
TL;DR: 本文提出了一种物理约束的多模态数据评估指标PCMDE，结合大型语言模型与推理、知识映射和视觉语言模型，以解决现有指标在语义和结构准确性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的评估指标如BLEU、CIDEr、VQA分数等，在领域特定或上下文依赖场景中往往无法准确捕捉语义或结构准确性。

Method: PCMDE架构包含三个阶段：(1)通过目标检测和视觉语言模型提取空间和语义信息的特征；(2)置信度加权组件融合进行自适应组件级验证；(3)使用大型语言模型进行物理引导推理，强制执行结构和关系约束。

Result: 该方法能够更有效地评估多模态数据的语义和结构准确性，特别是在需要物理约束的复杂场景中。

Conclusion: PCMDE指标通过结合多种先进技术，为多模态数据评估提供了更全面和准确的解决方案，克服了现有指标的局限性。
...
TL;DR: SkinGPT-R1是一个专注于皮肤科的视觉语言模型，通过显式的诊断推理链实现可验证的皮肤病诊断。该模型在DermBench基准测试中排名第一，比Vision-R1平均得分提高41%，并在三个皮肤病分类基准上保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够进行显式、逐步、可验证诊断推理的皮肤科专用视觉语言模型，以提升皮肤病诊断的透明度和准确性。

Method: 构建DermCoT语料库（包含10,000个训练案例和3,000个专家认证案例），定义DermEval六维评估器和DermBench基准，采用基于DermCoT的推理链监督和皮肤科感知的视觉蒸馏方法。

Result: 在DermBench上获得4.031/5的平均分，排名第一，比Vision-R1提高41%的得分；在三个皮肤病分类基准上表现稳定且具有竞争力；消融实验显示DermCoT监督和视觉蒸馏均带来显著改进。

Conclusion: SkinGPT-R1通过显式推理链和皮肤科专用训练数据，在皮肤病诊断质量和准确性方面取得了显著提升，证明了专业领域视觉语言模型的有效性。
...
TL;DR: SplitFlux是一个基于Flux模型的图像内容与风格解耦方法，通过分析Flux模型特性发现单流块在图像生成中的关键作用，并提出基于LoRA的微调策略实现内容与风格的有效分离。


<details>
  <summary>Details</summary>
Motivation: 现有SDXL方法难以实现高质量的内容风格解耦，而新提出的Flux模型由于特性未被充分探索，无法有效分离内容与风格。
...
Conclusion: SplitFlux通过系统分析Flux模型特性并设计有效的解耦策略，成功解决了图像内容与风格分离的挑战，为定制化图像生成提供了有效解决方案。
...
### [42] [IPTQ-ViT: Post-Training Quantization of Non-linear Functions for Integer-only Vision Transformers](https://arxiv.org/abs/2511.15369)
*Gihwan Kim,Jemin Lee,Hyungshin Kim*

Main category: cs.CV

TL;DR: IPTQ-ViT是一种无需重新训练的后训练量化框架，通过多项式GELU和位移Softmax近似函数实现完全整数化的视觉Transformer推理，在W8A8和W4A8配置下优于现有PTQ方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有量化方法在视觉Transformer中需要昂贵重新训练或无法实现完全整数推理的问题，为资源受限环境提供高效量化方案。

Method: 提出多项式GELU和位移Softmax近似函数，并设计统一指标选择每层最优近似函数，实现无需重新训练的完全整数化量化。

Result: 在图像分类任务中Top-1准确率提升最高6.44%（平均1.78%），目标检测提升1.0 mAP，性能与整数QAT方法相当。

Conclusion: IPTQ-ViT首次实现无需重新训练的完全整数化视觉Transformer量化，在准确率和延迟方面均表现出色。
...
TL;DR: 本文提出Graph Query Networks (GQN)框架，使用图查询在BEV空间中动态构建对象特定图，通过EdgeFocus和DeepContext Pooling模块处理雷达数据，在NuScenes数据集上显著提升检测性能。
...
Method: GQN将雷达感知的对象建模为图，使用图查询动态关注BEV空间，构建对象特定图，通过EdgeFocus进行关系推理，DeepContext Pooling进行上下文聚合。

Result: 在NuScenes数据集上，GQN相对mAP提升高达+53%，比最强雷达方法提高+8.2%，同时将峰值图构建开销降低80%，FLOPs成本适中。

Conclusion: GQN通过图查询和图处理模块有效解决了雷达数据稀疏不规则的问题，显著提升了3D雷达目标检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统X射线安检图像合成方法采用两阶段流程，需要人工提取前景目标，效率低下且成本高昂。为了解决数据不足问题，需要开发更高效的图像合成方法。

Method: 基于文本到图像生成的一阶段合成管道，包含交叉注意力细化策略（利用扩散模型交叉注意力图优化边界框标注）和背景遮挡建模策略（在潜在空间显式建模背景遮挡以增强图像复杂性）。
...
### [45] [Adapt-As-You-Walk Through the Clouds: Training-Free Online Test-Time Adaptation of 3D Vision-Language Foundation Models](https://arxiv.org/abs/2511.15311)
*Mehran Tamjidi,Hamidreza Dastmalchi,Mohammadreza Alimoradijazi,Ali Cheraghian,Aijun An,Morteza Saberi*

Main category: cs.CV

TL;DR: Uni-Adapter是一种无需重新训练、基于动态原型学习的在线测试时自适应策略，用于提升3D视觉语言基础模型在噪声、不完整或分布偏移数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 3D视觉语言基础模型在开放世界点云处理任务中表现出色，但在实际场景中面对噪声、不完整或分布偏移数据时性能下降，需要有效的自适应方法。

Method: 提出动态原型学习策略，定义3D缓存存储类别特定的聚类中心作为原型，通过图基标签平滑模块捕捉原型间相似性，并使用熵加权聚合统一原始模型和缓存预测。
...
### [46] [A Multimodal Transformer Approach for UAV Detection and Aerial Object Recognition Using Radar, Audio, and Video Data](https://arxiv.org/abs/2511.15312)
...
Method: 设计并评估了一个多模态Transformer模型，利用自注意力机制融合来自雷达、RGB视频、红外视频和音频的不同特征。

Result: 在独立测试集上表现出色，准确率0.9812，召回率0.9873，精确率0.9787，F1分数0.9826，特异性0.9954；计算效率高，推理速度41.11 FPS。

Conclusion: 该研究通过Transformer架构的多模态数据融合，为复杂空域中的无人机检测和监控提供了高精度和鲁棒性的解决方案，代表了空中物体分类的重要进展。
...
TL;DR: 本文提出了一种针对多模态检索增强生成系统的视觉攻击方法，通过向图像输入添加微小扰动来误导系统生成错误内容，而无需篡改其他组件。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注知识投毒攻击，但本文考虑仅通过视觉扰动攻击MRAG系统的新场景，这对系统的安全性提出了新挑战。
...
Conclusion: 视觉攻击是MRAG系统面临的新安全威胁，需要开发相应的防御机制来保护系统免受此类攻击。
...
Conclusion: Split DNNs存在比以往认知更严重的隐私威胁，FIA-Flow框架能够有效揭示这种风险。
...
Method: 构建了包含HSICT块、残差CNN模块、空间CNN块和通道注意力的混合架构，通过定制化ICT块学习全局上下文交互和局部纹理提取，H和S层学习空间同质性和精细结构细节，逆残差学习增强梯度消失问题。

Result: 在Kaggle基准和多样化MPox数据集上，分类准确率达到98.30%，F1分数达到98.13%，优于现有的CNN和ViT方法。

Conclusion: 提出的RS-CA-HSICT框架通过有效结合CNN和Transformer的优势，在MPox检测任务中取得了最先进的性能，证明了混合架构在医学图像分析中的有效性。
...
TL;DR: 提出了一种基于自适应阈值模式的指纹伪造检测算法，通过小波变换和SVM分类器来区分真假指纹，并特别关注了对像素缺失、块缺失和噪声污染等失真的抵抗能力。
...
Method: 采用自适应阈值模式，将输入图像的各向异性扩散通过三级小波变换，对不同层的系数进行自适应阈值处理并拼接成特征向量，最后使用SVM分类器进行分类。

Result: 在定量比较中，该方法在90%像素缺失场景下的准确率比同类方法高出约8%，在70x70块缺失场景下的准确率高出约5%，显示出对各类失真的改进抵抗能力。

Conclusion: 提出的方法在指纹伪造检测方面表现出色，特别是在抵抗各种失真方面具有显著优势，为指纹生物识别系统提供了更可靠的安全保障。
...
TL;DR: 论文提出了MILL数据集，包含不同光照强度下的低光图像，用于评估增强算法在不同光照条件下的性能，并提出了改进方法提升算法鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有低光增强方法依赖单一光照条件下的配对数据，缺乏光照强度多样性，限制了算法在不同光照条件下的性能理解。

Method: 引入MILL数据集，包含控制条件下不同光照强度的图像，并提出基于该数据集多光照结构的改进方法。

Result: 改进方法在Full HD图像上实现了DSLR相机10 dB PSNR提升和智能手机2 dB PSNR提升。

Conclusion: MILL数据集能够全面评估增强算法在不同光照条件下的性能，提出的改进方法显著提升了算法鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 开发可靠的无人机导航系统需要能够区分训练期间见过的物体和未见过的物体的鲁棒空对空目标检测器。现有方法通常只处理闭集检测或使用单一不确定性评分进行阈值处理，限制了灵活性且经常将未知物体与背景杂波混淆。

Method: 采用轻量级、模型无关的后处理框架，通过融合多个置信度估计和每个检测的特征，使用紧凑的多层感知机（MLP）进行三分类（已知目标、未知物体、背景）。将不同的logit变体纳入MLP中持续提升性能。

Result: 在二分类中平均比基于阈值的基线方法提高2.7% AUROC，同时保持或改善开放集mAP。在三分类中实现鲁棒性能，在闭集mAP上相对提升高达18%（9个点）。

Conclusion: 该方法为安全无人机导航提供了关键能力，其中未知物体必须主动避免，背景区域可以安全忽略，在AUROC和闭集mAP方面均优于竞争技术。
...
Result: 在三个运动语言数据集上实现最先进的运动定位性能，在HumanML3D基准上比之前方法提升+8.7% mAP，下游检索任务也有显著改进。

Conclusion: 建立了一种无需标注的运动理解新范式，在开放词汇场景中表现出色。
...
TL;DR: CompTrack是一个端到端的3D单目标跟踪框架，通过空间前景预测器和信息瓶颈引导的动态令牌压缩模块，有效消除点云中的空间冗余和信息冗余，实现高效准确的跟踪。
...
Method: 提出CompTrack框架，包含空间前景预测器（SFP）过滤背景噪声，以及基于信息瓶颈的动态令牌压缩（IB-DTC）模块，通过在线SVD分析自适应压缩冗余前景信息。
...
TL;DR: AutoPrune是一种新型的LLM剪枝方法，通过让LLM自动设计最优剪枝算法来克服专家知识限制，并引入图驱动的思维链优化提示，以及偏斜感知动态稀疏分配来解决高剪枝率下的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM剪枝方法严重依赖人工设计的剪枝算法，导致巨大的人工成本和专家知识需求。同时发现了在高剪枝率下由均匀稀疏性引起的严重异常值问题，导致性能急剧下降。

Method: 1. 利用LLM自动设计最优剪枝算法；2. 提出图驱动的思维链(GCoT)优化提示，增强推理过程；3. 引入偏斜感知动态稀疏分配(SDSA)解决异常值问题。

Result: 在主流LLM基准测试上的广泛实验表明，AutoPrune始终优于最先进的竞争对手。

Conclusion: AutoPrune证明了LLM可以自我剪枝，克服了专家知识限制和异常值问题，在保持性能的同时实现了高效的模型压缩。
...
<details>
  <summary>Details</summary>
Motivation: 深度分割模型缺乏对错误的明确统计保证，在临床决策中需要可靠的分割结果，特别是避免过度分割带来的临床风险。

Method: 使用预训练分割模型，通过增加得分阈值或形态学腐蚀构建收缩掩码系列，利用校准集选择收缩参数，确保假阳性比例低于用户指定容忍度。

Result: 在息肉分割基准测试中实现了目标级别的经验有效性，方法模型无关、无需重新训练，提供有限样本保证。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据无关量化技术在CLIP等视觉语言模型上应用时存在性能显著下降的问题，主要由于合成样本语义内容不足和图像内部多样性低两个关键限制。

Method: D4C框架包含三个关键组件：(1)提示引导语义注入，使用文本提示将生成图像与真实语义对齐；(2)结构对比生成，利用前景-背景对比合成重现自然图像的组合结构；(3)扰动感知增强，应用受控扰动提高样本多样性和鲁棒性。

Result: 在W4A8设置下，D4C在CLIP ResNet-50和ViT-B/32上取得了显著性能提升：在CIFAR-10上分别提升12.4%和18.9%，在CIFAR-100上提升6.8%和19.7%，在ImageNet-1K零样本分类上提升1.4%和5.7%。

Conclusion: D4C通过生成语义丰富且结构多样的伪图像，有效弥合了数据无关量化在CLIP上的性能差距，在各种比特宽度和模型上都验证了其有效性。
...
Method: 从DATTALION开源库构建真实世界数据集，使用多种最先进的语义分割模型在结构化城市场景上进行训练，分析训练数据环境的影响，并提出在目标图像无标注情况下实现有效导航性的初步方法。

Result: 报告了在WarNav数据集上的基线结果，分析了不同训练环境对模型性能的影响，为在无标注目标图像约束下实现挑战性环境中的有效导航提供了初步解决方案。

Conclusion: WarNav数据集为在冲突影响的无结构环境中开发自主导航系统提供了重要资源，促进了在数据标注有限情况下提高自主车辆在高风险场景中鲁棒性的研究。
...
Method: 提出EASD端到端脉冲相机检测器，采用双分支设计：时间纹理特征融合分支用于全局跨切片语义，熵选择注意力分支用于物体中心细节。

Result: 创建了首个面向驾驶场景的模拟脉冲检测基准DSEC Spike，填补了数据空白。

Conclusion: EASD通过双分支架构有效解决了脉冲相机输出稀疏离散的检测挑战，为自动驾驶中的物体检测提供了新解决方案。
...
Method: 提出Sigmma框架，包括多尺度对比对齐确保不同尺度表示在模态间保持一致，以及通过图表示细胞相互作用并整合图间和图内关系来捕捉组织微环境中的细胞-细胞相互作用。

Result: Sigmma在基因表达预测任务中平均提升9.78%，在跨模态检索任务中平均提升26.93%，并在下游分析中学习到有意义的多组织组织。

Conclusion: Sigmma通过学习层次表示有效捕捉了跨模态对应关系，在计算病理学任务中表现出显著改进。
...
TL;DR: 本研究开发了一个多阶段AI管道，用于从围网渔船电子监控系统的视频中自动识别和分类金枪鱼物种，特别关注区分大眼金枪鱼和黄鳍金枪鱼。


<details>
  <summary>Details</summary>
Motivation: 围网渔船捕获了全球约69%的热带金枪鱼，电子监控系统产生大量视频数据需要人工处理。AI集成可以减少工作量并提高报告准确性，但物种识别特别是区分大眼金枪鱼和黄鳍金枪鱼仍存在挑战。
...
Conclusion: YOLOv9-SAM2与分层分类相结合的方法在电子监控系统中对金枪鱼物种识别和分类方面表现出色，为渔业管理提供了可靠的自动化解决方案。
...
TL;DR: FunnyNodules是一个完全参数化的合成数据集，用于系统分析医学AI模型中的基于属性的推理，通过生成具有可控视觉属性的抽象肺结节形状来评估可解释AI模型。


<details>
  <summary>Details</summary>
Motivation: 密集标注的医学图像数据集稀缺，特别是包含诊断背后推理的标注，这对于开发和评估与放射科医生推理方式相似的可解释AI模型至关重要。

Method: 创建FunnyNodules数据集，生成具有可控视觉属性（如圆度、边缘锐度和毛刺）的抽象肺结节形状，目标类别来自预定义的属性组合，允许完全控制连接属性与诊断类别的决策规则。
...
TL;DR: EAR是一种新的自回归图像生成范式，模拟人类视觉系统的中心向外感知模式，通过螺旋顺序展开图像token，支持高效并行解码和长度自适应策略，在ImageNet上实现了最先进的保真度与效率平衡。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视觉生成方法存在效率问题，要么需要逐个token解码，要么多尺度表示复杂。受人类视觉系统中心向外感知模式的启发，希望开发更高效且符合认知的自回归生成方法。

Method: 提出扩展自回归表示(EAR)范式：1) 从中心向外按螺旋顺序展开图像token，保持空间连续性；2) 支持并行解码；3) 引入长度自适应解码策略，动态调整每步预测的token数量。

Result: 在ImageNet上的广泛实验表明，EAR在单尺度自回归模型中实现了保真度与效率之间的最先进平衡。
...
TL;DR: 本文评估了多种人体姿态预测算法，发现存在可复现性问题，并提出统一训练评估流程。通过借鉴语音理解模型，显著提升了姿态预测性能。同时评估了模型在噪声姿态数据下的鲁棒性，并展示了无监督微调对性能恢复的效果。
...
Result: 语音模型适配显著提升了姿态预测性能；估计姿态数据导致性能显著下降，但无监督微调可部分恢复性能。

Conclusion: 语音模型可有效提升姿态预测性能，但模型对噪声数据敏感，无监督微调是提升鲁棒性的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在干净条件下表现优异，但对抗攻击评估其安全性尚未充分探索，需要开发先进的对抗攻击方法来评估这类检测器的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR地点识别方法在新环境中难以适应且会遗忘先前学到的知识（灾难性遗忘），需要解决这一挑战。

Method: 提出KDF+框架，包含损失感知采样策略（根据损失值估计样本学习难度，优先选择困难样本）和排练增强机制（在训练新任务时微调记忆样本以增强长期知识保留）。
...
TL;DR: 提出了一种新颖的多模态深度学习方法，利用X射线图像信息来补全3D超声中被遮挡的脊柱解剖结构，解决了超声成像中骨骼遮挡导致的结构不完整问题。
...
Method: 通过生成配对的训练数据（模拟X射线扫描的2D侧位椎体视图和模拟超声成像遮挡的3D部分椎体表示），开发多模态深度学习模型，整合两种成像模式的形态信息。

Result: 在椎体重建方面相比现有3D超声椎体补全方法有显著改进（p < 0.001），在体模研究中实现了更准确、完整的腰椎体积可视化，无需与术前CT等模式配准。

Conclusion: 整合单个X射线投影可以减轻超声的关键局限性，同时保留其作为主要成像模式的优势，为未来临床转化提供了初步验证。
...
### [69] [Test-time Scaling of LLMs: A Survey from A Subproblem Structure Perspective](https://arxiv.org/abs/2511.14772)
*Zhuoyi Yang,Xu Guo,Tong Zhang,Huijuan Xu,Boyang Li*

Main category: cs.CL

TL;DR: 本文综述了通过增加推理时计算来提升预训练大语言模型预测准确性的方法，特别关注问题分解方式和子问题组织结构（顺序、并行或树状），统一了多种技术并分析了各自的优缺点。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过分配额外计算资源在推理阶段提升预训练大语言模型的预测性能，为不同应用场景提供有效的测试时扩展方法。

Method: 对测试时扩展方法进行分类，重点关注问题分解策略和子问题组织结构（顺序、并行、树状），将Chain-of-Thought、Branch-Solve-Merge、Tree-of-Thought等技术统一在同一框架下分析。

Result: 建立了统一的分类框架，系统分析了各种测试时扩展方法的优势和局限性，为方法选择提供了理论依据。

Conclusion: 提出了未来研究的几个有前景方向，为改进大语言模型推理时性能优化方法指明了路径。
...
### [70] [Temporal Predictors of Outcome in Reasoning Language Models](https://arxiv.org/abs/2511.14773)
*Joey David*

Main category: cs.CL

TL;DR: 研究表明，大型语言模型在推理过程的早期阶段就能预测最终答案的正确性，即使需要更长的输出来确定答案。
...
Method: 通过在推理过程的前t个标记后训练线性分类器来分析隐藏状态，预测最终答案的正确性。

Result: 模型在仅几个标记后就能高度预测最终正确性；对于难题，预测准确率下降揭示了选择偏差：难题在长链式思维中比例过高。

Conclusion: 推理模型的内部自我评估在几个标记后就会出现，这对可解释性和推理时控制具有重要意义。
...
### [71] [LiveCLKTBench: Towards Reliable Evaluation of Cross-Lingual Knowledge Transfer in Multilingual LLMs](https://arxiv.org/abs/2511.14774)
*Pei-Fu Guo,Yun-Da Tsai,Chun-Chia Hsu,Kai-Xin Chen,Ya-An Tsai,Kai-Wei Chang,Nanyun Peng,Mi-Yen Yeh,Shou-De Lin*

Main category: cs.CL

TL;DR: LiveCLKTBench是一个自动生成管道，专门用于隔离和测量大型语言模型中的跨语言知识迁移，通过时间敏感的知识实体来评估真实的跨语言迁移能力。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型中的跨语言知识迁移具有挑战性，因为目标语言中的正确答案可能来自真实的迁移，也可能来自预训练期间的先前接触。需要一种方法来隔离和测量真正的跨语言知识迁移。

Method: 开发了LiveCLKTBench自动化生成管道，识别现实领域中的自包含、时间敏感知识实体，基于时间发生进行过滤，并验证模型的知识。使用这些有效实体的文档生成事实性问题，翻译成多种语言来评估跨语言迁移能力。

Result: 在五种语言上评估多个LLM，发现跨语言迁移受到语言距离的强烈影响，并且在语言方向上通常是不对称的。较大的模型改进了迁移，但增益随规模增加而减少，并在不同领域间变化。
...
### [72] [COMPASS: Context-Modulated PID Attention Steering System for Hallucination Mitigation](https://arxiv.org/abs/2511.14776)
*Snigdha Pandya,Rohan Nagale,Kenji Sahay,Anna Lin,Shikhar Shiromani,Kevin Zhu,Dev Sunishchal*

Main category: cs.CL

TL;DR: COMPASS是一个轻量级、可解释的控制框架，通过基于模型的反馈回路在解码过程中动态调节注意力头，以减少LLM的上下文幻觉，同时提供对模型机制的透明洞察。


<details>
  <summary>Details</summary>
Motivation: 理解并引导LLM在上下文知识和参数知识之间的注意力分配行为，这对于可信部署和模型机制的科学可解释性至关重要。

Method: 引入COMPASS框架，通过上下文依赖分数(CRS)量化上下文依赖度，并使用PID控制器动态调节注意力头，无需重新训练或多轮解码。

Result: 在多个基准测试(HotpotQA、XSum、HaluEval、RAGTruth)上，COMPASS持续降低上下文幻觉率(绝对降低2.8%至5.8%)，并揭示了不同注意力头对证据对齐的贡献。

Conclusion: 反馈驱动的可解释性为科学理解LLM行为提供了一条途径，COMPASS在减少幻觉的同时提供了对模型内部机制的透明洞察。
...
### [73] [The Impact of Prosodic Segmentation on Speech Synthesis of Spontaneous Speech](https://arxiv.org/abs/2511.14779)
...
TL;DR: 本文评估了手动和自动韵律分割标注对巴西葡萄牙语语音合成质量的影响，发现使用韵律分割训练的模型能生成更清晰和自然的语音。


<details>
  <summary>Details</summary>
Motivation: 自发语音合成面临捕捉自然对话流程的挑战，包括话轮转换、停顿和不流畅性。现有研究很少探讨显性韵律分割数据集构建及其对自发语音合成的影响。
...
Conclusion: 韵律分割训练能提升语音合成的自然度，手动分割在韵律变化方面表现更好。所有数据集、源代码和训练模型已公开，支持可重复性研究。
...
### [74] [Human or LLM as Standardized Patients? A Comparative Study for Medical Education](https://arxiv.org/abs/2511.14783)
*Bingquan Zhang,Xiaoxiao Liu,Yuchi Wang,Lei Zhou,Qianqian Xie,Benyou Wang*

Main category: cs.CL

TL;DR: EasyMED是一个基于多智能体框架的标准化病人模拟器，通过Patient Agent、Auxiliary Agent和Evaluation Agent实现真实对话、事实一致性和可操作反馈，在SPBench基准测试中达到与人类标准化病人相当的学习效果，同时提供更好的灵活性、心理安全性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的标准化病人模拟器存在行为不一致、缺乏与人类标准化病人严格比较的问题，而传统标准化病人成本高、灵活性差且难以扩展。
...
Result: 实验表明EasyMED与人类标准化病人的学习效果相当，对基线水平较低的学生产生更大的技能提升，同时提供改进的灵活性、心理安全性和成本效率。
...
TL;DR: HTP通过分层令牌预置解决LLM嵌入中的因果注意力限制问题，在块级预置摘要令牌并采用均值池化，显著提升长文档嵌入性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的因果注意力机制限制了信息从后向前的流动，降低了表示质量。现有方法通过预置单个摘要令牌来解决此问题，但在长文档中会过度压缩信息，损害性能。

Method: 提出分层令牌预置(HTP)：1)将输入分区为块，在后续块中预置块级摘要令牌，创建多个后向信息流路径；2)用均值池化替代最后令牌池化，缓解读取级过度压缩。

Result: 在11个检索数据集和30个通用嵌入基准测试中实现一致的性能提升，特别是在长上下文设置下表现优异。
...
Method: 使用概率建模、信息理论、三角信号分析和贝叶斯不确定性估计来分析自回归误差传播，提出改进的不确定性度量指标，并开发对比解码、检索增强接地、事实对齐和弃权等缓解策略。

Result: 建立了一个统一的理论框架，将校准、检索和对齐等最新进展联系起来，为更安全可靠的大语言模型提供支持。

Conclusion: 该数学框架为理解和缓解大语言模型幻觉问题提供了系统性的理论基础和实践方法，有助于提升模型的可靠性和安全性。
...
TL;DR: TASA是一个学生感知的数学辅导框架，通过整合学生画像、记忆和遗忘动态来实现个性化学习，在LLM驱动的辅导系统中优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM辅导系统未能有效捕捉学生知识随能力、概念差距和遗忘模式的动态演变，特别是在需要精细校准学生掌握水平和认知保持的数学辅导中。

Method: TASA维护结构化学生画像记录能力概况和事件记忆记录先前学习交互，通过结合连续遗忘曲线和知识追踪，动态更新学生掌握状态并生成情境适当、难度校准的问题和解释。
...
<details>
  <summary>Details</summary>
Motivation: 印度作为世界上最多样化的地区之一，拥有近15亿人口和120多种主要语言。当前多语言VLM评估存在四个主要局限：依赖未经验证的自动翻译、任务/领域覆盖范围窄、样本量有限、缺乏文化和本地来源的问答数据。

Method: 提出了半自动数据集创建框架，结合回译、过滤和人工验证；创建了最全面的印地语和泰卢固语视觉语言基准，包括改编的英语数据集和本地新颖的印度数据集，每个语言约4000个问答对。

Result: 在所有模型中的5个任务中，有4个任务在印度语言上的表现相比英语出现退化，印地语平均退化8.3分，泰卢固语平均退化5.5分。

Conclusion: 通过分类常见失败模式，突出了多语言多模态理解中需要改进的具体领域，为开发更公平的AI提供了重要见解。
...
TL;DR: 本文首次全面研究了内在维度(ID)与可解释文本属性之间的关系，发现ID与基于熵的指标互补，在控制长度后两者不相关；ID在不同文体中呈现稳定分层：科学文本ID最低(~8)，百科内容中等(~9)，创意/观点写作最高(~10.5)；通过稀疏自编码器识别出科学信号降低ID，人性化信号增加ID。


<details>
  <summary>Details</summary>
Motivation: 内在维度是现代LLM分析中的重要工具，用于研究训练动态、缩放行为和数据集结构，但其文本决定因素尚未充分探索。本文旨在通过可解释的文本属性来理解ID。

Method: 采用交叉编码器分析、语言特征和稀疏自编码器(SAEs)进行多角度研究，包括控制长度后的相关性分析、不同文体的ID分层分析，以及通过SAEs识别因果特征并进行转向实验验证。

Result: 发现ID与熵指标正交，捕获几何复杂性；科学文本ID最低，创意写作ID最高；科学信号(正式语气、报告模板、统计数据)降低ID，人性化信号(个性化、情感、叙事)增加ID，转向实验证实这些效应具有因果性。

Conclusion: 对于当代模型，科学写作相对"简单"，而小说、观点和情感增加了表示自由度。多角度分析为ID的正确使用和基于ID结果的合理解释提供了实用指导。
...
Method: 提出OEMA框架，包含三个组件：自标注器生成示例、判别器通过SNOMED CT过滤示例、预测器使用实体描述进行准确推理。
...
Method: 采用级联架构：小型LLM作为第一阶段将长文本压缩为少量潜在token（32或64长度），大型LLM作为第二阶段在压缩后的上下文中执行解码任务。
...
TL;DR: 该论文报告了TeamNRC在BHASHA-Task 1语法错误纠正共享任务中的参与结果，针对5种印度语言使用零/少样本提示方法，在泰卢固语和印地语中分别获得第4和第2名，并扩展到泰米尔语、马拉雅拉姆语和孟加拉语，重点关注数据质量和评估指标问题。


<details>
  <summary>Details</summary>
Motivation: 探索小语言模型在印度语言语法错误纠正任务中的潜力，同时关注高质量数据集创建和适合印度语言脚本的评估指标问题。

Method: 使用零/少样本提示方法，在不同规模的语言模型（4B到大型专有模型）上进行实验。

Result: 在泰卢固语和印地语中分别获得GLEU分数83.78和84.31，排名第4和第2，并扩展到其他三种语言进行进一步分析。

Conclusion: 小语言模型在印度语言语法错误纠正中具有潜力，但需要关注数据质量和评估指标的适当性。
...
TL;DR: 本文使用SetFit框架对阿拉伯方言酒店评论进行情感分析，在官方评估集上获得73%的F1分数，在26个参与者中排名第12。
...
Conclusion: 这项工作突显了少样本学习在处理专业领域（如酒店评论）中细微阿拉伯方言文本时应对数据稀缺问题的潜力。
...
### [84] [Adversarial Poetry as a Universal Single-Turn Jailbreak Mechanism in Large Language Models](https://arxiv.org/abs/2511.15304)
...
TL;DR: 研究发现诗歌形式的对抗性提示可作为通用单轮越狱技术，在25个前沿大语言模型中，诗歌提示的攻击成功率很高，某些提供商超过90%。将1200个有害提示转换为诗歌形式后，攻击成功率比散文基线高出18倍。


<details>
  <summary>Details</summary>
Motivation: 探索风格变异是否能规避当代大语言模型的安全机制，揭示当前对齐方法和评估协议的根本局限性。

Method: 将有害提示转换为诗歌形式，使用标准化元提示生成诗歌变体，通过开放权重评判模型和人工验证子集评估输出，手动解决分歧。
...
### [85] [HEAD-QA v2: Expanding a Healthcare Benchmark for Reasoning](https://arxiv.org/abs/2511.15355)
*Alexis Correa-Guillén,Carlos Gómez-Rodríguez,David Vilares*

Main category: cs.CL

TL;DR: HEAD-QA v2是西班牙语/英语医疗多选推理数据集的扩展版本，包含超过12,000个问题，来自十年西班牙专业考试，用于评估LLM在医疗推理任务上的表现。
...
TL;DR: SPRING Lab开发的ASR系统，用于ASRU MADASR 2.0挑战赛，专注于改进8种语言33种方言的语言和方言识别，在Track 1和Track 2中采用多解码器架构和音素通用标签集作为中间表示。
...
Method: 使用多解码器架构，以音素通用标签集作为中间表示，并探讨了将音素空间增益转换回对应字素表示的各种方法。

Result: 在音素空间性能超过基线，在3种语言上WER/CER击败基线，在所有参赛团队中获得最高的语言ID和方言ID准确率。
...
TL;DR: LLM-MemCluster是一个新颖的LLM原生聚类框架，通过动态内存和双提示策略实现端到端文本聚类，无需复杂外部模块，在多个基准数据集上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在文本聚类中存在两个根本限制：缺乏状态记忆用于迭代优化，以及难以管理聚类粒度，导致需要依赖复杂的外部模块管道，无法实现真正的端到端方法。

Method: 提出LLM-MemCluster框架，将聚类重新定义为完全LLM原生任务，利用动态内存实现状态感知，采用双提示策略让模型推理并确定聚类数量。
...
TL;DR: 本文提出了LPDS数据结构和pelican nlp工具包，旨在解决语言处理领域的数据组织和处理方法标准化问题，构建端到端的可复现处理流程。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型等AI技术的发展，语言数据处理方法不断演进，但面临着数据组织和共享缺乏标准化、处理方法不可复现等挑战。

Method: 1. 提出LPDS数据结构，受神经科学BIDS标准启发，提供文件夹结构和文件命名规范；2. 开发pelican nlp模块化Python包，支持从数据清洗到复杂特征提取的完整处理流程，可通过单一配置文件执行。

Result: LPDS和pelican nlp共同构建了端到端的语言数据处理管道，能够生成预处理后的语言数据或标准化的语言学和声学特征提取结果。

Conclusion: 该研究为解决语言处理领域的标准化和可复现性问题提供了完整解决方案，确保方法透明性并增强研究可复现性。
...
TL;DR: Mera Multi是一个针对俄语的多模态评估框架，包含文本、图像、音频和视频四种模态的18个评估任务，为通用模型和特定模态架构提供基准测试。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏针对俄语的多模态基准测试，且多模态大语言模型的智能、局限性和风险理解不足，需要专门的评估框架。

Method: 构建了包含18个全新评估任务的多模态基准，采用指令式评估，涵盖文本、图像、音频和视频模态，并建立了通用多模态能力分类法。

Result: 提供了闭源和开源模型的基线结果，开发了防止基准泄漏的方法论，包括水印和私有数据集许可。

Conclusion: 该基准为构建斯拉夫语系等类型多样语言的多模态基准提供了可复制的方法论，目前专注于俄语评估。
...
Method: 提出课程调优框架，从初级到高级水平训练模型；构建基于语言学的评估系统，考察语法覆盖、写作错误、词汇句法复杂度和整体评分；构建HSKAgent，在1万篇学习者作文上微调。

Result: 实验结果表明HSKBenchmark不仅能有效建模中文二语习得，还能作为LLM动态写作评估的可靠基准。微调后的LLM写作表现与高级人类学习者相当，并展现出类人习得特征。
...
TL;DR: 本文介绍了FERMAT强化学习环境，用于自动化数学理论发现，并探索了自动评估数学对象趣味性的方法，使用基于LLM的进化算法在初等数论和有限域中发现比硬编码基线更好的趣味性度量。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能中自动化发现新数学理论这一重大挑战，通过构建符号化概念发现和定理证明环境，探索数学对象趣味性的自动评估方法。

Method: 引入FERMAT强化学习环境，使用基于LLM的进化算法，结合函数抽象技术，合成非平凡的趣味性度量函数。
...
TL;DR: Ask WhAI是一个系统级框架，用于检查和扰动多智能体交互中的信念状态。该框架通过记录和重放智能体交互、支持对每个智能体信念和推理的带外查询，以及启用反事实证据注入来测试信念结构对新信息的响应。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体科学推理中的信念形成和认知孤岛，通过可重现的方式使这些动态可见和可测试。

Method: 使用Ask WhAI框架，在医疗案例模拟器中应用多智能体共享内存（时间戳电子病历）和持有真实实验室结果的预言机智能体。对具有强烈角色先验的大型语言模型智能体进行压力测试，在关键诊断时刻设置断点进行前后信念查询。

Result: 模拟显示智能体信念往往反映现实世界学科立场，包括过度依赖规范研究和抵制反证据，这些信念可以以人类专家无法实现的方式进行追踪和询问。

Conclusion: Ask WhAI提供了一种可重现的方法来研究多智能体科学推理中的信念形成和认知孤岛，使这些动态可见和可测试。
...
TL;DR: 本文提出了一种完全自动化的LLM辅助工作流，用于处理灾害数据库中的非结构化位置文本信息，通过GPT-4o清理位置数据并利用三个独立的地理信息库进行几何分配和可靠性评分。
...
Method: 使用GPT-4o处理清理文本位置信息，通过交叉验证GADM、OpenStreetMap和Wikidata三个独立地理信息库来分配几何形状，并根据来源一致性和可用性为每个位置分配可靠性评分。

Result: 应用于2000-2024年EM-DAT数据集，成功地理编码14,215个事件，涉及17,948个独特位置，无需人工干预，覆盖所有灾害类型，支持跨源验证和灵活重映射。
...
Result: Rachel So成功发表了10多篇论文，获得了引用，并收到了同行评审邀请，表明AI身份在学术生态系统中被接受。

Conclusion: 这项工作为关于AI作者身份对出版商、研究人员和科学系统影响的辩论提供了实证数据，强调了在AI时代重新思考学术交流模式的必要性。
...
TL;DR: 提出一种概率方法来量化AI系统数据集的代表性，通过比较场景套件与目标操作域的统计分布特征，使用不精确贝叶斯方法处理有限数据和先验不确定性，生成区间值代表性估计。


<details>
  <summary>Details</summary>
Motivation: 确保AI系统（如自动驾驶车辆）的可信性和安全性依赖于训练和测试数据集的代表性，即场景数据反映系统设计运行条件（ODD）或预期遇到条件（TOD）的程度。

Method: 采用概率方法比较场景套件与TOD的特征统计分布，使用不精确贝叶斯方法处理有限数据和先验不确定性，生成区间值代表性估计而非单一值。

Result: 通过数值示例展示了在依赖性和先验不确定性下，跨操作类别（天气、道路类型、时间等）的场景套件与推断TOD分布的比较，估计了局部和全局代表性区间。

Conclusion: 该方法能够量化数据集代表性，处理数据有限性和不确定性，为AI系统的安全评估提供不确定性感知的代表性度量。
...
### [96] [Learning Human-Like RL Agents Through Trajectory Optimization With Action Quantization](https://arxiv.org/abs/2511.15055)
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习智能体虽然在许多领域超越了人类表现，但行为往往与人类不自然，这引发了可解释性和可信赖性的担忧。目标是设计能够表现出类人行为的强化学习智能体。

Method: 将类人程度建模为轨迹优化问题，引入MAQ框架，使用Vector-Quantized VAE从人类演示中蒸馏出宏动作，并采用后退水平控制作为可扩展实现。

Result: 在D4RL Adroit基准测试中，MAQ显著提高了类人程度，增加了轨迹相似度分数，并在人类评估研究中获得了所有RL智能体中最高的类人程度排名。
...
### [97] [Beyond GeneGPT: A Multi-Agent Architecture with Open-Source LLMs for Enhanced Genomic Question Answering](https://arxiv.org/abs/2511.15061)
...
Result: 在90%以上的基准测试任务中匹配或超越GeneGPT，在Gene-Turing和GeneHop上的平均得分分别为0.849和0.830，延迟降低40-50%。

Conclusion: 开源多智能体系统在基因组问答中具有巨大潜力，OpenBioLLM展示了在不牺牲能力的情况下提高效率和降低成本的可能性。
...
### [98] [ProRAC: A Neuro-symbolic Method for Reasoning about Actions with LLM-based Progression](https://arxiv.org/abs/2511.15069)
...
Method: 提取RAC问题中的动作和问题元素，逐步执行每个动作推导最终状态，然后评估查询与进展状态的关系来获得答案。
...
### [99] [Knowledge-Informed Automatic Feature Extraction via Collaborative Large Language Model Agents](https://arxiv.org/abs/2511.15074)
*Henrik Bradland,Morten Goodwin,Vladimir I. Zadorozhny,Per-Arne Andersen*

Main category: cs.AI

TL;DR: Rogue One是一个基于LLM的多智能体框架，通过三个专业智能体（科学家、提取器、测试器）的协作，结合检索增强生成（RAG）系统，实现知识驱动的自动特征提取，在特征探索和利用之间取得平衡，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自动特征提取方法存在架构单一、反馈机制简单、缺乏领域知识整合等问题，限制了特征工程的质量和效果。

Method: 采用多智能体框架，包含科学家、提取器和测试器三个专业智能体，结合RAG系统整合外部知识，使用"洪泛-剪枝"策略和丰富的定性反馈机制进行迭代特征发现、生成和验证。
...
### [100] [SafeRBench: A Comprehensive Benchmark for Safety Assessment in Large Reasoning Models](https://arxiv.org/abs/2511.15169)
...
Conclusion: SafeRBench为大型推理模型提供了首个端到端的安全评估基准，能够捕捉推理过程中的动态安全风险，为模型安全研究提供了重要工具。
...
### [101] [As If We've Met Before: LLMs Exhibit Certainty in Recognizing Seen Files](https://arxiv.org/abs/2511.15192)
*Haodong Li,Jingqi Zhang,Xiao Cheng,Peihua Mai,Haoyu Wang,Yang Pan*

Main category: cs.AI

TL;DR: COPYCHECK是一个利用不确定性信号检测LLM训练数据中版权内容的新框架，通过将LLM的过度自信转化为优势，可靠区分训练数据和非训练数据。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在大量数据集上训练，其中常包含受版权保护的材料，这引发了未经授权使用的担忧。现有的成员推理攻击方法因LLM的过度自信、缺乏真实训练数据和依赖经验阈值而面临挑战。

Method: COPYCHECK采用双重策略：(1)将文件战略性地分割成小片段以减少对大规模训练数据的依赖；(2)基于不确定性的无监督聚类来消除经验调整阈值的需求。

Result: 实验结果显示，COPYCHECK在LLaMA 7b和LLaMA2 7b上检测已见文件的平均平衡准确率分别达到90.1%和91.6%。相比最先进基线，实现了超过90%的相对改进，最高达到93.8%的平衡准确率，并在GPT-J 6B上保持高性能。
...
### [102] [Efficiency Will Not Lead to Sustainable Reasoning AI](https://arxiv.org/abs/2511.15259)
*Philipp Wiesner,Daniel W. O'Neill,Francesca Larosa,Odej Kao*

Main category: cs.AI

TL;DR: 本文认为，仅靠效率提升无法实现可持续的推理AI，需要将明确限制嵌入到这类系统的优化和治理中。


<details>
  <summary>Details</summary>
Motivation: 随着效率改进接近物理极限，新兴的推理AI缺乏可比较的饱和点，性能不再受可用训练数据量的限制，而是随着训练和推理中的指数级计算投资持续扩展。

Method: 讨论了将明确限制嵌入到推理AI系统优化和治理中的研究和政策方向。

Result: 效率提升无法单独实现可持续的推理AI，需要系统性的限制机制。

Conclusion: 必须将明确限制嵌入到推理AI的优化和治理中，以实现可持续性。
...
### [103] [Realist and Pluralist Conceptions of Intelligence and Their Implications on AI Research](https://arxiv.org/abs/2511.15282)
*Ninell Oldenburg,Ruchira Dhar,Anders Søgaard*

Main category: cs.AI

TL;DR: 本文分析了AI研究中存在的两种基本智力观：智力现实主义认为智力是单一、普遍的跨系统可度量能力；智力多元主义认为智力是多样化的、情境依赖的能力，无法简化为单一通用度量。这两种隐性观点深刻影响着AI研究的方法论、现象解释和风险评估。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究中的许多分歧源于对智力本质的不同隐性理解。作者旨在通过明确区分智力现实主义和智力多元主义这两种基本观点，帮助澄清AI研究中的争议，促进更清晰的学术讨论。

Method: 通过分析当前AI研究中的辩论，揭示这两种智力观如何在不同研究领域中塑造实证证据的解释方式，并具体考察它们对方法论、现象解释和风险评估三个方面的影响。

Result: 研究发现这两种智力观导致根本不同的研究路径：在方法论上影响模型选择、基准设计和实验验证；在解释上产生对相同经验现象（如能力涌现、系统局限）的矛盾解读；在AI风险上产生截然不同的评估——现实主义者关注超级智能风险并寻求统一对齐方案，多元主义者看到跨领域多样化威胁需要情境特定解决方案。

Conclusion: 明确这些基本假设有助于更清晰地理解AI研究中的分歧。将隐性的智力观显性化可以为AI研究提供更坚实的理论基础，促进更富有成效的学术对话。
...
Method: 设计了一个受《文明V》启发的综合性挑战环境，让多个经典RL挑战同时出现，要求智能体具备跨多个交互变量的集成、长视野理解能力。

Result: 提出了Terra Nova环境，排除了仅聚合不相关任务的基准，专注于测试智能体在复杂交互挑战中的深度推理能力。

Conclusion: Terra Nova为强化学习研究提供了一个更全面的测试平台，能够更好地评估智能体在复杂、交互式环境中的综合表现。
...
TL;DR: 本文提出IPR（交互式物理推理器），通过世界模型推演来评估和增强VLM策略，并引入PhysCode物理中心动作编码，在1000+游戏中预训练后，在生存、好奇心和实用性三个层次上表现稳健，整体匹配GPT-5并在好奇心方面超越。


<details>
  <summary>Details</summary>
Motivation: 研究智能体是否能够通过交互学习获得类人推理能力，并随着经验增加持续改进，探索以物理为中心的交互作为提升物理推理能力的路径。

Method: 提出IPR框架，使用世界模型推演来评分和增强VLM策略，引入PhysCode物理中心动作编码，提供预测和推理的共享动作空间，在1000+异构游戏上进行预训练。

Result: IPR在三个推理层次上表现稳健，整体性能匹配GPT-5，在好奇心方面超越GPT-5，性能随训练游戏和交互步骤增加而提升，并能零样本迁移到未见游戏。

Conclusion: 物理中心的交互是实现持续改进物理推理能力的有效路径，模型能够通过更多训练游戏和交互步骤不断提升性能。
...
Conclusion: TIM框架为理解DeFi用户动机提供了更可靠的方法，为复杂区块链活动提供情境感知解释。
