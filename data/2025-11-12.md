...
Conclusion: 所提出的方法通过显著性感知和不确定性建模有效解决了细粒度图像-文本对齐的关键挑战，为多模态学习提供了更强大的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 超声成像在临床诊断中应用广泛，但其有限的视野限制了新视角合成。现有方法难以准确模拟超声特有的物理特性，如深度衰减、反射和散射。

Method: 1) 深度感知高斯泼溅策略：为每个高斯分配可学习的视野，实现准确的深度预测和结构表示；2) SH-DARS渲染函数：结合低阶球谐函数和超声特定波物理特性（深度衰减、反射、散射）来精确建模组织强度；3) 贡献临床超声检查数据集作为基准。

Result: 在三个数据集上的广泛实验表明，UltraGS在PSNR（最高29.55）、SSIM（最高0.89）和MSE（低至0.002）方面达到最先进水平，同时实现64.69 fps的实时合成。

Conclusion: UltraGS通过深度感知的高斯泼溅和超声特定的物理建模，显著提升了超声图像新视角合成的质量和效率，为临床诊断提供了有力工具。
...
TL;DR: VectorSynth是一个基于扩散模型的卫星图像生成框架，通过多边形地理标注和语义属性进行条件生成，支持像素级精确的图像合成和空间编辑。


<details>
  <summary>Details</summary>
Motivation: 现有文本或布局条件模型无法实现像素级精确的卫星图像生成，需要开发能够对齐图像和语义矢量几何的框架，支持细粒度的空间编辑和地图信息内容生成。

Method: 使用视觉语言对齐模块从多边形语义生成像素级嵌入，这些嵌入指导条件图像生成框架，同时尊重空间范围和语义线索，支持语言提示与几何感知条件相结合的交互式工作流。

Result: 在语义保真度和结构真实性方面相比先前方法有显著改进，训练后的视觉语言模型展现出细粒度的空间定位能力。

Conclusion: VectorSynth框架能够实现像素级精确的卫星图像合成，支持快速假设模拟、空间编辑和地图信息内容生成，在语义和结构质量上优于现有方法。
...
TL;DR: 本文提出了PGCD和DAPD方法来解决医学图像分割中的类别增量学习问题，通过原型引导的校准蒸馏和双对齐原型蒸馏来更好地保留旧类知识。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么采用一刀切策略，平等对待所有空间区域和特征通道，阻碍了准确旧知识的保留；要么只关注将局部原型与全局原型对齐，忽略了新数据中的局部表示，导致知识退化。

Method: 提出了原型引导的校准蒸馏（PGCD）和双对齐原型蒸馏（DAPD）。PGCD利用原型到特征的相似性来校准不同空间区域中类别特定的蒸馏强度；DAPD将当前模型提取的旧类局部原型与全局原型和局部原型对齐。

Result: 在两个广泛使用的多器官分割基准上的综合评估表明，该方法优于最先进的方法，突显了其鲁棒性和泛化能力。

Conclusion: 所提出的PGCD和DAPD方法有效解决了类别增量医学图像分割中的知识保留问题，通过原型引导的校准和双对齐机制显著提升了旧类别的分割性能。
...
TL;DR: 本文揭示扩散模型中的初始噪声包含可分析的语义模式，提出基于信息论的语义擦除-注入方法，无需训练即可优化生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为扩散模型的初始噪声是随机元素，仅贡献生成多样性。但本文发现噪声中蕴含丰富的语义信息，可以通过简单方法进行语义操控。

Method: 提出基于信息论的两步法：首先从噪声中擦除不需要的语义，然后利用扩散模型生成过程与语义注入的等价性，将所需语义注入清理后的噪声。

Result: 实验证明该方法在基于DiT和UNet架构的各种文本驱动内容生成模型中均有效，提供了一种通用的优化生成工具。

Conclusion: 该方法为扩散模型生成优化提供了新视角，通过噪声语义操控实现一致生成，是一种无需训练的通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中多标签数据集普遍存在标签噪声，严重影响跨模态检索性能；现有方法忽略多标签数据的部分语义重叠，限制了鲁棒性和泛化能力

Method: SCBCH框架包含两个模块：跨模态语义一致性分类（CSCC）利用跨模态语义一致性估计样本可靠性减少噪声标签影响；双向软对比哈希（BSCH）基于多标签语义重叠动态生成软对比样本对，实现跨模态语义相似和相异样本的自适应对比学习

Result: 在四个广泛使用的跨模态检索基准上的大量实验验证了方法的有效性和鲁棒性，在噪声多标签条件下持续优于最先进方法
...
Method: 提出DCDNet，包含三个模块：ACFD模块通过对比学习和对抗学习解耦特征；MGDF模块动态融合基础、共享和私有特征；CAM模块在微调阶段通过调制机制增强泛化。

Result: 在四个具有挑战性的数据集上的大量实验表明，DCDNet超越了现有的CD-FSS方法，在跨域泛化和少样本适应方面达到了新的最先进水平。
...
TL;DR: PC-Diffusion是一个用于扩散模型人类偏好对齐的新框架，通过轻量级可训练偏好分类器直接建模样本间相对偏好，避免了整个模型微调和参考模型依赖，显著降低了训练成本并实现了高效稳定的偏好引导生成。


<details>
  <summary>Details</summary>
Motivation: 现有DPO方法在扩散模型中存在两个关键限制：1）高计算成本（需要整个模型微调）；2）对参考模型质量敏感（导致不稳定性和偏差）。需要克服这些限制来改善扩散模型与人类偏好的对齐。

Method: 提出PC-Diffusion框架，使用轻量级可训练偏好分类器直接建模样本间相对偏好，将偏好学习限制在分类器中，从而解耦偏好对齐与生成模型，无需整个模型微调和参考模型依赖。

Result: PC-Diffusion在实现与DPO相当的偏好一致性的同时，显著降低了训练成本，并实现了高效稳定的偏好引导生成。

Conclusion: PC-Diffusion通过解耦偏好对齐与生成模型，提供了一种计算效率高且稳定的方法来解决扩散模型的人类偏好对齐问题，具有理论保证和实证有效性。
...
TL;DR: 本文提出了一种用于SAR土地覆盖分类的通用基础模型DI3CL，通过动态实例和轮廓一致性对比学习框架，在大规模SARSense数据集上预训练，在多个下游任务中表现出优越性能。
...
Method: 提出了动态实例和轮廓一致性对比学习（DI3CL）预训练框架，包含动态实例模块（增强全局上下文感知）和轮廓一致性模块（关注SAR地物几何轮廓），并在包含460,532张SAR图像的大规模SARSense数据集上进行预训练。

Result: 在SAR土地覆盖制图、水体检测和道路提取等多种任务上的实验结果表明，DI3CL方法优于现有方法。

Conclusion: DI3CL作为SAR土地覆盖分类的基础模型，为下游模型开发提供了强大基础，显著提升了泛化能力和性能表现。
...
TL;DR: 提出Q-Scorer框架，通过轻量级回归模块和IQA专用评分标记解决MLLMs在图像质量评估任务中离散输出与连续评分不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图像质量评估任务中存在离散标记输出与连续质量评分之间的不匹配问题，这严重影响了MLLM-based IQA方法的性能。之前的转换方法存在转换误差，且级别标记引入的语义混淆进一步限制了性能。

Method: 提出Q-Scorer框架，包含轻量级回归模块和IQA专用评分标记，集成到MLLM流程中。

Result: 在多个IQA基准测试中达到最先进性能，在混合数据集上泛化良好，与其他方法结合时性能进一步提升。

Conclusion: Q-Scorer通过理论分析和简单有效的设计，成功解决了MLLMs在IQA任务中的关键挑战，取得了优异的性能表现。
...
Conclusion: Sparse3DPR通过结合预训练LLM的推理能力和创新的场景表示方法，为免训练3D场景理解提供了高效准确的解决方案，具有很好的实用价值。
...
TL;DR: 本研究探讨了合成相关扩散成像（CDI^s）作为标准扩散成像协议的增强方法，用于提升前列腺癌病灶分割性能。通过对200名患者的六种最先进分割架构进行综合评估，发现CDI^s集成在94%的配置中可靠地提升或保持了分割性能，最高可实现72.5%的相对改进。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习的前列腺癌病灶分割方法在大规模患者队列中表现有限，Dice评分仅为0.32或更低。需要开发更有效的成像增强方法来提升分割性能。

Method: 使用200名患者的共配准CDI^s、DWI和ADC序列，在六种最先进的分割架构上进行全面评估。CDI^s从现有DWI采集数据中衍生，无需额外扫描时间或架构修改。

Result: CDI^s集成在94%的评估配置中可靠地提升或保持了分割性能，单个架构最高实现72.5%的统计显著相对改进。CDI^s + DWI组合是最安全的增强路径，在一半评估架构中实现显著改进且无性能下降实例。

Conclusion: CDI^s作为现有DWI采集的衍生技术，无需额外扫描时间或架构修改，可在临床工作流中立即部署，为前列腺癌病灶分割任务提供了实用的即插即用增强方案。
...
TL;DR: 提出了SSOMotion框架，通过统一的场景语义占据（SSO）表示来合成3D场景中的人体运动，结合场景语义理解和结构分析，在杂乱场景中实现高性能运动合成。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景中人体运动合成方法主要关注场景结构而忽略语义理解，需要同时考虑场景结构和语义信息来提升运动合成的质量和准确性。

Method: 设计双向三平面分解获得紧凑的SSO表示，通过CLIP编码和共享线性降维将场景语义映射到统一特征空间，结合场景提示和运动方向进行逐帧场景查询的运动控制。

Result: 在ShapeNet家具、PROX和Replica数据集上的实验表明，该方法在杂乱场景中达到领先性能，验证了其有效性和泛化能力。

Conclusion: SSOMotion框架通过统一的场景语义占据表示，成功实现了结合场景语义和结构的3D人体运动合成，在多个数据集上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D物体检测存在深度模糊和视野受限的问题，导致几何线索不足，在遮挡或截断场景中精度降低。现有方法虽然引入了额外深度信息，但忽略了视觉线索对鲁棒识别的重要性。
...
TL;DR: 本文提出了一种基于流匹配的通用视觉感知框架，能够跨多个任务生成不同的视觉表示，突破了传统"单任务-单模型"的限制。


<details>
  <summary>Details</summary>
Motivation: 受到大语言模型跨领域泛化能力的启发，旨在解决当前扩散模型在视觉任务中局限于单一任务模型的问题，提高模型在多任务场景下的通用性和可扩展性。

Method: 将视觉感知过程构建为从图像块标记到任务特定表示的通用流匹配问题，利用自监督基础模型作为锚点，引入多尺度循环任务嵌入机制，学习通用速度场来桥接异构任务之间的差距。

Result: 在分类、检测、分割、深度估计和图像文本检索等任务上的广泛实验表明，该模型在零样本和微调设置下均取得了有竞争力的性能，超越了先前的一般模型和多个专业模型。
...
TL;DR: 提出了一种分层自回归的草图生成方法，通过三个阶段（笔画预测、锚定和翻译）逐个生成笔画，支持在生成过程中随时调整笔画嵌入来实现灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要在生成前一次性提供所有笔画条件，无法在生成过程中进行动态调整。为了更灵活地控制草图绘制过程，需要支持在生成过程中随时进行笔画级别的操作。

Method: 采用分层自回归生成过程：1）预测笔画嵌入表示要绘制的笔画；2）在画布上锚定预测的笔画位置；3）将嵌入转换为绘制动作序列。整个过程自回归进行，考虑已生成笔画及其位置来预测下一个笔画。

Result: 实现了在草图生成过程中随时调整笔画嵌入的灵活控制能力，能够根据已生成笔画和位置动态预测合适的后续笔画。

Conclusion: 提出的分层自回归方法为草图生成提供了更灵活的笔画级控制，支持在生成过程中随时进行编辑操作，提高了草图绘制的可控性和灵活性。
...
TL;DR: 本文对文本极性检测和图像二值化中的功率律变换现象进行了理论分析，解释了为什么暗文本在亮背景上时类间方差会增加，而亮文本在暗背景上时类间方差会减少。


<details>
  <summary>Details</summary>
Motivation: 文本极性检测是图像二值化的重要预处理步骤，现有文献中基于功率律变换的直观方法观察到了类间方差随文本极性的变化现象，但缺乏理论解释。

Method: 对功率律变换后的图像进行理论分析，研究文本和背景作为两个类别时，类间方差随文本极性的变化规律。

Result: 理论分析证实了经验观察：暗文本在亮背景上时最大类间方差增加，亮文本在暗背景上时最大类间方差减少。

Conclusion: 本文为文本极性检测中的功率律变换现象提供了理论依据，完善了图像二值化的理论基础。
...
TL;DR: 该论文提出了首个大规模细粒度水下分割数据集AquaOV255和首个水下开放词汇分割基准UOVSBench，并开发了无需训练的地球到海洋框架Earth2Ocean，通过几何引导视觉掩码生成器和类别-视觉语义对齐模块，将陆地视觉语言模型迁移到水下领域。


<details>
  <summary>Details</summary>
Motivation: 现有分割数据集和模型主要局限于陆地场景，缺乏针对水下生物分割的大规模数据集和基准，限制了海洋生物多样性监测和生态评估的发展。

Method: 提出Earth2Ocean框架，包含两个核心组件：几何引导视觉掩码生成器(GMG)通过自相似几何先验优化视觉特征以感知局部结构；类别-视觉语义对齐模块(CSA)通过多模态大语言模型推理和场景感知模板构建增强文本嵌入。

Result: 在UOVSBench基准上的广泛实验表明，Earth2Ocean实现了显著的性能提升，同时保持高效的推理速度。

Conclusion: 该工作填补了水下开放词汇分割领域的空白，为海洋生物监测提供了有效的解决方案，证明了无需额外水下训练即可将陆地视觉语言模型成功迁移到水下领域。
...
TL;DR: 本文提出HD²-SSC框架，通过高维语义解耦模块和高密度占用优化模块，解决相机3D语义场景补全中的维度差距和密度差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有SSC方法存在输入输出维度差距和标注-现实密度差距，2D平面视图和稀疏标注导致对真实世界3D密集占用的预测效果不佳。

Method: 使用高维语义解耦模块扩展2D图像特征到伪第三维度，解耦粗粒度像素语义；高密度占用优化模块采用'检测-优化'架构，利用上下文几何和语义结构增强语义密度。
...
TL;DR: 本文提出了一种基于图像的路径规划算法，使用计算机视觉技术生成地形视差图来定义路径候选点，并与A*和PRM算法进行比较验证。


<details>
  <summary>Details</summary>
Motivation: 传统二维图像无法区分地形中的坑洼和山丘等深度信息，这会影响路径规划的安全性，因此需要结合深度信息来改进路径规划算法。

Method: 使用无人机生成地形视差图，应用边缘、直线和角点检测等计算机视觉技术，结合立体深度重建技术来定义路径候选点，使用ArUco标记姿态估计和圆检测技术自动识别起始点和目标点。

Result: 在V-REP仿真程序和实验室物理环境中进行测试，与A*和PRM算法相比，提出的算法表现良好，证明了其有效性。

Conclusion: 提出的基于计算机视觉的路径规划算法能够有效利用地形深度信息，提高路径规划的安全性和准确性，实验结果令人满意。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像分析中隐私保护需求与深度学习模型性能之间的平衡问题，应对联邦学习中的数据异构性和资源成本挑战，特别是视觉语言模型在联邦学习中的部署困难。

Method: 1) 使用掩码特征适应模块(FAM)作为通信模块减少通信负载；2) 冻结CLIP编码器降低计算开销；3) 采用掩码MLP作为本地分类器；4) 设计自适应KL散度蒸馏正则化方法实现FAM与MLP互学习；5) 结合模型压缩传输FAM参数并使用集成预测进行分类。

Result: 在四个公开医学数据集上的实验表明，该方法在ISIC2019数据集上比次优基线性能提高8%，且资源成本显著降低（比FedAVG快120倍）。

Conclusion: FedMedCLIP方法在保持合理资源成本的同时提供了可行的性能，为医学图像联邦学习提供了有效的解决方案。
...
TL;DR: DiffRegCD是一个统一的密集配准和变化检测框架，将对应关系估计重新定义为高斯平滑分类任务，利用预训练扩散模型的多尺度特征，在多种数据集上表现优于现有基线方法。
...
Method: 将对应关系估计重新定义为高斯平滑分类任务以实现亚像素精度；利用预训练去噪扩散模型的冻结多尺度特征；通过标准CD数据集上的受控仿射扰动提供监督，无需伪标签。

Result: 在航空（LEVIR-CD、DSIFN-CD、WHU-CD、SYSU-CD）和地面（VL-CMU-CD）数据集上的广泛实验表明，DiffRegCD始终优于最近的基线方法，在广泛的时间和几何变化下保持可靠。

Conclusion: 扩散特征和基于分类的对应关系为统一变化检测建立了坚实基础，DiffRegCD在多种数据集上表现优异且稳定。
...
TL;DR: 本文提出ISExplore策略，通过自动选择信息丰富的5秒参考视频片段来优化说话人脸生成，相比传统需要几分钟参考视频的方法，在保持高保真度的同时将数据处理和训练速度提升5倍以上。


<details>
  <summary>Details</summary>
Motivation: 当前基于NeRF或3DGS的说话人脸生成方法需要处理几分钟的参考视频，耗时数小时，严重限制了实际应用价值。研究发现视频信息质量比长度更重要，仅使用几秒的信息丰富片段就能达到甚至超过完整视频的效果。

Method: 提出ISExplore策略，基于三个关键数据质量维度自动识别信息丰富的5秒参考视频片段：音频特征多样性、唇部运动幅度和相机视角数量。

Result: 实验表明该方法在NeRF和3DGS方法中将数据处理和训练速度提升超过5倍，同时保持高保真度的输出质量。
...
TL;DR: 本文提出了一种多模态原型多实例学习方法，通过双向交互机制解决病理图像分析中计算成本高和标签稀疏的问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在病理图像分析中因缺乏细粒度医学知识而产生的偏差问题，以及现有视觉语言多实例学习方法中单向指导的限制。

Method: 使用冻结的大语言模型生成任务特定的病理实体描述作为文本原型，视觉分支学习实例级原型以减少对冗余数据的依赖，并采用基于相似度度量的立体最优传输算法进行融合。
...
TL;DR: 提出了一个纯Mamba架构的人物重识别框架ReIDMamba，通过多粒度特征提取器和排序感知三元组正则化来增强特征鲁棒性，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer在人物重识别中面临的计算复杂度高、内存需求大的问题，同时提升特征的判别性和鲁棒性。

Method: 设计基于Mamba的基线模型，引入多类令牌；开发多粒度特征提取器模块形成多粒度特征；提出排序感知三元组正则化减少特征冗余。

Result: ReIDMamba参数仅为TransReID的三分之一，GPU内存使用更低，推理速度更快，在五个基准测试中达到最先进性能。

Conclusion: 纯Mamba架构在人物重识别中具有显著优势，能够有效平衡性能和效率，为相关研究提供了新的方向。
...
TL;DR: 该研究针对胸部X射线图像中罕见但临床重要的骨折病理检测和描述问题，开发了专门的骨折特异性视觉语言模型，相比通用模型在生成准确骨折描述方面有显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前通用放射学报告生成模型在描述罕见但临床重要的病理（如骨折）方面存在不足，需要开发专门针对骨折病理检测和描述的模型。

Method: 使用MAIRA-2和CheXagent的编码器训练骨折特异性视觉语言模型，分析模型输出在骨折类型、位置和年龄方面的表现。

Result: 骨折特异性模型在生成准确骨折描述方面显著优于通用模型，不同视觉语言模型架构在骨折检测和描述方面展现出不同的优势和局限性。

Conclusion: 开发了高性能的骨折报告模型并公开发布，为未来准确报告罕见病理的研究提供了支持。
...
### [27] [Hardware-Aware YOLO Compression for Low-Power Edge AI on STM32U5 for Weeds Detection in Digital Agriculture](https://arxiv.org/abs/2511.07990)
*Charalampos S. Kouzinopoulos,Yuri Manna*

Main category: cs.CV

TL;DR: 本文提出了一种基于YOLOv8n的优化低功耗边缘AI系统，用于在STM32U575ZI微控制器上实现杂草检测，通过多种压缩技术实现实时检测，能耗仅为51.8mJ/推理。
...
Result: 系统实现了检测精度与效率的平衡，支持实时原位杂草检测，每推理能耗仅51.8mJ，可在功率受限的农业环境中规模化部署。

Conclusion: 该低功耗边缘AI系统为可持续农业提供了可行的杂草管理解决方案，能够在资源受限环境下实现高效杂草检测。
...
### [28] [Sharp Eyes and Memory for VideoLLMs: Information-Aware Visual Token Pruning for Efficient and Reliable VideoLLM Reasoning](https://arxiv.org/abs/2511.08003)
*Jialong Qin,Xin Zou,Di Lu,Yibo Yan,Xuming Hu*

Main category: cs.CV

TL;DR: SharpV是一种高效的自适应视觉令牌和KV缓存剪枝方法，通过动态调整剪枝比例和基于信息瓶颈的分层缓存剪枝，解决了VideoLLMs中的二次计算复杂度和KV缓存扩展问题。
...
Method: SharpV采用自适应剪枝机制，基于时空信息动态调整剪枝比例，并在KV缓存剪枝阶段通过自校准方式剪枝退化的视觉特征，实现分层缓存剪枝。

Result: 在多个公开基准测试中表现出优越性，甚至在某些情况下性能超过密集模型，且无需访问注意力分数，完全兼容硬件加速技术。

Conclusion: SharpV为VideoLLMs提供了一种新的自适应剪枝范式，从信息瓶颈角度提供了对模型信息流的新见解。
...
### [29] [EAGLE: Episodic Appearance- and Geometry-aware Memory for Unified 2D-3D Visual Query Localization in Egocentric Vision](https://arxiv.org/abs/2511.08007)
*Yifei Cao,Yu Liu,Guolong Wang,Zhu Liu,Kai Wang,Xianjie Zhang,Jizhe Yu,Xun Tu*

Main category: cs.CV

TL;DR: EAGLE框架通过外观感知元学习记忆和几何感知定位记忆的统一机制，在自我中心视觉中实现了2D-3D视觉查询定位的突破，在Ego4D-VQ基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决自我中心视觉查询定位中因相机运动、视角变化和外观变化带来的挑战，需要开发能够统一处理2D和3D任务的鲁棒方法。

Method: 提出EAGLE框架，集成外观感知元学习记忆（AMM）引导的分割和几何感知定位记忆（GLM）驱动的跟踪，通过结构化外观和几何记忆库存储高置信度检索样本，支持目标外观变化的长期和短期建模。

Result: 在Ego4D-VQ基准上实现了最先进的性能，能够精确轮廓描绘和鲁棒空间判别，显著提高了检索准确性。

Conclusion: EAGLE通过记忆整合机制有效解决了自我中心视觉查询定位的核心挑战，实现了2D-3D任务的高效统一，为具身AI和VR/AR应用提供了可靠解决方案。
...
### [30] [Invisible Triggers, Visible Threats! Road-Style Adversarial Creation Attack for Visual 3D Detection in Autonomous Driving](https://arxiv.org/abs/2511.08015)
*Jian Wang,Lijun He,Yixing Yong,Haixia Bi,Fan Li*

Main category: cs.CV

TL;DR: AdvRoad是一种生成多样化道路风格对抗性海报的方法，通过在道路表面放置自然外观的对抗性图案，诱导3D物体检测器感知不存在的物体，实现隐蔽攻击。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的3D物体检测系统容易受到对抗性攻击，但现有方法使用不自然的对抗性海报容易被人类察觉且容易被防御。需要开发更隐蔽、自然的攻击方法。

Method: 采用两阶段方法：道路风格对抗性生成和场景关联适应，确保对抗性海报具有自然道路外观同时最大化攻击效果。

Result: 实验表明AdvRoad在不同检测器、场景和欺骗位置上都表现良好，物理攻击进一步证明了其在真实环境中的实际威胁。

Conclusion: AdvRoad展示了在自动驾驶场景中生成自然外观对抗性攻击的可行性，揭示了现有3D检测系统的安全隐患。
...
### [31] [High-Quality Proposal Encoding and Cascade Denoising for Imaginary Supervised Object Detection](https://arxiv.org/abs/2511.08018)
*Zhiyuan Chen,Yuelin Guo,Zitong Huang,Haoyu He,Renhao Lu,Weizhe Zhang*

Main category: cs.CV

TL;DR: 本文提出了Cascade HQP-DETR方法，通过高质量合成数据生成、基于高质量建议的查询编码和级联去噪算法，解决了合成数据训练对象检测模型的三个主要问题，在仅使用合成数据训练12个epoch的情况下，在PASCAL VOC 2007上达到了61.04% mAP@0.5的SOTA性能。
...
Conclusion: Cascade HQP-DETR通过高质量合成数据生成、图像特定先验查询初始化和自适应去噪策略，有效解决了ISOD的关键挑战，为减少对象检测对大规模人工标注数据的依赖提供了可行方案。
...
### [32] [Perceptual Quality Assessment of 3D Gaussian Splatting: A Subjective Dataset and Prediction Metric](https://arxiv.org/abs/2511.08032)
*Zhaolin Wan,Yining Diao,Jingqi Xu,Hao Wang,Zhiyang Li,Xiaopeng Fan,Wangmeng Zuo,Debin Zhao*

Main category: cs.CV

TL;DR: 本文提出了首个针对3D高斯泼溅(3DGS)的主观质量评估数据集3DGS-QA，并开发了一种无需参考的质量预测模型，直接在3D高斯基元上操作，无需渲染图像或真实参考。


<details>
  <summary>Details</summary>
Motivation: 随着3DGS技术的快速发展，虽然已有研究关注算法性能和视觉保真度，但在不同重建条件下3DGS渲染内容的感知质量仍未被系统研究。实践中，视角稀疏性、有限训练迭代、点云下采样、噪声和颜色失真等因素会显著降低视觉质量，但其感知影响尚未得到系统分析。

Method: 构建了包含15种对象类型、225个退化重建的3DGS-QA数据集；提出了无需参考的质量预测模型，直接从3D高斯表示中提取空间和光度线索，以结构感知的方式估计感知质量；对现有质量评估方法进行了基准测试。

Result: 实验结果表明，该方法在3DGS内容评估中始终实现优越性能，突出了其鲁棒性和有效性。

Conclusion: 3DGS-QA数据集和提出的质量预测模型填补了3DGS感知质量评估的研究空白，为未来3DGS质量评估研究提供了基础。数据集和代码已公开可用。
...
### [33] [WEDepth: Efficient Adaptation of World Knowledge for Monocular Depth Estimation](https://arxiv.org/abs/2511.08036)
...
### [34] [Generalized-Scale Object Counting with Gradual Query Aggregation](https://arxiv.org/abs/2511.08048)
...
<details>
  <summary>Details</summary>
Motivation: 现有的少样本计数器在处理包含不同大小对象和密集小对象区域的图像时存在困难，因为它们采用临时解决方案（如上采样和分块）来应对计算和内存需求。

Method: 提出GECO2方法，使用新的密集查询表示在多尺度上逐步聚合特定示例的特征信息，生成高分辨率密集查询，从而能够检测大小不同的对象。

Result: GECO2在计数和检测精度上比现有最先进方法提升10%，同时运行速度快3倍，GPU内存占用更小。
...
### [35] [Taming Identity Consistency and Prompt Diversity in Diffusion Models via Latent Concatenation and Masked Conditional Flow Matching](https://arxiv.org/abs/2511.08061)
...
<details>
  <summary>Details</summary>
Motivation: 主题驱动图像生成需要在保持核心身份特征的同时实现多样化的上下文合成，但身份一致性和提示多样性之间存在根本性权衡。

Method: 使用LoRA微调扩散模型，采用潜在空间拼接策略联合处理参考图像和目标图像，结合掩码条件流匹配目标；引入两阶段蒸馏数据策展框架，第一阶段通过数据恢复和VLM过滤创建高质量种子数据集，第二阶段用于参数高效微调；提出CHARIS细粒度评估框架进行质量评估。

Result: 该方法能够在无需架构修改的情况下实现鲁棒的身份保持，并扩展生成能力到各种主题和上下文。

Conclusion: 提出的方法有效解决了主题驱动图像生成中身份一致性与提示多样性的权衡问题，通过创新的训练策略和评估框架实现了高质量的图像生成。
...
### [36] [Radar-APLANC: Unsupervised Radar-based Heartbeat Sensing via Augmented Pseudo-Label and Noise Contrast](https://arxiv.org/abs/2511.08071)
*Ying Wang,Zhaodong Sun,Xu Cheng,Zuxian He,Xiaobai Li*

Main category: cs.CV

TL;DR: 提出了首个用于雷达心跳感应的无监督框架Radar-APLANC，通过增强伪标签和噪声对比方法，在不依赖昂贵生理信号标签的情况下实现与有监督方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统雷达心跳感应方法因噪声而性能下降，基于学习的方法需要昂贵的标记信号进行监督训练，需要克服这些限制。

Method: 使用雷达距离矩阵中的心跳范围和噪声范围分别构建正负样本，提出噪声对比三元组损失(NCT)，仅使用正样本、负样本和传统雷达方法生成的伪标签信号，避免依赖真实生理信号。设计了具有自适应噪声感知标签选择的伪标签增强方法。

Result: 在Equipleth数据集和自收集雷达数据集上的广泛实验表明，该无监督方法实现了与最先进有监督方法相当的性能。

Conclusion: Radar-APLANC是首个用于雷达心跳感应的无监督框架，通过噪声对比学习和伪标签增强，在不依赖真实标签的情况下实现了优异的性能。
...
### [37] [CLIP is All You Need for Human-like Semantic Representations in Stable Diffusion](https://arxiv.org/abs/2511.08075)
*Cameron Braunstein,Mariya Toneva,Eddy Ilg*

Main category: cs.CV

TL;DR: 本文研究了Stable Diffusion等潜在扩散模型在文本到图像生成过程中内部表示是否包含对人类有意义的语义信息。通过回归层预测语义属性并与人类标注对比，发现语义理解主要来自CLIP文本编码而非扩散过程。


<details>
  <summary>Details</summary>
Motivation: 研究潜在扩散模型（如Stable Diffusion）在生成图像时是否真正理解了图像的语义内容，以及这些语义信息在模型内部是如何表示的。

Method: 在Stable Diffusion上使用简单的回归层来预测对象的语义属性，并将这些预测与人类标注进行比较，分析不同语义属性的解码准确性差异。

Result: 发现语义理解的成功主要归因于CLIP的文本编码，而不是反向扩散过程；不同语义属性的解码准确性差异显著；在反向扩散过程中，属性之间的区分度逐渐降低。

Conclusion: CLIP视觉语言模型决定了类人的语义表示，而扩散过程主要扮演视觉解码器的角色。
...
### [38] [Beyond the Pixels: VLM-based Evaluation of Identity Preservation in Reference-Guided Synthesis](https://arxiv.org/abs/2511.08087)
...
TL;DR: 本文提出了一个名为Beyond the Pixels的层次化评估框架，用于解决生成模型中身份保持评估的挑战。该框架通过结构化推理将身份评估分解为特征级转换，使用(type, style) -> attribute -> feature的决策树，并提示具体转换而非抽象相似度分数。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标依赖全局嵌入或粗略的VLM提示，无法捕捉细粒度身份变化且诊断洞察有限，因此需要开发更精细的身份保持评估方法。

Method: 引入层次化评估框架，将主体分解为(type, style) -> attribute -> feature决策树，通过结构化推理指导VLMs进行具体转换分析，而非抽象相似度评分。

Result: 在四个最先进的生成模型上验证了该框架，显示与人类判断在身份一致性测量方面有强对齐。同时创建了包含1,078个图像-提示对的新基准，涵盖多样化主体类型和6-7个转换轴。

Conclusion: 该框架通过将VLM分析基于可验证的视觉证据，减少了幻觉并提高了一致性，为生成模型的身份保持评估提供了更可靠的方法。
...
### [39] [StableMorph: High-Quality Face Morph Generation with Stable Diffusion](https://arxiv.org/abs/2511.08090)
*Wassim Kabbani,Kiran Raja,Raghavendra Ramachandra,Christoph Busch*

Main category: cs.CV

TL;DR: 提出StableMorph方法，使用基于扩散的图像合成技术生成高质量、无伪影的人脸融合图像，解决了现有方法生成的模糊、有伪影图像容易被检测的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸融合攻击生成方法产生的图像质量差、有伪影，容易被检测，不能代表最危险的攻击场景，需要开发更高质量的融合图像来评估和开发有效的检测系统。

Method: 使用现代基于扩散的图像合成技术生成人脸融合图像，能够生成全头部图像，避免常见视觉缺陷，并提供对视觉属性的无与伦比的控制。

Result: StableMorph生成的图像不仅媲美或超过真实人脸图像质量，还能有效欺骗人脸识别系统，对现有检测解决方案构成更大挑战。

Conclusion: StableMorph通过创建更真实有效的攻击改进了生物识别安全评估，支持开发更鲁棒的检测系统，为研究和操作测试设定了新的融合质量标准。
...
### [40] [Introducing Nylon Face Mask Attacks: A Dataset for Evaluating Generalised Face Presentation Attack Detection](https://arxiv.org/abs/2511.08114)
*Manasa,Sushrut Patwardhan,Narayan Vetrekar,Pavan Kumar,R. S. Gad,Raghavendra Ramachandra*

Main category: cs.CV

TL;DR: 本文介绍了针对尼龙面罩（NFMs）这一新型3D欺骗攻击手段的数据集，使用iPhone 11 Pro收集了3,760个真实样本和51,281个攻击样本，评估了5种先进PAD方法的性能。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统在智能手机认证、门禁控制等应用中广泛部署，但仍易受呈现攻击（PAs）威胁。尼龙面罩因其弹性结构和逼真外观能精确模拟受害者面部几何特征，构成严重安全风险。

Method: 创建了专注于尼龙面罩攻击的新数据集，使用iPhone 11 Pro在真实智能手机使用条件下收集数据，包含100个受试者的3,760个真实样本和51,281个NFM攻击样本，涵盖人类和人体模型的四种不同呈现场景。

Result: 使用5种最先进的PAD方法对数据集进行基准测试，结果显示不同方法在未见攻击条件下的性能存在显著差异，突显了NFM带来的挑战。

Conclusion: 尼龙面罩对现有PAD技术构成严峻挑战，强调了开发能够有效泛化到新兴欺骗威胁的PAD技术的重要性。
...
### [41] [LatentPrintFormer: A Hybrid CNN-Transformer with Spatial Attention for Latent Fingerprint identification](https://arxiv.org/abs/2511.08119)
*Arnab Maity,Manasa,Pavan Kumar C,Raghavendra Ramachandra*

Main category: cs.CV

TL;DR: 本文提出了一种名为LatentPrintFormer的新型潜指纹识别方法，该方法结合CNN和Transformer架构，通过空间注意力模块增强脊线区域特征，在公开数据集上优于现有技术。
...
Conclusion: 提出的多模态特征融合方法能有效提升潜指纹识别性能，为低质量指纹识别提供了新的解决方案。
...
### [42] [Foam Segmentation in Wastewater Treatment Plants: A Federated Learning Approach with Segment Anything Model 2](https://arxiv.org/abs/2511.08130)
*Mehmet Batuhan Duman,Alejandro Carnero,Cristian Martín,Daniel Garrido,Manuel Díaz*

Main category: cs.CV

TL;DR: 本文提出了一种结合联邦学习和SAM2图像分割模型的新框架，用于污水处理厂泡沫检测，解决了数据隐私和标注数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 污水处理厂的泡沫形成会降低处理效率和增加成本，但传统机器学习方法需要大量标注数据，且不同工厂因隐私问题不愿共享数据。

Method: 使用联邦学习范式结合SAM2图像分割模型，通过Flower框架在分布式客户端上微调SAM2，中央雾服务器聚合模型权重而不访问私有数据。

Result: 该框架加速了训练收敛，提高了分割性能，即使本地数据集有限也能获得良好效果，并在真实和合成数据集上进行了验证。
...
### [43] [PEOD: A Pixel-Aligned Event-RGB Benchmark for Object Detection under Challenging Conditions](https://arxiv.org/abs/2511.08140)
...
Result: 在完整测试集和正常子集上，融合模型表现优异；但在光照挑战子集上，顶级事件模型优于所有融合模型，融合模型仍优于RGB模型，表明当帧模态严重退化时现有融合方法存在局限。
...
### [44] [Boomda: Balanced Multi-objective Optimization for Multimodal Domain Adaptation](https://arxiv.org/abs/2511.08152)
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习面临标注数据稀缺的问题，而现有的无监督领域自适应方法主要针对单模态场景，在多模态场景下的研究较少。异构多模态领域自适应的主要挑战是不同模态从源域到目标域的领域偏移各不相同。

Method: 首先使用信息瓶颈方法独立学习每个模态的表示，然后在表示空间中使用相关性对齐匹配源域和目标域。将问题表述为多目标任务，寻求帕累托最优解，通过模型特定性质将问题简化为二次规划问题，进一步近似得到闭式解。

Result: 广泛的实证结果表明，所提出的方法有效且优于竞争方案。Boomda在异构多模态领域自适应任务中表现出色。

Conclusion: Boomda方法通过平衡多目标优化有效解决了异构多模态领域自适应问题，为多模态学习在标注数据稀缺情况下的应用提供了有效解决方案。
...
### [45] [Non-Aligned Reference Image Quality Assessment for Novel View Synthesis](https://arxiv.org/abs/2511.08155)
...
<details>
  <summary>Details</summary>
Motivation: 解决新视角合成图像质量评估的挑战，特别是在缺乏像素对齐真实参考的情况下，现有全参考方法在不对齐时失效，而无参考方法泛化能力不足。

Method: 构建大规模合成失真图像数据集，使用对比学习框架结合LoRA增强的DINOv2嵌入，仅使用合成失真数据训练以避免过拟合，提升泛化能力。
...
### [46] [LandSegmenter: Towards a Flexible Foundation Model for Land Use and Land Cover Mapping](https://arxiv.org/abs/2511.08156)
*Chenying Liu,Wei Huang,Xiao Xiang Zhu*

Main category: cs.CV

TL;DR: 本文提出了LandSegmenter，一个用于土地利用与土地覆盖（LULC）映射的基础模型框架，通过弱监督数据、跨模态特征提取和置信度融合策略，在零样本设置下实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 当前LULC模型通常针对特定模态和固定类别分类法开发，限制了通用性。基础模型虽有潜力，但需要大量标注数据或微调，在遥感领域成本高昂且不实用。

Method: 提出LandSegmenter框架：1）输入层面构建LAS数据集，使用现有LULC产品的弱标签；2）模型层面集成遥感特定适配器和文本编码器；3）输出层面采用类别置信度引导融合策略。

Result: 在6个精确标注的LULC数据集上的实验表明，LandSegmenter在迁移学习和零样本设置中达到竞争性或更优性能，特别是在零样本场景下转移到未见数据集时。

Conclusion: 该框架证明了弱监督在构建任务特定基础模型中的有效性，为大规模LULC模型训练提供了可扩展且成本效益高的解决方案。
...
### [47] [KPLM-STA: Physically-Accurate Shadow Synthesis for Human Relighting via Keypoint-Based Light Modeling](https://arxiv.org/abs/2511.08169)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散方法在生成逼真且几何准确的阴影方面仍存在不足，特别是在合成图像中。IC-Light等现有技术无法同时实现高外观真实性和几何精度。
...
Conclusion: 该方法显著提升了合成图像中阴影的真实性和几何精度，为图像合成任务提供了有效的阴影生成解决方案。
...
### [48] [Distributed Zero-Shot Learning for Visual Recognition](https://arxiv.org/abs/2511.08170)
*Zhi Chen,Yadan Luo,Zi Huang,Jingjing Li,Sen Wang,Xin Yu*

Main category: cs.CV

TL;DR: 提出分布式零样本学习框架DistZSL，通过跨节点属性正则器和全局属性-视觉共识解决分布式数据中的异构性问题，提升对未见类别的学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法通常假设数据集中存储，但在实际分布式环境中，数据分布在多个节点且存在异构性，这给零样本学习带来挑战。
...
Conclusion: DistZSL框架通过处理数据异构性问题，能够有效利用分布式数据学习未见类别的模型，显著提升零样本学习性能。
...
### [49] [VLMDiff: Leveraging Vision-Language Models for Multi-Class Anomaly Detection with Diffusion](https://arxiv.org/abs/2511.08173)
*Samet Hicsonmez,Abd El Rahman Shabayek,Djamila Aouada*

Main category: cs.CV

TL;DR: 提出了一种名为VLMDiff的无监督多类视觉异常检测框架，结合潜在扩散模型和视觉语言模型，通过VLM自动生成图像描述作为扩散模型的额外条件，无需人工标注或额外训练即可实现多类异常检测。
...
Method: 集成预训练视觉语言模型和潜在扩散模型，使用VLM自动提取图像描述作为LDM的额外条件输入，学习鲁棒的正常图像特征表示。

Result: 在Real-IAD数据集上像素级PRO指标提升高达25点，在COCO-AD数据集上提升8点，优于现有最先进的基于扩散的方法。

Conclusion: VLMDiff框架通过结合VLM和LDM，无需人工标注即可实现有效的多类视觉异常检测，在多个数据集上表现出色。
...
### [50] [Knowledge-Guided Textual Reasoning for Explainable Video Anomaly Detection via LLMs](https://arxiv.org/abs/2511.07429)
*Hari Lee*

Main category: cs.CV

TL;DR: 提出了基于文本的可解释视频异常检测框架TbVAD，通过语言驱动的方式在文本域内完成异常检测和解释，无需依赖显式视觉特征。


<details>
  <summary>Details</summary>
Motivation: 传统弱监督视频异常检测模型依赖显式视觉特征，缺乏可解释性。TbVAD旨在通过语言表示视频语义，实现可解释和基于知识的推理。

Method: 三阶段框架：1) 使用视觉语言模型将视频内容转换为细粒度描述；2) 将描述组织为四个语义槽（动作、对象、上下文、环境）构建结构化知识；3) 生成槽级解释，揭示哪些语义因素对异常决策贡献最大。

Result: 在UCF-Crime和XD-Violence两个公开基准测试中验证了TbVAD的有效性，证明文本知识推理能为真实世界监控场景提供可解释且可靠的异常检测。

Conclusion: TbVAD框架通过完全在文本域内进行异常检测和解释，实现了基于语言的可解释视频异常检测，为监控应用提供了新的解决方案。
...
### [51] [WarpGAN: Warping-Guided 3D GAN Inversion with Style-Based Novel View Inpainting](https://arxiv.org/abs/2511.08178)
*Kaitao Huang,Yan Yan,Jing-Hao Xue,Hanzi Wang*

Main category: cs.CV

TL;DR: WarpGAN提出了一种新的3D GAN反演方法，通过扭曲和修复策略将图像修复融入3D GAN反演，以解决单视角图像中遮挡区域生成质量差的问题。
...
Conclusion: WarpGAN通过结合图像修复技术有效提升了3D GAN反演中遮挡区域的生成质量。
...
### [52] [Pixel-level Quality Assessment for Oriented Object Detection](https://arxiv.org/abs/2511.08186)
*Yunhui Zhu,Buliao Huang*

Main category: cs.CV

TL;DR: 提出了一种新的像素级质量评估框架PQA，用于解决现有目标检测器中基于框级IoU预测的定位质量评估存在的结构耦合问题。


<details>
  <summary>Details</summary>
Motivation: 现有的目标检测器通常通过预测边界框与真实框的IoU来评估定位质量，但这种方法存在结构耦合问题：由于预测框是从检测器对真实框的内部估计推导出来的，对于定位较差的框，预测的IoU可能会被高估。

Method: 提出像素级质量评估框架PQA，用像素级空间一致性集成替代框级IoU预测。PQA测量每个像素相对于预测框的相对位置与其相对于真实框的对应位置之间的对齐程度，避免了直接比较预测框与估计的真实框。

Result: 在HRSC2016和DOTA数据集上的大量实验表明，PQA可以无缝集成到各种定向目标检测器中，持续提高性能（例如，在Rotated RetinaNet上AP50:95提升+5.96%，在STD上提升+2.32%）。

Conclusion: PQA框架通过像素级空间一致性评估，有效解决了框级IoU预测的结构耦合问题，提供了更准确的定位质量近似，显著提升了定向目标检测器的性能。
...
### [53] [UI2Code$^\text{N}$: A Visual Language Model for Test-Time Scalable Interactive UI-to-Code Generation](https://arxiv.org/abs/2511.08195)
...
Conclusion: 交互式UI到代码范式能更好地反映真实工作流程，提高性能上限，UI2Code⁰展示了在多模态编码方面的基础性改进。
...
### [54] [Evaluating Gemini LLM in Food Image-Based Recipe and Nutrition Description with EfficientNet-B4 Visual Backbone](https://arxiv.org/abs/2511.08215)
*Rizal Khoirul Anam*

Main category: cs.CV

TL;DR: 本文提出了一种解耦的多模态食物识别流水线，评估了EfficientNet-B4视觉骨干与Gemini LLM的组合，分析了视觉分类准确性、模型效率和生成输出质量之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 数字食品应用的普及需要自动营养分析和烹饪指导的鲁棒方法，同时需要解决公共数据集中存在的文化偏见问题。

Method: 使用解耦的多模态流水线，集成专门的视觉骨干（EfficientNet-B4）与生成式大语言模型（Gemini LLM），并与替代视觉骨干（VGG-16、ResNet-50、YOLOv8）和轻量级LLM（Gemma）进行基准测试。引入"语义错误传播"（SEP）形式化分析视觉模块分类错误如何级联到生成输出中。

Result: 实验结果表明，EfficientNet-B4（89.0% Top-1准确率）在准确性和效率之间提供了最佳平衡，Gemini（9.2/10事实准确性）提供了优越的生成质量，但系统的整体效用受到视觉前端感知准确性的根本瓶颈限制。

Conclusion: 系统的整体性能受到视觉前端准确性的限制，高语义相似度被识别为最关键的错误模式，视觉分类的准确性对生成输出的质量有决定性影响。
...
### [55] [Modulo Video Recovery via Selective Spatiotemporal Vision Transformer](https://arxiv.org/abs/2511.07479)
*Tianyu Geng,Feng Ji,Wee Peng Tay*

Main category: cs.CV

TL;DR: 本文提出了SSViT（选择性时空视觉Transformer），这是首个用于模数视频重建的深度学习框架。该框架通过令牌选择策略提高效率并专注于关键区域，在模数视频恢复方面实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像传感器动态范围有限，模数相机通过将入射辐照度折叠到有限范围内来解决这个问题，但需要专门的展开算法来重建原始信号。尽管模数图像恢复已提出十多年，但在使用现代深度学习技术方面进展缓慢。

Method: 提出了选择性时空视觉Transformer（SSViT）框架，采用令牌选择策略来提高效率并专注于最关键的区域，这是首个用于模数视频重建的深度学习框架。

Result: 实验证实SSViT能够从8位折叠视频中产生高质量重建，并在模数视频恢复方面实现了最先进的性能。

Conclusion: Transformer能够捕捉全局依赖性和时空关系，这对于解决折叠视频帧至关重要，但需要新颖的技术来适应现有的Transformer架构进行模数恢复。
...
### [56] [2D Representation for Unguided Single-View 3D Super-Resolution in Real-Time](https://arxiv.org/abs/2511.08224)
...
<details>
  <summary>Details</summary>
Motivation: 消除对高分辨率RGB引导的需求，简化3D超分辨率流程，使其更适用于实际场景，特别是在无法获取高分辨率RGB数据的情况下。
...
Conclusion: 该几何引导管道为现实场景提供了一个简单而可行的实用解决方案，特别是在无法访问高分辨率RGB数据的情况下。
...
### [57] [Laplacian Score Sharpening for Mitigating Hallucination in Diffusion Models](https://arxiv.org/abs/2511.07496)
*Barath Chandran. C,Srinivas Anumasa,Dianbo Liu*

Main category: cs.CV

TL;DR: 本文提出了一种在推理过程中对扩散模型的得分函数进行后验调整的方法，利用拉普拉斯算子（锐度）来减少无条件扩散模型中的模式插值幻觉，在1D、2D和高维图像数据上均有效。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然成功，但已知存在产生不连贯或不现实样本的幻觉问题。最近的研究将其归因于模式插值和得分平滑现象，但缺乏在采样过程中防止其生成的方法。

Method: 提出在推理过程中对得分函数进行后验调整，利用拉普拉斯算子来减少模式插值幻觉。针对高维情况，使用Hutchinson迹估计器的有限差分变体推导出高效的拉普拉斯近似。

Result: 该方法在玩具1D/2D分布和高维图像数据集上显著减少了幻觉样本的比例。

Conclusion: 该校正方法有效减少了扩散模型中的幻觉问题，同时分析了拉普拉斯算子与得分不确定性之间的关系。
...
TL;DR: 本文提出了一种基于语义和关系改进多模态对齐与融合的方法，通过提取多级语义特征、学习相关语义分组，并使用可继承的交叉注意力来融合视觉和文本特征，在视觉问答和图像描述任务上优于现有方法。
...
Method: 1. 从不同视觉编码器提取多级语义特征以捕捉更多视觉关系线索；2. 学习将视觉特征投影到相关语义组；3. 使用可继承的交叉注意力融合视觉和文本特征，通过丢弃低相关性的视觉-语言特征对来全局去除冗余的视觉关系。
...
### [60] [Toward the Frontiers of Reliable Diffusion Sampling via Adversarial Sinkhorn Attention Guidance](https://arxiv.org/abs/2511.07499)
*Kwanyoung Kim*

Main category: cs.CV

TL;DR: ASAG是一种新颖的扩散模型引导方法，通过最优传输视角重新解释注意力机制，使用Sinkhorn算法故意破坏传输成本来提升生成质量。
...
TL;DR: NERVE是一种无需训练的开源词汇语义分割方法，通过整合全局和局部信息，利用稳定扩散模型的自注意力层邻域结构，并引入随机游走进行亲和力优化，无需传统后处理技术。


<details>
  <summary>Details</summary>
Motivation: 现有无需训练的OVSS方法存在计算成本高、注意力图融合效果差、依赖固定大小高斯核等问题，需要更有效的解决方案。

Method: 结合全局和局部信息，利用稳定扩散模型的自注意力层邻域结构，引入基于熵的不确定性选择相关注意力图，使用随机游走进行亲和力优化。

Result: 在7个流行的语义分割基准测试中实现了最先进的零样本分割性能。

Conclusion: NERVE为开源词汇语义分割提供了一种有效的方法，能够有效描绘任意形状的物体，无需传统后处理技术。
...
TL;DR: 本文提出LayerEdit框架，通过多层解耦编辑解决多目标图像编辑中的注意力纠缠问题，实现无冲突的对象分层编辑。


<details>
  <summary>Details</summary>
Motivation: 现有多目标图像编辑方法主要遵循定位-编辑范式，关注独立对象定位和编辑，但忽视了关键的对象间交互。被忽视的注意力纠缠在对象冲突区域会阻碍解耦的多目标编辑，导致对象间编辑泄漏或对象内编辑约束。
...
Conclusion: LayerEdit是一种无需训练的方法，首次通过精确的对象分层分解和连贯融合，实现了无冲突的对象分层编辑，解决了多目标图像编辑中的注意力纠缠问题。
...
<details>
  <summary>Details</summary>
Motivation: 从航拍视角生成地面图像面临极端视角差异、遮挡和有限视野的挑战，现有方法通常依赖中间表示，限制了生成质量和效率。
...
TL;DR: ImageBindDC是一个新颖的多模态数据压缩框架，在ImageBind的统一特征空间中操作，通过特征函数损失实现更精确的统计对齐，支持单模态、跨模态和联合模态三个层次的分布一致性。
...
Result: 在NYU-v2数据集上，仅用每类5个压缩数据点训练的模型实现了与完整数据集训练相当的无损性能，比之前最佳方法绝对提升8.2%，压缩时间减少超过4倍。

Conclusion: ImageBindDC通过特征函数损失和多层次分布对齐，在多模态数据压缩中实现了最先进的性能，显著提升了压缩效率和模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 在极端条件下（如光线不足、相机剧烈运动），RGB图像信息严重损失，而事件模态虽然高速高动态但存在与RGB的异构性，导致现有多模态方法特征不匹配和优化困难。

Method: 提出边缘感知语义一致性框架：1）边缘感知潜在重编码，通过重编码分布引导将事件-RGB特征对齐到统一语义空间，利用预建边缘字典作为线索；2）重编码整合和不确定性优化，利用重编码边缘特征和不确定性指标解决异构融合问题。

Result: 在两个合成和一个真实世界事件-RGB语义分割数据集上的实验表明，该方法在DERS-XS数据集上比现有最佳方法提升2.55% mIoU，并在空间遮挡下具有优越的鲁棒性。

Conclusion: 所提出的边缘感知语义一致性框架能有效解决极端条件下事件和RGB模态的异构融合问题，显著提升语义分割性能并增强对空间遮挡的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 大规模组织病理学图像标注是开发深度学习模型的主要瓶颈，传统的文件夹标注工作流程缓慢、疲劳且难以扩展。
...
TL;DR: 提出了一种机制感知的无监督通用图像融合方法MAUGIF，基于双交叉图像自编码器，根据不同融合任务的内在机制分类为加性和乘性融合，通过双编码器捕获共享内容并隔离模态特定细节，双解码器作为特征注入器选择性整合各模态特征生成融合图像。
...
Method: 基于双交叉图像自编码器，将融合机制分类为加性和乘性，双编码器映射到共享潜在空间捕获共同内容，双解码器根据融合机制变化架构，作为特征注入器选择性整合模态特定特征。

Result: 在多种融合任务上进行了广泛实验，验证了方法的有效性和泛化能力。

Conclusion: MAUGIF方法通过机制感知的融合策略，在保持通用性的同时适应不同融合任务的具体机制，提高了性能和可解释性。
...
TL;DR: SkelSplat是一个基于可微分高斯渲染的多视角3D人体姿态估计框架，无需3D真实标注即可优化3D高斯骨架，在跨数据集和遮挡场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的多视角方法依赖大量标注数据，在测试场景变化时泛化能力差，需要一种无需3D真实标注就能融合多视角信息的方法。

Method: 将人体姿态建模为每个关节的3D高斯骨架，通过可微分渲染优化，提出one-hot编码方案实现关节独立优化，无需3D监督即可融合任意相机视角。

Result: 在Human3.6M和CMU数据集上优于不依赖3D真实标注的方法，相比学习方法将跨数据集误差降低47.8%，在遮挡场景下表现出鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统不断更新，保持连续版本间的一致性至关重要。在图像分类中，模型更新可能导致负翻转问题，即新模型对之前正确分类的样本进行错误分类。随着训练类别数量增加，这个问题变得更加严重，因为新增类别会减少每个类的边界并可能引入冲突模式。

Method: 提出了一种新颖方法：1）通过引入显式的边界校准项来保持原始模型的边界；2）结合双重焦点蒸馏损失，同时从先前模型和新独立训练模型中学习适当的决策边界；3）在边界校准下平衡新旧数据的决策边界学习。

Result: 在图像分类基准测试上的广泛实验表明，该方法在保持高整体准确率的同时，持续降低了负翻转率。

Conclusion: 该方法有效解决了模型更新过程中的负翻转问题，通过边界保持和双重蒸馏策略，在引入新类别时保持了模型的一致性和性能。
...
Result: 基于图像的配准在所有指标上表现最佳，能生成平滑、解剖学上合理的变形场，而将基于图像的变形场应用于特征空间可获得最佳风险预测性能。

Conclusion: 基于图像的变形场在纵向风险建模中至关重要，能提高预测准确性和鲁棒性，有望增强个性化筛查并为高风险个体提供更早干预。
...
<details>
  <summary>Details</summary>
Motivation: 水下实例分割在海洋资源勘探和生态保护中至关重要，而大规模预训练视觉基础模型（如DINO）在复杂下游任务中表现出色，因此探索其在UIS任务中的应用潜力。

Method: 提出DiveSeg框架，包含两个关键组件：(1) AquaStyle Aligner：在水下颜色风格特征嵌入到DINO微调过程中，促进对水下领域的更好适应；(2) ObjectPrior Prompter：通过基于二值分割的提示提供对象级先验，为需要对象级和实例级推理的实例分割任务提供必要指导。

Result: 在UIIS和USIS10K数据集上进行了全面实验，结果显示DiveSeg实现了最先进的性能。

Conclusion: DINO可以作为UIS任务的有效特征学习器，DiveSeg框架通过结合水下风格适应和对象级先验，显著提升了水下实例分割的性能。
...
TL;DR: 提出了一种基于扩散的sEMG手势识别数据增强方法SASG-DA，通过语义表示引导和稀疏感知采样策略，生成既忠实又多样化的训练样本，有效缓解过拟合并提升识别性能。
...
Method: 提出SASG-DA方法：1）语义表示引导机制增强生成忠实性；2）高斯建模语义建模策略实现灵活多样样本生成；3）稀疏感知语义采样策略探索欠表示区域，提升分布覆盖。
...
TL;DR: 提出了一种分层向量量化变分自编码器方法，通过解耦运动伪影特征来实现通用的MRI运动校正，无需针对特定伪影进行训练即可处理未见过的运动模式。


<details>
  <summary>Details</summary>
Motivation: 现有MRI运动校正方法难以泛化到不同运动类型和身体区域，特别是基于机器学习的方法通常针对特定应用和数据集定制。

Method: 使用分层向量量化变分自编码器学习运动到干净图像特征的解耦嵌入，部署码本在多个分辨率捕获有限运动模式集合，结合自回归模型学习无运动图像的先验分布。

Result: 在模拟全身运动伪影上验证，观察到在不同运动严重程度下都能实现鲁棒校正，模型有效解耦了模拟运动扫描的物理运动特征。

Conclusion: 该方法提高了基于机器学习的MRI运动校正的泛化能力，解耦运动特征的方法有望应用于不同解剖区域和运动类型。
...
TL;DR: 本文质疑了RoPE位置编码成功的关键因素，提出了非交换生成器的Spherical RoPE方法，在计算机视觉任务中表现优于传统相对位置编码。


<details>
  <summary>Details</summary>
Motivation: 探讨RoPE位置编码成功的原因，质疑严格等变性在性能中的重要性，并寻找更通用的多维数据位置编码方案。

Method: 提出Spherical RoPE方法，使用非交换生成器替代传统RoPE中的交换生成器，打破了严格等变性的限制。

Result: 实验表明Spherical RoPE在计算机视觉任务中具有与等变方法相当或更好的学习性能。

Conclusion: 相对位置编码在计算机视觉中可能不如普遍认为的重要，这为开发更快、泛化能力更强的视觉位置编码方法开辟了新方向。
...
TL;DR: 本文提出了基于文本的空地行人检索任务(TAG-PR)，通过构建TAG-PEDES数据集和TAG-CLIP检索框架，解决了跨异构视角的行人图像检索问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于文本的行人检索仅关注地面视角图像，而空地视角之间存在巨大视点差异，具有更大的实际意义和独特挑战。

Method: 提出TAG-CLIP框架，包含分层路由专家混合模块学习视角特定和视角无关特征，以及视角解耦策略实现更好的跨模态对齐。

Result: 在提出的TAG-PEDES数据集和现有T-PR基准上验证了TAG-CLIP的有效性。

Conclusion: TAG-PR任务具有重要实用价值，提出的数据集和框架为解决跨视角行人检索问题提供了有效解决方案。
...
TL;DR: RAPTR提出了一种基于弱监督的雷达室内3D人体姿态估计方法，仅使用3D边界框和2D关键点标签，通过两阶段姿态解码器架构实现准确的3D姿态估计。


<details>
  <summary>Details</summary>
Motivation: 传统雷达室内3D人体姿态估计依赖昂贵的3D关键点标注，在复杂室内环境中获取成本高。本文旨在利用更易获取的3D边界框和2D关键点标签进行弱监督学习。

Method: 采用两阶段姿态解码器架构：姿态解码器使用3D模板损失估计初始3D姿态，关节解码器使用2D关键点标签和3D重力损失进行细化，并引入伪3D可变形注意力机制增强多视角雷达特征。

Result: 在两个室内雷达数据集上评估，RAPTR显著优于现有方法，在HIBER数据集上关节位置误差减少34.3%，在MMVR数据集上减少76.9%。

Conclusion: RAPTR证明了在弱监督条件下仅使用3D边界框和2D关键点标签即可实现准确的雷达室内3D人体姿态估计，为低成本、可扩展的姿态估计提供了可行方案。
...
Result: Anatomy-VLM在分布内和分布外数据集上都表现出色，在下游图像分割任务中验证了其性能，表明其细粒度对齐捕获了解剖和病理相关知识。

Conclusion: Anatomy-VLM的编码器支持零样本解剖学解释，提供了强大的专家级临床解释能力，通过模拟人类临床工作流程实现了准确的疾病诊断。
...
TL;DR: CoMa是一种压缩预训练方法，作为对比学习的预热阶段，能够将视觉语言模型转化为具有竞争力的嵌入模型，仅需少量预训练数据即可在MMEB基准上实现最先进性能。
...
Method: 提出CoMa压缩预训练阶段作为对比学习的预热阶段，通过少量预训练数据将视觉语言模型转化为嵌入模型，实现效率和效果的双重优化。

Result: 实验表明，仅使用少量预训练数据，CoMa就能在MMEB基准上实现与同类规模视觉语言模型相比的最新最优结果。

Conclusion: CoMa方法证明了解耦预训练和对比学习的有效性，为视觉语言模型的高效嵌入学习提供了新思路。
...
Method: 通过公民科学平台eBird收集数据，构建包含40,000多名参与者、1,700万道选择题、涵盖10,000多种鸟类物种的大规模知识追踪基准数据集。

Result: 追踪学习者的知识具有挑战性，特别是在不同参与者子组和问题类型之间，不同形式的上下文信息提供不同程度的预测效益。

Conclusion: CleverBirds是同类中最大的基准之一，提供了显著更多的可学习概念，有望为研究视觉专业知识随时间和个体发展的新途径提供支持。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用需要复杂的迭代工作流来结合视频处理的各种能力，而现有专用AI模型只能处理孤立任务，无法满足这种需求。

Method: 采用Plan-and-Act双智能体架构：规划智能体解释用户意图并分解为结构化步骤，执行智能体通过模块化MCP工具服务器执行分析、生成、编辑等任务，使用分层多级内存维持长期推理和上下文连续性。
...
### [81] [3D4D: An Interactive, Editable, 4D World Model via 3D Video Generation](https://arxiv.org/abs/2511.08536)
*Yunhong He,Zhengqing Yuan,Zhengzhong Tu,Yanfang Ye,Lichao Sun*

Main category: cs.CV

TL;DR: 3D4D是一个交互式4D可视化框架，通过结合WebGL和Supersplat渲染技术，将静态图像和文本转换为连贯的4D场景，并采用注视点渲染策略实现高效实时多模态交互。
...
Method: 构建了包含四个核心模块的框架，集成WebGL与Supersplat渲染技术，并采用注视点渲染策略优化实时交互性能。

Result: 成功实现了将静态图像和文本转换为连贯4D场景的能力，支持高效实时的多模态交互，项目代码已开源。

Conclusion: 3D4D框架为复杂4D环境的自适应、用户驱动探索提供了有效的解决方案，展示了在4D可视化领域的应用潜力。
...
### [82] [RePose-NeRF: Robust Radiance Fields for Mesh Reconstruction under Noisy Camera Poses](https://arxiv.org/abs/2511.08545)
...
<details>
  <summary>Details</summary>
Motivation: 现实环境中获取精确相机位姿具有挑战性，现有基于NeRF的方法严重依赖准确的外参估计，且其隐式体积表示与广泛采用的多边形网格差异很大，导致在标准3D软件中渲染和操作效率低下

Method: 联合优化相机位姿同时学习隐式场景表示，捕获精细几何细节和逼真外观，生成与常见3D图形和机器人工具兼容的网格

Result: 在标准基准测试中证明该方法在位姿不确定性下实现准确鲁棒的3D重建
...
### [83] [Vision Transformer Based User Equipment Positioning](https://arxiv.org/abs/2511.08549)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习定位模型存在两个主要缺点：对全部输入给予相同注意力权重；不适用于非顺序数据（如瞬时CSI）。

Method: 采用基于注意力机制的视觉变换器架构，重点关注CSI矩阵中的角度延迟剖面特征。

Result: 在DeepMIMO数据集上室内RMSE为0.55m，室外为13.59m；在ViWi数据集上室外阻塞场景RMSE为3.45m，比最先进方案提升约38%。
...
TL;DR: 提出SENCA-st架构，通过共享编码器和邻域交叉注意力机制，有效整合组织病理学图像和空间转录组学数据，在保持两种模态特征的同时，强调结构相似但功能不同的区域，显著提升了肿瘤异质性和肿瘤微环境区域的检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前的组织病理学-空间转录组学区域分割方法存在两个极端：要么过度依赖空间转录组学数据，要么使用普通对比学习导致组织病理学图像特征过于突出，都会丢失关键信息。需要一种能够平衡两种模态特征并突出结构相似但功能不同区域的方法。

Method: 提出SENCA-st（共享编码器与邻域交叉注意力）架构，使用共享编码器处理两种模态数据，通过交叉注意力机制强调组织病理学中结构相似但空间转录组学中功能不同的区域。

Result: 模型在检测肿瘤异质性和肿瘤微环境区域方面表现出色，超越了现有最先进方法，在临床关键方面具有显著优势。

Conclusion: SENCA-st架构能够有效整合多模态数据，在保持各自特征的同时突出重要区域差异，为癌症药物耐药性相关的肿瘤亚结构识别提供了有力工具。
...
### [85] [A Preliminary Study of RAG for Taiwanese Historical Archives](https://arxiv.org/abs/2511.07445)
*Claire Lin,Bo-Han Feng,Xuanjun Chen,Te-Lun Yang,Hung-yi Lee,Jyh-Shing Roger Jang*

Main category: cs.CL

TL;DR: 本研究首次将RAG方法应用于台湾历史档案，分析了查询特征和元数据集成策略对检索质量、答案生成和系统性能的影响。


<details>
  <summary>Details</summary>
Motivation: 虽然RAG在知识密集型任务中表现出色，但很少有研究将其应用于台湾历史档案领域，因此需要探索RAG在该领域的适用性。

Method: 构建RAG管道，应用于两个繁体中文历史数据集（热兰遮城和台湾省议会公报），系统研究查询特征和元数据集成策略的影响。

Result: 早期元数据集成能同时提升检索和答案准确性，但也揭示了RAG系统面临的挑战，包括生成过程中的幻觉问题以及处理时间性或多跳历史查询的困难。

Conclusion: RAG在台湾历史档案应用中具有潜力，但需要解决幻觉和时间性查询处理等挑战，元数据集成是提升性能的关键策略。
...
### [86] [Large Language Models for Scientific Idea Generation: A Creativity-Centered Survey](https://arxiv.org/abs/2511.07448)
...
TL;DR: 本调查系统综述了LLM驱动的科学创意生成方法，将其分为五大类：外部知识增强、基于提示的分布引导、推理时缩放、多智能体协作和参数级适应，并使用Boden创造力分类法和Rhodes 4Ps框架来分析不同方法的创造力特性和贡献。


<details>
  <summary>Details</summary>
Motivation: 科学创意生成是科学发现的核心，但LLM在这方面的创造力表现不一致且理解不足。需要系统梳理现有方法，明确不同方法如何平衡创造力与科学严谨性，推动LLM在科学发现中的可靠应用。

Method: 采用结构化综述方法，将现有LLM科学创意生成方法分为五大类，并使用Boden创造力分类法（组合性、探索性、变革性创造力）和Rhodes 4Ps框架（人、过程、压力、产品）来分析比较。

Result: 建立了LLM科学创意生成方法的系统分类框架，揭示了不同方法在创造力类型和侧重点上的差异，为理解LLM创造力提供了理论支撑。

Conclusion: 通过将方法论进展与创造力框架对齐，本调查澄清了该领域现状，并为实现LLM在科学发现中可靠、系统和变革性应用指明了关键方向。
...
### [87] [It Takes Two: A Dual Stage Approach for Terminology-Aware Translation](https://arxiv.org/abs/2511.07461)
*Akshat Singh Jaswal*
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够有效处理术语约束的机器翻译系统，解决传统方法在术语处理上的局限性。

Method: 两阶段架构：第一阶段使用在大型合成数据上微调的术语感知NMT模型，第二阶段使用基于提示的LLM进行后编辑和术语遵守强化。

Result: 在英语到德语、英语到西班牙语和英语到俄语的翻译任务中，LLM作为上下文驱动修改器比严格约束执行产生更高质量的翻译。

Conclusion: LLM最适合作为高质量翻译的上下文驱动修改器而非生成器，揭示了术语处理中的关键权衡。
...
### [88] [Motif 2 12.7B technical report](https://arxiv.org/abs/2511.07464)
...
TL;DR: Motif-2-12.7B是一个新的开放权重基础模型，通过架构创新和系统级优化提升大型语言模型的效率。该模型在受限计算预算下实现可扩展的语言理解和强大的指令泛化能力。
...
Method: 基于Motif-2.6B构建，集成分组差分注意力(GDA)机制，在55万亿token上进行预训练，使用课程驱动的数据调度器，采用MuonClip优化器和自定义高性能内核，包括融合的PolyNorm激活和并行Muon算法。

Result: 在多样化基准测试中表现出竞争力，证明经过深思熟虑的架构扩展和优化训练设计可以与更大模型的性能相媲美。

Conclusion: 通过架构创新和系统级优化，Motif-2-12.7B展示了在受限计算预算下实现高效语言模型的可能性，其性能可与更大模型竞争。
...
### [89] [Focusing on Language: Revealing and Exploiting Language Attention Heads in Multilingual Large Language Models](https://arxiv.org/abs/2511.07498)
...
<details>
  <summary>Details</summary>
Motivation: 多头自注意力在LLM多语言能力中的作用尚未充分探索，需要开发有效方法来理解其机制以提升多语言性能。

Method: 提出LAHIS方法，通过单次前向和反向传播识别注意力头在多语言能力中的重要性，并引入轻量级适配学习软头掩码。

Result: 在Aya-23-8B等模型中发现语言特定和通用注意力头，语言特定头支持跨语言注意力转移，仅需20个可调参数即可提升XQuAD准确率。
...
### [90] [LLM Optimization Unlocks Real-Time Pairwise Reranking](https://arxiv.org/abs/2511.07555)
*Jingyu Wu,Aditya Shrivastava,Jing Zhu,Alfy Samuel,Anoop Kumar,Daben Liu*

Main category: cs.CL

TL;DR: 本文研究了如何优化基于大语言模型的文档重排序方法，通过多种优化技术将延迟从61.36秒降低到0.37秒，性能损失极小，使LLM重排序在实时应用中变得可行。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的成对重排序方法虽然有效，但由于算法复杂性和高计算需求，在实时应用中存在延迟问题，需要优化以提高实用性。

Method: 采用多种优化技术：使用更小的模型、限制重排序集大小、降低精度、通过单向顺序推断减少位置偏差、限制输出标记数量。

Result: 优化后延迟显著降低166倍（从61.36秒到0.37秒），Recall@k指标性能损失极小，使LLM重排序在实时部署中变得可行。

Conclusion: 通过精心设计的优化方法，可以显著提高基于LLM的重排序效率，使其在延迟敏感的实时应用中具有可行性，同时保持较好的性能表现。
...
### [91] [LLMs vs. Traditional Sentiment Tools in Psychology: An Evaluation on Belgian-Dutch Narratives](https://arxiv.org/abs/2511.07641)
*Ratna Kandala,Katie Hoemann*

Main category: cs.CL

TL;DR: 荷兰语LLMs在佛兰芒语情感分析中表现不如传统方法，Pattern工具表现最佳，挑战了LLM在情感分析任务中的优越性假设。


<details>
  <summary>Details</summary>
Motivation: 理解日常语言中的情感细微差别对计算语言学和情感研究至关重要，传统词典工具如LIWC和Pattern是基础工具，而LLMs承诺提供更好的上下文理解能力。

Method: 评估了三个荷兰语特定LLMs（ChocoLlama-8B-Instruct、Reynaerde-7B-chat和GEITje-7B-ultra）与LIWC和Pattern在佛兰芒语情感预测中的表现，使用来自102名荷兰语参与者的约25000个自发文本响应数据集。

Result: 令人惊讶的是，尽管有架构进步，荷兰语调优的LLMs表现不如传统方法，Pattern显示出最佳性能。

Conclusion: 这些发现挑战了LLM在情感分析任务中优越性的假设，突显了在自发真实世界叙事中捕捉情感效价的复杂性，强调需要为低资源语言变体开发文化和语言定制的评估框架。
...
### [92] [Revisiting NLI: Towards Cost-Effective and Human-Aligned Metrics for Evaluating LLMs in Question Answering](https://arxiv.org/abs/2511.07659)
*Sai Shridhar Balamurali,Lu Cheng*

Main category: cs.CL

TL;DR: 本文重新评估了使用现成的自然语言推理（NLI）评分结合简单词汇匹配标志的轻量级方法，发现在长格式问答任务中，这种几十年前的技术能达到与GPT-4o相当的准确率（89.9%），同时所需参数数量少几个数量级。
...
Method: 采用现成的自然语言推理（NLI）评分，并辅以简单的词汇匹配标志，构建轻量级评估方法。同时创建了DIVER-QA基准数据集，包含3000个样本，涵盖五个问答数据集和五个候选LLM。

Result: 基于NLI的评估方法在长格式问答任务中达到89.9%的准确率，与GPT-4o相当，但所需参数数量显著减少。DIVER-QA基准显示这种廉价方法仍然具有竞争力。

Conclusion: 廉价的基于NLI的评估方法在大语言模型答案评估中仍然具有竞争力，DIVER-QA可作为未来度量研究的开放资源。
...
### [93] [Stress Testing Factual Consistency Metrics for Long-Document Summarization](https://arxiv.org/abs/2511.07689)
*Zain Muhammad Mujahid,Dustin Wright,Isabelle Augenstein*

Main category: cs.CL

TL;DR: 本文系统评估了六种广泛使用的无参考事实性度量在长文档摘要中的可靠性，发现这些原本用于短文本摘要的度量在长文档场景下表现不稳定，对语义等效摘要产生不一致评分，且在信息密集声明上可靠性下降。


<details>
  <summary>Details</summary>
Motivation: 评估抽象文本摘要的事实一致性仍然是一个重要挑战，特别是在长文档场景中，传统度量方法面临输入长度限制和长距离依赖问题。

Method: 通过七种事实性保持扰动（释义、简化、同义词替换、逻辑等价否定、词汇缩减、压缩和源文本插入）来探测度量鲁棒性，并分析其对检索上下文和声明信息密度的敏感性。

Result: 在涵盖科幻、法律和科学领域的三个长文档基准数据集上，现有短文本度量对语义等效摘要产生不一致评分，对信息密集声明的可靠性下降，扩展检索上下文在某些领域能提高稳定性，但没有度量能在长上下文条件下持续保持事实对齐。
...
### [94] [CAPO: Confidence Aware Preference Optimization Learning for Multilingual Preferences](https://arxiv.org/abs/2511.07691)
*Rhitabrat Pokharel,Yufei Tao,Ameeta Agrawal*

Main category: cs.CL

TL;DR: 提出CAPO方法，通过基于相对奖励的动态损失缩放机制替代DPO的固定偏好对处理，增强多语言环境中对噪声和低边际比较的鲁棒性。
...
Method: CAPO方法使用动态损失缩放机制，根据每个偏好对的置信度调整学习信号，替代DPO的固定处理方式。

Result: CAPO在奖励准确率上比现有基线方法至少提升16%，并通过扩大首选和非首选响应之间的差距来改善多语言对齐效果。
...
### [95] [Critical Confabulation: Can LLMs Hallucinate for Social Good?](https://arxiv.org/abs/2511.07722)
*Peiqi Sui,Eamon Duede,Hoyt Long,Richard Jean So*

Main category: cs.CL

TL;DR: 论文提出'关键虚构'方法，利用LLM的幻觉来填补因社会政治不平等造成的档案空白，重建历史'隐藏人物'的多样化但基于证据的叙事。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM会产生幻觉，但经过精心约束的虚构可以具有社会价值，特别是用于填补历史档案中因不平等造成的空白。

Method: 使用开放式叙事填空任务，让LLM生成角色时间线中被掩盖的事件，评估不同模型在受控幻觉提示下的表现。

Result: 验证了LLM具备执行关键虚构的基础叙事理解能力，展示了受控幻觉如何支持知识生产应用而不损害历史准确性。

Conclusion: 精心规范和受控的LLM幻觉可以成为知识生产的有效工具，在保持历史准确性的同时填补档案空白。
...
### [96] [Back to the Future: The Role of Past and Future Context Predictability in Incremental Language Production](https://arxiv.org/abs/2511.07752)
*Shiva Upadhye,Richard Futrell*

Main category: cs.CL

TL;DR: 本研究探讨了上下文可预测性如何影响在线语言产生中的词汇形式和选择，提出了整合未来和过去上下文的信息理论可预测性度量，并通过两个自然语音语料库研究验证了该度量在词汇时长和替换错误中的效果。


<details>
  <summary>Details</summary>
Motivation: 虽然词汇在给定过去上下文时的可预测性效应已被充分理解，但给定未来上下文的向后可预测性效应仍不清楚，这可能与未来规划相关。

Method: 使用改进的度量和更强大的语言模型，提出新的信息理论可预测性度量；第一个研究重新审视可预测性对词汇时长的经典效应；第二个研究在生成框架内分析替换错误，独立建模词汇、上下文和交际因素对词汇选择的影响。

Result: 提出的向后可预测性替代方法在两个研究中产生定性相似的效果；通过替换错误的细粒度分析，揭示了说话者在词汇规划过程中如何优先处理形式、意义和基于上下文的信息。

Conclusion: 这些发现阐明了过去和未来上下文在说话者编码和选择词汇中的功能作用，为上下文可预测性效应和句子规划机制之间搭建了桥梁。
...
### [97] [Design, Results and Industry Implications of the World's First Insurance Large Language Model Evaluation Benchmark](https://arxiv.org/abs/2511.07794)
*Hua Zhou,Bing Ma,Yufei Zhang,Yi Zhao*

Main category: cs.CL

TL;DR: 本文详细介绍了CUFEInse v1.0保险领域大模型评估基准的构建方法、多维评价体系和设计理念，并对11个主流大语言模型进行了全面评估，揭示了在保险垂直场景中的性能瓶颈和发展方向。
...
Conclusion: CUFEInse基准的建立为学术模型优化和工业模型选择提供了权威参考，未来保险大模型的核心发展方向是"领域适应+推理增强"。
...
### [98] [From Experience to Strategy: Empowering LLM Agents with Trainable Graph Memory](https://arxiv.org/abs/2511.07800)
*Siyu Xia,Zekun Xu,Jiajun Chai,Wentian Fan,Yan Song,Xiaohan Wang,Guojun Yin,Wei Lin,Haifeng Zhang,Jun Wang*

Main category: cs.CL

TL;DR: 本文提出了一种可训练的、基于图的记忆框架，通过将智能体轨迹抽象为状态机中的结构化决策路径，并进一步提炼为高层战略元认知，增强LLM智能体在复杂环境中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体获取经验的方式存在局限性：通过训练获得的隐式记忆容易发生灾难性遗忘且可解释性差，而通过提示获得的显式记忆缺乏适应性。需要一种更好的方法来利用先验经验指导当前决策。

Method: 提出多层级图记忆框架，将原始智能体轨迹抽象为状态机中的结构化决策路径，并提炼为高层战略元认知。采用基于强化学习的权重优化程序评估每个元认知的效用，并通过元认知提示动态整合到LLM智能体的训练循环中。

Result: 可学习的图记忆框架实现了稳健的泛化能力，提高了LLM智能体的战略推理性能，并在强化学习训练过程中提供了持续的好处。

Conclusion: 该可训练的图记忆框架有效增强了LLM智能体利用参数信息的能力，通过结构化的经验表示和自适应集成机制，显著提升了智能体在复杂开放环境中的任务解决能力。
...
### [99] [AlignSurvey: A Comprehensive Benchmark for Human Preferences Alignment in Social Surveys](https://arxiv.org/abs/2511.07871)
*Chenxi Lin,Weikang Yuan,Zhuoren Jiang,Biao Huang,Ruitao Zhang,Jianan Ge,Yueqian Xu,Jianxing Yu*

Main category: cs.CL

TL;DR: AlignSurvey是首个系统复制和评估完整社会调查流程的基准，定义了与调查关键阶段对齐的四个任务，并提供了多层级数据集架构和评估指标。


<details>
  <summary>Details</summary>
Motivation: 传统社会调查面临固定问题格式、高成本、有限适应性和跨文化等效性等挑战，而现有LLM模拟调查的研究大多局限于结构化问题，忽视了完整调查流程，且存在训练数据偏见导致边缘群体代表性不足的风险。

Method: 引入AlignSurvey基准，定义了四个与调查阶段对齐的任务：社会角色建模、半结构化访谈建模、态度立场建模和调查响应建模。构建了多层级数据集架构，包括社会基础语料库和完整流程调查数据集，并通过两阶段微调开源LLM获得SurveyLM系列模型。

Result: 开发了包含44K+访谈对话和400K+结构化调查记录的社会基础语料库，以及专家标注的AlignSurvey-Expert数据集和两个具有全国代表性的跨文化调查数据集。提供了任务特定的评估指标来评估对齐保真度、一致性和公平性。

Conclusion: AlignSurvey为透明和负责任的社会科学研究提供了完整的基准、数据集和模型资源，支持在个体和群体层面评估LLM在社会调查中的表现，特别关注人口多样性。
...
### [100] [Planned Event Forecasting using Future Mentions and Related Entity Extraction in News Articles](https://arxiv.org/abs/2511.07879)
*Neelesh Kumar Shukla,Pranay Sanghvi*

Main category: cs.CL

TL;DR: 本文开发了一个预测社会动荡事件的系统，通过主题建模和word2vec过滤相关新闻文章，使用NER识别实体，并通过时间标准化处理未来日期。提出了相关实体提取方法来识别真正参与事件的实体。


<details>
  <summary>Details</summary>
Motivation: 在印度等民主国家，公民表达诉求可能导致抗议、集会等社会动荡事件。这些事件往往未经许可且具有破坏性，预测这些事件有助于行政官员采取必要措施。由于抗议活动通常提前宣布以吸引更多参与者，通过分析新闻公告可以提前预测计划事件。

Method: 使用主题建模和word2vec过滤相关新闻文章；应用命名实体识别方法识别人员、组织、地点和日期等实体；进行时间标准化以将未来日期转换为标准格式；提出相关实体提取方法识别真正参与事件的实体。

Result: 开发了一个地理独立、通用的模型来识别过滤社会动荡事件的关键特征；能够从众多实体中提取真正相关的实体；建立了预测社会动荡事件的完整系统。

Conclusion: 本文提出的系统能够有效预测社会动荡事件，通过相关实体提取方法提高了事件预测的准确性，为行政管理部门提供了有价值的决策支持工具。
...
### [101] [Breaking the Adversarial Robustness-Performance Trade-off in Text Classification via Manifold Purification](https://arxiv.org/abs/2511.07888)
*Chenhao Dang,Jing Ma*

Main category: cs.CL

TL;DR: MC^2F是一种用于文本分类的两模块系统，通过建模干净样本在编码器嵌入流形中的分布，在保持干净数据性能的同时增强对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决文本分类中增强模型对抗鲁棒性通常会降低干净数据性能的挑战。
...
Conclusion: MC^2F方法成功解决了文本分类中对抗鲁棒性与干净数据性能之间的权衡问题。
...
### [102] [Last Layer Logits to Logic: Empowering LLMs with Logic-Consistent Structured Knowledge Reasoning](https://arxiv.org/abs/2511.07910)
*Songze Li,Zhiqiang Liu,Zhaoyan Gong,Xiaoke Guo,Zhengke Gui,Huajun Chen,Wen Zhang*

Main category: cs.CL

TL;DR: 论文提出Logits-to-Logic框架，通过logits增强和过滤模块来纠正LLM在结构化知识推理中的逻辑漂移问题，显著提升逻辑一致性。
...
Conclusion: 通过直接操作LLM的logits输出可以有效纠正逻辑缺陷，为提升结构化知识推理的逻辑一致性提供了新思路。
...
### [103] [Unified Work Embeddings: Contrastive Learning of a Bidirectional Multi-task Ranker](https://arxiv.org/abs/2511.07969)
*Matthias De Lange,Jens-Joris Decorte,Jeroen Van Hautte*

Main category: cs.CL

TL;DR: WorkBench是首个统一评估套件，涵盖6个工作相关任务，通过构建统一工作嵌入模型UWE，在零样本场景下显著优于通用嵌入模型。


<details>
  <summary>Details</summary>
Motivation: 工作领域自然语言处理需求增长，但工作相关任务具有长尾分布、多标签和稀缺数据等复杂性，需要评估通用嵌入模型在工作领域的性能。

Method: 构建WorkBench评估套件，基于跨任务转移洞察构建任务特定二分图，使用多对多InfoNCE目标和任务无关软延迟交互训练UWE模型。

Result: UWE在未见目标空间上实现零样本排序性能，支持低延迟推理，在宏平均MAP和RP@10指标上显著优于通用嵌入模型。

Conclusion: UWE模型通过利用训练数据结构实现了工作领域的有效嵌入表示，为工作相关NLP任务提供了强大基础。
...
### [104] [NOTAM-Evolve: A Knowledge-Guided Self-Evolving Optimization Framework with LLMs for NOTAM Interpretation](https://arxiv.org/abs/2511.07982)
*Maoqi Liu,Quan Fang,Yuhao Wu,Can Zhao,Yang Yang,Kaiquan Cai*

Main category: cs.CL

TL;DR: 提出了NOTAM-Evolve框架，通过自演进学习使大语言模型能够自主掌握复杂的NOTAM（航行通告）解析，在结构化NOTAM解析任务上比基础LLM准确率提升30.4%。


<details>
  <summary>Details</summary>
Motivation: NOTAM的压缩和加密语言给手动和自动处理带来重大挑战，现有自动化系统通常仅限于浅层解析，无法提取操作决策所需的可操作情报。

Method: 将完整解析任务形式化为深度解析，提出NOTAM-Evolve自演进框架，结合知识图谱增强检索模块进行数据基础，通过闭环学习过程使LLM从自身输出逐步改进。

Result: NOTAM-Evolve在结构化NOTAM解析任务上比基础LLM准确率提升30.4%，建立了新的最先进水平。

Conclusion: NOTAM-Evolve框架通过自演进学习有效解决了NOTAM深度解析的挑战，显著提升了自动化解析的准确性。
...
### [105] [Self-Correction Distillation for Structured Data Question Answering](https://arxiv.org/abs/2511.07998)
*Yushan Zhu,Wen Zhang,Long Jin,Mengshu Sun,Ling Zhong,Zhiqiang Liu,Juan Li,Lei Liang,Chong Long,Chao Deng,Junlan Feng*

Main category: cs.CL

TL;DR: 提出自校正蒸馏方法，通过错误提示机制和两阶段蒸馏策略，提升小规模LLM在结构化数据问答中的性能，在多个基准测试中达到最佳效果。
...
Result: 在5个基准测试和3种结构化数据类型上的实验表明，该方法在小规模LLM上达到最佳性能，并在某些数据集上接近GPT4的表现。

Conclusion: 自校正蒸馏方法有效提升了小规模LLM在结构化数据问答中的能力，同时错误提示机制也能帮助大规模LLM在多数数据集上超越现有最佳结果。
...
### [106] [Estranged Predictions: Measuring Semantic Category Disruption with Masked Language Modelling](https://arxiv.org/abs/2511.08109)
*Yuxuan Liu,Haim Dubossarsky,Ruth Ahnert*

Main category: cs.CL

TL;DR: 本研究使用掩码语言模型测量科幻小说中人类、动物和机器概念之间的渗透性，发现科幻小说具有更高的概念流动性，特别是机器概念表现出显著的跨类别替代和分散，而人类术语保持语义连贯性。


<details>
  <summary>Details</summary>
Motivation: 旨在通过计算方式验证达科·苏文的陌生化理论，探索科幻小说如何通过语言扰动来解构本体论类别，为计算文学研究提供新方法。

Method: 使用RoBERTa掩码语言模型生成词汇替代，通过Gemini分类，采用保留率、替换率和熵三个指标量化概念滑动，比较科幻小说和一般小说语料库。

Result: 科幻小说表现出更高的概念渗透性，机器概念边界最不稳定，人类术语保持语义锚定作用，科幻小说在人类中心逻辑内进行类别重构。

Conclusion: 科幻小说的陌生化是对语义规范的受控扰动，可通过概率建模检测；掩码语言模型可作为解释性工具揭示体裁条件化的本体论假设。
...
### [107] [Sentence-Anchored Gist Compression for Long-Context LLMs](https://arxiv.org/abs/2511.08128)
*Dmitrii Tarasov,Elizaveta Goncharova,Kuznetsov Andrey*

Main category: cs.CL

TL;DR: 该研究探索使用学习压缩令牌来压缩大型语言模型的上下文，以减少处理长序列时的内存和计算需求。通过微调预训练LLM，可在2倍到8倍的压缩比下保持性能，并在30亿参数LLaMA模型上达到与其他压缩技术相当的效果。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型处理长序列时面临的内存和计算需求过高的问题，通过上下文压缩来提升效率。

Method: 对预训练的大型语言模型进行微调，使其学习压缩令牌来压缩上下文，压缩比可达2倍到8倍。

Result: 在短上下文和长上下文基准测试中，压缩后的模型性能没有显著下降；在30亿参数LLaMA模型上，该方法与其他压缩技术效果相当但压缩比更高。
...
### [108] [Relation as a Prior: A Novel Paradigm for LLM-based Document-level Relation Extraction](https://arxiv.org/abs/2511.08143)
*Qiankun Pi,Yepeng Sun,Jicang Lu,Qinlong Fan,Ningbo Huang,Shiyu Wang*

Main category: cs.CL

TL;DR: 本文提出了一种新的关系提取范式RelPrior，通过将关系作为先验来改进基于LLM的文档级关系提取性能，解决了无关实体对噪声和预定义关系标签限制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的文档级关系提取方法采用'先提取实体再预测关系'的范式存在两个主要问题：大量无关实体对引入噪声干扰，以及超出预定义关系集的关系标签被错误处理。

Method: 提出RelPrior范式：利用二元关系作为先验来过滤无关实体对，减少预测噪声；利用预定义关系作为先验来匹配实体三元组，避免严格的预定义关系标签导致的误判。

Result: 在两个基准测试上的广泛实验表明，RelPrior实现了最先进的性能，超越了现有的基于LLM的方法。

Conclusion: RelPrior通过将关系作为先验的新范式，有效解决了LLM在文档级关系提取中的性能差距问题，显著提升了关系提取的准确性和鲁棒性。
...
### [109] [Still Not There: Can LLMs Outperform Smaller Task-Specific Seq2Seq Models on the Poetry-to-Prose Conversion Task?](https://arxiv.org/abs/2511.08145)
...
Conclusion: 对于梵语诗歌转散文等复杂任务，领域特定的微调模型优于通用LLM，但当领域特定语料不可用时，提示策略可作为替代方案。
...
### [110] [Do Syntactic Categories Help in Developmentally Motivated Curriculum Learning for Language Models?](https://arxiv.org/abs/2511.08199)
*Arzu Burcu Güven,Anna Rogers,Rob van der Goot*

Main category: cs.CL

TL;DR: 研究分析了BabyLM语料库和CHILDES中不同年龄组的句法特性，发现CHILDES在年龄上的句法区分不明显，但训练数据的句法知识有助于解释模型在语言任务上的表现。课程学习方面，探索了发展性和其他认知启发式课程方法，发现某些课程有助于阅读任务，但主要性能提升来自使用可句法分类的数据子集而非完整嘈杂语料库。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解儿童语言数据的句法特性如何影响语言模型的性能解释，并探索认知启发式课程学习方法的有效性。

Method: 分析BabyLM语料库和CHILDES语料库的句法特性，比较不同年龄组的句法差异，并测试多种课程学习方法，包括发展性课程和其他认知启发式方法。

Result: CHILDES语料库在年龄上未表现出明显的句法区分；训练数据的句法知识有助于解释模型性能；某些课程学习方法对阅读任务有帮助；主要性能提升来自使用可句法分类的数据子集。

Conclusion: 句法知识对解释语言模型性能很重要，使用经过句法筛选的数据子集比使用完整嘈杂语料库能带来更好的性能提升，课程学习在特定任务上有效。
...
### [111] [Encoder Fine-tuning with Stochastic Sampling Outperforms Open-weight GPT in Astronomy Knowledge Extraction](https://arxiv.org/abs/2511.08204)
...
Method: 实现了一个基于SciBERT模型的多任务transformer系统，通过从训练数据中随机采样片段进行微调，并在推理时使用多数投票。
...
### [112] [Benchmarking Educational LLMs with Analytics: A Case Study on Gender Bias in Feedback](https://arxiv.org/abs/2511.08225)
*Yishan Du,Conrad Borchers,Mutlu Cukurova*

Main category: cs.CL

TL;DR: 本文提出了一个基于嵌入的基准测试框架，用于检测大型语言模型在形成性反馈中的性别偏见。通过600篇真实学生论文构建受控反事实，研究发现所有模型对性别词汇替换都表现出不对称的语义响应，GPT和Llama模型还对显式性别线索敏感。


<details>
  <summary>Details</summary>
Motivation: 随着教师在教育实践中越来越多地使用生成式AI，需要稳健的方法来为教学目的基准测试大型语言模型，特别是检测其在形成性反馈中的偏见问题。

Method: 使用600篇真实学生论文构建受控反事实，通过词汇替换（隐式线索）和提示中的作者背景（显式线索）两个维度操纵性别信息。采用余弦和欧几里得距离量化响应差异，通过置换测试评估显著性，并使用降维可视化结构。

Result: 所有模型对男性-女性反事实的隐式操纵都产生了比女性-男性更大的语义偏移。只有GPT和Llama模型对显式性别线索敏感。定性分析显示一致的反馈语言差异，如男性线索下更多自主支持性反馈，女性线索下更多控制性反馈。

Conclusion: 即使是最先进的大型语言模型也表现出对性别替换的不对称语义响应，表明其提供的反馈存在持续的性别偏见。研究为教学生成式AI的公平性审计提供了基准，并提出了反事实评估报告标准和确保公平反馈的实用指南。
...
### [113] [VocalBench-zh: Decomposing and Benchmarking the Speech Conversational Abilities in Mandarin Context](https://arxiv.org/abs/2511.08230)
...
Conclusion: VocalBench-zh填补了普通话语音到语音评估的空白，为开发者和用户提供了系统评估和公平比较的工具，推动了下一代语音交互系统的发展。
...
### [114] [Prompt Tuning for Natural Language to SQL with Embedding Fine-Tuning and RAG](https://arxiv.org/abs/2511.08245)
*Jisoo Jang,Tien-Cuong Bui,Yunjun Choi,Wen-Syan Li*

Main category: cs.CL

TL;DR: 本文提出了一种基于提示调优的NL-to-SQL错误校正框架，结合生成式预训练大语言模型和RAG技术，通过类似医疗诊断的过程来诊断错误类型、识别原因并提供修正指导，显著提升了自然语言到SQL转换的准确性。


<details>
  <summary>Details</summary>
Motivation: 随着自然语言接口的广泛应用，需要高效准确地将自然语言查询转换为SQL表达式。现有方法在复杂场景下仍存在准确性不足的问题，需要更智能的错误校正机制。

Method: 提出新颖框架，集成错误校正机制：诊断错误类型、识别原因、提供修正指导并应用修正。结合嵌入微调和RAG技术，利用外部知识库提升准确性和透明度。

Result: 通过全面实验验证，该框架相比现有基线方法实现了12%的准确率提升。

Conclusion: 该框架有潜力彻底改变当代数据驱动环境中的数据访问和处理方式，展示了集成错误校正机制和外部知识检索在NL-to-SQL任务中的显著优势。
...
### [115] [ParliaBench: An Evaluation and Benchmarking Framework for LLM-Generated Parliamentary Speech](https://arxiv.org/abs/2511.08247)
*Marios Koniaris,Argyro Tsipi,Panayiotis Tsanakas*

Main category: cs.CL

TL;DR: 提出了ParliaBench基准测试，用于评估议会演讲生成的质量，包括语言质量、语义连贯性和政治真实性三个维度，并引入了基于嵌入的新指标来量化意识形态定位。


<details>
  <summary>Details</summary>
Motivation: 议会演讲生成对大型语言模型提出了特定挑战，需要政治真实性和意识形态一致性，而现有模型缺乏专门训练，评估方法也忽视了政治真实性。

Method: 构建了英国议会演讲数据集，提出了结合计算指标和LLM作为评判者的评估框架，引入了政治光谱对齐和党派对齐两个新指标，并对五个大型语言模型进行了微调。

Result: 微调在大多数指标上产生了统计显著的改进，新指标在政治维度上表现出强大的区分能力。

Conclusion: ParliaBench基准测试有效解决了议会演讲生成中的政治真实性问题，微调方法显著提升了模型性能，新指标为政治维度评估提供了有力工具。
...
### [116] [Hierarchical structure understanding in complex tables with VLLMs: a benchmark and experiments](https://arxiv.org/abs/2511.08298)
*Luca Bindini,Simone Giovannini,Simone Marinai,Valeria Nardoni,Kimiya Noor Ali*

Main category: cs.CL

TL;DR: 该研究探讨了视觉大语言模型理解科学论文中表格结构的能力，特别是推断表格层次结构的能力，使用PubTables-1M数据集中的复杂层次表格作为基准测试。


<details>
  <summary>Details</summary>
Motivation: 探索通用视觉大语言模型是否能够理解表格的层次结构，而无需额外的专门处理，为将结构化数据理解整合到通用VLLMs中提供指导。

Method: 采用一系列提示工程策略，实验不同提示格式和写作风格，评估多个最先进的开源VLLMs，包括现成版本和针对任务微调的版本，并与人类表现进行比较。

Result: 实验结果表明，未专门设计用于理解表格结构的通用VLLMs能够执行此任务，揭示了VLLMs处理复杂表格的潜力和局限性。

Conclusion: 该研究为未来在通用VLLMs中整合结构化数据理解提供了见解和指导，证明了VLLMs在理解表格层次结构方面的潜力。
...
### [117] [Adaptive Multi-Agent Response Refinement in Conversational Systems](https://arxiv.org/abs/2511.08319)
*Soyeong Jeong,Aparna Elangovan,Emine Yilmaz,Oleg Rokhlenko*

Main category: cs.CL

TL;DR: 本文提出了一种多智能体框架来改进LLM的对话响应，通过分配不同智能体负责事实性、个性化和连贯性等关键方面，并采用动态通信策略来协调智能体协作，显著提升了对话质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在对话系统中表现出色，但在处理个性化或特定知识时存在不足。现有方法难以同时考虑对话所需的多个方面，且不切实际依赖用户检测错误。

Method: 采用多智能体框架，每个智能体专门负责一个对话质量方面（事实性、个性化、连贯性），引入动态通信策略自适应选择和协调相关智能体，合并反馈来改进响应。

Result: 在具有挑战性的对话数据集上验证，该方法显著优于相关基线方法，特别是在涉及知识或用户画像的任务中表现突出。

Conclusion: 多智能体框架结合动态通信策略能有效提升LLM对话响应的质量，尤其在处理个性化知识和多维度对话需求方面具有明显优势。
...
### [118] [AgentPRM: Process Reward Models for LLM Agents via Step-Wise Promise and Progress](https://arxiv.org/abs/2511.08325)
...
TL;DR: 本文提出了一种名为AgentPRM的过程奖励模型，用于改进大型语言模型在多轮决策任务中的表现。该方法通过评估每个决策与目标的接近程度和进展贡献来指导智能体决策，相比传统方法具有更高的计算效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多轮决策任务中仍面临挑战，传统方法依赖复杂的提示工程或专家轨迹微调。本文从不同角度探索构建过程奖励模型来评估每个决策并指导决策过程。

Method: 提出AgentPRM过程奖励模型，捕捉序列决策间的相互依赖关系及其对最终目标的贡献。采用基于时间差分和广义优势估计的方法来高效获取训练数据。

Result: 在不同智能体任务上的实验表明，AgentPRM比基线方法计算效率高出8倍以上，并且在扩展测试时计算资源时表现出稳健的改进。

Conclusion: AgentPRM方法能有效跟踪进展并平衡探索与利用，为LLM智能体的强化学习提供了新的思路和见解。
...
### [119] [DPRM: A Dual Implicit Process Reward Model in Multi-Hop Question Answering](https://arxiv.org/abs/2511.08364)
*Xinyi Wang,Yiping Song,Zhiliang Tian,Bo Liu,Tingjin Luo,Minlie Huang*

Main category: cs.CL

TL;DR: 本文提出DPRM（双重隐式过程奖励模型），通过训练两个隐式PRM分别处理知识图谱和思维链推理，解决了多跳问答任务中传统方法无法处理图结构约束和推理路径不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 在多跳问答任务中，传统的结果奖励模型无法评估多步推理过程，而传统过程奖励模型需要昂贵的人工标注。现有的隐式PRM仅适用于纯文本场景，无法处理知识图谱的结构约束和思维链与KG路径之间的不一致性。

Method: 提出DPRM模型，包含KG-PRM和CoT-PRM两个隐式过程奖励模型，分别从结果信号中推导步骤级奖励。KG-PRM使用偏好对从知识图谱中学习结构约束，并引入一致性约束使两个PRM相互验证和优化推理路径。

Result: 实验结果显示，该方法在多个数据集上优于13个基线方法，在Hit@1指标上最高提升16.6%。
...
### [120] [The Dynamic Articulatory Model DYNARTmo: Dynamic Movement Generation and Speech Gestures](https://arxiv.org/abs/2511.08372)
*Bernd J. Kröger*
...
<details>
  <summary>Details</summary>
Motivation: 提供一个神经生物学启发的计算框架，模拟从语言表达到发音-声学实现的层次化语音产生控制。

Method: 使用语音手势概念和手势评分，通过手势库结构和手势协调，将手势转换为控制DYNARTmo声道模型的连续发音器轨迹。

Result: 开发了能够生成连续发音器运动的动态发音模型DYNARTmo。

Conclusion: DYNARTmo模型为语音产生的层次化控制提供了一个有效的计算框架，实现了从语言表达到发音实现的完整模拟。
...
### [121] [TurkEmbed: Turkish Embedding Model on NLI & STS Tasks](https://arxiv.org/abs/2511.08376)
*Özay Ezerceli,Gizem Gümüşçekiçci,Tuğba Erkoç,Berke Özenç*

Main category: cs.CL

TL;DR: TurkEmbed是一个新的土耳其语嵌入模型，在NLI和STS任务上表现优于现有模型，通过多样化数据集和先进训练技术实现了1-4%的性能提升。
...
Result: 在土耳其STS-b-TR数据集上使用Pearson和Spearman相关性指标评估，在语义相似性任务中表现出显著改进，在All-NLI-TR和STS-b-TR基准测试中超越了当前最先进的Emrecan模型。
...
### [122] [PCRLLM: Proof-Carrying Reasoning with Large Language Models under Stepwise Logical Constraints](https://arxiv.org/abs/2511.08392)
*Tangrui Li,Pei Wang,Hongzheng Wang Christian Hahm,Matteo Spatola,Justin Shi*

Main category: cs.CL

TL;DR: 提出了Proof-Carrying Reasoning with LLMs (PCRLLM)框架，通过单步推理约束和显式指定前提、规则与结论，增强LLM的逻辑推理可验证性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在逻辑推理中缺乏明确的推理规则遵循，存在可信度问题，需要一种能够支持验证的推理框架。

Method: PCRLLM框架将推理约束为单步推理，保持自然语言表述，同时显式指定前提、规则和结论，支持多LLM协作和中间步骤比较。

Result: 该框架能够在黑盒设置下支持链级验证，并引入了结合自然语言表达性和形式严谨性的大规模步级推理数据基准生成方案。

Conclusion: PCRLLM通过形式化约束和验证机制，有效提升了LLM推理的可信度和协作能力。
...
### [123] [Interaction Dynamics as a Reward Signal for LLMs](https://arxiv.org/abs/2511.08394)
*Sian Gooding,Edward Grefenstette*

Main category: cs.CL

TL;DR: TRACE提出了一种基于对话嵌入轨迹几何特性的新型奖励信号，仅使用交互动态就能达到与基于完整文本分析的LLM基线相当的准确率（68.20% vs 70.04%），结合文本分析的混合模型性能最高（80.17%）。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型多轮对话对齐主要依赖文本内容奖励信号，忽视了交互动态这一丰富补充信号源。

Method: 引入TRACE（基于轨迹的智能体协作评估奖励），从对话嵌入轨迹的几何特性中推导奖励信号，即'对话几何学'概念。

Result: 仅基于结构信号的奖励模型达到68.20%的成对准确率，与基于完整文本分析的LLM基线（70.04%）相当；结合交互动态和文本分析的混合模型性能最高，达80.17%。

Conclusion: 在交互设置中，智能体的沟通方式与其沟通内容同样重要，这提供了一个新的隐私保护框架，既能对齐智能体，又能作为理解成功协作驱动因素的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交机器人检测器在基准测试中表现良好，但在多样化真实场景中的鲁棒性有限，主要由于不清楚的真实标签和多样化的误导线索。特别是捷径学习（模型依赖虚假相关性而非因果任务相关特征）的影响尚未得到充分关注。

Method: 设计了一系列捷径场景，通过构建用户标签与表面文本线索之间的虚假关联来评估模型鲁棒性。提出基于大语言模型的缓解策略，利用反事实数据增强从数据和模型角度在三个层面解决问题。

Result: 结果显示不相关特征分布的偏移显著降低了社交机器人检测器性能，基线模型的平均相对准确率下降32%。提出的缓解策略在捷径场景下实现了平均相对性能提升56%。
...
TL;DR: SPEAR-MM框架通过选择性参数评估和模型融合，在保持金融领域适应性的同时，有效缓解大语言模型在领域适应中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在金融领域适应过程中对通用推理能力的灾难性遗忘问题，这对于客户交互和复杂金融分析至关重要。

Method: 通过后验分析近似层级对基准测试的影响，然后通过球面插值融合选择性冻结或恢复Transformer层。

Result: 在LLaMA-3.1-8B上，SPEAR-MM实现了91.2%的通用能力保留（标准持续预训练为69.7%），同时保持了94%的领域适应收益，计算成本降低90%。

Conclusion: 该方法为资源受限的金融机构提供了可解释的权衡控制和显著的计算成本降低，是实用的领域适应解决方案。
...
TL;DR: 本文提出S-RAG方法，专门处理需要从大量文档中聚合信息的查询问题，在HOTELS和WORLD CUP数据集上显著优于传统RAG系统和长上下文LLM。


<details>
  <summary>Details</summary>
Motivation: 当前RAG方法主要关注少量相关文档的查询，无法有效处理需要从大量文档中聚合信息的复杂查询问题。

Method: S-RAG在数据摄入阶段构建语料库的结构化表示，在推理阶段将自然语言查询转换为对该结构化表示的形式化查询。

Result: 在新引入的HOTELS和WORLD CUP数据集以及公共基准测试中，S-RAG显著优于传统RAG系统和长上下文LLM。

Conclusion: S-RAG为聚合查询提供了有效的解决方案，填补了当前RAG方法在处理需要从大量文档中收集和推理信息的查询方面的空白。
...
TL;DR: 本文介绍了Bangla-SGP数据集，包含1000个人工标注的句子-手语对和3000个合成生成的对，用于孟加拉手语句子级翻译任务。作者微调了多个transformer模型并评估了其性能。
...
Method: 构建了包含人工标注和规则增强生成的平行数据集，使用基于规则的检索增强生成(RAG)流水线进行数据增强，并微调了mBart50、Google mT5、GPT4.1-nano等transformer模型。

Result: 创建了Bangla-SGP数据集，包含4000个句子-手语对，通过BLEU分数评估模型性能，并与RWTH-PHOENIX-2014T基准进行比较。

Conclusion: 该工作填补了孟加拉手语句子级翻译数据集的空白，为低资源手语翻译任务提供了有效的数据集和基准模型。
...
TL;DR: 本文系统研究了大型推理模型（LRMs）通过思维链（CoT）进行监控的可行性和挑战，探讨了模型在CoT中真实表达决策因素的程度以及基于CoT的监控可靠性，并提出了新的监控范式MoME。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型的详细推理轨迹为AI安全监控创造了新机会，但面临两个关键挑战：模型不一定在CoT中真实反映内部决策过程，以及监控器可能过于敏感或不够敏感。

Method: 从两个核心视角进行研究：(i) 言语化程度：LRMs在CoT中真实表达决策因素的程度；(ii) 监控可靠性：基于CoT的监控器可靠检测模型不当行为的程度。通过实证证据和相关分析，研究不同CoT干预方法对监控效果的影响，并提出MoME监控范式。

Result: 在数学、科学和伦理领域提供了言语化质量、监控可靠性和LLM性能之间的实证证据和相关分析，发现不同CoT干预方法会影响监控有效性。

Conclusion: CoT监控性是一个有前景但具有挑战性的研究方向，需要系统解决言语化真实性和监控可靠性问题，提出的MoME范式为模型间相互监控提供了新思路。
...
TL;DR: 本文提出了一种从OntoNotes 5.0语料库的WSJ部分构建高质量语义角色标注数据集的方法，并将其适配于意见角色标注任务。通过可复现的提取流程，生成了97,169个谓词-论元实例，并映射到意见角色标注的框架中。
...
Method: 基于PropBank标注框架，实现可复现的提取流程：对齐谓词-论元结构与表层文本，将句法树指针转换为连贯的跨度，并进行严格的清洗以确保语义保真度。

Result: 构建了包含97,169个谓词-论元实例的数据集，具有明确定义的Agent、Predicate和Patient角色，并映射到意见角色标注的Holder、Expression和Target模式。
...
TL;DR: 本文研究了大型语言模型在角色扮演情境下的道德响应，通过道德基础问卷量化了道德易感性和道德鲁棒性，发现模型家族对道德鲁棒性影响最大，而模型尺寸对道德易感性影响更明显。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地在社会环境中应用，需要分析它们如何表达和改变道德判断，特别是在角色扮演情境下的道德响应。

Method: 使用道德基础问卷(MFQ)构建基准，通过角色扮演提示让LLM扮演特定角色，量化道德易感性和道德鲁棒性两个属性。

Result: Claude家族在道德鲁棒性上表现最佳，其次是Gemini和GPT-4；模型尺寸对道德鲁棒性无系统性影响，但对道德易感性有明显影响，较大模型更易受影响；鲁棒性和易感性呈正相关。

Conclusion: 角色扮演条件化显著影响大型语言模型的道德行为，模型家族是道德鲁棒性的主要决定因素，而模型尺寸主要影响道德易感性。
...
TL;DR: TaH是一种动态潜在思考方法，仅在困难标记处进行深度迭代，通过轻量级决策器触发潜在迭代，使用LoRA模块专注于困难标记优化，并引入双因果注意力机制保持并行性，显著提升LLM推理性能。


<details>
  <summary>Details</summary>
Motivation: 解决循环变换器中存在的潜在过度思考现象——简单标记在第一次前向传播后已经正确预测，但在额外迭代中有时被错误修正。

Method: 提出Think-at-Hard方法：1）轻量级神经决策器仅在可能错误的标记处触发潜在迭代；2）LoRA模块将LLM目标从通用下一个标记预测转向专注困难标记优化；3）双因果注意力机制扩展注意力到迭代深度维度。

Result: 在五个挑战性基准测试中提升LLM推理性能，相比对所有输出标记迭代两次的基线方法，准确率提升8.1-11.3%，同时免除94%标记的第二次迭代。与使用相同数据微调的强单次迭代模型相比，准确率提升4.0-5.0%。

Conclusion: TaH方法能有效提升LLM推理能力，同时保持参数数量不变，在资源受限的实际应用中具有重要价值。
...
TL;DR: 语言模型可以通过微调学习生成对其内部计算的自然语言描述，包括特征编码信息、激活因果结构和输入令牌影响。利用自身内部访问权限的解释模型比使用其他模型效果更好，即使其他模型能力更强。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否能够利用对其内部计算的特殊访问权限来生成忠实的行为解释，以及这种自我解释是否比外部模型解释更有效。

Method: 使用现有可解释性技术作为真实标签，微调语言模型生成三类自然语言描述：特征编码信息、内部激活的因果结构、特定输入令牌对输出的影响。仅需数万个解释示例进行训练。

Result: 解释模型对新查询展现出非平凡的泛化能力。利用模型自身解释其计算通常比使用不同模型解释效果更好，即使其他模型能力更强。

Conclusion: 语言模型可以学习可靠地解释其内部计算，这种解释方法为现有可解释性技术提供了可扩展的补充。
...
TL;DR: REFLEX是一个基于大语言模型判断的无参考日志摘要评估指标，通过LLM作为零样本评估器来评估摘要质量，解决了传统指标依赖词汇重叠和缺乏高质量参考摘要的问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于规则的强化学习奖励在小型模型上经常导致低质量推理链或推理过程与最终答案不一致的问题，限制了资源有限组织在小规模模型上进行直接强化学习训练的能力。

Method: 开发了置信度奖励模型，不仅惩罚错误答案，还惩罚低置信度的正确答案；通过静态评估、Best-of-N推理测试和基于PPO的强化学习训练进行验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有解决方案主要针对基于事实的检索，无法构建时空锚定的叙事表示来跟踪实体在事件中的演变，需要新的记忆框架来增强大语言模型的长上下文推理能力。
...
Conclusion: GSW为赋予大语言模型类似人类的情景记忆提供了具体蓝图，为能够在长时间范围内进行推理的更强大智能体铺平了道路。
...
TL;DR: 本文提出了一种AI增强的团队贡献评估框架，通过整合多种数据源和LLM分析来解决团队协作中的不公平评估问题。


<details>
  <summary>Details</summary>
Motivation: 团队中个人贡献的公平评估存在挑战，冲突和工作量不均会导致不公平表现评估，需要成本高昂的人工干预。现有工具在冲突解决方法和AI集成方面存在空白。

Method: 提出一个框架和实现设计，将异构工件（提交物、通信、协调记录、同行评估、上下文信息）组织到三个维度的九个基准中：贡献、互动和角色。客观指标被标准化、按维度聚合，并与不平等度量（基尼指数）配对以识别冲突标记。使用LLM架构对这些度量进行验证和上下文分析，生成可解释的咨询判断。

Result: 框架设计了实用的分析指标（情感分析、任务保真度、字数/代码行数等），包含偏见防护措施，并论证了在当前法规和机构政策下的可行性。

Conclusion: 该AI增强工具为解决团队贡献评估中的冲突和公平性问题提供了可行方案，同时指出了局限性和实际挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在可验证领域表现出色，但在具有不确定结果的高风险战略决策中可靠性不足，这种差距源于人类和AI系统中相互强化的认知偏见，威胁到行业估值的可辩护性和投资的可持续性。

Method: 通过对7个前沿级大语言模型和3个面向市场的风险案例进行系统定性评估，开发了包含7阶段校准序列的框架，建立在4阶段初始化过程之上，并采用5层保护架构，包括偏见自我监控、人机对抗挑战、伙伴关系状态验证、性能退化检测和利益相关者保护。

Result: 研究发现：通过有序校准可以实现伙伴关系状态，但需要紧急维护协议；当架构漂移和上下文耗尽同时发生时可靠性会下降；解散纪律可以避免在根本错误方向上付出昂贵代价。跨模型验证揭示了不同LLM架构之间的系统性性能差异。

Conclusion: 该方法证明人类-AI团队可以实现认知伙伴关系，在高风险决策中预防可避免的遗憾，满足依赖于AI系统支持关键决策而不引入可预防认知陷阱的投资回报期望。
...
TL;DR: 本文提出了一种基于LLM的多智能体多模态推理框架，用于自动生成军事训练场景中的复杂文档（如作战命令OPORDs），通过分层分解任务和使用专门智能体来克服传统方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统模拟训练场景生成过程劳动密集且资源消耗大，而前LLM时代的AI工具无法生成足够复杂或可适应的场景，因此需要开发更先进的自动化解决方案。

Method: 采用多智能体多模态推理框架，将场景生成分解为层次化子问题，使用专门LLM智能体分别处理不同子任务，支持三种AI参与模式：生成选项供人选择、生成候选产品供人审批修改、完全自动生成文本制品。

Result: 通过概念验证成功生成了OPORD的机动方案和移动部分，同时估计了地图位置和移动路径，证明了框架的可行性和准确性。
...
TL;DR: WaterMod是一种新的文本水印方法，通过概率感知的模块化规则在保持生成质量的同时嵌入可检测的水印信号，支持零位和多位设置，适用于自然语言生成、数学推理和代码合成等任务。


<details>
  <summary>Details</summary>
Motivation: 现有基于logit的水印方法在随机划分词汇表时可能排除最高概率的token，从而影响生成质量。WaterMod旨在解决这一限制，在满足欧盟AI法案等监管要求的同时保持文本生成的流畅性。

Method: 使用概率感知的模块化规则：首先按模型概率降序排列词汇表，然后通过余数rank mod k进行分区，将语义相似的相邻token分配到不同类别。在零位设置中，通过熵自适应门选择偶数或奇数奇偶性作为绿色列表；在多位设置中，当前有效载荷数字d选择满足rank mod k = d的颜色类别。

Result: 实验结果表明，WaterMod在零位和多位设置下都能保持强大的水印检测性能，同时维持生成质量，在自然语言生成、数学推理和代码合成等多种任务中表现出鲁棒性。

Conclusion: WaterMod通过模块化算术同时支持二进制归属和丰富有效载荷，在保持文本生成质量的同时实现了可靠的水印嵌入和检测，为合成内容的溯源提供了有效解决方案。
...
TL;DR: 该研究开发了一种基于EEG的失语症患者沟通支持系统，通过分析正确发音和发音错误时的脑电信号差异，使用多任务学习框架来解码患者的意图，即使在发音错误时也能有效识别。


<details>
  <summary>Details</summary>
Motivation: 失语症严重限制了语言沟通能力，但目前基于脑电图的沟通支持系统很少针对失语症患者开发。研究旨在填补这一空白，开发能够支持现实世界中不完美语音条件的辅助系统。

Method: 招募一名表达性失语症患者进行韩语自动语音任务，记录EEG信号并根据发音正确与否标记试验。使用频谱分析识别神经激活模式差异，开发基于最大均值差异正则化的软多任务学习框架，重点关注delta特征。

Result: 频谱分析显示发音错误试验在广泛通道中表现出过度的delta功率，前额区域theta-alpha活动增加。提出的模型在正确试验中达到58.6%准确率，发音错误试验达到45.5%准确率，比基线提高了45%以上。
...
TL;DR: 提出一种通过大语言模型生成数据文本描述的方法，结合外部知识库和影响力估计来选择最有信息量的描述，并通过跨模态迁移分类任务验证描述的有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型的行为理解仍是一个黑盒，现有可解释AI方法主要关注模型预测的解释，而本文旨在通过语言这一人类易理解的媒介来解释数据本身。

Method: 构建管道使用大语言模型生成数据文本描述，结合外部知识库，利用影响力估计和CLIP评分选择最相关的描述，并提出跨模态迁移分类基准任务。

Result: 在零样本设置下，生成的文本描述比其他基线描述更有效，在九个图像分类数据集上成功提升了仅使用图像训练的模型性能，GPT-4o评估进一步支持了这些结果。

Conclusion: 该方法能够提供对模型决策过程内在可解释性的洞察，通过文本描述有效解释数据并提升模型性能。
...
TL;DR: DANS-KGC是一种基于扩散的自适应负采样方法，用于知识图谱补全，通过难度评估、自适应负采样和动态训练机制生成不同难度的负样本，在多个基准数据集上取得SOTA结果。
...
Method: 提出DANS-KGC框架，包含三个核心组件：难度评估模块（DAM）整合语义和结构特征评估实体学习难度；自适应负采样模块（ANS）使用条件扩散模型和难度感知噪声调度生成不同难度负样本；动态训练机制（DTM）在训练过程中动态调整负样本难度分布。

Result: 在六个基准数据集上的广泛实验表明，DANS-KGC在UMLS和YAGO3-10数据集的所有三个评估指标上均取得了最先进的结果，证明了方法的有效性和泛化能力。

Conclusion: DANS-KGC通过创新的扩散模型和自适应采样策略，有效解决了传统负采样方法的局限性，为知识图谱表示学习提供了更强大的负采样方案。
...
TL;DR: 本研究通过脑电图和卡片分类任务研究人类适应性推理的神经机制，发现delta-theta-alpha振荡协调是适应性推理的关键神经特征，而多模态大语言模型缺乏真正的层次规则抽象能力。


<details>
  <summary>Details</summary>
Motivation: 适应性推理使人类能够在环境规则或情境变化时灵活调整推理策略，但其潜在的神经动力学机制尚不清楚。本研究旨在揭示适应性推理的神经生理机制。

Method: 使用卡片分类范式结合脑电图技术，比较人类表现与多模态大语言模型的性能，进行刺激锁定和反馈锁定的神经活动分析。

Result: 发现协调的delta-theta-alpha动态：早期delta-theta活动反映探索性监测和规则推断，而枕叶alpha参与表明成功规则识别后的注意确认稳定化。多模态大语言模型仅表现出短期反馈驱动调整，缺乏层次规则抽象或真正的适应性推理。
...
<details>
  <summary>Details</summary>
Motivation: 为严重表达性语言障碍的失语症患者开发临床通信支持的想象语音脑机接口，解决实际临床环境中环境变异性和最小预处理约束下的可靠性能问题。
...
Conclusion: 该框架证明了在实际临床约束下部署基于扩散的EEG解码的可行性，推进了想象语音脑机接口向临床通信支持的转化应用。
...
TL;DR: 本文提出了同构数学问题生成（IMPG）任务，旨在生成与源问题结构一致的问题变体，并开发了基于LLM的CBIT框架，通过元级生成和模板化选择变异实现高数学正确性和结构一致性。


<details>
  <summary>Details</summary>
Motivation: 个性化数学教育快速发展，对大量相似练习题的需求激增，但现有数学问题生成研究主要关注数据增强而非直接教育部署，需要填补这一空白。

Method: 定义了IMPG任务，探索了基于LLM的自动生成框架，建立了CBIT方法，采用元级生成和模板化选择变异策略。

Result: CBIT在生成准确性和成本效益方面表现优异，生成问题的错误率比专家编写问题低17.8%，在商业教育平台上部署到6,732名学习者，产生186,870次交互。

Conclusion: CBIT框架能够高效生成结构一致的同构数学问题，满足个性化数学教育的需求，具有实际部署价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有脑机接口研究主要局限于静态固定环境，限制了实际应用。为了推动脑机接口走向实用化，需要开发能够在日常环境中使用的灵活系统。

Method: 开发了一个实时无线想象语音EEG解码系统，包含用户识别模块实现个性化服务，使用实验室流层管理实时EEG信号流，构建端到端的处理管道。

Result: 系统在有线设备上实现4类分类准确率62.00%，在便携无线头戴设备上达到46.67%的准确率，证明了从有线设备向无线便携硬件扩展的可行性。

Conclusion: 这项工作向真正实用和易用的脑机接口技术迈出了重要一步，为未来稳健、实用和个性化的神经接口研究确立了明确方向。
...
TL;DR: 提出了Thinker模型，通过多轮交互进行深度搜索的分层思维模型，将复杂问题分解为可独立解决的子问题，使用自然语言和逻辑函数双重表示，并通过知识边界确定避免不必要的外部搜索。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要采用端到端强化学习，缺乏对推理过程的监督，难以保证逻辑连贯性和严谨性。

Method: 将复杂问题分解为子问题，每个子问题用自然语言和逻辑函数双重表示，通过逻辑函数传递子问题间依赖关系，并进行知识边界确定。

Result: 仅需数百个训练样本即可与现有基线竞争，在完整训练集上显著优于其他方法，在不同数据集和模型规模上表现优异。

Conclusion: Thinker模型使推理过程可监督和可验证，提高了问题解决的逻辑连贯性，在少量训练数据下也能取得良好效果。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列生成是一个重要研究课题，但面临两个关键挑战：传统扩散模型计算效率低（需要数百到数千次函数评估），而普通流匹配(ODE)无法显式捕捉时间序列的固有随机性和局部变化。

Method: 提出TimeFlow框架：1）基于SDE的流匹配方法，自然适合建模随机性；2）采用仅编码器架构；3）设计分量分解的速度场来捕捉时间序列的多方面结构；4）在标准流匹配优化中增加随机项以增强表达能力。

Result: 在多个数据集上的广泛实验表明，TimeFlow在生成质量、多样性和效率方面持续优于强基线模型。

Conclusion: TimeFlow是一个灵活通用的框架，支持无条件和条件生成任务，通过SDE-based流匹配有效解决了时间序列生成中的随机性建模和效率问题。
...
TL;DR: VARS是一种基于图表示的创新心电图诊断方法，能够统一处理不同配置的ECG信号，并通过对比学习增强风险信号检测能力，在多个数据集上超越现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习ECG诊断方法面临的两个主要障碍：缺乏处理多样化ECG信号的通用性，以及由于样本不平衡导致的风险信号检测不足。

Method: 使用基于图的表示方法统一建模异构ECG信号，将ECG信号转换为图结构以捕获关键诊断特征，结合去噪重建和对比学习来保留原始信息并突出病理模式。

Result: 在三个不同ECG数据集上的评估显示，VARS在所有数据集上持续超越现有最先进模型，并在风险信号识别方面有显著改进，同时提供可解释性以辅助临床决策。

Conclusion: VARS有望成为全面心脏健康评估的宝贵工具，能够处理多样化ECG配置并增强风险信号检测能力。
...
TL;DR: 提出了一种细粒度反事实解释框架，通过对象级和部分级可解释性来解决模型误分类问题，识别关键特征并定位影响区域。
...
Method: 采用非生成式方法生成可解释反事实，通过量化相似性和加权组件贡献来识别感兴趣区域；引入基于Shapley值贡献的显著性分区模块来隔离具有区域特定相关性的特征。

Result: 大量实验表明该方法在捕捉更细粒度、直观有意义的区域方面表现优越，超越了现有的细粒度方法。

Conclusion: 该框架能够有效解决模型误分类问题，提供更细粒度的解释，有助于理解模型决策过程。
...
<details>
  <summary>Details</summary>
Motivation: 现有法律基准往往混淆事实记忆与真正推理，片段化推理过程，忽视推理质量，需要开发能评估复杂法律推理能力的数据集。

Method: 基于IRAC框架构建多步骤法律推理数据集，设计可扩展的人机协作标注流程，评估多种LLMs性能，比较自主生成与人工设计思维链提示的效果。

Result: LLMs在MSLR上表现中等，自主生成的思维链提示在推理连贯性和质量上优于人工设计提示。

Conclusion: MSLR为推进LLM推理和思维链策略提供了开放资源，揭示了适应复杂法律推理的挑战，自主生成提示方法具有优势。
...
TL;DR: ST-SAM是一种新颖高效的时空自注意力模型，用于交通流量预测，通过自注意力机制联合捕捉时空依赖关系，在准确性和效率上都显著优于现有方法。
...
Method: 使用区域嵌入层学习时间特定嵌入，然后基于自注意力机制的时空依赖学习模块来联合捕捉附近和远处区域的时空依赖关系。

Result: 在两个真实世界数据集上的实验表明，ST-SAM在RMSE上平均提升15%，MAPE上提升17%，训练时间快32倍。

Conclusion: ST-SAM完全依赖自注意力来捕捉局部和全局时空相关性，使其既有效又高效，显著优于现有最先进方法。
...
TL;DR: 该研究将脑调谐方法扩展到多模态音频-视频模型，通过针对社交处理关键区域STS进行训练，提高了脑对齐度并改善了社交认知任务（情景喜剧中的讽刺检测）性能。
...
Method: 使用多模态音频-视频模型，通过脑调谐方法对STS区域进行微调，使模型更好地预测相应的fMRI活动。

Result: 显著提高了与STS及相邻ROI的脑对齐度，并在与训练数据相关的社交认知任务（情景喜剧讽刺检测）上表现改善。
...
TL;DR: 该论文提出了一种新方法VSPO，使用LLaMA-3.1-8B-Instruct微调模型来自动生成验证本体语义陷阱的能力问题(CQs)，相比GPT-4.1在精度和召回率上分别提升26%和28.2%。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要基于与现有数据集的相似性评估生成的CQs，无法可靠检测语义陷阱如'Misusing allValuesFrom'等，需要自动化验证语义陷阱的CQ生成方法。

Method: 使用LLMs生成类和属性的自然语言定义，通过移除公理或改变逻辑运算符引入定义与本体之间的不对齐，然后微调LLaMA-3.1-8B-Instruct来生成验证这些语义差异的CQs。

Result: 微调模型在生成验证陷阱的CQs方面表现优于基线，相比GPT-4.1精度提高26%，召回率提高28.2%，能够检测比现有公共数据集更广泛的建模错误。

Conclusion: 该研究实现了使用LLMs自动生成验证TBox的CQs，显著减少了人工工作量，同时改善了本体与专家知识之间的语义对齐，是首个使用LLMs针对CQ生成中语义陷阱验证的研究。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种专注于将多元时间序列转换为单变量时间序列的异常检测方法，以简化复杂多元数据的异常检测问题。

Method: 使用FCM聚类和模糊积分技术将多元时间序列转换为单变量时间序列，然后应用HMM模型进行异常检测，并比较不同的转换方法。

Result: 通过一系列实验研究和比较分析验证了所提出方法的有效性。

Conclusion: 该方法能够有效检测多元时间序列中的异常，通过转换技术简化了检测过程，HMM模型在异常检测中表现良好。
...
TL;DR: 提出新的扰动框架评估LLMs在复杂环境中的数学推理能力，通过注入语义无关的扰动句子并逐步增加扰动强度，发现LLMs对含数字的扰动更敏感，性能下降明显，且在没有核心问题指令时仍能保持一定准确率，表明可能依赖记忆模板而非逻辑推理。
...
Method: 提出扰动框架，注入语义无关的扰动句子并逐步增加扰动强度，同时使用核心问题指令缺失的额外扰动方法，分析LLMs的问题解决机制。

Result: LLMs在面对不含数字的扰动句子时表现稳定但存在鲁棒性边界；面对含数字的扰动时性能显著下降，开源模型下降近10%以上，最大下降达51.55%，商业LLMs也有3%-10%下降；在核心问题指令缺失时仍能保持20%-40%准确率。

Conclusion: LLMs对含数字信息的扰动更敏感，容易在无关数字信息干扰下给出错误答案；可能依赖记忆模板或模式匹配而非逻辑推理完成任务；揭示了当前LLMs推理能力的缺陷，对LLMs进一步发展具有重要意义。
...
TL;DR: 提出了一种知识增强的长链推理框架，通过结合大型语言模型与基于知识图谱的多跳推理链，解决生物分子推理中的逻辑不一致和领域知识缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理生物分子推理任务时存在逻辑不一致、缺乏领域知识基础的问题，且现有基准测试在规模和深度上受限，无法支持深度推理链的评估。

Method: 开发了知识增强的长链推理框架，通过知识图谱上的引导多跳遍历和剪枝构建机制链，结合监督微调和强化学习来提升事实基础和推理可靠性。

Result: 在PrimeKGQA和现有数据集上的实验表明，随着推理深度增加，该方法在多跳任务上表现出明显优势，实现了最先进的性能。

Conclusion: 将结构化知识与先进推理策略相结合，能够实现可靠且可解释的生物分子推理，为复杂生物机制理解提供了有效解决方案。
...
TL;DR: 提出了一种基于心理学理论的Scaffold Reasoning框架用于代码调试，通过三个流（Scaffold、Analytic、Integration）实现高效推理，在DebugBench上达到88.91%通过率和5.36秒平均推理时间。
...
Method: 提出Scaffold Reasoning框架，包含三个流：Scaffold流构建参考代码，Analytic流分析错误代码，Integration流整合两者结果。

Result: 在DebugBench上达到88.91%的通过率，平均推理时间5.36秒/问题，优于其他推理方法，在不同问题难度和错误类型下表现稳定。

Conclusion: 该框架在推理准确性和效率上均优于现有方法，其设计符合人类认知过程，为LLM推理优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展和广泛应用，计算资源需求激增，测试时缩放进一步加剧了模型能力与资源消耗之间的矛盾。目前缺乏一个能够准确反映不同模型规模和架构下LLM效率的统一指标。

Method: 基于压缩与智能之间的相关性，引入信息容量这一指标，通过文本压缩性能相对于计算复杂度的比值来衡量模型效率。对主流开源模型进行实证评估，分析不同规模模型在系列中的信息容量一致性。

Result: 实证评估表明，同一系列中不同规模的模型展现出一致的信息容量。该指标能够实现跨模型系列的公平效率比较和系列内性能的准确预测。信息容量还考虑了通常被忽视的分词器效率因素。

Conclusion: 信息容量是一个有效的LLM效率评估指标，能够公平比较不同模型系列，并在49个模型和5个异构数据集上观察到分词器效率、预训练数据和专家混合架构影响的一致性结果。
...
<details>
  <summary>Details</summary>
Motivation: 多变量时间序列异常检测在科学和工程中有广泛应用，但由于需要同时考虑时间和变量关系而具有挑战性。现有方法在检测振幅和形状异常方面存在局限。

Method: 使用滑动窗口生成多变量子序列，应用扩展模糊聚类揭示结构，采用重构准则重建子序列，构建置信指数量化异常水平，并使用粒子群优化进行优化。
...
Method: 采用五支柱架构和RAIRAB基准，在六个任务族中评估检索基础配置的性能表现。

Result: 检索基础配置实现了更高的基础准确性(0.90)，幻觉和解释漂移减少约40%，透明度几乎翻倍。

Conclusion: 当治理明确、数据可追溯且保证可验证时，现有审慎原则已能容纳可靠的AI，降低了风险转移和资本配置中的信息摩擦。
...
TL;DR: 本文通过参数化复杂性理论重新审视Pearl因果层次(PCH)的可满足性问题，识别出首个可处理性的突破口。研究提出了针对概率和反事实片段可满足性的固定参数和XP算法，使用原始树宽和变量数量等参数，并提供了匹配的硬度结果来界定可处理性的边界。
...
Method: 研究采用参数化复杂性方法，使用原始树宽和变量数量等参数，开发了针对关键概率和反事实片段可满足性的固定参数和XP算法。技术上，研究偏离了通常用于树宽算法的动态规划范式，转而利用结构良好的因果模型的结构特征。

Result: 研究获得了固定参数和XP算法用于PCH关键片段的可满足性，同时提供了匹配的硬度结果，精确地映射了可处理性的边界。

Conclusion: 本文为因果推理提供了一个新的算法工具包，通过参数化复杂性理论首次识别出PCH可满足性问题的可处理性突破口，并明确了可处理性的确切边界。
...
TL;DR: 本文研究了在注塑件质量分类中使用XAI技术进行特征缩减的效果，通过SHAP、Grad-CAM和LIME分析LSTM模型的特征重要性，将19个输入特征缩减到9个和6个，评估了模型准确性、推理速度和可解释性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 机器学习在工业质量控制中应用广泛，但模型复杂性和缺乏可解释性限制了实际应用，且工业设备往往缺乏全面的传感器技术，导致数据采集不完整。可解释人工智能(XAI)通过提供模型决策洞察和识别最重要分类特征来解决这些问题。

Method: 应用SHAP、Grad-CAM和LIME三种XAI技术分析长短期记忆(LSTM)模型在真实生产数据上的特征重要性，将原始19个输入特征缩减到9个和6个，评估模型准确性、推理速度和可解释性的权衡。

Result: 特征缩减可以改善泛化能力，同时保持高分类性能，推理速度有小幅提升。在特征缩减到9个和6个时，模型仍能保持良好性能。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习在优化工业过程中具有巨大潜力，但数据获取成本高且耗时。合成数据可以解决数据不足的问题，提高机器学习模型的鲁棒性。
...
TL;DR: SpeechCARE是一个多模态语音处理框架，用于通过语音分析早期检测阿尔茨海默病和相关痴呆症，结合声学、语言学和人口统计学特征，在认知障碍分类方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 超过一半的认知衰退患者未被诊断，传统语音处理方法性能有限且泛化能力不足，需要更有效的早期检测工具。

Method: 采用预训练的多语言声学和语言转换器模型，结合动态融合架构整合多模态输入，包括自动转录、LLM异常检测和任务识别等预处理步骤。

Result: 在认知健康、轻度认知障碍和阿尔茨海默病分类中达到AUC=0.88和F1=0.72，轻度认知障碍检测达到AUC=0.90和F1=0.62，偏差分析显示除80岁以上人群外差异较小。

Conclusion: SpeechCARE在认知障碍检测方面表现出色，未来将在真实医疗环境中部署，并为纽约市服务不足人群提供EHR集成的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统在特定科学任务上表现出专家级性能，但仍是狭窄且手工构建的，需要开发能够跨学科和难度级别自适应推理策略的通用科学推理系统。

Method: 采用分层多智能体架构：协调器智能体分析问题领域和复杂度，动态编排由符号推理、概念建模、数值计算和验证子智能体组成的专业工作系统，协作构建和优化推理管道。

Result: 在数学和物理奥林匹克竞赛（IMO、IMC、IPhO、CPhO）中持续达到或超越人类金牌选手表现，在国际化学奥林匹克和Humanity's Last Exam基准测试中也表现出跨学科泛化能力。

Conclusion: SciAgent是迈向通用科学智能的具体步骤，展示了在专家水平上进行连贯跨学科推理的AI系统能力。
...
Method: 从零开始开发图像生成模型，使用仅版权清理图像进行训练，采用特殊架构设计以在有限数据集下生成高质量图像。

Result: 成功开发'oboro:'图像生成模型，公开发布基础模型权重和推理代码，成为日本首个完全本土开发的开源商业图像生成AI。

Conclusion: 该项目标志着日本首个开源商业图像生成AI的发布，通过保持开发过程透明度，旨在为日本AI研究社区和工程师社区做出贡献，推动国内AI开发生态系统发展。
...
### [169] [Towards Provably Unlearnable Examples via Bayes Error Optimisation](https://arxiv.org/abs/2511.08191)
*Ruihan Zhang,Jun Sun,Ee-Peng Lim,Peixin Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种基于最大化贝叶斯误差的新方法来构建不可学习样本，该方法在理论上有保证，并且在混合干净数据时仍保持有效性。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型依赖大规模数据进行训练，用户数据保护成为重要问题。现有不可学习样本方法缺乏理论保证，且在混合干净数据时失效。

Method: 采用基于优化的方法，通过投影梯度上升系统性地最大化贝叶斯误差来构建不可学习样本。

Result: 实验结果表明该方法能有效限制数据可学习性，在多种数据集和模型架构上均表现一致。

Conclusion: 提出的方法在理论上具有保证，在实践中能有效保护用户数据隐私，解决了现有方法在混合数据场景下的局限性。
...
### [170] [EHRStruct: A Comprehensive Benchmark Framework for Evaluating Large Language Models on Structured Electronic Health Record Tasks](https://arxiv.org/abs/2511.08206)
*Xiao Yang,Xuejiao Zhao,Zhiqi Shen*

Main category: cs.AI

TL;DR: 本文介绍了EHRStruct基准测试，用于评估大语言模型在结构化电子健康记录数据上的性能，包含11个任务和2200个评估样本，并提出了EHRMaster方法提升模型表现。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏标准化的评估框架和明确定义的任务，难以系统评估和比较大语言模型在结构化EHR数据上的性能。

Method: 开发EHRStruct基准测试，包含11个代表性任务和2200个评估样本；评估20个先进LLM；分析影响性能的关键因素；提出EHRMaster代码增强方法。

Result: 许多结构化EHR任务对LLM的理解和推理能力要求很高；EHRMaster方法实现了最先进的性能。

Conclusion: EHRStruct为结构化EHR数据的LLM评估提供了标准化框架，EHRMaster方法有效提升了模型在临床任务中的表现。
...
### [171] [MADD: Multi-Agent Drug Discovery Orchestra](https://arxiv.org/abs/2511.08217)
...
<details>
  <summary>Details</summary>
Motivation: 传统命中识别方法需要大量实验资源，而现有AI工具复杂性限制了湿实验室研究人员的可访问性。多智能体系统结合了LLMs的可解释性和专业模型的精确性。
...
Conclusion: MADD为药物设计的智能体化未来做出了贡献，引入了包含300多万个化合物的查询-分子对和对接得分的新基准。
...
### [172] [Beyond Distributions: Geometric Action Control for Continuous Reinforcement Learning](https://arxiv.org/abs/2511.08234)
*Zhihao Lin*
...
### [173] [Towards Outcome-Oriented, Task-Agnostic Evaluation of AI Agents](https://arxiv.org/abs/2511.08242)
...
<details>
  <summary>Details</summary>
Motivation: 当前仅基于基础设施指标（如延迟、首token时间、token吞吐量）评估AI智能体性能已不足够，这些指标无法捕捉智能体决策质量、操作自主性或最终业务价值。

Method: 提出了11个基于结果的、任务无关的性能指标，并通过大规模模拟实验，在四个不同智能体架构（ReAct、思维链、工具增强、混合）和五个不同领域（医疗、金融、营销、法律、客户服务）中验证框架效果。

Result: 实验结果显示不同智能体设计之间存在显著性能权衡，混合智能体在大多数指标上表现最一致，平均目标完成率达到88.8%，投资回报率最高。
...
### [174] [Where and What Matters: Sensitivity-Aware Task Vectors for Many-Shot Multimodal In-Context Learning](https://arxiv.org/abs/2511.08246)
*Ziyu Ma,Chenhui Gou,Yiming Hu,Yong Wang,Xiangxiang Chu,Bohan Zhuang,Jianfei Cai*

Main category: cs.AI

TL;DR: 提出了一种敏感性感知任务向量插入框架（STV），通过识别激活模式中的敏感位置，并使用强化学习选择最合适的插入值，解决了多模态模型在少样本学习中的上下文长度限制和推理成本问题。
...
Method: 提出STV框架，通过分析查询-上下文对的激活差异来识别敏感位置，为每个位置构建预聚类的激活库，然后使用强化学习选择最合适的插入值。

Result: 在多种多模态模型（如Qwen-VL、Idefics-2）和任务（如VizWiz、OK-VQA）上评估STV，证明其有效性，并在先前任务向量方法基础上显示出持续改进和强泛化能力。

Conclusion: STV框架通过敏感性感知的位置识别和强化学习驱动的值选择，有效解决了多模态模型多样本上下文学习中的挑战，提供了可靠的性能提升。
...
### [175] [Multi-Agent GraphRAG: A Text-to-Cypher Framework for Labeled Property Graphs](https://arxiv.org/abs/2511.08274)
*Anton Gusarov,Anastasia Volkova,Valentin Khrulkov,Andrey Kuznetsov,Evgenii Maslov,Ivan Oseledets*

Main category: cs.AI

TL;DR: 本文提出Multi-Agent GraphRAG系统，利用Cypher查询语言和标签属性图数据库作为GraphRAG的可扩展推理引擎，通过多智能体工作流实现文本到Cypher查询的自动生成和执行。


<details>
  <summary>Details</summary>
Motivation: 现有GraphRAG研究主要关注RDF知识图谱，而Cypher和标签属性图数据库在GraphRAG管道中作为可扩展有效推理引擎的潜力尚未得到充分探索。

Method: 提出模块化LLM智能体系统，包含自动Cypher查询生成和执行的工作流，使用Memgraph作为图数据库后端，通过迭代内容感知校正和规范化确保查询的语义和句法优化。

Result: 在CypherBench图数据集上评估系统性能，涵盖多个通用领域的不同类型查询，并在基于IFC数据的属性图上展示工作流性能。

Conclusion: 该方法能够将AI与大规模现实世界应用连接，实现工业数字自动化用例，展示了标签属性图在GraphRAG中的实际应用潜力。
...
### [176] [DiagramIR: An Automatic Pipeline for Educational Math Diagram Evaluation](https://arxiv.org/abs/2511.08283)
*Vishal Kumar,Shubhra Mishra,Rebecca Hao,Rizwaan Malik,David Broman,Dorottya Demszky*

Main category: cs.AI

TL;DR: DiagramIR是一个自动且可扩展的几何图形评估管道，通过中间表示方法评估LLM生成的图表，比LLM-as-a-Judge基准具有更高的人类评分一致性，并能以10倍更低的推理成本实现与大型模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为学习工具主要限于文本形式，在需要可视化表达的领域（如数学）应用受限。虽然LLM能够生成编译为教育图形的代码，但缺乏可扩展的图表评估方法成为主要瓶颈。

Method: 提出DiagramIR评估管道，基于LaTeX TikZ代码的中间表示（IRs）进行自动和可扩展的几何图形评估。

Result: 该方法与人类评分者的一致性高于其他评估基准（如LLM-as-a-Judge），使GPT-4.1-Mini等较小模型能够以10倍更低的推理成本实现与GPT-5等大型模型相当的性能。
...
### [177] [Smarter Together: Creating Agentic Communities of Practice through Shared Experiential Learning](https://arxiv.org/abs/2511.08301)
*Valentin Tablan,Scott Taylor,Gabriel Hurtado,Kristoffer Bernhem,Anders Uhrenholt,Gabriele Farei,Karo Moilanen*

Main category: cs.AI

TL;DR: Spark是一个新型的共享智能体记忆架构，旨在模拟人类开发者社区的集体智能和知识，帮助AI编程智能体实现集体持续学习，提升代码生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统的人类中心软件开发知识共享环境正在被以智能体为中心的实践所颠覆，而AI编程智能体缺乏有效的共享学习资源库，导致大量生成的代码无法受益于集体经验。

Method: 提出Spark共享智能体记忆架构，使AI编程智能体能够贡献和获取持续演化的经验记忆，作为软件开发生成任务的教练。

Result: Spark的推荐显著提升了通用代码生成模型的代码质量，300亿参数的小型模型在Spark帮助下能达到与更大规模最先进模型相当的代码质量；推荐的有用性在最高两个定性有用性等级中达到98.2%。

Conclusion: Spark架构成功模拟了人类开发者社区的集体智能，为AI编程智能体提供了有效的知识共享机制，显著提升了代码生成质量和小型模型的性能表现。
...
Conclusion: 基于云的数据可视化应用显著减少了数据分析过程的人工输入，同时保持高质量的可视化输出和用户体验。
...
TL;DR: 本文提出了一种利用自组织映射(SOMs)从语言模型中提取多个拒绝方向的新方法，证明该方法优于传统的单方向方法，并能更有效地抑制模型的拒绝行为。


<details>
  <summary>Details</summary>
Motivation: 现有研究将拒绝行为编码为潜在空间中的单一方向，但新兴证据表明LLM中的概念通常以低维流形形式编码。本文受此启发，旨在探索多个拒绝方向。

Method: 使用自组织映射(SOMs)在有害提示表示上训练以识别多个神经元，通过从每个神经元减去无害表示的质心来推导多个拒绝方向。

Result: 实验验证表明，从模型内部消融多个方向不仅优于单方向基线，还优于专门的越狱算法，能有效抑制拒绝行为。

Conclusion: 通过分析方法的机制含义，证实了多方向方法在拒绝行为抑制方面的优越性。
...
TL;DR: 本文提出了FaithEval用于评估多模态推理中的忠实性，并开发了FaithAct框架来在每一步推理中强制证据基础，显著提高了感知忠实性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在推理过程中经常产生看似合理但缺乏依据的推理链，与感知证据或最终结论存在偏差，这构成了持续性的忠实性问题。

Method: 区分行为忠实性（推理与输出的对齐）和感知忠实性（推理与输入的对齐），提出FaithEval量化步骤级和链级忠实性，并开发FaithAct框架在每一步推理中强制证据基础。

Result: 在多个推理基准测试中，FaithAct将感知忠实性提高了高达26%，同时不降低任务准确性，相比基于提示和工具增强的基线方法表现更优。

Conclusion: 将忠实性作为指导原则不仅能缓解幻觉问题，还能产生更稳定的推理轨迹，为多模态推理中的忠实性评估和执行建立了统一框架。
...
TL;DR: 本文提出了一个基于ISO/PAS 8800指南的安全数据集开发框架，专注于自动驾驶AI感知系统，涵盖数据收集、标注、管理和维护的全生命周期，通过严格的安全分析和验证策略确保数据集完整性。
...
Method: 引入AI数据飞轮和数据集生命周期概念，结合严格的安全分析来识别危险和减轻数据集不足带来的风险，定义数据集安全要求建立流程和验证策略。

Result: 提出了一个全面的数据集安全框架，整合了最佳实践和新兴趋势，为自动驾驶应用提供安全保证。

Conclusion: 通过整合不同视角，该框架旨在推进自动驾驶应用中稳健、安全保证的AI系统发展。
...
TL;DR: 提出了一种轻量级模块化的LLM安全补丁方法，通过添加仅占0.003%参数的紧凑可学习前缀，快速修复现有模型的安全漏洞，而无需昂贵的大版本更新。


<details>
  <summary>Details</summary>
Motivation: 现有LLM厂商发布改进版本成本高、频率低且难以定制，导致已发布模型存在已知安全漏洞，需要更轻量、快速的修复方案。

Method: 在现有模型前添加紧凑可学习前缀作为"补丁"，仅引入极少量额外参数，将模型行为引导至更安全的参考模型方向。

Result: 在毒性缓解、偏见减少和有害拒绝三个关键领域，安全补丁实现了与下一代安全对齐模型相当的安全改进，同时保持流畅性。

Conclusion: LLM可以像软件一样打补丁，为厂商和从业者提供了在大版本发布之间分发可扩展、高效且可组合安全更新的实用机制。
...
TL;DR: 该研究通过两个实证研究探讨了人类与LLM在评估数学问题趣味性和难度方面的对齐程度，发现LLM虽然大致认同人类的趣味性概念，但无法完全捕捉人类判断的分布模式，且与人类选择趣味性理由的相关性较弱。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统（如LLM）越来越多地参与数学研究和教育，了解它们对人类趣味性判断的认知对齐程度变得重要，这关系到AI能否成为有效的数学思维伙伴。

Method: 通过两个实证研究，分别对众包平台参与者和国际数学奥林匹克竞赛选手进行调查，比较人类和LLM对数学问题趣味性和难度的评估。

Result: LLM大致认同人类的趣味性概念，但无法完全捕捉人类判断的分布模式；大多数LLM与人类选择趣味性理由的相关性较弱。

Conclusion: 当前LLM在捕捉人类数学趣味性判断方面既有前景也存在局限，需要进一步改进才能成为有效的数学AI思维伙伴。
...
### [184] [Simulating the Visual World with Artificial Intelligence: A Roadmap](https://arxiv.org/abs/2511.08585)
*Jingtong Yue,Ziqi Huang,Zhaoxi Chen,Xintao Wang,Pengfei Wan,Ziwei Liu*

Main category: cs.AI

TL;DR: 本文综述了视频生成模型向世界模型发展的演变过程，将现代视频基础模型概念化为隐式世界模型和视频渲染器的组合，探讨了四代视频生成技术的核心能力演进。


<details>
  <summary>Details</summary>
Motivation: 视频生成领域正从生成视觉吸引人的片段转向构建支持交互和保持物理合理性的虚拟环境，这指向了视频基础模型作为隐式世界模型的发展方向。

Method: 将视频基础模型分解为两个核心组件：隐式世界模型（编码物理规律、交互动态和智能体行为）和视频渲染器（将潜在模拟转换为逼真视觉观察）。

Result: 通过四代技术演进，视频生成模型最终发展成具有内在物理合理性、实时多模态交互和跨时空尺度规划能力的世界模型。

Conclusion: 下一代世界模型面临开放挑战，包括智能体智能在塑造和评估这些系统中的作用，需要建立相应的设计原则。
