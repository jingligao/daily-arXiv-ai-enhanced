...
<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法仅在训练数据的变换上学习不变性，无法完全捕捉这些属性。作者希望开发一种能够直接在整个输入空间中强制执行已知不变性的方法。

Method: 使用在训练集上训练的生成模型来近似输入分布并生成未标记的虚拟图像，然后在半监督方式下同时训练分类和提示目标，利用未标记的虚拟样本来指导模型学习期望的提示属性。
...
Conclusion: 生成提示方法提供了一种有效的方式来直接在整个输入空间中强制执行已知不变性，相比传统数据增强方法具有更好的性能表现。
...
TL;DR: ProM3E是一个概率掩码多模态嵌入模型，用于生态学中任意到任意多模态表示生成，通过掩码模态重建学习从上下文模态推断缺失模态，支持嵌入空间中的模态反转，并提出新颖的跨模态检索方法。
...
Method: 基于掩码模态重建在嵌入空间中学习，通过概率模型分析不同模态融合的可行性，提出混合模态间和模态内相似度的跨模态检索方法。

Result: 模型在跨模态检索任务中表现出优越性能，并在线性探测任务中展示了强大的表示学习能力。

Conclusion: ProM3E为生态学多模态数据分析提供了有效的任意到任意生成框架，具有模态融合分析和跨模态检索的实用价值。
...
TL;DR: EvtSlowTV是一个从YouTube视频构建的大规模事件相机数据集，包含超过130亿个事件，用于解决事件相机深度估计中标注数据不足的问题，支持自监督学习框架。
...
Method: 从公开的YouTube慢动作视频构建EvtSlowTV大规模数据集，采用自监督学习框架，无需基于帧的标注，保留事件数据的异步特性。

Result: EvtSlowTV比现有事件数据集大一个数量级，训练后的模型在复杂场景和运动中的泛化能力得到提升。

Conclusion: EvtSlowTV为事件相机深度估计提供了大规模、无约束的自然场景数据，展示了自监督学习在利用事件数据高动态范围潜力方面的适用性。
...
TL;DR: SCALE-VLP是一个软加权对比视觉语言预训练框架，专门针对医学CT等体积数据设计，通过整合体积空间语义和领域感知知识，在有限监督下实现结构一致和语义基础的表征。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要局限于2D数据和二元监督，忽略了CT等体积数据的连续结构化依赖关系，且通常将体积扫描视为独立2D切片处理，损害了空间连贯性和临床语义的充分利用。

Method: 提出SCALE-VLP框架，整合：(i)体积空间语义以保持解剖结构；(ii)领域感知、知识注入的语义（如放射学本体）来指导对齐，采用软加权对比学习方法。

Result: 相比先前最先进方法，SCALE-VLP实现高达4.3倍的CT报告检索top-1准确率，异常分类提升10个百分点，报告生成达到ROUGE-L 0.44和BERT-F1 0.89。在零样本外部数据集评估中也观察到一致增益。

Conclusion: SCALE-VLP展示了强大的跨任务可迁移性（检索、报告生成和分类）和跨领域泛化能力，无需进一步微调即可获得一致性能提升。
...
TL;DR: 提出了一种基于自监督学习的高效标签方法，仅使用1000个标注图像块实现州级1米分辨率土地覆盖分类，准确率达87.14%。
...
Method: 使用BYOL自监督预训练策略在大量未标记彩色红外航空图像上预训练ResNet-101编码器，然后将学习到的权重迁移到多个语义分割架构进行微调。
...
TL;DR: 提出一个用于脑部MRI的基础模型，能够处理不同成像序列组合，通过可学习模态嵌入、条件层归一化和掩码自编码目标来处理缺失模态，使用方差-协方差正则化器稳定特征学习。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法需要为每种模态单独训练模型的问题，开发能够适应不同模态组合和缺失模态的灵活模型。

Method: 使用单一编码器配合可学习模态嵌入、条件层归一化、掩码自编码目标，并应用方差-协方差正则化器，在约60,000个多中心MRI上进行自监督重建和模态插补训练。

Result: 初步结果显示方法可行，能够适应不同模态设置，计划在脑肿瘤、多发性硬化症分割和病变分类等任务上进行详细评估。

Conclusion: 该模型能够灵活处理不同MRI序列组合，无需为每种模态单独训练模型，代码和预训练模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 在扫描到BIM、变形分析和3D建模等高精度应用中，评估移动激光扫描点云的不确定性至关重要，但获取地面真值成本高且在实际应用中不可行。

Method: 提出了一个学习框架，集成了最优邻域估计和几何特征提取，使用XGBoost模型进行不确定性预测。
...
TL;DR: ISC-Perception是首个专门用于ISC组件检测的混合数据集，结合了CAD渲染图像、游戏引擎照片级真实感场景和有限真实照片，自动标注合成部分，大幅减少人工标注时间81.7%，检测器性能显著优于仅使用合成或真实数据的模型。


<details>
  <summary>Details</summary>
Motivation: 解决ISC系统机器人感知中缺乏专用图像数据集的问题，因为在活跃建筑工地收集照片存在物流困难和安全隐私顾虑。

Method: 创建混合数据集ISC-Perception，包含程序化渲染的CAD图像、游戏引擎照片级真实感场景和少量精选真实照片，实现合成部分的完全自动标注。

Result: 生成10,000张图像数据集的人工时间仅为30.5小时，相比手动标注节省81.7%；检测器在IoU 0.50下达到0.756的平均精度，在1,200帧基准测试中达到0.943/0.823的mAP。
...
TL;DR: 提出DentalSplat框架，解决正畸治疗中仅使用稀疏三视图（前视图和双侧颊视图）进行3D重建的挑战，通过先验引导的点云初始化、尺度自适应剪枝和几何约束优化，在稀疏输入下实现高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 在远程正畸治疗中，从多视角观察患者牙颌关系对临床决策至关重要。传统3DGS方法需要密集多视图输入和精确相机位姿，而正畸病例通常只有三个稀疏图像，导致重建质量严重下降。

Method: 使用先验引导的密集立体重建模型初始化点云，采用尺度自适应剪枝策略提高3DGS训练效率和重建质量。在极稀疏视角下，结合光流作为几何约束和梯度正则化来增强渲染保真度。

Result: 在包含950个临床病例的大规模数据集和195个模拟远程正畸成像条件的视频测试集上验证，方法能有效处理稀疏输入场景，在牙颌可视化新视角合成质量上优于现有技术。

Conclusion: DentalSplat框架成功解决了正畸稀疏图像3D重建的挑战，为远程正畸治疗提供了有效的牙颌可视化解决方案。
...
TL;DR: 提出IC DefectNCD框架，利用IC SEM图像的固有先验进行缺陷检测和新类别发现，无需支持集。包含自归一化信息引导的缺陷检测、自适应二值化策略和自缺陷信息引导的缺陷分类。


<details>
  <summary>Details</summary>
Motivation: 集成电路制造过程复杂，缺陷会导致良率损失和产品可靠性下降。监督方法需要大量人工标注且难以处理新类别和罕见缺陷，聚类方法因缺乏先验而性能不稳定。
...
Conclusion: IC DefectNCD框架能够有效利用图像固有先验，实现无需支持集的缺陷检测和新类别发现，在真实工业场景中具有实用价值。
...
TL;DR: \sysname系统通过算法和系统优化加速视觉引导的物理属性推理，将端到端延迟从10-20分钟降至6秒以内，实现62.9-287.2倍加速，同时在物体级物理属性估计精度上达到或超过SOTA基线。


<details>
  <summary>Details</summary>
Motivation: 解决视觉引导物理属性推理过程中运行延迟高的问题，实现实时增强视觉认知能力。

Method: 采用快速几何3D重建、高效语义特征融合和并行视图编码等算法和系统优化方法。

Result: 在ABO数据集上实现62.9-287.2倍加速，物体级物理属性估计精度达到或超过SOTA基线，在材料分割和体素级推理方面表现更优。在真实环境IKEA家具店的案例研究中，即使视图较少也能提供稳健的属性估计。

Conclusion: \sysname系统通过优化显著降低了物理属性推理的延迟，在保持精度的同时实现了实时性能，为智能眼镜等设备提供了实用的增强视觉认知能力。
...
TL;DR: 本文开发并部署了首个用于无人机图像建筑物损坏评估的AI/ML系统，在联邦宣布的飓风灾害中实际运行，解决了灾害响应中图像数据过载的问题。


<details>
  <summary>Details</summary>
Motivation: 灾害响应中无人机团队每天收集大量图像数据（47GB-369GB），远超过专家处理能力，导致响应延迟。需要计算机视觉技术来自动化损坏评估。

Method: 基于最大的灾后无人机图像数据集（21,716个建筑物损坏标签）训练模型，并对91名灾害从业者进行操作培训，开发了建筑物损坏评估模型。

Result: 最佳模型在飓风Debby和Helene响应中部署，评估了415座建筑物，耗时约18分钟，实现了实际灾害中的AI/ML应用。

Conclusion: 这项工作建立了无人机图像损坏评估的实践标准，为AI/ML研究和用户社区提供了实际应用文档和经验教训。
...
TL;DR: 提出了一种基于深度引导的肝脏标志点分割框架，结合视觉基础编码器提取RGB和深度特征，并引入SRFT-GaLore高效微调方法，在腹腔镜肝脏手术中实现精确分割。


<details>
  <summary>Details</summary>
Motivation: 在腹腔镜肝脏手术中，2D视频流限制了深度感知并复杂化了标志点定位，需要融合RGB和深度特征来提升分割精度，同时需要高效适应大规模视觉模型到手术领域。

Method: 使用SAM2编码器提取RGB特征，DA2编码器提取深度感知特征，引入SRFT-GaLore低秩梯度投影方法替代计算昂贵的SVD，通过交叉注意力融合模块整合RGB和深度线索。

Result: 在L3D数据集上，Dice相似系数提升4.85%，平均对称表面距离降低11.78点；在新建的LLSD数据集上保持竞争力，显著优于SAM基线，展示强跨数据集鲁棒性。
...
Method: 收集了490名诊断为各种恶性肿瘤患者的全身18F-FDG PET/CT扫描和相应放射学报告，主要包含肺癌、肝癌、乳腺癌、前列腺癌和卵巢癌等常见癌症类型。

Result: 构建了包含配对PET和CT图像、去标识化文本报告和结构化临床元数据的PETWB-REP数据集。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在多图像场景中存在两个关键问题：缺乏跨不同图像的细粒度感知能力，以及有效推理和综合多视觉输入信息的能力下降。现有研究主要关注单图像设置或特定受限场景，缺乏对通用复杂多图像推理任务的理解和解决方案。

Method: 提出QG-CoC（问题引导的标题链）方法，这是一种通用的零样本提示方法，能够有效处理任意数量图像的问题。该方法通过问题引导的方式生成图像标题链，实现感知与推理的无缝集成。

Result: 在各种开源和闭源多模态大语言模型上的多图像和单图像基准测试表明，QG-CoC在不同任务中表现出竞争力，并在现有提示方法失败的挑战性场景中显示出稳健的改进。

Conclusion: QG-CoC是一种有效的零样本提示方法，能够显著提升多模态大语言模型在多图像场景中的感知和推理能力，特别是在复杂挑战性场景中表现优异。
...
TL;DR: 提出了一种结合样本混合和扩散合成的数据增强方法MCPMix，通过掩码一致性配对混合和真实锚定学习退火策略，在保持像素级语义的同时增强多样性，实现了内窥镜分割的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统的数据增强方法存在局限性：样本混合会产生软标签歧义，扩散合成则忽略了掩码条件化的结构优势并引入合成-真实域偏移。需要融合两者的优势来提升密集预测任务的性能。

Method: 提出MCPMix方法：为每个真实图像生成相同掩码下的合成对应物，然后进行掩码一致性配对混合，只混合图像外观而监督始终使用原始硬掩码。同时采用RLA策略自适应调整混合强度和混合样本的损失权重。

Result: 在Kvasir-SEG、PICCOLO、CVC-ClinicDB、私有NPC-LES队列和ISIC 2017等多个数据集上实现了最先进的分割性能，相比基线方法获得了一致的性能提升。

Conclusion: 将标签保持混合与扩散驱动的多样性相结合，配合自适应重新锚定策略，能够产生稳健且可泛化的内窥镜分割结果。
...
Conclusion: 该方法建立了具有线性复杂度的细粒度建模范式，在超分辨率任务中实现了更好的性能和效率平衡。
...
TL;DR: 提出了一种结合U-Mamba和热传导方程的混合架构，用于医学图像分割，能够同时实现高效的全局上下文建模和长距离依赖推理。
...
TL;DR: 本文提出了一个针对工业设备组件的3D异常检测数据集IEC3D-AD，并基于此开发了新的3D异常检测方法GMANet，通过几何形态分析生成合成点云样本，并通过空间差异优化提高检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测数据集无法充分捕捉真实工业环境中的复杂性和细微缺陷，限制了工业设备组件异常检测研究的精度。

Method: 开发了IEC3D-AD数据集，直接从实际生产线收集点云数据；提出了GMANet方法，通过几何形态分析生成合成点云样本，并使用空间差异优化来减小正常与异常点级特征之间的差距。
...
TL;DR: 提出了一种统一的长视频修复和外延方法，通过扩展文本到视频扩散模型来生成任意长度的高保真空间编辑视频。


<details>
  <summary>Details</summary>
Motivation: 生成长视频是一个基本挑战，实现视频修复和外延的高可控性尤其困难。需要同时解决这两个挑战，实现长视频剪辑的可控修复和外延。

Method: 利用LoRA高效微调预训练视频扩散模型（如Wan 2.1）进行掩码区域视频合成，采用重叠混合时间协同去噪策略和高阶求解器来保持长序列的一致性。

Result: 在数百帧的修复/外延任务上验证，在质量指标（PSNR/SSIM）和感知真实性（LPIPS）上优于基线方法Wan 2.1和VACE。

Conclusion: 该方法实现了实用的长距离视频编辑，以最小开销在参数效率和性能之间取得了平衡，能够无缝合或漂移地生成任意长度的视频。
...
TL;DR: 本文提出Diffusion-SDPO方法，解决扩散模型直接偏好优化中的关键问题：扩大偏好边际不一定提升生成质量，反而可能增加重建误差。新方法通过自适应缩放失败者梯度来保护获胜者，确保偏好输出的误差不增加。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的直接偏好优化方法存在病理问题，扩大偏好边际不一定改善生成质量，反而可能同时增加获胜者和失败者的重建误差，导致偏好分支也受到不利影响。

Method: 提出Diffusion-SDPO方法，采用保护性更新规则，根据失败者梯度与获胜者梯度的对齐程度自适应缩放失败者梯度。通过一阶分析得到闭式缩放系数，保证每个优化步骤中偏好输出的误差不增加。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术视频问答方法局限于静态图像特征，缺乏对时间动态的建模，而手术过程的时间连贯性对于准确理解手术过程至关重要。

Method: 提出SurgViVQA模型，使用掩码视频-文本编码器融合视频和问题特征，捕捉运动和组织-工具交互等时间线索，然后通过微调的大型语言模型解码为连贯答案。

Result: 在REAL-Colon-VQA和EndoVis18-VQA数据集上的实验表明，SurgViVQA在关键词准确率上比PitVQA分别提高了11%和9%，扰动研究证实了其对问题表述变化的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有开源音频-视频生成方法由于缺乏有效的跨模态建模，常常表现出唇部同步不佳和语义一致性不足的问题。

Method: 采用双分支联合合成架构，包含两个并行扩散变换器构建统一的跨模态潜在空间；核心是不对称跨模态交互机制，实现双向时间对齐的跨注意力；通过面部感知调制模块动态优先处理显著区域；引入模态感知无分类器引导策略增强跨模态相关性。
...
TL;DR: 本文提出AAPL方法，通过引入对抗性token嵌入来解耦图像增强带来的表面视觉变化与类别相关语义表示，从而提高提示学习在零样本学习任务中的泛化能力。
...
Conclusion: 图像级增强特别是引入属性特定变化能够支持和增强提示学习，AAPL方法通过解耦表面变化与语义表示有效提升了泛化性能。
...
TL;DR: 提出了一种基于PCA的自动化方法，用于标准化3D超声图像中胚胎的对齐，通过三种策略选择标准方向，在2166个图像上达到98.5%的准确率。
...
Method: 给定胚胎分割掩码，应用PCA提取胚胎的主轴，从中推导出四个候选方向。使用三种策略之一选择标准方向：基于皮尔逊相关性的启发式方法、通过归一化互相关与图谱进行图像匹配、以及随机森林分类器。

Result: 在2166个图像中，PCA在99.0%的图像中正确提取了胚胎的主轴。皮尔逊启发式、基于图谱和随机森林方法分别正确选择了97.4%、95.8%和98.4%的图像。这些选择方法的多数据投票准确率达到98.5%。

Conclusion: 该流程的高准确性使得在妊娠早期能够实现一致的胚胎对齐，支持临床和研究环境中的可扩展分析。
...
TL;DR: 本文提出了一种能够处理拓扑变化的形状模板重建方法，通过迭代分割模板空间域来最小化能量函数，实现了在撕裂和切割等拓扑变化情况下的鲁棒重建。
...
Method: 使用经典SfT解初始化，然后迭代分割模板空间域，最小化结合物理合理性和重投影一致性的能量函数。

Result: 该方法能够鲁棒地捕捉包括撕裂和切割在内的各种拓扑变化，在合成和真实数据实验中均优于基线方法。

Conclusion: 建立了第一个通用的拓扑变化感知的形状模板重建框架，为处理拓扑变化的变形重建提供了有效解决方案。
...
TL;DR: Anny是一个基于人体测量学知识的开源、可微分人体模型，无需3D扫描即可生成从婴儿到老人的多样化人体形态，支持毫米级扫描拟合和可控合成数据生成。


<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型依赖昂贵的3D扫描和专有技术，存在人口统计学覆盖范围窄、成本高的问题，需要开发更开放、更具代表性的替代方案。

Method: 基于MakeHuman社区的人体测量学知识，构建连续可解释的形状空间，使用表型参数（性别、年龄、身高、体重）控制混合形状，并利用WHO人口统计数据进行校准。

Result: Anny模型能够生成真实且人口统计学基础扎实的人体形状变化，Anny-One数据集包含80万张照片级真实感人体图像，基于Anny训练的HMR模型性能可与基于扫描的模型相媲美。

Conclusion: Anny提供了一个开放、语义可控的人体建模基础，在保持简单性的同时实现了高精度和广泛代表性，为3D人体建模提供了可访问的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交互动检测方法忽略了细微的视觉线索（如面部表情、注视、手势），主要依赖个体整体表示，且直接检测社交群体而不显式建模个体间互动，导致无法捕捉局部社交信号并在基于细微线索推断群体配置时产生歧义。

Method: 首先检测个体并使用部位感知线索增强其特征，然后通过基于相似性的推理关联个体来推断群体配置，该推理不仅考虑空间关系还考虑表示互动的细微社交线索。
...
TL;DR: DANCE是一个用于视频动作识别的可解释性框架，通过解耦运动动态、物体和场景概念来提供清晰的解释，在保持竞争力的同时显著提升了解释的清晰度。


<details>
  <summary>Details</summary>
Motivation: 现有基于显著性的方法产生纠缠的解释，无法区分预测是基于运动还是空间上下文；基于语言的方法由于运动的隐含特性而难以解释运动。需要一种能够解耦运动动态和上下文概念的方法。

Method: 提出DANCE框架，使用解耦的概念类型：将运动动态概念定义为人体姿态序列，使用大语言模型自动提取物体和场景概念，基于前概念瓶颈设计强制通过这些概念进行预测。

Result: 在KTH、Penn Action、HAA500和UCF-101四个数据集上的实验表明，DANCE显著提高了解释清晰度且保持竞争力性能，用户研究验证了其优越的可解释性，同时有益于模型调试、编辑和失败分析。

Conclusion: DANCE框架成功解决了视频动作识别模型解释中的纠缠问题，通过解耦运动动态、物体和场景概念，提供了更清晰和可解释的预测机制。
...
TL;DR: 本文提出了LEGO-Eval评估框架和LEGO-Bench基准，用于评估细粒度指令与生成3D场景的对齐程度，解决当前方法在评估3D场景生成质量方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前使用大语言模型生成的3D场景缺乏真实世界的空间布局和物体属性，这源于指令不够详细。不真实的场景会导致具身智能体学习与现实世界物理和语义相悖的先验知识，降低部署性能。

Method: 开发了LEGO-Eval评估框架，配备多样化工具来显式地锚定场景组件，实现更准确的对齐评估；同时构建了LEGO-Bench基准，包含详细指令来指定复杂布局和真实环境属性。

Result: 实验表明LEGO-Eval在评估场景-指令对齐方面比VLM-as-a-judge高出0.41 F1分数。使用LEGO-Bench基准测试发现当前生成方法存在显著局限性，所有评估方法生成完全对齐细粒度指令场景的成功率最多仅为10%。

Conclusion: 需要更可靠的评估方法来验证3D场景生成与细粒度指令的对齐，LEGO-Eval和LEGO-Bench为解决这一问题提供了有效工具，揭示了当前生成方法的不足。
...
TL;DR: 提出了一个分析-修订-微调(ARF)管道，使较小的开源语言模型在客户服务摘要任务中超越更大的专有模型。通过分析教师模型错误、使用编辑器模型修订数据、微调学生模型，实现了成本效率和数据隐私的提升。
...
Method: ARF管道：1)分析教师模型(GPT-3.5)的摘要错误并分类；2)使用紧凑编辑器模型(Llama 3.1 70B)进行针对性修订，生成高质量训练数据；3)在精炼数据上微调学生模型(Llama 3.1 8B)。

Result: 微调后的8B学生模型在摘要性能上超越了GPT-3.5，同时提高了成本效率和数据隐私保护。
...
TL;DR: 本文提出了一种新的ABSA评估方法FTS-OBP，研究了小型生成式语言模型在低资源教育评论ABSA任务中的表现，并发布了首个教育评论ABSA资源集。


<details>
  <summary>Details</summary>
Motivation: ABSA研究主要集中在商业领域，而教育和医疗等高需求低资源领域的分析需求未得到满足。传统评估方法对边界变化过于严格，且现有方法依赖资源密集的知识注入。

Method: 提出FTS-OBP评估方法；研究小型解码器生成语言模型在数据无关和数据轻量微调下的表现；提出多任务微调策略；发布教育评论ABSA资源集。

Result: FTS-OBP与传统指标强相关并提供细粒度诊断；1.5-3.8B参数模型在单GPU上仅用200-1000个示例即可超越专有大模型并接近基准结果。

Conclusion: 该方法解决了低资源领域ABSA的挑战，为教育和医疗等领域的细粒度意见挖掘提供了有效解决方案。
...
TL;DR: ROBOTO2是一个开源的基于网页的平台，利用大语言模型辅助进行临床试验的偏倚风险评估，通过结合PDF解析、检索增强的LLM提示和人工在环审查，简化了传统的ROB2标注流程。
...
Method: 开发了交互式界面，结合PDF解析、检索增强的LLM提示和人工在环审查，用户可以上传临床试验报告，接收ROB2信号问题的初步答案和支持证据，并提供实时反馈或修正。

Result: 构建并发布了包含521份儿科临床试验报告的数据集（8954个信号问题，1202个证据段落），使用该数据集对4个LLM进行了ROB2性能基准测试。

Conclusion: ROBOTO2平台公开可用，代码和数据已发布以促进可重复性和采用，同时分析了当前模型能力和在自动化这一系统评价关键方面面临的持续挑战。
...
TL;DR: 本文研究单边对话问题(1SC)，即在只能记录对话一方的情况下推断和学习对话内容。通过评估提示和微调模型，发现访问未来一轮对话和话语长度信息能改善重建效果，占位符提示有助于减少幻觉，大模型通过提示就能生成有希望的重建结果，而小模型需要微调。高质量摘要可以在不重建缺失轮次的情况下生成。


<details>
  <summary>Details</summary>
Motivation: 现实世界中许多场景（如远程医疗、呼叫中心、智能眼镜）只能记录对话的一方，这限制了对话AI的应用。本文旨在解决这一单边对话问题，开发能够从单边对话记录中推断和学习的方法。

Method: 使用MultiWOZ、DailyDialog和Candor数据集，评估提示和微调模型在两种任务上的表现：实时用例中重建缺失说话者的轮次，以及从单边转录生成摘要。采用人工A/B测试和LLM作为评判指标。

Result: 研究发现：访问未来一轮对话和话语长度信息能改善重建效果；占位符提示有助于减少幻觉；大模型通过提示就能生成有希望的重建结果，而小模型需要微调；高质量摘要可以在不重建缺失轮次的情况下生成。
...
Method: 提出基于提示的LLM方法PolyNorm，并开发语言无关的自动数据管理和评估流程。

Result: 在8种语言上的实验显示，相比生产级系统，词错误率持续降低。

Conclusion: PolyNorm能有效减少人工干预，支持多语言文本归一化，并发布了多语言数据集PolyNorm-Benchmark。
...
TL;DR: 本研究使用计算语言学方法分析精神分裂症患者的语言障碍，重点关注惊奇度和语义连贯性两个指标，探讨它们与症状严重程度的关系。


<details>
  <summary>Details</summary>
Motivation: 精神分裂症患者的语言障碍表现为言语混乱和语篇连贯性受损，这些异常反映了潜在的认知障碍，有潜力作为症状严重程度和诊断的客观标记。

Method: 通过计算模型计算语言中的惊奇度和语义连贯性，比较精神分裂症患者与健康对照组在这些指标上的差异。

Result: 研究发现精神分裂症患者与健康对照组在惊奇度和语义连贯性指标上存在显著差异，且这些语言障碍指标随症状严重程度而变化。

Conclusion: 计算语言学指标如惊奇度和语义连贯性能够有效表征精神分裂症的语言障碍，为症状评估和诊断提供客观量化工具。
...
Result: CARMA在规模和多样性上超越现有资源，分析揭示了特定心理健康状况的语言标记，分类实验显示了在阿拉伯语等代表性不足语言中推进心理健康检测的潜力。

Conclusion: 该研究展示了利用大规模自动标注数据集推进阿拉伯语心理健康检测的可行性，为代表性不足语言的心理健康研究提供了新途径。
...
TL;DR: 本文提出了一个基于控制理论的框架，通过控制屏障函数来确保大型语言模型生成符合用户期望的文本。该框架将安全过滤器应用于基线LLM生成的预测token，无需微调即可实现对齐。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在文本生成时可能产生不符合用户期望的内容，需要一种无需微调就能确保生成文本安全性的方法。

Method: 使用控制屏障函数作为安全过滤器，对基线LLM生成的预测token进行干预。该过滤器是附加型设计，可以直接应用于任何已有评估模型的过滤器设计。

Result: 基于开源语言模型实现了整体文本生成系统，能够生成积极的文本内容。

Conclusion: 提出的控制屏障函数框架为LLM对齐提供了一种有效且灵活的方法，无需微调即可确保生成文本符合期望特性。
...
TL;DR: MME-CC是一个多模态认知能力评估基准，专注于视觉中心的认知行为，包含11个代表性推理任务，分为空间、几何和知识推理三类，用于系统评估MLLMs的认知能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准过度强调文本推理或未能系统捕捉视觉中心认知行为，导致MLLMs认知能力评估不足，需要专门针对视觉认知的评估工具。

Method: 构建MME-CC基准，将11个推理任务组织为空间、几何和知识推理三类，对16个代表性MLLMs进行广泛实验分析。

Result: 闭源模型整体领先（Gemini-2.5-Pro 42.66 vs GLM-4.5V 30.45），空间和几何推理普遍较弱（≤30%），发现常见错误模式如方向错误、跨视图身份保持脆弱、反事实指令遵循差等。

Conclusion: 希望推动将MLLMs认知能力作为评估和模型设计的核心考量，促进多模态认知能力的发展。
...
TL;DR: 该论文提出了一个基于LLM的利益相关者风险感知评估框架，通过使用Risk Atlas Nexus和GloVE解释方法，生成可解释的、特定于利益相关者的风险政策，并在医疗AI、自动驾驶和欺诈检测三个实际用例中验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 理解不同利益相关者对AI系统风险的感知对于负责任地部署AI至关重要，需要开发能够预测和解释风险、揭示利益相关者间冲突的透明评估方法。

Method: 使用LLM作为评判者来预测和解释风险，结合Risk Atlas Nexus和GloVE解释方法，生成利益相关者特定的可解释政策，并通过交互式可视化展示冲突产生的原因。

Result: 研究结果表明，利益相关者视角显著影响风险感知和冲突模式，框架能够有效揭示不同利益相关者对相同风险的分歧与共识。

Conclusion: 该工作强调了利益相关者感知解释的重要性，使基于LLM的评估更加透明、可解释，并与以人为中心的AI治理目标保持一致。
...
TL;DR: 本文对大型语言模型(LLMs)中12种不确定性估计方法进行了实证研究，评估它们在问答任务中处理偶然不确定性和认知不确定性的效果，发现在分布内和分布外数据集上不同方法表现各异。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在各行业的广泛应用，确保其输出的可信度至关重要，其中不确定性估计起着关键作用。需要系统评估不同UE方法在LLMs中的鲁棒性和有效性。

Method: 使用12种不同的不确定性估计方法和4种生成质量指标（包括LLMScore），在问答任务的分布内和分布外数据集上评估LLM生成答案的不确定性。

Result: 基于信息的方法在分布内设置中表现优异；基于密度的方法和P(True)指标在分布外场景中表现更好；语义一致性方法在不同数据集和生成指标上表现可靠。

Conclusion: 不同不确定性估计方法在不同情境下各有优势，没有单一方法在所有情况下都是最优的，需要根据具体应用场景选择合适的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有伦理基准主要基于英语和西方框架，忽视了文化细微差别，而孟加拉语作为全球第六大语言（2.85亿使用者），其伦理对齐研究不足，需要开发适合当地文化背景的评估工具。

Method: 通过母语者共识标注构建基准，涵盖日常活动、习惯、育儿、家庭关系和宗教活动五个道德领域，使用美德伦理、常识伦理和正义伦理三种伦理视角进行标注，并对主流多语言LLM进行零样本评估。

Result: 不同模型表现差异显著（准确率50-91%），定性分析显示模型在文化基础、常识推理和道德公平性方面存在一致弱点。
...
Conclusion: LGM能够有效提升大语言模型的概念解释能力，无需截断即可处理任意长度文本，在性能上优于传统RAG方法。
...
TL;DR: ISI的SARAL团队在MATERIAL项目中开发了跨语言信息检索方法，专注于检索查询相关文档集而非仅排序文档列表，在第三阶段评估中在5/6条件下超越其他团队。


<details>
  <summary>Details</summary>
Motivation: 推进跨语言信息检索技术，特别是开发能够检索查询相关文档集的方法，而不仅仅是生成排序的文档列表。

Method: 开发新颖的跨语言信息检索方法，强调检索查询相关文档集，适用于多种语言环境。

Result: 在MATERIAL第三阶段评估中，SARAL在涉及三种语言（波斯语、哈萨克语、格鲁吉亚语）的六个评估条件中的五个条件下表现优于其他团队。

Conclusion: SARAL的方法在跨语言信息检索方面表现出色，特别是在检索文档集方面具有优势，验证了其方法的有效性。
...
TL;DR: IndicSuperTokenizer是一个针对印度多语言大语言模型的tokenizer，结合了子词和多词分词方法，通过语言特定的预分词实现了更好的语言对齐，在生育率分数上达到新的最优水平。


<details>
  <summary>Details</summary>
Motivation: 为多语言LLM设计有效的tokenizer具有挑战性，因为存在不同的文字和丰富的形态变化。虽然BPE等子词方法被广泛采用，但它们在多语言环境中的有效性仍有待探索。

Method: 结合子词和多词分词方法，采用语言特定的预分词策略，实现更符合语言学的token对齐。

Result: 在英语、22种印度语言和代码数据上的评估显示，平均生育率分数比LLaMA4提高39.5%，比Sutra提高18%。推理吞吐量比LLaMA4提高44%，同时在英语和印度语言基准测试中保持相当性能。

Conclusion: IndicSuperTokenizer通过结合子词和多词分词以及语言特定预分词，在多语言环境中实现了更好的性能，为多语言LLM的tokenizer设计提供了有效解决方案。
...
TL;DR: 本研究比较了四种开源LLM（Mistral-7b-instruct、LLaMa2-7b-chat、Falcon-7b-instruct、Orca-mini-v3-7b）和GPT-3.5在计算机科学文献问答任务中的性能，使用RAG技术增强模型能力。GPT-3.5表现最佳，而Mistral-7b-instruct在开源模型中表现最好。


<details>
  <summary>Details</summary>
Motivation: 随着RAG技术与LLMs的日益重要，研究者希望比较不同LLMs在多样化领域问答任务中的性能表现，特别是在计算机科学文献领域。

Method: 使用RAG技术支持，对四种开源LLM和GPT-3.5进行问答任务评估。评估指标包括：二元问题的准确率和精确率，人类专家排名，Gemini AI模型排名，以及长答案问题的余弦相似度。

Result: GPT-3.5结合RAG在二元和长答案问题中表现最佳。在开源LLMs中，Mistral-7b-instruct表现最好。Orca-mini-v3-7b响应延迟最短，LLaMa2-7b-chat延迟最高。

Conclusion: 研究表明开源LLMs可以与GPT-3.5等专有模型相媲美，特别是在具备更好基础设施的情况下。RAG技术有效减少了LLMs的幻觉问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前持续预训练的进展更多依赖于扩展正确的结构而非仅仅增加参数数量，需要解决在增加模型能力的同时避免严重遗忘的问题。

Method: 提出SCALE架构，包含SCALE-Preserve（保护优先）、SCALE-Adapt（适应优先）和SCALE-Route（路由扩展）三种变体，通过持久性保护原则保持基础模型行为，通过协作适应原则选择性训练扩展组件获取新知识。

Result: 在合成传记基准测试中，SCALE缓解了深度扩展中观察到的严重遗忘问题，同时在韩语语料库的持续预训练中，SCALE变体在英语评估中表现出较少遗忘，在韩语基准测试中取得有竞争力的增益。

Conclusion: SCALE架构提供了最佳的稳定性-可塑性权衡，其保护与适应的相互作用相比标准持续学习设置能稳定优化过程。
...
TL;DR: 本文评估了推理型多模态大语言模型在临床任务中的表现，发现开启思维模式相比标准非思维模式带来的性能提升有限，特别是在复杂医疗任务中表现仍不理想。


<details>
  <summary>Details</summary>
Motivation: 随着具有"双状态"（思维模式和非思维模式）的推理型MLLMs的快速发展，需要严格评估这些模型增强的推理过程如何影响临床任务中的性能和可靠性。

Method: 评估了Seed1.5-VL和Gemini-2.5-Flash两个领先MLLMs在思维模式下的能力，使用VQA-RAD和ROCOv2数据集在四个视觉医疗任务上进行测试。

Result: 研究发现，对于大多数任务，激活思维模式相比标准非思维模式的改进幅度有限。在开放式VQA和医学图像解释等复杂医疗任务中表现仍然不理想。

Conclusion: 当前推理型MLLMs在医疗应用中的表现表明，需要领域特定的医疗数据和更先进的医学知识整合方法。
...
TL;DR: 这篇综述系统评估了生成式AI在生物信息学中的变革性应用，涵盖基因组学、蛋白质组学、结构生物学和药物发现等领域，提出了六个研究问题来分析方法学进展、预测性能和专业化优势。


<details>
  <summary>Details</summary>
Motivation: 系统识别和评估生成式AI在生物信息学中日益增长的发展，评估有影响力的策略在方法学进步、预测性能和专业化方面的表现，并确定先进建模、数据密集型发现和整合生物学分析的有前景方法。

Method: 采用系统综述和荟萃分析方法，提出六个研究问题(RQs)，分析生成式AI在序列分析、分子设计、整合数据建模等子领域的应用，比较专门化模型架构与通用模型的性能差异。

Result: 生成式AI在多个生物信息学子领域表现出优于传统方法的性能，专门化模型架构通过针对性预训练和上下文感知策略超越通用模型，在分子分析、数据整合、结构建模和功能预测方面取得显著改进。

Conclusion: 生成式AI已成为生物信息学的变革性方法，在分子数据集、细胞数据集和文本资源的支持下展现出强大潜力，但面临可扩展性不足和数据偏差等约束，未来需要关注稳健评估和生物学基础建模。
...
TL;DR: 本文提出了"沉默偏见"概念，指安全对齐大语言模型中潜在存在但被掩盖的不公平偏好，并开发了Silenced Bias Benchmark（SBB）基准来揭示这些偏见。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常将模型拒绝回答视为公平性表现，这掩盖了模型潜在的不公平偏好，造成虚假的公平感。

Method: 采用激活引导技术减少模型在问答过程中的拒绝行为，从而揭示被安全对齐掩盖的潜在偏见。

Result: 在多个大语言模型上的实验表明，模型的直接响应与底层公平性问题存在显著差异，暴露了被掩盖的偏见问题。
...
Result: 在ReClor和LogiQA数据集上的实验表明，预训练模型的逻辑推理准确率显著提升，证实了LFC-DA在LLM引导的逻辑数据增强中的有效性。
...
TL;DR: 研究发现对称BPE分词方法在机器翻译中并非最优，提出非对称BPE方法，在源语言使用高合并操作数，目标语言使用低合并操作数，显著提升低资源机器翻译性能。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译研究通常使用对称BPE分词方法，但这种方法在不同语言对和数据规模下可能不是最优的。本文旨在探索更有效的分词策略。

Method: 研究不同数据量级和语言对下的BPE分词方法，比较对称BPE和非对称BPE的性能差异，其中非对称BPE在源语言和目标语言使用不同的合并操作数。

Result: 非对称BPE在低资源设置下显著优于对称BPE，在英语-印地语翻译中平均提升5.32、4.46和0.7 CHRF++分数，在6个额外语言对中10/12个系统显示出统计显著改进。

Conclusion: 源语言使用高合并操作数(4K-32K)，目标语言使用低合并操作数(0.5K-2K)的非对称BPE策略能获得最佳结果，特别有利于低资源机器翻译。
...
TL;DR: 本文研究了小型语言模型在关系抽取任务中处理RDF图中数据类型属性和对象属性的能力，发现稀有属性的长尾分布是关键瓶颈，并评估了多种策略来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 探索小型语言模型在完整RDF图抽取中如何处理数据类型属性和对象属性，特别是解决稀有属性长尾分布带来的挑战。

Method: 评估了多种策略：分层抽样、加权损失、数据集扩展和基于模板的合成数据增强，以解决稀有属性的不平衡问题。

Result: 最佳策略是构建一个训练集，其中每个属性的出现次数超过给定阈值，这样可以在不平衡的目标属性上实现同等良好的性能。
...
TL;DR: 3TF框架通过思想训练和无思想推理，让模型在推理时内部执行结构化推理但外部输出简洁，无需显式生成推理步骤。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要压缩冗长的推理输出，但仍依赖显式推理。3TF采用短到长视角，旨在让模型内部化推理过程，在推理时无需显式生成推理步骤。

Method: 首先训练混合模型支持推理和非推理模式，然后在CoT标注数据上进一步训练以内部化结构化推理，推理时使用非推理模式强制简洁输出。

Result: 3TF训练模型在无思想推理下在推理基准上获得显著改进，证明高质量推理可以隐式学习和执行。

Conclusion: 3TF框架证明模型能够学习并隐式执行高质量推理，无需显式逐步生成推理步骤，实现了推理质量与效率的平衡。
...
Method: 提出QuestionRAG框架：1）通过外部知识（如搜索结果、相关实体）增强输入以解决误解问题；2）使用强化学习将模型目标与精确修正对齐，避免过度修正。

Result: 知识增强对理解错误问题至关重要，基于RL的对齐方法比传统监督微调更有效，显著提升了模型遵循指令和泛化的能力。

Conclusion: 通过整合知识增强和强化学习两种策略，QuestionRAG充分释放了LLMs在问题修正任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 生物医学领域需要批判性评估科学文献的能力，虽然大型语言模型能提供支持，但其在专业领域批判性推理的可靠性仍然有限。

Method: 创建CareMedEval数据集，基于法国医学生真实考试内容，包含534个问题，涵盖37篇科学文章，专门评估基于科学文献的批判性阅读和推理能力。

Result: 最先进的通用和生物医学专用LLM在各种上下文条件下都难以完成任务，即使生成中间推理标记能显著改善结果，但所有模型的精确匹配率均未超过0.5。

Conclusion: CareMedEval为基于科学文献的推理提供了一个具有挑战性的基准，揭示了当前LLM的局限性，特别是关于研究局限性和统计分析的问题，为未来开发自动化批判性评估支持铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于SHACL形状的RDF模式提取方法在专业领域知识库补全和精炼方面存在局限性，需要更高效的方法来提升模型泛化能力

Method: 将单SHACL形状验证重构为评估形状派生属性的所有可能组合，选择最优组合进行训练，并采用迭代学习过程精炼噪声知识库

Result: 显著提升了模型泛化能力和性能，能够发现新的相关事实

Conclusion: Kastor框架为专业领域知识库的补全和精炼提供了有效的解决方案，通过优化训练策略提升了小语言模型的效能
...
TL;DR: BanglaSTEM是一个包含5000个孟加拉语-英语句对的数据集，专门针对STEM领域的技术术语翻译问题，通过训练T5模型显著提升了技术内容的翻译准确性。
...
Method: 构建BanglaSTEM数据集，包含5000个从STEM领域精心挑选的孟加拉语-英语句对，使用语言模型生成超过12000个翻译，然后通过人工评估选择最高质量的对，训练基于T5的翻译模型。
...
TL;DR: 提出了HaluMem基准测试，这是首个针对记忆系统的操作级幻觉评估基准，包含三个评估任务来揭示不同操作阶段的幻觉行为。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统在存储和检索过程中经常出现幻觉问题，包括捏造、错误、冲突和遗漏，但现有评估主要是端到端的问答，难以定位幻觉产生的具体操作阶段。
...
TL;DR: 提出了一个可扩展的多轮指令遵循评估框架EvolIF，通过三层机制解耦语言表面形式和用户意图模拟，动态构建包含状态变化和回溯的基准测试，在用户耐心耗尽时终止对话。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常限于固定轮次，容易饱和且无法考虑用户交互体验，需要评估大语言模型在多主题对话中遵循用户指令的能力。

Method: 采用三层机制跟踪约束、指令和主题，模拟用户-LLM交互，动态构建包含状态变化和回溯的基准测试，在用户耐心耗尽时终止对话。

Result: 构建了EvolIF基准，包含九种约束类型。GPT-5表现最佳，平均维持18.54轮对话，鲁棒性达70.31%，显著优于Gemini-2.5-Pro 11.41%。

Conclusion: 提出的框架能有效评估多轮指令遵循能力，GPT-5在该任务上表现最优，数据和代码将公开。
...
TL;DR: SOLVE-Med是一个多智能体医疗问答系统，通过结合领域专业化的小语言模型来处理复杂医疗查询，在意大利医疗论坛数据上表现出色，超越了参数更大的独立模型。
...
Method: 采用多智能体架构，包括用于动态专家选择的Router Agent、十个针对特定医疗领域微调的专门化模型（每个1B参数），以及用于合成响应的Orchestrator Agent。

Result: 在十个医疗专业的意大利医疗论坛数据上评估，SOLVE-Med实现了ROUGE-1为0.301和BERTScore F1为0.697的优异性能，优于参数高达14B的独立模型，同时支持本地部署。

Conclusion: SOLVE-Med通过多智能体专业化方法有效解决了医疗问答系统的关键挑战，在保持高性能的同时实现了本地部署能力。
...
TL;DR: 该研究创建了MultiZebraLogic数据集，包含多种语言、主题和难度的斑马谜题，用于评估大型语言模型的逻辑推理能力。研究发现4x5大小的谜题对推理模型o3-mini具有挑战性，而5个干扰线索会使其准确率下降15%。


<details>
  <summary>Details</summary>
Motivation: 为了全面评估大型语言模型的多任务能力，需要创建大规模、高质量的多语言逻辑推理基准数据集，涵盖不同难度级别以适应各种推理能力的模型。

Method: 通过生成多种语言、主题、大小和线索类型的斑马谜题，包括14种不同线索类型和8种干扰线索类型，创建了MultiZebraLogic数据集。

Result: 研究发现2x3大小的谜题对非推理模型GPT-4o mini具有挑战性，4x5大小的谜题对推理模型o3-mini具有挑战性。5个干扰线索使o3-mini在4x5谜题上的准确率下降15±7%。语言和主题对模型表现无显著影响。

Conclusion: 成功创建了MultiZebraLogic数据集，包含9种日耳曼语言的128+1024个谜题，并发布了可适应更多语言和主题的谜题生成代码，为评估LLMs的逻辑推理能力提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统语言模型仅依赖分布式表示，缺乏可解释性。本文旨在开发一种能够动态控制表示局部化程度的新架构，为需要透明性和能力的受监管领域提供实用框架。

Method: 使用两层Transformer架构在WikiText语料库上进行实验，系统性地改变局部性参数λ从1.0（完全局部）到0.0（完全分布式），通过明确的惩罚阈值和信息论设计原则实现控制。

Result: 局部配置显著降低注意力熵（λ=1.0时为5.36比特，λ=0.0时为7.18比特），同时保持更高的指针保真度。中间局部性值（λ=0.6）在可解释性和性能之间达到最优平衡，测试困惑度为4.65，准确率为84.7%。

Conclusion: 局部语言模型为需要透明性和能力的受监管领域提供了实用框架，通过明确的惩罚阈值和信息论设计原则实现了对可解释性-性能谱系的精确数学控制。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法主要关注表示层面的解耦，而该研究旨在通过仅使用大边界合成数据来实现迭代控制和高度表达性，避免对嵌入先验或辅助模块的依赖。
...
Conclusion: 该模型代表了对传统表示层面解耦方法的根本性转变，通过大边界学习实现了高度表达性和迭代控制能力。
...
TL;DR: 本文提出了中文多文档问答数据集ChiMDQA，包含6个领域的6,068个高质量问答对，分为10个细粒度类别，为中文文档理解和智能问答系统提供重要资源。
...
Method: 通过细致的文档筛选和系统化的问题设计方法，构建包含学术、教育、金融、法律、医疗、新闻等6个领域的长文档问答数据集。

Result: 成功构建了包含6,068个高质量问答对的ChiMDQA数据集，涵盖10个细粒度类别，保证了数据的多样性和高质量。

Conclusion: ChiMDQA数据集为中文文档理解、知识抽取和智能问答系统等NLP任务提供了重要基础，代码和数据已公开可用。
...
TL;DR: 本研究通过监督微调LLM并结合RAG方法，开发了专门支持政策制定者理解、分析和制定法律规制的AI工具。
...
Method: 使用针对法律领域定制的监督数据集对LLM进行微调，并集成检索增强生成（RAG）方法，使模型能够访问和整合最新的外部法律知识。

Result: 该方法显著提升了法律研究和法规制定的效率，模型不仅能处理法律信息，还能主动协助政策制定者解释法规和起草新法规。

Conclusion: 结合微调和RAG增强的方法为法律领域提供了有价值的工具，能够有效支持政策制定者在不断变化的法律环境中开展工作。
...
TL;DR: 本文提出了一个视角主义标注方案，用于分析协作对话中的共同基础建立过程，特别关注参与者可能认为达成一致但实际上指向不同实体的误解情况。


<details>
  <summary>Details</summary>
Motivation: 在非对称协作对话中，参与者可能以为自己达成共识但实际上在指代不同实体，这种误解会影响对话质量。研究旨在追踪理解如何形成、分歧和修复的过程。

Method: 使用方案约束的LLM标注流水线对HCRC MapTask语料库进行标注，获得13k个带可靠性估计的标注参考表达，分析理解状态。

Result: 结果显示，一旦词汇变体统一后，完全误解很少见，但多重性差异会系统性地导致分歧，揭示了表面上的共同基础可能掩盖指代不对齐。

Conclusion: 该框架为研究基础性误解提供了资源和分析视角，并可用于评估(V)LLMs在协作对话中建模视角依赖基础的能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer模型的兴起，关系抽取领域出现了大量新研究，需要对这些工作进行系统梳理，为研究者和从业者提供全面的参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本立场检测方法存在泛化性问题、缺乏文本与目标之间的一致性，以及过度依赖显式推理、缺乏细粒度解释、难以解释模型预测过程等问题。

Method: IRIS框架将立场检测视为信息检索排序任务，基于文本中的隐式推理序列和基于语言度量的显式推理来理解输入对目标的立场，无需真实推理标签即可提供内在可解释性。

Result: 在VAST、EZ-STANCE、P-Stance和RFD基准数据集上使用50%、30%甚至10%训练数据的广泛实验证明了模型具有良好的泛化能力。

Conclusion: IRIS框架通过其提出的架构和可解释设计，在零样本立场检测任务中表现出优异的性能，为理解作者对给定目标的态度提供了可解释的方法。
...
### [72] [Evaluating Control Protocols for Untrusted AI Agents](https://arxiv.org/abs/2511.02997)
*Jon Kutasov,Chloe Loughridge,Yuqi Sun,Henry Sleight,Buck Shlegeris,Tyler Tracy,Joe Benton*

Main category: cs.AI

TL;DR: 本文系统评估了AI控制协议在SHADE-Arena环境中的安全性，发现关键行动延迟协议对红队攻击具有高度鲁棒性，而重采样策略在红队获得协议内部知识时安全性大幅下降。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统作为智能体广泛部署，确保其安全运行变得至关重要。AI控制通过监控智能体行为并在必要时干预或审计来降低风险，需要评估这些协议的有效性和对自适应攻击的鲁棒性。

Method: 在SHADE-Arena多样化智能体环境中系统评估控制协议，首先评估蓝队协议（包括信任模型延迟、重采样和关键行动延迟）对抗默认攻击策略，然后迭代红队策略对抗这些协议。

Result: 重采样和关键行动延迟协议将安全性从50%提升到96%。但当红队策略获得额外能力（如知道重采样发生时间或模拟监控器）时，重采样策略的安全性降至17%。关键行动延迟协议即使面对最强红队策略也保持高度鲁棒性。

Conclusion: 关键行动延迟协议对自适应攻击具有高度鲁棒性，证明了拒绝攻击策略访问协议内部信息的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 开放数据仓库对非专家用户难以访问，需要数据集发现、模式映射和统计分析的专业知识。虽然大语言模型在单个任务中表现良好，但在端到端分析工作流中存在注意力稀释、专业推理模式干扰和错误传播等根本限制。

Method: 提出PublicAgent多智能体框架，将分析工作流分解为四个专门智能体：意图澄清、数据集发现、分析和报告。这种架构在每个智能体上下文中保持专注注意力，并在每个阶段实现验证。

Result: 在5个模型和50个查询的评估中，发现：1) 专业化独立于模型强度提供价值；2) 智能体分为通用型（发现、分析）和条件型（报告、意图）；3) 不同智能体缓解不同的失败模式；4) 架构优势在不同任务复杂度下持续存在；5) 不同模型间智能体效果差异显著。
...
TL;DR: ScalingEval是一个大规模基准测试研究，系统比较了36个大型语言模型作为评估者的能力，通过多智能体框架和共识驱动协议实现无需人工标注的可复现评估。
...
Method: 采用多智能体框架，通过模式审计和问题代码聚合为真实标签，使用可扩展的多数投票机制，在互补商品推荐场景中进行大规模评估。

Result: 发现Claude 3.5 Sonnet决策置信度最高，Gemini 1.5 Pro整体性能最佳，GPT-4o在延迟-准确性-成本权衡最优，GPT-OSS 20B在开源模型中领先；结构化领域共识强，生活方式类别存在分歧。
...
TL;DR: 本文构建了首个基准测试来评估LLMs是否能够内化描述现实世界人口的经验分布，结果显示LLMs在概率分布知识方面表现不佳，无法自然内化现实世界统计数据。


<details>
  <summary>Details</summary>
Motivation: AI系统在科学领域具有巨大潜力，但需要进一步发展以实现更通用的智能。关键区别在于事实性知识和概率性知识，本文旨在测试LLMs是否能够内化描述现实世界概率分布的知识。

Method: 开发首个基准测试，直接评估LLMs是否能够访问描述现实世界人口的经验分布，涵盖经济、健康、教育和社会行为等领域。

Result: LLMs整体表现不佳，似乎无法自然内化现实世界统计数据。在Pearl因果层次框架下，语言模型缺乏对观察分布的知识。

Conclusion: LLMs在概率分布学习方面存在根本性限制，无法有效内化现实世界统计数据，这限制了它们在因果层次中干预性和反事实性知识的能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型KV缓存占用大量内存的问题，同时克服现有技术在工业部署中的兼容性和精度不确定性障碍。

Method: 开发SnapStream KV缓存压缩方法，在静态图和连续批处理框架中实现稀疏KV注意力技术。

Result: 在DeepSeek-671B模型上实现4倍内存使用改进，128k上下文长度下达到1832 tokens/秒的推理速度，在多个基准测试中精度损失最小。

Conclusion: SnapStream是首个在生产推理系统中成功部署的稀疏KV注意力技术，为大规模LLM部署提供了有效的内存优化解决方案。
...
TL;DR: 本文提出了一种基于能力的大语言模型监控新方法，替代传统的基于任务的监控方式，以适应LLMs在医疗领域的通用特性。


<details>
  <summary>Details</summary>
Motivation: 现有监控方法继承自传统机器学习，基于任务性能退化的假设，但LLMs并非针对特定任务训练，无法假设模型性能必然因人群变化而退化。

Method: 提出能力基础监控方法，围绕共享模型能力（如摘要、推理、翻译、安全防护）组织监控，而非独立评估每个下游任务。

Result: 该方法能够实现跨任务检测系统性弱点、长尾错误和涌现行为，这些都是基于任务的监控可能遗漏的。
...
TL;DR: 论文分析了miniF2F基准测试中形式化与非形式化陈述的差异，发现超过一半问题存在不一致性。通过修正这些错误创建了miniF2F-v2，将完整定理证明流程的准确率从36%提升到70%。


<details>
  <summary>Details</summary>
Motivation: 评估AI系统在数学奥林匹克竞赛环境中的表现，分析形式化与非形式化陈述之间的差异对定理证明准确率的影响。

Method: 对miniF2F基准进行彻底分析，识别形式化与非形式化陈述之间的差异，修正所有错误和不一致性，创建miniF2F-v2数据集，并评估完整定理证明流程的性能。

Result: 原始miniF2F中完整定理证明流程的最佳准确率约为36%，而修正后的miniF2F-v2将准确率提升到70%。发现超过一半的问题存在形式化与非形式化陈述之间的差异。

Conclusion: 高质量基准测试对于评估形式推理领域的进展至关重要，能更好诊断自动形式化和定理证明模型的失败与成功模式。miniF2F-v2为社区提供了更可靠的评估工具。
...
TL;DR: 本文提出了一种利用多模态大语言模型辅助设计烟花算法的新方法，通过引入关键部分概念扩展烟花算法到复杂高维任务，在旅行商问题和电子设计自动化问题上取得了优于或达到当前最优水平的结果。


<details>
  <summary>Details</summary>
Motivation: 随着优化问题日益复杂多样，传统零阶或一阶方法效率低、梯度信息不准确、优化信息利用不足，难以有效应对非凸性、高维性、黑盒特性等挑战。大语言模型在语言理解和代码生成能力上的显著提升为优化算法设计提供了新思路。

Method: 以烟花算法为基础优化器，结合多模态大语言模型辅助设计，提出关键部分概念扩展烟花算法到复杂高维任务，利用大语言模型的多模态特性充分利用优化过程信息。

Result: 在旅行商问题和电子设计自动化问题上的实验结果表明，新框架下生成的烟花算法在许多问题实例上达到或超越了当前最优水平。

Conclusion: 大语言模型辅助的优化算法设计框架能够有效提升传统优化算法在复杂高维问题上的性能，为优化算法创新提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 形式化XAI相比非形式化方法具有理论严谨性保证，但实际实现验证研究较少，需要验证形式化解释器的正确性。

Method: 开发了一种新颖的验证形式化解释器的方法论，并对PyXAI解释器进行了实验评估。

Result: 实验发现PyXAI在大多数分析的数据集上计算出的解释存在错误，验证了所提验证方法的重要性。

Conclusion: 形式化解释器的实际实现需要严格验证，所提出的验证方法对确保形式化XAI的可靠性至关重要。
...
TL;DR: 提出一个多智能体AI框架，通过专业化的知识驱动智能体协作生成和优化工程设计方案，并以NACA翼型气动优化为例验证其有效性。
...
Method: 构建包含三个核心AI智能体的框架：图本体学家使用LLM构建领域知识图谱，系统工程师制定技术需求，设计工程师生成候选设计，通过迭代反馈循环进行优化。

Result: 成功应用于4位数NACA翼型的气动优化，能够有效生成满足技术需求的翼型设计并优化升阻比等性能指标。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI助手在企业环境中具有提升生产力、简化信息访问和改善用户体验的巨大潜力，但面临数据稀疏、质量保证和快速部署等现实挑战。

Method: 采用人在回路开发工作流程，结合提示工程、检索增强和轻量级人工验证，构建系统架构并实施敏捷的反馈驱动开发。

Result: 成功开发并部署了Summit Concierge AI助手，能够处理广泛的活动相关查询，在冷启动场景下实现了可扩展且可靠的AI助手。

Conclusion: 敏捷的反馈驱动开发方法能够在冷启动场景下实现可扩展且可靠的AI助手，为类似企业应用提供了有价值的实践经验。
...
TL;DR: 大型语言模型能够从少量Big Five人格量表数据中准确模拟人类心理特质的相关结构，通过两阶段推理过程生成其他心理量表的响应，其表现接近直接在数据集上训练的机器学习算法。


<details>
  <summary>Details</summary>
Motivation: 研究探索大型语言模型是否能够从最小化的定量输入中建模人类心理特质之间的相关结构，验证LLMs在心理学模拟中的潜力。
...
Conclusion: LLMs通过抽象和推理过程能够从最小数据中精确预测个体心理特质，为心理学模拟提供了强大工具，并揭示了其新兴的推理能力。
...
TL;DR: 提出AAA框架，通过人机协作模式实现可扩展的Web可访问性审计，包含GRASP图采样方法和MaC多模态LLM助手。


<details>
  <summary>Details</summary>
Motivation: 当前网站可访问性审计方法资源密集且难以扩展，WCAG-EM标准执行需要大量人工努力。

Method: 基于WCAG-EM标准，开发了GRASP图采样方法确保页面覆盖代表性，以及MaC多模态LLM助手提供跨模态推理支持。

Result: 实验证明方法有效，小规模语言模型经过微调后可作为专家系统。贡献了四个用于审计流程基准测试的新数据集。

Conclusion: AAA框架实现了可扩展的端到端Web可访问性审计，通过AI增强辅助为实际应用带来影响。
...
TL;DR: 本文对机器学习模型中解释问题的参数化复杂度进行理论分析，聚焦于具有透明内部机制的模型，研究归纳式和对比式两种主要解释问题及其本地和全局变体。


<details>
  <summary>Details</summary>
Motivation: 填补可解释AI领域的理论空白，为各种机器学习模型生成解释的复杂度提供基础理解，促进AI系统的透明度和问责制。

Method: 理论分析决策树、决策集、决策列表、布尔电路及其集成模型等多样化ML模型的解释问题参数化复杂度。

Result: 为可解释AI领域提供了关于生成解释复杂度的基础理论理解。
...
TL;DR: Solly是第一个在简化版Liar's Poker中达到精英人类水平的AI智能体，使用无模型、演员-评论家深度强化学习算法通过自博弈训练，在单挑和多玩家游戏中均表现优异，胜率超过50%，并优于大型语言模型。
...
Method: 使用无模型、演员-评论家深度强化学习算法进行自博弈训练，开发了Solly AI智能体。

Result: Solly在单挑和多玩家Liar's Poker中达到精英人类水平，胜率超过50%，在资金收益方面也表现优异；优于具有推理能力的大型语言模型；开发了新颖的竞标策略，有效随机化游戏，不易被世界级人类玩家利用。

Conclusion: Solly证明了深度强化学习在复杂多人不完全信息游戏中可以达到精英人类水平，为研究更复杂的多人动态环境提供了重要进展。
